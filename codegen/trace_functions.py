...
op_name = c10::Symbol::fromQualString("npu::${trace_name}");
""")
...
ADD_TRACE_INPUT = CodeTemplate("""torch::jit::tracer::addInputs(node, "${name}", ${input});""")
...
def format_trace_inputs(f: NativeFunction) -> str:
    def dispatch_trace_input(arg: Union[Argument, TensorOptionsArguments]) -> Sequence[str]:
        if isinstance(arg, TensorOptionsArguments):
            ...
            if str(arg.type) == 'Tensor?[]':
                return [f'torch::jit::tracer::addInputs(node, "{name}", {name});']
            else:
    ...
    if f.func.is_out_fn():
        ...
        inplace = [ADD_TRACE_INPUT.substitute(name=f.func.arguments.out[i].name, input=f.func.arguments.out[i].name)
...
RENAME_TRACE_ADD_ARGS = {
    'fill': '''\
    torch::jit::tracer::addInputs(node, "options", c10::optional<ScalarType>());
    torch::jit::tracer::addInputs(node, "options", layout_or_default(c10::nullopt));
    torch::jit::tracer::addInputs(node, "options", device_or_default(c10::nullopt));
    torch::jit::tracer::addInputs(node, "options", pinned_memory_or_default(c10::nullopt));
    c10::optional<MemoryFormat> memory_format = c10::MemoryFormat::Preserve;
    torch::jit::tracer::addInputs(node, "memory_format", memory_format);
''',
    'zero': '''\
    torch::jit::tracer::addInputs(node, "options", c10::optional<ScalarType>());
    torch::jit::tracer::addInputs(node, "options", layout_or_default(c10::nullopt));
...
torch::jit::tracer::ensureUniqueIfOutOfPlaced("${name}", ${mutable_input});
""")
...
def format_postrecord_trace(f: NativeFunction) -> str:
    ...
    if f.func.is_out_fn():
        ...
        if output_names_outplace == output_names_inplace:
            outputs = [f'torch::jit::tracer::addOutput(node, {n});' for n in output_names_outplace]
            return POST_RECORD_TRACE.substitute(add_trace_outputs=outputs)

        selection = SELECT.substitute(
            cond='force_outplace',
            true='\n'.join(f'torch::jit::tracer::addOutput(node, {n});' for n in output_names_outplace),
            false='\n'.join(f'torch::jit::tracer::addOutput(node, {n});' for n in output_names_inplace),
        )
        return POST_RECORD_TRACE.substitute(add_trace_outputs=selection)
    else:
        output_names = cpp.return_names(f)
        outputs = [f'torch::jit::tracer::addOutput(node, {n});' for n in output_names]
        return POST_RECORD_TRACE.substitute(add_trace_outputs=outputs)
...
${assign_return_values} ${impl_name}(${args_exprs_str});""")
...
def emit_trace_body(f: NativeFunction) -> List[str]:
    ...
    # We could probably work harder to ensure that the fast variants are called instead,
    # but the perf benefit would be minimal.
...
@with_native_function
def compute_trace_method_declaration(f: NativeFunction):
    assert cpp.name(f.func) not in MANUAL_TRACER
    ...
    result = f"TORCH_API {returns_type} {name}({args_str});\n"
    return [result]
