...
class DbParser(BaseParser):
    def __init__(self, name: str, param_dict: dict):
        ...
        self._ascend_db_path = os.path.join(self._output_path, DbConstant.DB_ASCEND_PYTORCH)
        self._conn = None
        self._cur = None
    ...
    def run(self, depth_data: dict):
        try:
            ...
            self.save_session_info_to_db()
        except RuntimeError:
            print_error_msg("Failed to generate ascend_pytorch db file.")
    ...
    def save_session_info_to_db(self):
        ProfilerConfig().load_info(self._profiler_path)
        DbManager.create_table_with_headers(self._conn, self._cur, DbConstant.TABLE_SESSION_INFO, TableColumnsManager.TableColumns.get(DbConstant.TABLE_SESSION_INFO))
        DbManager.insert_data_into_table(self._conn, DbConstant.TABLE_SESSION_INFO, [[ProfilerConfig().rank_id]])