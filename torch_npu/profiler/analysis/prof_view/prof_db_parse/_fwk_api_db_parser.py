...
from ...prof_common_func._id_manager import (
    CallChainIdManager,
    ConnectionIdManager,
    Str2IdManager,
)
...
class FwkApiDbParser(BaseParser):
    ...
    def run(self, deps_data: dict):
        ...
        try:
            ...
            self.logger.exception("Failed to generate framework api table")
    ...
    def get_mstx_mark_op_connection_ids_with_cann_api(
        self, task_enqueues: list, task_dequeues: list, mstx_mark_apis: list
    ):
        if not mstx_mark_apis:
            return
        sql = f"select startNs, endNs, globalTid, connectionId from {DbConstant.TABLE_MSTX_EVENTS} order by startNs"
        ...
        if task_enqueues and task_dequeues:
            self.get_torch_op_connection_ids_with_task_queue(
                task_enqueues, task_dequeues, mstx_mark_apis, mstx_op_len, cann_tx_apis
            )

    def get_torch_op_connection_ids_with_cann_api(
        self, task_enqueues: list, task_dequeues: list, torch_op_apis: list
    ):
        if not torch_op_apis:
            return
        sql = f"select id from {DbConstant.TABLE_STRING_IDS} where value = 'launch'"
        ...
        sql = (
            ...
            f"where name = {node_launch_str_id} and type = 10000 order by startNs"
        )
        # 10000 : node level
        ...
        if task_enqueues and task_dequeues:
            ...
            self.get_torch_op_connection_ids_without_task_queue(
                torch_op_apis, torch_op_len, node_launch_apis
            )

    def get_torch_op_connection_ids_with_task_queue(
        self,
        task_enqueues: list,
        task_dequeues: list,
        torch_op_apis: list,
        torch_op_len: int,
        node_lauch_apis: list,
    ):
        connection_id_manager = ConnectionIdManager()
        enqueue_corr_ids = {
            connection_id_manager.get_connection_ids_from_id(
                task_enqueue[TaskQueueDataOri.CORRELATION_ID]
            )[0]
            for task_enqueue in task_enqueues
        }
        dequeue_corr_ids = {
            connection_id_manager.get_connection_ids_from_id(
                task_dequeue[TaskQueueDataOri.CORRELATION_ID]
            )[0]
        ...
        enqueue_list = [
            ...
            if connection_id_manager.get_connection_ids_from_id(
                enqueue[TaskQueueDataOri.CORRELATION_ID]
            )[0]
            in matched_corr_ids
        ]
        dequeue_list = [
            ...
            if connection_id_manager.get_connection_ids_from_id(
                dequeue[TaskQueueDataOri.CORRELATION_ID]
            )[0]
            in matched_corr_ids
        ]
        ...
        for node_launch_api in node_lauch_apis:
            for idx in range(last_dequeue_index, dequeue_len):
                if (
                    node_launch_api[CannNodeLaunchApiOri.GLOBAL_TID]
                    != dequeue_list[idx][TaskQueueDataOri.GLOBAL_TID]
                ):
                    continue
                if (
                    node_launch_api[CannNodeLaunchApiOri.START_NS]
                    > dequeue_list[idx][TaskQueueDataOri.START_NS]
                    and node_launch_api[CannNodeLaunchApiOri.END_NS]
                    < dequeue_list[idx][TaskQueueDataOri.END_NS]
                ):
                    last_dequeue_index = idx
                    enqueue = enqueue_list[idx]
                    last_torch_op_index = self.get_torch_op_connection_ids_with_enqueue(
                        torch_op_apis,
                        torch_op_len,
                        enqueue,
                        last_torch_op_index,
                ...
                if (
                    dequeue_list[idx][TaskQueueDataOri.START_NS]
                    > node_launch_api[CannNodeLaunchApiOri.END_NS]
                ):
                    last_dequeue_index = idx
                    break

    def get_torch_op_connection_ids_with_enqueue(
        self,
        torch_op_apis: list,
        torch_op_len: int,
        enqueue: list,
        last_torch_op_index: int,
        connection_id: int,
    ) -> int:
        last_op_api = None
        for idx in range(last_torch_op_index, torch_op_len):
            if (
                enqueue[TaskQueueDataOri.GLOBAL_TID]
                != torch_op_apis[idx][TorchOpDataOri.GLOBAL_TID]
            ):
                continue
            if (
                enqueue[TaskQueueDataOri.START_NS]
                > torch_op_apis[idx][TorchOpDataOri.END_NS]
            ):
                continue
            if (
                enqueue[TaskQueueDataOri.START_NS]
                > torch_op_apis[idx][TorchOpDataOri.START_NS]
                and enqueue[TaskQueueDataOri.END_NS]
                < torch_op_apis[idx][TorchOpDataOri.END_NS]
            ):
                ...
                break
            if (
                enqueue[TaskQueueDataOri.END_NS]
                < torch_op_apis[idx][TorchOpDataOri.START_NS]
            ):
                last_torch_op_index = idx
                break
        if last_op_api:
            torch_op_apis[last_torch_op_index][TorchOpDataOri.CONNECTION_ID].append(
                connection_id
            )
        return last_torch_op_index

    def get_torch_op_connection_ids_without_task_queue(
        self, torch_op_apis: list, torch_op_len: int, node_lauch_apis: list
    ):
        last_op_api = None
        last_op_index = 0
        for node_launch_api in node_lauch_apis:
            for idx in range(last_op_index, torch_op_len):
                ...
                if (
                    ...
                    torch_op_apis[last_op_index][TorchOpDataOri.CONNECTION_ID].append(
                        node_launch_api[CannNodeLaunchApiOri.CORRELATION_ID]
                    )
                    last_op_api = None
                    break
    ...
    def init_db_connect(self) -> None:
        if not TorchDb().create_connect_db():
            raise RuntimeError(
                f"Failed to connect to db file: {TorchDb().get_db_path()}"
            )
    ...
    def save_string_ids(self):
        ...
        TorchDb().insert_data_into_table(
            DbConstant.TABLE_STRING_IDS, Str2IdManager().get_all_string_2_id_data()
        )
    ...
    def sava_connection_ids(self):
        ...
        TorchDb().insert_data_into_table(
            DbConstant.TABLE_CONNECTION_IDS, save_connection_ids
        )
    ...
    def save_callchain_ids(self):
        ...
        TorchDb().insert_data_into_table(
            DbConstant.TABLE_PYTORCH_CALLCHAINS, save_callchain_ids
        )
