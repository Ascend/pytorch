...
from torch_npu import _op_plugin_docs, profiler
...
from torch_npu.utils._error_code import _except_handler, ErrCode, pta_error
...
def _apply_distributed_methods_patch():
    ...
    torch.distributed.gather_object = (
        torch_npu.distributed.distributed_c10d._gather_object
    )
    ...
    torch.distributed._symmetric_memory.enable_symm_mem_for_group = (
        torch_npu.distributed._symmetric_memory._enable_symm_mem_for_group
    )
    torch.distributed.nn.functional._AllGatherBase.backward = (
        torch_npu.distributed.nn.functional._allgather_base_backward_hccl
    )
...
if _transfer_to_npu_env == "1":
    ...
    raise ValueError(
        f"Invalid value for TORCH_TRANSFER_TO_NPU: {_transfer_to_npu_env}. Only '0' or '1' is supported."
    )
