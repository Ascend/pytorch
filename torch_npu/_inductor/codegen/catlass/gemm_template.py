        ...
        **kwargs,
        ...
        if epilogue_nodes:
            ...
            try:
                ...
                log.debug(  # noqa: G200
                    "Cannot fuse epilogue nodes on-fly, reason: %s, will use EVG to fuse.",
                    e,
                )
...
class CATLASS1xGemmTemplate(CATLASSGemmTemplate):
    ...
    @staticmethod
    def is_mixed_template(op: "GemmKernelBase") -> bool:  # type: ignore[name-defined]  # noqa: F821
        return getattr(op, "is_mix", False)

    @staticmethod
    def epilogue_fusion_type(op: "GemmKernelBase") -> int:  # type: ignore[name-defined]  # noqa: F821
        fusion_type = 0
    ...
    def _are_inputs_layout_compatible(self, layouts: List[Layout]) -> bool:
        ...
        if len(B_layout.size) < 1:
            return False
        A_size = list(V.graph.sizevars.optimization_hints(A_layout.size, fallback=8192))
        B_size = list(V.graph.sizevars.optimization_hints(B_layout.size, fallback=8192))
        if len(A_size) < 2:
            A_size.insert(0, 1)
        if len(B_size) < 2:
            A_size.insert(1, 1)
        ...
        if "add" in used_ops:
            ...
            for name in read_names:
                ...
                if name in V.graph.name_to_buffer:
                    ...
                    raise KeyError(
                        f"Could not resolve buffer for name {name} (maybe removed)."
                    )
