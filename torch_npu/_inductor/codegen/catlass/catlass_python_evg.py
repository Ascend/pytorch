...
class CatlassEVGOpsMixIn:
    ...
    @staticmethod
    def _prefix_tri_op(op: str, a: str, b: str, c: str) -> str:
        return f"{op}({a}, {b}, {c})"
    ...
    def mul(x0: str, x1: str) -> str:
        return CatlassEVGOpsMixIn._infix_bin_op("*", x0, x1)
    ...
    @staticmethod
    def relu(x0: str) -> str:
        return CatlassEVGOpsMixIn._prefix_un_op("relu", x0)

    @staticmethod
    def sigmoid(x0: str) -> str:
        return CatlassEVGOpsMixIn._prefix_un_op("sigmoid", x0)
    ...
    @staticmethod
    def tanh(x0: str) -> str:
        raise NotImplementedError("tanh is not supported in CATLASS python evg")


class MockCatlassHandler(CatlassEVGOpsMixIn, WrapperHandler):
    """Passthrough handler for catlass ops, used for running epilogue nodes for memory planning"""


class _AssignmentFormatter(DefaultHandler):
    def __init__(self, parent_handler: "CatlassEVGCodegen"):
...
class CatlassEVGCodegen(CatlassEVGOpsMixIn):
    """
    Notes:
        * Used by CATLASSGemmTemplate.
        * This class should not be instantiated by users, it is intended to be used
            by calling CatlassEVGCodegen.ir_to_evg_python_code(...)
    ...
    def __init__(self, accumulator_node_name: str, removed_buffers: OrderedSet[str]):
        ...
        Initializes a CatlassEVGEpilogueArgumentFormatter object. Do not instantiate directly.
        Use the CatlassEVGCodegen.ir_to_evg_python_code static method.
    ...
    @staticmethod
    def ir_to_evg_python_code(
        npu_template_node_name: str,
        epilogue_nodes: list[BaseSchedulerNode],
        removed_buffers: OrderedSet[str],
    ) -> tuple[list[str], list[str], dict[str, Any], str]:
        codegen = CatlassEVGCodegen(npu_template_node_name, removed_buffers)
        ...
        with virtualized.V.set_ops_handler(handler):
            for s_node in epilogue_nodes:
                ...
                with codegen.set_cur_node(node):
                    index_vars = CatlassEVGCodegen.get_index_vars(node)
    ...
    def _check_indexing(self, name: str, index: sympy.Expr) -> None:
        ...
        # CATLASS doesn't support arbitrary indexing
        buffer_name = (
