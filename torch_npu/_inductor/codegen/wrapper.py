...
class _RuntimeHelperDefinitionsLine(DeferredLineBase):
    def __init__(self, definitions: IndentedBuffer, prefix: str = ""):
        super().__init__(prefix)
        self.definitions = definitions

    def __call__(self):
        source = self.definitions.getvalue()
        if not source:
            return None
        if not self.line:
            return source
        return "\n".join(
            f"{self.line}{line}" if line else line for line in source.splitlines()
        )

    def _new_line(self, line):
        return _RuntimeHelperDefinitionsLine(self.definitions, line)
...
class NPUPythonWrapperCodeGen(_NPUKernelCodegenMixin, PythonWrapperCodegen):
    ...
    def make_buffer_free(self, buffer: Union[BufferLike, TorchBindObject]):
        if is_multi_stream():
            ...
            if hasattr(buffer, "multi_stream_intent"):
                multi_stream_intent = " " * buffer.multi_stream_intent
            elif (
                self.buffer_args_multi_stream_intent is not None
                and buffer.get_name() in self.buffer_args_multi_stream_intent.keys()
            ):
        ...
        *,
        ...
        if is_multi_stream():
            ...
            if origin_node is not None and hasattr(origin_node, "multi_stream_name"):
                ...
                multi_stream_intent = " " * origin_node.multi_stream_intent
            else:
                multi_stream_intent = ""
                stream_name = PythonWrapperCodegen.write_get_raw_stream(
                    self, device.index, V.graph
            ...
            super().generate_kernel_call(
                kernel_name, call_args, device=device, triton=triton,
                arg_types=arg_types, raw_keys=raw_keys, raw_args=raw_args,
                triton_meta=triton_meta, inductor_meta=inductor_meta,
                original_fxnode_name=original_fxnode_name,
            )
    ...
    def make_buffer_reuse(self, old: BufferLike, new: BufferLike, delete_old: bool):
        if is_multi_stream():
            if not (old.get_dtype() == new.get_dtype()):
                raise AssertionError("old dtype not equals new dtype")
    ...
    def codegen_exact_buffer_reuse(self, old_name: str, new_name: str, del_line: str):
        if is_multi_stream():
            multi_stream_intent_str = self.get_buffer_define_multi_stream_by_name(new_name)
            return (
                f"{multi_stream_intent_str}{self.declare_maybe_reference}{new_name} = "
                f"{old_name}{del_line}{self.ending}  {self.comment} reuse"
            )
    ...
    def codegen_deferred_allocation(self, name: str, view: ir.ReinterpretView) -> None:
        if is_multi_stream():
            ...
            self.writeline(
                DeferredLine(
                    name,
                    f"{multi_stream_intent_str}{self.declare}{name} = "
                    f"{view.codegen_reference()}{self.ending}  {self.comment} alias",
                )
            )
        else:
    ...
    def handle_cross_stream_del_buf(self):
        ...
        for idx, line in enumerate(self.lines):
            ...
            for sub_stream_line in sub_streams_line_no:
                ...
                if (
                    idx > sub_stream_line[0] and idx < sub_stream_line[1]
                    and isinstance(line, WrapperLine) and hasattr(line, "node")
                    and line.node.get_name() not in self.buffer_args_multi_stream_intent.keys()
                ):
            ...
            for i in range(1, n):
                ...
                if (
                    prev_end < idx < curr_start
                    and isinstance(line, WrapperLine) and hasattr(line, "node")
                    and line.node.get_name() in self.buffer_args_multi_stream_intent.keys()
                ):
