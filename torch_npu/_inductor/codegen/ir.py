...
def detect_flattened_dims(kernel, index):
    ...
    def detect_flattened_axis(expr):
        ...
        if isinstance(expr, ModularIndexing):
            ...
            # Check if var is a simple symbol, otherwise recursively process it
            if isinstance(var, sympy.Symbol):
                init_new_vars(var, length)
                new_vars[var][length][1] = (expr, divisor, length)
            else:
                # var is a complex expression (e.g., s77**2), recursively process it
                detect_flattened_axis(var)
        elif isinstance(expr, FloorDiv):
            var, divisor = expr.args
            # Check if var is a simple symbol
            if isinstance(var, sympy.Symbol):
