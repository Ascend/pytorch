...
面向A5+PyTorch-v2.10.0，Inductor-Ascend在Triton后端precompile阶段提供CostModel预筛选能力。该能力通过Triton-Ascend CostModel后端对候选config进行静态性能预测，并将预测耗时较短的config优先送入后续编译和实测profiling流程，从而降低候选config数量较多时的首次编译开销。
