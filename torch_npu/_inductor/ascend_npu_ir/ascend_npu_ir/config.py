...
Add extra commands before model execute, code like:
...
"default" refers to the mode of fully compiling with MLIR. Currently, it is not
fully supported, but it will be set as the default once the capability matures.
"complete_fallback" refers to completely falling back to the eager execution mode
of the FX graph, without performing any MLIR compilation. It is primarily used
for debugging.
"auto_fallback" refers to automatically falling back to the fx_graph_backend when
compilation fails.
auto_fallback mechanism is designed to provide a fallback strategy when the
primary compilation process encounters an issue. It works in conjunction with
the fx_graph_backend configuration, allowing for the fallback approach:
Fallback to fx_graph_backend: If the first fallback attempt fails, the system
falls back to the fx_graph_backend.
...
def _get_compile_mode():
    ...
    if isinstance(m.compile_mode, str):
        ...
        raise ValueError(
            f"Please use the *str* type to set the compile mode, "
            f"current type is {type(compile_mode)=}"
        )
