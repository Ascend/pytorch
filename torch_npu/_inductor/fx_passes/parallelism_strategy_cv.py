...
class CVParallelismStrategy(ParallelStrategyBase):
    ...
    def extract_special_groups(
        self, group: List[BaseSchedulerNode], nodes: List[BaseSchedulerNode]
    ) -> tuple[List[BaseSchedulerNode], List[BaseSchedulerNode], List[BaseSchedulerNode]]:
        if not group:
            return [], [], []
            ...
            if curr_type == ComputeType.CUBE and not cube_part:
                while i < n:
                    if group[i].compute_unit == ComputeType.CUBE:
                        ...
                        i + 1 < n
                        and group[i].compute_unit == ComputeType.MLP_VECTOR
                        and group[i + 1].compute_unit == ComputeType.CUBE
                    ):
                        cube_part.append(group[i])
    ...
    def _get_longest_continuous_segment(
        self, candidates: List[BaseSchedulerNode], nodes: List[BaseSchedulerNode]
    ...
    def assign_parallel_groups(self, nodes: List[BaseSchedulerNode]) -> Dict[str, List[BaseSchedulerNode]]:
        ...
        log.info("cv parallel group len: %s", len(final_groups))
