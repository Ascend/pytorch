...
def _msc_build_node(graph, cat, run):
    ...
    except Exception as exc:  # a gap in matching; skip rather than emit a wrong graph
        log.debug("[inductor_fx] multi_slice_concat segment plan validation failed: %s", exc)
        return None
