...
def check_view(node):
    if (
        not check_op(node, torch.ops.aten._unsafe_view.default)
        and not check_op(node, torch.ops.aten.view.default)
        and not check_op(node, torch.ops.aten.reshape.default)
    ):
        return False
    return True
