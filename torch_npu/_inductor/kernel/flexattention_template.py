...
def forward_block_mn_sparse_mask(
    ...
    if not IS_FULL_BLOCKS:
        ...
        # Q_OFFSETS is laid out like KV_NUM_BLKS (there is no extra sentinel
        # row).  Use its actual strides so broadcasted batch/head metadata is
        # indexed correctly when the block mask has more than one head.
...
def sparse_mask_grid(*_args, **_kwargs):
    ...
    # ``TypeError: cannot determine truth value of Relational`` while the
    # graph is being scheduled.  Launch a fixed vector-core grid instead;
    # each program already strides over the runtime entry count, so extra
    # programs are harmless (including the zero-entry case).
    return (num_vector_cores, 1, 1)
...
_FWD_MASK_OUT_SIGNATURE = (
    ...
    '"KV_NUM_BLKS", "KV_IDX", "LSE", "MAX_SCORES", '
    '"FULL_KV_NUM_BLKS", "FULL_KV_IDX")}}'
)
_FWD_MASK_IN_SIGNATURE = (
    '{{def_kernel("Q", "K", "V", "KV_NUM_BLKS", "KV_IDX", "LSE", "MAX_SCORES", '
    '"FULL_KV_NUM_BLKS", "FULL_KV_IDX")}}'
)
...
{{def_kernel("Q", "K", "V", "LSE", "DELTA", "DO", "DQ", "SPARSE_MASK", "Q_OFFSETS", "SPARSE_MASK_BLOCK_POS", "KV_NUM_BLKS", "KV_IDX", "Q_NUM_BLKS", "Q_IDX", "FULL_KV_NUM_BLKS", "FULL_KV_IDX", "FULL_Q_NUM_BLKS", "FULL_Q_IDX")}}
    ...
    for task_id in range(pid, DQ_NUM_TASKS, num_core):
        ...
        # Q_OFFSETS follows KV_NUM_BLKS' [batch, metadata-head, q-block]
        # layout; index it with the metadata head rather than the compact
        # mask head-count specialization.
        q_offsets_idx = (
            sparse_idx_z * {{stride("Q_OFFSETS", 0)}}
            + sparse_h * {{stride("Q_OFFSETS", 1)}}
            + sparse_q_block * {{stride("Q_OFFSETS", 2)}}
        )
...
flex_attention_backward_dkdv_tasklist_source = (
    r"""
{{def_kernel("Q", "K", "V", "LSE", "DELTA", "DO", "DV", "DK", "SPARSE_MASK", "Q_OFFSETS", "SPARSE_MASK_BLOCK_POS", "KV_NUM_BLKS", "KV_IDX", "Q_NUM_BLKS", "Q_IDX", "FULL_KV_NUM_BLKS", "FULL_KV_IDX", "FULL_Q_NUM_BLKS", "FULL_Q_IDX", "WORK_ITEMS", "TASK_OFFSETS", "DK_PARTIAL", "DV_PARTIAL")}}
    stride_qz, stride_qh, stride_qm, stride_qd = {{stride("Q")}}
    stride_kz, stride_kh, stride_kn, stride_kd = {{stride("K")}}
    stride_vz, stride_vh, stride_vn, stride_vd = {{stride("V")}}
    stride_doz, stride_doh, stride_dom, stride_dod = {{stride("DO")}}
...
@SymbolicGridFn
def flex_decoding_npu_grid(batch_size, kv_heads, gqa_group_size, n_keys, d_model, meta):
    return (batch_size * kv_heads, meta["SPLIT_KV"], 1)


flex_decoding_npu_source = r"""
    ...
    # Load Q directly as a flat [BLOCK_M, D] tile. On Ascend, the original
    # [G, BLOCK_M_PER_HQ, D] load followed by reshape produces wrong values
    # for later query heads in a GQA group.
    q_rows = tl.arange(0, BLOCK_M)
    q_group = q_rows // BLOCK_M_PER_HQ
    q_m = q_rows % BLOCK_M_PER_HQ
    q_range = (
        q_group[:, None] * stride_qg
        + q_m[:, None] * stride_qm
        + offs_d[None, :] * stride_qk
    )
    if not SAFE_M_BOUNDARY and not SAFE_HEAD_DIM:
        q_mask = (q_m[:, None] < Q_LEN) & (offs_d[None, :] < QK_HEAD_DIM)
        q = tl.load(Q + q_offset + q_range, mask=q_mask, other=0.0)
    elif SAFE_M_BOUNDARY and not SAFE_HEAD_DIM:
        q = tl.load(Q + q_offset + q_range, mask=offs_d[None, :] < QK_HEAD_DIM, other=0.0)
    elif not SAFE_M_BOUNDARY and SAFE_HEAD_DIM:
        q = tl.load(Q + q_offset + q_range, mask=q_m[:, None] < Q_LEN, other=0.0)
    else:
    ...
    ml_mask = off_m[None, :] < Q_LEN
    m_offsets = (
        off_hkv * G * stride_mh
        + off_g[:, None] * stride_mh
        + off_m[None, :] * stride_mm
    )
    l_offsets = (
        off_hkv * G * stride_lh
        + off_g[:, None] * stride_lh
        + off_m[None, :] * stride_lm
    )
    tl.store(M + m_offset + m_offsets, m_i, mask=ml_mask)
    tl.store(L + l_offset + l_offsets, l_i, mask=ml_mask)
...
flex_decoding_npu = NPUTritonTemplate(
    name="flex_decoding_npu",
    grid=flex_decoding_npu_grid,
    source=flex_decoding_npu_source,
)
