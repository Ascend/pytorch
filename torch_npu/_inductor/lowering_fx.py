...
import sympy
import torch._ops
from sympy.core import Expr, Integer, Symbol  # noqa: F401
from torch._inductor import ir
from torch._inductor import lowering
from torch._inductor import scheduler
from torch._inductor.fx_passes.post_grad import view_to_reshape
from torch._inductor.ir import (  # noqa: F401
    ExpandView,
    is_triton,
    ops_wrapper,
    PermuteView,
...
from torch._inductor.lowering import mul, new_empty
from torch._inductor.scheduler import (  # noqa: F401
    Dep,
    WeakDep,
    Scheduler,
    SchedulerNode,
...
def create_fx_from_snodes_by_traced_graph(snodes: List[scheduler.SchedulerNode]):
    ...
    try:
        ...
        log.warning("Could not rebuild fx graph for %s, reason: %s", snodes, e)  # noqa: G200
...
def create_compile_kwargs(final_kernel, fx_call_args, fx_args):
    ...
    if kernel_call_args_str_set != set(fx_call_args_no_dy):
        ...
        if not scalar_only_mismatch:
            log.warning("[ACC-DEBUG] kernel=%s skipped: buffer mismatch (fx_only=%s)",
                        final_kernel.kernel_name, fx_only)
            return None
    ...
    for i, idx in enumerate(call_args_mapping):
        ...
        if found is None:
            ...
            log.warning("[ACC-DEBUG] kernel=%s skipped: cannot resolve scalar at pos=%s from any tensor shape",
                        final_kernel.kernel_name, i)
            return None
...
def run():
    ...
    fx_inputs = [
        fx_args[idx].contiguous() if idx in non_contiguous_indices['inputs']
        else fx_args[idx]
...
def dump_fx_graph_code(code, dump_path, traced_graph_hash):  # noqa: F811
    py_path = os.path.join(dump_path, traced_graph_hash + '.py')
...
def clone(x, *, memory_format=None):  # noqa: F811
    # TODO(jansel): memory format
    input_graphs = fetch_graphs(x)
    node_name = f'clone_{next(node_id)}'
    new_graph = merge_traced_graphs(input_graphs, aten.clone, node_name)
...
def _register_npu_inductor_fallbacks_fx(make_reduction):
    ...
    def expand(x, sizes):
        ...
        if not free_unbacked_symbols(x.get_size()):
            x_size_product = V.graph.sizevars.optimization_hint(sympy_product(x.get_size()))
            ...
            if x_size_product > 0 and not free_unbacked_symbols(sizes):
                ...
                x.mark_reuse(
                    V.graph.sizevars.optimization_hint(sympy_product(sizes)) // x_size_product
                )
    ...
    def repeat(x, repeats):
        ...
        old_size_product = V.graph.sizevars.optimization_hint(sympy_product(old_size))
        if old_size_product > 0:
            # maybe realize the input
            x.mark_reuse(
                V.graph.sizevars.optimization_hint(sympy_product(new_size)) // old_size_product
            )
    ...
    def empty_strided(
        ...
        if not isinstance(size, (list, tuple)):
            raise RuntimeError("assert Expected list or tuple")
        if stride is not None and not isinstance(stride, (list, tuple)):
            raise RuntimeError("assert Expected list or tuple or None")
        lowering.assert_nyi(not pin_memory, "pin_memory")
        lowering.assert_nyi(layout in (None, torch.strided), f"layout={layout}")
        ...
        if not isinstance(buffer, ir.ComputedBuffer):
            raise RuntimeError("assert Expected ir.ComputedBuffer")
        size = [sympy.expand(s) for s in size]
        stride = (
