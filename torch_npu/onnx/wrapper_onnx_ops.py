...
class _NPUFusedInferAttentionScoreOP(torch.autograd.Function):
    ...
    def symbolic(g, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        ...
        return g.op("npu::NPUFusedInferAttentionScoreOP", self, query, key, value,
                    ...
                    key_shared_prefix, value_shared_prefix, actual_shared_prefix_len, query_rope, key_rope,
...
class _NPUMoeInitRoutingOP(torch.autograd.Function):
...
def _wrapper_npu_fused_infer_attention_score(self, query, key, value, pse_shift, atten_mask, actual_seq_lengths, actual_seq_lengths_kv,
                                            dequant_scale1, quant_scale1, dequant_scale2, quant_scale2, quant_offset2,
                                            antiquant_scale,
...
def _wrapper_npu_moe_init_routing(x, row_idx, expert_idx, active_num):
    return _NPUMoeInitRoutingOP.apply(x, row_idx, expert_idx, active_num)
...
def _add_onnx_ops():
    ...
    torch_npu.npu_fused_infer_attention_score = _wrapper_npu_fused_infer_attention_score
    ...
    torch_npu.npu_moe_init_routing = _wrapper_npu_moe_init_routing
