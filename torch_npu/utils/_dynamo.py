...
def patch_inductor_wrapper():
    ...
    def new_apply_options(self, options: Optional[dict[str, Any]]):
        if (
            options is not None
            and "npu_backend" in options
            and not isinstance(options["npu_backend"], str)
        ):
            backend_type = type(options["npu_backend"]).__name__
            raise RuntimeError(
                "Unexpected type of attr npu_backend, "
                f"got {backend_type} should be str"
            )
        src_apply_options(self, options)
        # Shape handling is installed in new_call, after the selected backend
        # scope has loaded the matching NPU Inductor backend.
...
def add_dynamo_methods_init():
    steps = (
        ...
        ("variable_builder", patch_variable_builder),
