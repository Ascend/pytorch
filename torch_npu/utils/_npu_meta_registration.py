...
def _add_op_to_meta_table(op, fn, avoid_fallback_flag=False, inductor_decomp=False):
    ...
    for op_overload in overloads:
        if op_overload in npu_meta_table:
            raise RuntimeError(
                f"duplicate registrations for npu_meta_table {op_overload}"
            )
...
def patch_torch_inductor_decompositions():
    ...
    for op_overload in inductor_decomp_table:
        if op_overload in npu_meta_table:
            inductor_decomposition.decompositions[op_overload] = npu_meta_table[
                op_overload
            ]
...
def patch_torch_decomp_decompositions():
    ...
    def torch_decomp_decompositions_new(func):
        if func in npu_meta_table:
            return True
        return src_func(func)

    _subclasses_fake_tensor.torch_decomp_decompositions = (
        torch_decomp_decompositions_new
    )
...
def meta_native_dropout_patch(tensor_input: Tensor, p: float, train: bool | None):
