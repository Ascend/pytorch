...
def _register_npu_cp_sharding_rules() -> None:
    """Register NPU CP sharding rules and keep handles for restoring them."""
    global _npu_cp_strategy_contexts
    ...
    from torch.distributed.tensor.experimental._context_parallel._sharding_rules import (
        _op_strategy_context,
    )

    npu_cp_strategies = [
        (_npu_fa, _scaled_dot_product_attention_cp_strategy, RuntimeSchemaInfo(1)),
    ]
