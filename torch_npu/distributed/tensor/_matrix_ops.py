...
def npu_grouped_matmul_strategy(mesh: DeviceMesh, op_schema: OpSchema) -> OpStrategy:
    ...
    for key in unsupported_arguments:
        ...
        if schema is not None and isinstance(schema, TupleStrategy) and len(schema.childs) > 0:
            full_mesh_strategies = expand_to_full_mesh_op_strategy(
                mesh, op_schema, strategies, input_index=y_num
            )
            if y_num == 1:
    ...
    full_mesh_strategies = expand_to_full_mesh_op_strategy(mesh, op_schema, strategies,
                                                           input_index=y_num)
...
def npu_apply_adam_w_strategy(mesh: DeviceMesh, op_schema: OpSchema) -> OpStrategy:
    # npu_apply_adam_w(
...
def custom_dropout_forward_sharding(mesh: DeviceMesh, op_schema: OpSchema):
    ...
    if input_spec.placements[0].is_shard():
        ...
        if input_spec.shape[shard_dim] < input_spec.mesh.size(0):
            ...
            output_strategy = OpStrategy([
                PlacementStrategy(output_specs=output_target_specs, input_specs=input_target_specs)
            ])
            return output_strategy
...
def custom_dropout_backward_sharding(mesh: DeviceMesh, op_schema: OpSchema) -> OpStrategy:
    input_target_specs = []
    ...
    output_strategy = OpStrategy([
        PlacementStrategy(output_specs=op_schema.args_schema[0].strategies[0].output_spec, input_specs=input_target_specs)
    ])
