...
def _all_gather_sharded_tensor(
    ...
    npu_device = torch.device("npu", torch_npu.npu.current_device())
    if shards:
        local_tensor = shards[0].tensor.flatten()
        if not local_tensor.is_npu:
            move_to_cpu = torch.ones(1, device=npu_device)
            local_tensor = local_tensor.npu()
        else:
            move_to_cpu = torch.zeros(1, device=npu_device)
        num_padding = chunk_size - local_tensor.numel()
        if num_padding > 0:
            local_tensor = F.pad(local_tensor, [0, num_padding])
    else:
        local_tensor = torch.zeros(
            chunk_size, dtype=sharded_tensor.dtype, device=npu_device
        )
        move_to_cpu = torch.zeros(1, device=npu_device)
...
def _gather_state_dict(
    ...
    for key, tensor in state_dict.items():
        if isinstance(tensor, ShardedTensor):
            ...
            if tensor.local_shards() and tensor.local_shards()[0].tensor.is_npu:
