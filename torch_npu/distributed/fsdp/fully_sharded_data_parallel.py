...
from torch_npu.distributed.algorithms._comm_hooks import (
    LOW_PRECISION_HOOKS,
    default_hooks,
)
from .utils import (
...
class CPUOffload:
    ...
    offload_params: Offloading parameters to CPUs when these parameters are
                    not used for computation on NPU. This implicitly enables
...
class BackwardPrefetch(Enum):
    ...
    BACKWARD_PRE: prefetch right before current layer's backward computation
                  ...
                  the prefetched full parameters will be kept in the NPU memory
...
class FullStateDictConfig(StateDictConfig):
    ...
    context manager as follows:
        ...
        >>> fsdp = FSDP(model, device_id=torch.npu.current_device(), auto_wrap_policy=..., sync_module_states=True)
...
class FullyShardedDataParallel(nn.Module):
    ...
    .. warning::
        If the destination NPU device has ID ``dev_id``, either (1)
        ``module`` should already be placed on that device, (2) the device
        should be set using ``torch_npu.npu.set_device(dev_id)``, or (3)
        ...
        device. For (1) and (3), the FSDP initialization always occurs on NPU.
    ...
    .. warning::
        ...
        on NPU, or to use ``device_id`` argument to specify a NPU device that
    ...
    .. note::
        ...
        not have to manually move inputs from CPU -> NPU.
    ...
    Args:
        ...
        param_init_fn (Optional[Callable[[nn.Module], None]]):
            ...
            Example::
                ...
                >>> print(next(fsdp_model.parameters()).device) # current NPU device
                >>> # With torchdistX
                >>> module = deferred_init.deferred_init(MyModule, device="npu")
                >>> # Will initialize via deferred_init.materialize_module().
                >>> fsdp_model = FSDP(module, auto_wrap_policy=size_based_auto_wrap_policy)

        device_id (Optional[Union[int, torch.device]]): An ``int`` or ``torch.device``
            describing the NPU device the FSDP module should be moved to determining where
            ...
            different NPU device, an error will be thrown. (Default: ``None``)
        ...
        limit_all_gathers (bool): If ``False``, then FSDP allows the CPU
            ...
            help lower the number of NPU malloc retries.
    """
    ...
    def __init__(
        ...
        torch.nn.Module._load_state_dict_post_hooks = OrderedDict()
    ...
    def _get_device_from_device_id(
        ...
        if device == torch.device("npu"):
            warnings.warn(
                ...
                f"FSDP will use the current device {torch_npu.npu.current_device()}. "
                "If this is incorrect, please explicitly call `torch_npu.npu.set_device()` "
    ...
    def _materialize_module(
        ...
        if (
            ...
            materialization_device = device_from_device_id or torch_npu.npu.current_device()
    ...
    def _move_module_to_device(
        ...
        if device_from_device_id is not None:
            ...
            warnings.warn(
                ...
                " run on CPU, which is less efficient than on NPU. We recommend passing in "
                "`device_id` argument which will enable FSDP to put module on NPU device,"
                " module must also be on NPU device to work with `sync_module_states=True` flag"
                " which requires NPU communication."
            )
    ...
    def _get_compute_device(
        ...
        is always a NPU device with its explicit index.

        Precondition: ``_check_single_device_module()`` and
        ``_move_module_to_device()``.
        """
        # If the module is on NPU already, then that NPU device has priority
        # over the current device
        param = next(self._get_orig_params(module, ignored_params), None)
        if param is not None and param.device.type == "npu":
            compute_device = param.device
        else:
            compute_device = torch.device("npu", torch_npu.npu.current_device())
        if (
    ...
    def _sync_module_states(
        ...
        if params and any(param.device == torch.device("cpu") for param in params):
            raise ValueError(
                ...
                "This only works for NPU module, please specify `device_id` argument or move"
    ...
    def _unshard(
        ...
        with torch_npu.npu.stream(self._streams["pre_all_gather"]):
        ...
        with torch_npu.npu.stream(self._streams["all_gather"]):
            for handle in handles:
                handle.unshard()
                handle.post_unshard()
    ...
    def _reshard(
        ...
        for handle, free_unsharded_flat_param in zip(
            ...
            if self.limit_all_gathers and free_unsharded_flat_param:
                free_event = torch_npu.npu.Event()
                free_event.record()
    ...
    def _lazy_init(self) -> None:
        ...
        if not torch_npu.npu.is_available():
            # Allow the FSDP constructor to run even with NPU but check this
    ...
    def _init_streams(self) -> None:
        ...
        self._streams["post_backward"] = torch_npu.npu.Stream()
        # Stream for pre-all-gather copies (e.g. H2D or precision cast).
        self._streams["pre_all_gather"] = torch_npu.npu.Stream()
            ...
            if expected_state_dict_config_type != type(state_dict_config):
                raise RuntimeError(
                    f"Expected state_dict_config of type {expected_state_dict_config_type} \
                        but got {type(state_dict_config)}"
                )
    ...
    def _full_post_state_dict_hook(
        ...
        # the original buffer's NPU memory and require reallocating
        # that memory later; this only affects the state dict's buffer
        # variable and leaves the original buffer's NPU memory intact
        if offload_to_cpu:
        ...
        with self._summon_full_params(recurse=False, writeback=False):
            for fqn, _, _ in self._param_fqns:
                ...
                state_dict[f"{prefix}{fqn}"] = _ext_chunk_tensor(
                    ...
                    num_devices_per_node=torch_npu.npu.device_count(),
    ...
    def state_dict(self, *args, **kwargs):
        ...
        fit on a single NPU. In that case, users may pass in a
    ...
    def _sharded_pre_load_state_dict_hook(
        ...
        for (param_name, _, module_name) in self._fsdp_wrapped_module.handle.flat_param._param_infos:
            ...
            if shards:
                ...
                if not local_tensor.is_npu:
                    local_tensor = local_tensor.npu()
                num_padding = chunk_size - local_tensor.numel()
                if num_padding > 0:
                    local_tensor = F.pad(local_tensor, [0, num_padding])
            else:
                local_tensor = torch.zeros(chunk_size, dtype=param.dtype).npu()
            tensor = torch.empty(
    ...
    def load_state_dict(
        ...
        in NPU OOM. As a result, :func:`state_dict_type` API is available to
    ...
    def forward(self, *args: Any, **kwargs: Any) -> Any:
        ...
        with torch.autograd.profiler.record_function("FullyShardedDataParallel.forward"):
            ...
            free_unsharded_flat_params = [
                handle._config.sharding_strategy == HandleShardingStrategy.FULL_SHARD
                for handle in self._handles
            ]
    ...
    def _pre_forward(
        ...
        Args:
            ...
            input_t (Any): Unused; expected by the hook signature.
        """
    ...
    def _pre_forward_unshard(
        ...
        if handles:
            ...
            torch_npu.npu.current_stream().wait_stream(self._streams["all_gather"])
    ...
    def _post_forward(
        ...
        Args:
            ...
            module (nn.Module): Unused; expected by the hook signature.
            input_t (Any): Unused; exepcted by the hook signature.
    ...
    def summon_full_params(
        ...
        .. warning:: Note that ``offload_to_cpu`` and ``rank0_only=False`` will
            ...
            NPUs that reside on the same machine, which may incur the risk of
    ...
    def _post_backward_hook(
        ...
        with torch.autograd.profiler.record_function(
            ...
            self._streams["post_backward"].wait_stream(torch_npu.npu.current_stream())

            with torch_npu.npu.stream(self._streams["post_backward"]):
                ...
                if handle._config.offload_params:
                    ...
                    grad.data.record_stream(torch_npu.npu.current_stream())
    ...
    def _cast_grad_to_param_dtype(
        ...
        if (
            ...
            low_prec_grad_data.record_stream(torch_npu.npu.current_stream())
    ...
    def _wait_for_post_backward(self) -> None:
        ...
        if self._sync_gradients:
            torch_npu.npu.current_stream().wait_stream(self._streams["post_backward"])
            if self.cpu_offload.offload_params:
                # We need to wait for the non-blocking NPU ->
                # CPU grad transfers to finish. We need to do this for NPU -> CPU
                # copies because when grad is on CPU, it won't wait for any NPU
                # stream to finish NPU -> CPU copies unless we explicitly block the
    ...
    def clip_grad_norm_(
        ...
        local_norm = _calc_grad_norm(self.params_with_grad, norm_type_float).npu()  # type: ignore[arg-type]
        if norm_type_float == math.inf:
            total_norm = local_norm
            dist.all_reduce(total_norm, op=torch.distributed.ReduceOp.MAX, group=self.process_group)
        else:
            total_norm = local_norm ** norm_type_float
            dist.all_reduce(total_norm, group=self.process_group)
            total_norm = total_norm ** (1.0 / norm_type_float)
    ...
    def shard_full_optim_state_dict(
        ...
        .. note:: Both :meth:`shard_full_optim_state_dict` and
            ...
            where rank 0 moves each shard to NPU memory (for NCCL) and
    ...
    def scatter_full_optim_state_dict(
        ...
        # Check for a valid broadcast device, preferring NPU when available
        using_nccl = dist.distributed_c10d._check_for_nccl_backend(group)
        broadcast_device = torch.device("npu") if torch_npu.npu.is_available() \
            else torch.device("cpu")
        if using_nccl and not torch_npu.npu.is_available():
            raise RuntimeError("NCCL requires a NPU for collectives")
    ...
    def register_comm_hook(self, state: object, hook: callable):
        ...
        Args:
            ...
            hook (Callable): Callable, which has one of the following signatures:
                            ...
                            Callables with signature 1 are expected to handle gradient communication for a `NO_SHARD`
                            case.
...
def _calc_grad_norm(parameters: List[torch.nn.Parameter], p: float) -> torch.Tensor:
    ...
    if p == math.inf:
        ...
        local_norm = torch.norm(
            torch.stack(
                [
                    torch.norm(par.grad.detach().view(-1), p, dtype=torch.float32)
