...
#define TORCH_NPU_ENUM_OFFSET(new_name, old_name) \
  new_name = static_cast<int>(old_name) + g_toAclOffset,

#define ENUM_OFFSET(new_name, old_name)                                                   \
  _Pragma(                                                                                \
      "GCC warning \"'ENUM_OFFSET' is deprecated, use 'TORCH_NPU_ENUM_OFFSET' instead\"") \
...
enum class DType {
  UNDEFINED = -1,
  TORCH_NPU_ENUM_OFFSET(FLOAT, ACL_FLOAT) TORCH_NPU_ENUM_OFFSET(
      FLOAT16,
      ACL_FLOAT16) TORCH_NPU_ENUM_OFFSET(INT8, ACL_INT8)
      TORCH_NPU_ENUM_OFFSET(INT32, ACL_INT32) TORCH_NPU_ENUM_OFFSET(
          UINT8,
          ACL_UINT8) TORCH_NPU_ENUM_OFFSET(INT16, ACL_INT16)
          TORCH_NPU_ENUM_OFFSET(UINT16, ACL_UINT16) TORCH_NPU_ENUM_OFFSET(
              UINT32,
              ACL_UINT32) TORCH_NPU_ENUM_OFFSET(INT64, ACL_INT64)
              TORCH_NPU_ENUM_OFFSET(UINT64, ACL_UINT64) TORCH_NPU_ENUM_OFFSET(
                  DOUBLE,
                  ACL_DOUBLE) TORCH_NPU_ENUM_OFFSET(BOOL, ACL_BOOL)
                  TORCH_NPU_ENUM_OFFSET(
                      STRING,
                      ACL_STRING) TORCH_NPU_ENUM_OFFSET(COMPLEX64, ACL_COMPLEX64)
                      TORCH_NPU_ENUM_OFFSET(
                          COMPLEX128,
                          ACL_COMPLEX128) TORCH_NPU_ENUM_OFFSET(BF16, ACL_BF16)
                          TORCH_NPU_ENUM_OFFSET(
                              INT4,
                              ACL_INT4) TORCH_NPU_ENUM_OFFSET(UINT1, ACL_UINT1)
