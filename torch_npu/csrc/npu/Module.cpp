...
void RegisterNpuPluggableAllocator(PyObject* module)
{
    ...
    py::class_<
        ...
        [](torch::npu::NPUPluggableAllocator::NPUPluggableAllocator& self,
            uint64_t func_ptr) {
            using FuncType = c10_npu::NPUCachingAllocator::SnapshotInfo();
            std::function<FuncType> func =
                reinterpret_cast<FuncType*>(func_ptr);
...
{
    HANDLE_TH_ERRORS
    THPUtils_assert(THPUtils_checkString(args), "invalid value of module, module must be string", PTA_ERROR(ErrCode::PARAM));
