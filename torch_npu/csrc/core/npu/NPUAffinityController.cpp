...
const std::unordered_map<ThreadType, std::string> threadTypeToNameMap = { { releaseThread, "release_thread" },
...
const std::unordered_map<std::string, ThreadType> threadNameToTypeMap = { { "release_thread", releaseThread },
...
std::string GetAffinityMapAsString(const std::unordered_map<ThreadType, coreIdRange> &threadToCoreidMap,
    c10::DeviceIndex device_id)
{
    ...
    oss << "threadToCoreidMap plan to bind device " << static_cast<unsigned int>(device_id) << " to "
        ...
        << " [" << threadToCoreidMap.at(hcclCommWatchdogThread).start << "," <<
        threadToCoreidMap.at(hcclCommWatchdogThread).end << "]";
...
std::unordered_map<ThreadType, coreIdRange> GetCpuAffinityMap(c10::DeviceIndex device_id)
{
    ...
    std::initializer_list<ThreadType> thread_types = { unknownThread, mainThread,    backwardThread,
        aclThread,     releaseThread, hcclCommWatchdogThread };
    ...
    if (core_nums < thread_types.size()) {
        ASCEND_LOGW("Available core numbers (%d) are insufficient for all %zu thread types. Binding available cores to "
                    "all threads.",
        ...
        for (auto thread_type : thread_types) {
            if (thread_type == ThreadType::unknownThread) {
                threadToCoreidMap[ThreadType::unknownThread] =
                    coreIdRange{ current_core_range.start + remaining_type_count, current_core_range.end };
            } else {
    ...
    ASCEND_LOGD("Thread affinity map for device %d: %s", device_id,
        GetAffinityMapAsString(threadToCoreidMap, device_id).c_str());
                    ...
                    if (device_id < device_nums) {
                        ...
                        if (dashPos != std::string::npos) {
                            ...
                            if (isAllDigits(startStr) && isAllDigits(endStr)) {
                                ...
                                ASCEND_LOGW("core range is %s-%s, should be all digits", startStr.c_str(),
                                    endStr.c_str());
                            }
                        } else {
                            if (isAllDigits(value)) {
