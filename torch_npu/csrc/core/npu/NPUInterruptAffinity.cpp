...
std::string getPciAddress(int physical_id) {
  ...
  ASCEND_LOGD(
      "Physical device-%d: PCIe fields: vendor=0x%x subvendor=0x%x "
...
bool bindIrqAffinity(int device_id, const CoreIdList& irq_cores) {
  ...
  // In non-container scenarios, the physical device ID is the same as the
  // logical device ID.
...
void stopIrqbalance() {
  ...
  if (ret == 0) {
    ...
    if (stop_ret == 0) {
      ASCEND_LOGW(
          "To bind IRQ affinity, the irqbalance service has been stopped successfully. "
