...
class AsyncBatchCopyTask {
 public:
  ...
  void LaunchBatchCopyTask(
      ...
      size_t numAttrs,
      aclrtStream stream);
  QueueParas LaunchBatchCopyTaskEnQueue(
      const string& op_name,
      const PROC_FUNC& acl_call);
};
...
void AsyncCopyTask::LaunchCopyTask() {
  ...
  if (!cur_stream.isSyncLaunchStream() &&
    ...
    if (c10_npu::acl::AclrtMemcpyAsyncWithConditionExist() &&
        copyParam_.kind == aclrtMemcpyKind::ACL_MEMCPY_DEVICE_TO_HOST) {
      NPU_CHECK_ERROR(c10_npu::acl::AclrtMemcpyAsyncWithCondition(
          copyParam_.dst,
          copyParam_.dstLen,
          copyParam_.src,
          copyParam_.srcLen,
...
QueueParas AsyncBatchCopyTask::LaunchBatchCopyTaskEnQueue(
    const string& op_name,
    const PROC_FUNC& acl_call) {
  ...
  QueueParas params(
      EXECUTE_OPAPI_V2,
      sizeof(at_npu::native::ExecuteParasOpApiV2),
      &execParams);
  c10_npu::enCurrentNPUStream(&params);
  return params;
}

void AsyncBatchCopyTask::LaunchBatchCopyTask(
    void** dsts,
    size_t* dstLens,
    void** srcs,
    size_t* srcLens,
  ...
  if (!cur_stream.isSyncLaunchStream() &&
    ...
    auto attrs_vec =
        std::vector<aclrtMemcpyBatchAttr>(attrs, attrs + numBatches);
    auto attrsIndexes_vec =
        std::vector<size_t>(attrsIndexes, attrsIndexes + numBatches);
    auto acl_call = [dsts_vec,
                     dstLens_vec,
                     srcs_vec,
                     srcLens_vec,
                     numBatches,
      ...
      return c10_npu::acl::AclrtMemcpyBatchAsync(
          dsts,
          dstLens,
          srcs,
          srcLens,
    ...
    QueueParas params =
        LaunchBatchCopyTaskEnQueue("aclrtMemcpyBatchAsync", acl_call);
#ifndef BUILD_LIBTORCH
    at_npu::native::NpuUtils::ProfReportMarkDataToNpuProfiler(
        1, batch_copy_paras, params.correlation_id);
#endif
  } else {
    size_t failIdx = SIZE_MAX;
    NPU_CHECK_ERROR(c10_npu::acl::AclrtMemcpyBatchAsync(
        dsts,
        dstLens,
        srcs,
        srcLens,
...
void EventTask::LaunchRecordTask(
  ...
  if (!npuStream.isSyncLaunchStream() &&
      c10_npu::option::OptionsManager::GetTaskQueueEnable()) {
#ifndef BUILD_LIBTORCH
    at_npu::native::NpuUtils::ProfReportMarkDataToNpuProfiler(
...
#ifndef BUILD_LIBTORCH
    at_npu::native::NpuUtils::ProfReportMarkDataToNpuProfiler(
        1, EventParas::EVENT_PARAS_MAP[RECORD_EVENT], prof_correlation_id);
#endif
  } else {
    if (flags == ACL_EVENT_EXTERNAL &&
        c10_npu::acl::IsExistValueWaitAndWrite()) {
      NPU_CHECK_ERROR(
          c10_npu::acl::AclrtValueWrite(eventParam_.event, 1, npuStream));
      ASCEND_LOGI(
          "External Event: aclrtValueWrite is successfully executed, stream=%p, event=%p",
...
void EventTask::LaunchWaitTask(
  ...
  if (!npuStream.isSyncLaunchStream() &&
    ...
    uint64_t prof_correlation_id = 0;
    {
      ...
      QueueParas params(eventType, sizeof(EventParas), &eventParam_);
      c10_npu::NPUEventManager::GetInstance().IncreaseUnwaitedCount(
          eventParam_.event);
      c10_npu::enCurrentNPUStream(&params);
      prof_correlation_id = params.correlation_id;
    }
...
#ifndef BUILD_LIBTORCH
    at_npu::native::NpuUtils::ProfReportMarkDataToNpuProfiler(
        1, EventParas::EVENT_PARAS_MAP[WAIT_EVENT], prof_correlation_id);
#endif
  } else {
    if (flags == ACL_EVENT_EXTERNAL &&
        c10_npu::acl::IsExistValueWaitAndWrite()) {
      NPU_CHECK_ERROR(
          c10_npu::acl::AclrtValueWait(eventParam_.event, npuStream));
      NPU_CHECK_ERROR(
          c10_npu::acl::AclrtValueWrite(eventParam_.event, 0, npuStream));
      ASCEND_LOGI(
          "External Event: aclrtValueWait and aclrtValueWrite is successfully executed, stream=%p, event=%p",
          npuStream.stream(false),
          eventParam_.event);
    } else {
