  ...
  if (!cur_stream.isSyncLaunchStream() && c10_npu::option::OptionsManager::GetTaskQueueEnable()) {
    ...
    if (c10_npu::acl::AclrtMemcpyAsyncWithConditionExist() &&
      ...
      NPU_CHECK_ERROR(c10_npu::acl::AclrtMemcpyAsyncWithCondition(
          copyParam_.dst, copyParam_.dstLen, copyParam_.src, copyParam_.srcLen, copyParam_.kind, stream));
    } else {
...
void AsyncBatchCopyTask::LaunchBatchCopyTask(
  ...
  if (!cur_stream.isSyncLaunchStream() && c10_npu::option::OptionsManager::GetTaskQueueEnable()) {
    ...
    auto acl_call =
        [dsts_vec, dstLens_vec, srcs_vec, srcLens_vec, numBatches, attrs_vec, attrsIndexes_vec, numAttrs, stream]()
        -> int {
      ...
      return c10_npu::acl::AclrtMemcpyBatchAsync(
          dsts, dstLens, srcs, srcLens, numBatches, attrs, attrsIndexes, numAttrs, &failIdx, stream);
    };
...
aclError LaunchRecordEventTask(aclrtEvent event, c10_npu::NPUStream npuStream, unsigned int flags) {
  ...
  if (C10_UNLIKELY(trigger)) {
    trigger->traceNpuEventRecord(
        reinterpret_cast<uintptr_t>(event), reinterpret_cast<uintptr_t>(npuStream.stream(false)));
  }
#endif
  return ACL_ERROR_NONE;
}
...
#ifndef BUILD_LIBTORCH
    at_npu::native::NpuUtils::ProfReportMarkDataToNpuProfiler(
        1, EventParas::EVENT_PARAS_MAP[LAZY_DESTROY_EVENT], params.correlation_id);
#endif
  } else {
