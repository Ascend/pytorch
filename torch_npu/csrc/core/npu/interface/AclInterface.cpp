...
aclError AclrtCreateStreamWithConfig(aclrtStream *stream, uint32_t priority, uint32_t flag) {
  ...
  if (ret == ACL_SUCCESS && stream != nullptr) {
    ...
    if (!c10_npu::IsSupportInfNan()) {
      TORCH_CHECK(AclrtSetStreamOverflowSwitch(*stream, 1) == ACL_SUCCESS, "SET StreamOverflowSwitch Failed.", PROF_ERROR(ErrCode::NOT_FOUND));
    }
...
aclError AclrtSynchronizeStreamWithTimeout(aclrtStream stream) {
  ...
  if (func != nullptr) {
    ...
    TORCH_NPU_WARN_ONCE(func, "Failed to find function", "aclrtSynchronizeStreamWithTimeout", PROF_ERROR(ErrCode::NOT_FOUND));
...
bool can_device_access_peer(c10::DeviceIndex device_id, c10::DeviceIndex peer_device_id) {
  ...
  TORCH_CHECK(device_id >= 0 && device_id < num_npus, PROF_ERROR(ErrCode::PARAM));
  TORCH_CHECK(peer_device_id >= 0 && peer_device_id < num_npus, PROF_ERROR(ErrCode::PARAM));
