  ...
  if (ret == ACL_SUCCESS && stream != nullptr) {
    ...
    static bool forceOverflowCheck = []() -> bool {
      ...
      if (!IsGteCANNVersion("9.1.0")) {
        ASCEND_LOGW(
            "FORCE_OVERFLOW_CHECK=1 requires CANN >= 9.1.0, current CANN "
...
aclError AclrtMallocPhysical(aclrtDrvMemHandle* handle, size_t size, const aclrtPhysicalMemProp* prop, uint64_t flags) {
  ...
  if (ret != ACL_RT_SUCCESS && (prop->memAttr == ACL_HBM_MEM_HUGE1G)) {
    ...
    aclrtPhysicalMemProp prop_update = {
        prop->handleType,
        prop->allocationType,
        ACL_HBM_MEM_HUGE,
...
aclError AclrtSynchronizeDeviceWithTimeout(void) {
  ...
  if (func != nullptr) {
    ...
    if (timeout > 0) {
      TORCH_NPU_WARN_ONCE(
          "The ACL_DEVICE_SYNC_TIMEOUT does not take effect. If you want to enable this env, please upgrade CANN to the matching version.");
    }
    ...
    TORCH_CHECK(
        func_backup,
        "Failed to find function ",
        "aclrtSynchronizeDeviceWithTimeout and aclrtSynchronizeDevice",
