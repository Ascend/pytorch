...
int DcmiInit(void) {
  ...
  if (func == nullptr) {
    TORCH_CHECK(
        false,
        "Failed to find function dcmi_init, "
        " maybe your hdk version is too low, please upgrade it.",
...
int DcmiGetCardNumList(int* card_num, int* card_list, int list_len) {
  ...
  if (func == nullptr) {
    TORCH_CHECK(
        false,
        "Failed to find function dcmi_get_card_num_list, "
        " maybe your hdk version is too low, please upgrade it.",
        PTA_ERROR(ErrCode::NOT_FOUND))
  }
...
int DcmiGetDeviceIdInCard(int card_id, int* device_id_max, int* mcu_id, int* cpu_id) {
  ...
  if (func == nullptr) {
    TORCH_CHECK(
        false,
        "Failed to find function dcmi_get_device_id_in_card, "
        " maybe your hdk version is too low, please upgrade it",
        PTA_ERROR(ErrCode::NOT_FOUND))
  }
...
int DcmiGetDevicePcieInfoV2(int card_id, int device_id, DcmiPcieInfo* pcie_info) {
  ...
  if (func == nullptr) {
    TORCH_CHECK(
        false,
        "Failed to find function dcmi_get_device_pcie_info_v2, "
        " maybe your hdk version is too low, please upgrade it.",
        PTA_ERROR(ErrCode::NOT_FOUND))
  }
