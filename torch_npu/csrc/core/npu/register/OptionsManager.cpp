  ...
  const static uint32_t status_save_interval = []() -> uint32_t {
    ...
    if (env_val != nullptr) {
      ...
      if (envFlag <= 0) {
        envFlag = 2;
        TORCH_NPU_WARN_ONCE(
            "Get env TORCH_HCCL_STATUS_SAVE_INTERVAL less than or equal to 0, so reset it to the default value.");
      }
    }
    return static_cast<uint32_t>(envFlag);
  }();
...
std::pair<double, double> OptionsManager::GetSilenceThresh(
...
uint32_t OptionsManager::GetAclOpInitMode() {
  const static uint32_t acl_op_init_mode = []() -> uint32_t {
    ...
    if (is_aclnn_only && env_mode != 2) {
      TORCH_NPU_WARN_ONCE(
          "This device only supports ACL_OP_INIT_MODE=2 (aclops disabled), but got ",
          env_mode,
...
uint32_t OptionsManager::GetStreamsPerDevice() {
  const static uint32_t streams_per_device = []() -> uint32_t {
    ...
    if (streams_per_device != 8 && streams_per_device != 32) {
      ...
      TORCH_NPU_WARN_ONCE(
          "STREAMS_PER_DEVICE only support 8 or 32, but get other value, so reset it to the default value 32");
    }
...
void OptionsManager::SetTaskQueueEnable(int32_t mode) {
  ...
  TORCH_CHECK(
      valid_modes.find(mode) != valid_modes.end(),
      "TASK_QUEUE_ENABLE should be 0, 1 or 2, but got ",
      mode,
...
bool OptionsManager::ShouldPrintWarning() {
  static bool should_print = []() {
    char* disabled_warning = get_and_log_env("TORCH_NPU_WARNING_DISABLE");
    if (disabled_warning == nullptr) {
      disabled_warning = get_and_log_env("TORCH_NPU_DISABLED_WARNING");
    }
...
bool OptionsManager::IsOomSnapshotEnable() {
  ...
  const static char* env_val = get_and_log_env("OOM_SNAPSHOT_ENABLE");
  int64_t envFlag = (env_val != nullptr) ? strtol(env_val, nullptr, 10) : 0;
#ifndef BUILD_LIBTORCH
  if (isFirstCall) {
    switch (envFlag) {
...
uint64_t OptionsManager::GetShmemSymmetricSize() {
  static uint64_t symmetricSize = []() -> uint64_t {
    ...
    char* env_val = get_and_log_env("NPU_SHMEM_SYMMETRIC_SIZE");
    if (env_val == nullptr) {
