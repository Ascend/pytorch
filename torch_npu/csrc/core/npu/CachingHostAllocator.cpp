        ...
        if (!block_found) {
            if (params.err == ACL_ERROR_RT_MEMORY_ALLOCATION) {
                AT_ERROR("Malloc pin memory failed, host out of memory, Rried to allocate size: ", orig_size,
                    ", total_allocated_memory: ", stats.active_bytes.current, ", total_reserved_memory: ", stats.allocated_bytes.current,
...
private:
    ...
    void unmap_block(ExpandableBlock *block)
    {
        ...
        block->mapped = false;
        update_stat(stats.allocated_bytes, -static_cast<std::int64_t>(unmapped.size));
    ...
    void free_expandable_block(ExpandableBlock *block)
    {
        ...
        update_stat(stats.active_bytes, -static_cast<std::int64_t>(block->size_));
    ...
    bool map_block(ExpandableBlock *to_map, size_t size, BlockPool *map_pool)
    {
        ...
        update_stat(stats.allocated_bytes, static_cast<std::int64_t>(mapped_range.size));
...
std::unique_ptr<NPUCachingHostAllocatorImpl> cachingHostAllocatorImpl = std::make_unique<NPUCachingHostAllocatorImpl>();
std::unique_ptr<NPUCachingHostAllocatorImpl> expandableHostAllocatorImpl = std::make_unique<NPUExpandableHostAllocatorImpl>();

struct NPUCachingHostAllocator final
    : public at::CachingHostAllocatorInterface<NPUCachingHostAllocatorImpl, raw_local_deleter> {
