...
MempoolId_t graph_pool_handle() {
  ...
  // from aclmdlRICaptureGetInfo id_s in capture_begin.
 ...
 *
 * A1: Convenience.
 * A2: To ensure valid numerics on replay, some native NPU ops (like RNG ops
...
void NPUGraph::capture_begin(MempoolId_t pool, aclmdlRICaptureMode capture_mode, bool report_shape) {
  ...
  TORCH_CHECK(
      stream != c10_npu::getDefaultNPUStream(),
  ...
  // Addendum: beginAllocateToPool is now called before AclmdlRICaptureBegin to
  ...
  NPUGRAPH_LOGD(
      "NPUGRAPH Capture begin done: model_ri=%p, capture_dev=%d, "
      "mempool_id=(%lu,%lu), stream=%p, capture_mode=%d",
      static_cast<void*>(model_ri_),
      capture_dev_,
      mempool_id_.first,
      mempool_id_.second,
      static_cast<void*>(capture_stream_.stream(false)),
...
void NPUGraph::capture_end() {
  ...
  NPUGRAPH_LOGD(
      "NPUGRAPH Capture end done: model_ri=%p, has_graph_exec=%d",
      static_cast<void*>(model_ri_),
      static_cast<int>(has_graph_exec_));
}
...
void NPUGraph::reset() {
  ...
  // NPUGraph, the generator, and the allocator could end up in all kinds of
  ...
  if (has_graph_exec_) {
    NPUGRAPH_LOGD(
        "NPUGRAPH Reset releasing graph: model_ri=%p, capture_dev=%d, "
        "mempool_id=(%lu,%lu)",
        static_cast<void*>(model_ri_),
        capture_dev_,
        mempool_id_.first,
...
std::function<bool(aclrtStream)> NPUGraph::create_allocate_filter() const {
  return [this](aclrtStream stream) -> bool {
    ...
    NPUGRAPH_LOGD(
        "NPUGRAPH AllocateToPool filter: stream=%p, capture_id=%llu, expected_capture_id=%llu, "
        "mempool_id=(%lu,%lu)",
        static_cast<void*>(stream),
        capturing_id.value_or(0),
        capture_id_,
        mempool_id_.first,
        mempool_id_.second);
...
std::function<bool(aclrtStream)> NPUGraph::create_child_allocate_filter(aclmdlRI child_model_ri) const {
  ...
  return [this, child_capture_id](aclrtStream stream) -> bool {
    auto capturing_id = c10_npu::captureIdMayInitCtx(stream);
    NPUGRAPH_LOGD(
        "NPUGRAPH Child AllocateToPool filter: stream=%p, capture_id=%llu, "
...
void NPUGraph::begin_capture_to_if_node(const at::Tensor& scalar_npu_pred_tensor) {
  ...
  NPUGRAPH_LOGD(
      ...
      static_cast<void*>(parent_model_ri),
      static_cast<void*>(child_model_ri),
      static_cast<void*>(parent_stream.stream(false)),
      mempool_id_.first,
      mempool_id_.second);
  ...
  NPUGRAPH_LOGD(
      "NPUGRAPH Conditional capture to child modelRI: child_model_ri=%p, child_stream=%p, "
      "mempool_id=(%lu,%lu)",
      static_cast<void*>(child_model_ri),
...
void NPUGraph::end_capture_to_conditional_node() {
  ...
  NPUGRAPH_LOGD(
      ...
      static_cast<void*>(child_model_ri),
      static_cast<void*>(stream.stream(false)),
      mempool_id_.first,
      mempool_id_.second);
  ...
  if (conditional_model_ri_stack_.empty()) {
    NPUGRAPH_LOGD(
        "NPUGRAPH Conditional restore root allocate filter: model_ri=%p, mempool_id=(%lu,%lu)",
        static_cast<void*>(model_ri_),
        mempool_id_.first,
        mempool_id_.second);
    ...
    NPUGRAPH_LOGD(
        "NPUGRAPH Conditional restore parent allocate filter: parent_model_ri=%p, mempool_id=(%lu,%lu)",
