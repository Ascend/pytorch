...
int64_t VersionToNum(std::string versionStr) {
  ...
  int64_t minor = -1;
  int64_t release = -1;
  int64_t RCVersion = -51; // Ensure that when there is no matching format, the
                           // calculation result of the corresponding item is 0.
  int64_t TVersion = -1;
  ...
  if (tokens.size() == tokenNum3) {
    ...
    if (!parsed && StartsWith(tokens[index2], "T") &&
        tokens[index2].length() > validLength1) { // ([0-9]+).([0-9]+).T([0-9]+)
...
int64_t DriverVersionToNum(std::string versionStr) {
  ...
  // pattern is major.minor.release.patch. release:num or RC+num or T+num,
  // patch: num or alpha+num or beta+num.
  std::vector<std::string> tokens = SplitVersionStr(versionStr);
  ...
  if (!parsed && tokens.size() == tokenNum4) {
    ...
    if (!parsed && StartsWith(tokens[index2], "RC") && tokens[index2].length() > validLength2 &&
        ...
        tokens[index3].length() >
            validLength1) { // ([0-9]+).([0-9]+).RC([0-9]+).beta([0-9]+) or ([0-9]+).([0-9]+).RC([0-9]+).b([0-9]+)
      ...
      size_t prefixLen =
          (StartsWith(tokens[index3], "beta") && tokens[index3].length() > validLength4) ? validLength4 : validLength1;
...
bool IsGteDriverVersion(const std::string driverVersion) {
  // if cann does not support AclsysGetCANNVersion，GetCANNVersion("DRIVER")
  // will return "". The result of this function will be false, even if current
  // driver version meets the requirement.
