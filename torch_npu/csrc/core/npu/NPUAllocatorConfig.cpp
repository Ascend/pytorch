    ...
    namespace NPUCachingAllocator {
        ...
        size_t CachingAllocatorConfig::parseMaxSplitSize(const std::vector<std::string> &config, size_t i)
        {
            ...
            if (++i < config.size()) {
                ...
                TORCH_CHECK(val1 > minMaxSplitSizeMb / kMB,
                    "CachingAllocator option max_split_size_mb too small, must be > ", minMaxSplitSizeMb / kMB,
                    OPS_ERROR(ErrCode::VALUE));
                val1 = std::max(val1, minMaxSplitSizeMb / kMB);
                val1 = std::min(val1, (std::numeric_limits<size_t>::max() / kMB));
                m_max_split_size = val1 * kMB;
            } else {
