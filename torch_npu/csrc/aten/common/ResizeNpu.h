...
static void storage_resize_npu(
  ...
  auto& storage_desc =
      torch_npu::NPUBridge::GetNpuStorageImpl(&storage)->npu_desc_;
  if (!FormatHelper::IsBaseFormatType(storage_desc.npu_format_)) {
    TORCH_CHECK(
        false,
        "Cannot resize storage without base format",
        OPS_ERROR(ErrCode::NOT_SUPPORT));
    return;
  }
  ...
  if ((size % static_cast<ptrdiff_t>(itemsize)) != 0) {
    AT_ERROR(
        "The specified storage nbytes cannot be divided by item size.",
  ...
  if (force_refresh) {
    ...
    const c10::IntArrayRef& refresh_size =
        new_data_numel > new_shape_numel ? resize_shape : new_size;
    ...
    if (dim_ > 0) {
      ...
      for (auto i = last_idx - 1; i >= 0; --i) {
        new_stride[i] =
            new_stride[i + 1] * std::max<int64_t>(refresh_size[i + 1], 1);
      }
    }
