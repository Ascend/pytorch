...
bool NpuP2pCtrl::get_p2p_access(int32_t source_dev, int32_t dest_dev)
{
    ...
    TORCH_CHECK(source_dev >= 0 && source_dev < num_devices_, source_dev, " is not a device", PTA_ERROR(ErrCode::PARAM));
    TORCH_CHECK(dest_dev >= 0 && dest_dev < num_devices_, dest_dev, " is not a device", PTA_ERROR(ErrCode::PARAM));
