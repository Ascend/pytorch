...
void set_storage_nd_npu(
  ...
  resize_nd_npu(
      self.unsafeGetTensorImpl(), nDimension, size.data(), stride.data());
}
...
bool CheckStorageDesc(const at::Tensor& self, const c10::Storage src) {
  ...
  int64_t new_size =
      static_cast<int64_t>(src.nbytes() / self.dtype().itemsize());
  int64_t nelements =
      c10::multiply_integers(self.unsafeGetTensorImpl()->sizes());
...
at::Tensor set_tensor_with_storage_format(c10::Storage src) {
  if (StorageDescHelper::CheckDescInit(src)) {
    ...
    auto dist_tensor = NPUNativeFunctions::empty(
        ...
        c10::MemoryFormat::Contiguous);
    int64_t new_size =
        static_cast<int64_t>(src.nbytes() / dist_tensor.dtype().itemsize());
