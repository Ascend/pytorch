...
// Write canonical int_repr bytes into dst; NPU quantized tensors may expose
// multiple int slabs and a separate qtensor data_ptr — retry until
// dst.int_repr() matches expected_sum (small tensors only).
...
at::Tensor empty_like_npu(
  ...
  if (self.is_quantized()) {
    ...
    // at::Tensor clone(const at::Tensor& src,
    // c10::optional<c10::c10::MemoryFormat> optional_memory_format) if
    // (self.is_non_overlapping_and_dense()) -> _empty_affine_quantized_strided
    ...
    // defaulting process, by explicitly passing nullopt for the
    // c10::MemoryFormat argument.  When codegen is adjusted so we can delete
    // this argument from the method signature, the argument will just disappear
    // entirely.
    //
...
at::Tensor NPUNativeFunctions::unsafe_empty_with_format(
    ...
    int64_t dst_format,
    bool keep_format) {
  // This is a special interface that can adjust the memory application results.
  // Check before use.

  // Some ops cannot operate directly based on ND format, such as MatMul,
...
at::Tensor NPUNativeFunctions::clone(const at::Tensor& src, c10::optional<c10::MemoryFormat> format) {
  ...
  // produced non-deterministic or partially wrong buffers vs int_repr semantics
  // (see contiguous() twice differing). Always materialize via Byte stride-copy
  // matching CopyKernel.
  if (src.is_quantized()) {
    TORCH_CHECK(
    ...
    // When MetaDataAreMatch(src.int_repr)==0, NPU quantized tensors can expose
    // multiple int slabs; npu_stride_copy_out into dst.int_repr().reshape(...)
    // still left later dst.int_repr() reading a different buffer
    // (test_view_ops.TestOldViewOpsPRIVATEUSE1.test_ravel_npu nc). int_repr CPU
    // byte staging (plain Byte on CPU, not QuantizedCPU) when q or int_repr
    // NPUStorageDesc disagrees with tensor sizes/strides. Do not use
    // src.to(CPU) / QuantizedCPU / r.to(NPU) on QTensor.
    ...
    // on CPU, then H2D into empty_like dst + slab propagation
    // (quantize_per_tensor on NPU can leave dst.int_repr() on a different slab
    // than the quantized payload).
    if (npu_src_q_meta_mismatch || npu_src_repr_meta_mismatch) {
      ...
      if (src.numel() == 0) {
        ...
        // MetaData(q)==0 only (e.g. ravel/view rank-1): int_repr meta already
        // matches src; do not as_strided via qtensor desc (can yield numel 1);
        // D2H the logical int_repr view.
    ...
    op_plugin::npu_stride_copy_out(
        ...
        dst_repr);
    if (src.device().type() == c10::DeviceType::PrivateUse1) {
      // npu_stride_copy_out writes dst_repr; successive dst.int_repr() may
      // return different physical slabs (post_copy: same_data_ptr=0,
      // again!=frozen on first clone). One copy_(canonical, dst_repr) is
      // insufficient if int_repr() is unstable. Propagate the stride-copied
      // buffer through int_repr() hops until stable or cap, syncing after each
      // copy.
      c10_npu::getCurrentNPUStream(src.device().index()).synchronize();
      c10_npu::npuSynchronizeDevice();
      // Do not break early when cur.data_ptr() == written: int_repr() can
      // return the same slab for several calls (so stride_copy data is
      // visible), then a later call returns a different slab (post_copy
      // again!=frozen). Run a fixed number of copy+sync rounds so every
      // returned slab receives the stride-copied payload.
      at::Tensor written = dst_repr;
      constexpr int kMaxIntReprSlabHops = 8;
      for (int hop = 0; hop < kMaxIntReprSlabHops; ++hop) {
        at::Tensor cur = dst.int_repr();
