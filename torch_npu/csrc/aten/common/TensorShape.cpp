...
at::Tensor NPUNativeFunctions::as_strided(
  ...
  if (!FormatHelper::IsOpInputBaseFormat(dst)) {
    if (InferFormat::IsDefiniteTensorWhenMetaDataChanges(dst, size)) {
      TORCH_WARN_ONCE(
          "current tensor is running as_strided, don't perform inplace operations on the returned value."
...
// Stride pattern for row-major memory (sizes with 1 are neutral for contiguity
// checks).
...
// alias_with_sizes_and_strides_npu leaves NPUStorageDesc describing the storage
// owner shape; a view with new sizes/strides can show MetaDataAreMatch(out)==0
// while int_repr still matches (see logs). Do not SetDesc in-place on shared
// storage (would corrupt the base tensor). Materialize via clone so the
// flattened QTensorImpl regains consistent NPUStorageDesc; skipping clone when
...
// ATen's _unsafe_view wraps view_impl and still validates strides via
// computeStride. For quantized NPU, ravel()/view(-1) after a buggy
// is_contiguous() short-circuit (or contiguous() passthrough) must match
...
// NPUNativeFunctions::empty_strided but QuantizedPrivateUse1 does not use that
// registration.
...
// native_functions.yaml dispatches QuantizedCPU/CUDA empty_like ->
// empty_like_quantized. QuantizedPrivateUse1 is not listed, so composite
// empty_like runs (calls empty_symint -> empty.memory_format), which hits
// make_unknown_quantizer and breaks qscheme() during asserts. Mirror
// QuantizedCUDA by forwarding to NPUNativeFunctions::empty_like (same as
...
// is_contiguous_or_false() and may view-flatten without the row-major clone
// path that Quantized+NPU tensors need after transpose. Match explicit
// contiguous().view(-1) (test_view_ops.TestOldViewOpsPRIVATEUSE1.test_ravel).
at::Tensor npu_quantized_ravel(const at::Tensor& self) {
  // Dispatch through aten::contiguous (npu_quantized_contiguous), not
  // NPUNativeFunctions::contiguous — the latter only checks is_contiguous(),
  // not row-major stride truth, and can return transpose as-is.
...
// Quantized NPU tensors carry DispatchKey::QuantizedPrivateUse1; register view
// to match native _unsafe_view (reshape copy path) and wire other factory/copy
// hooks used by functionalization.
