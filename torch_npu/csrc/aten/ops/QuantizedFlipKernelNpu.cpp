...
at::Tensor quantized_flip(const at::Tensor& self, at::IntArrayRef dims) {
  TORCH_CHECK(
      self.scalar_type() != at::kQUInt4x2 && self.scalar_type() != at::kQUInt2x4,
  ...
  TORCH_CHECK(self.qscheme() == at::kPerTensorAffine, "Setting strides is possible only on uniformly quantized tensor");

  at::Tensor repr = self.int_repr();
  at::Tensor flipped = at::flip(repr, dims);
  at::Tensor result = at::_empty_affine_quantized(
      self.sizes(), self.options(), self.q_scale(), self.q_zero_point(), self.suggest_memory_format());
