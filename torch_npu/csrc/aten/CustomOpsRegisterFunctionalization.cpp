...
// Regster functionalization for custom op npu_quant_update_scatter_.
// To Get these mutable operators to work with functionalization requires some extra work.
// We need to register a corresponding out-of-place variant of the op,
// and register a functionalization kernel that performs some boilerplate to
// teach functionalization how to map from the mutable op to the out-of-place op.
at::Tensor &npu_quant_update_scatter__functionalization(
    at::Tensor& self,
    const at::Tensor& indices,
    const at::Tensor& updates,
...
{
    {
        ...
        at::AutoDispatchSkipFunctionalize func_guard;
        c10::impl::ExcludeDispatchKeyGuard guard(exclude_keys_for_meta_dispatch);
        static auto npu_quant_update_scatter__handle = c10::Dispatcher::singleton()
            // specify namespace::op_name, op_overload_name
            .findSchemaOrThrow("npu::npu_quant_update_scatter_", "")
            // specify the C++ schema of the original mutable op
            .typed<at::Tensor & (at::Tensor &, const at::Tensor &, const at::Tensor &, const at::Tensor &,
                                 const c10::optional<at::Tensor> &, c10::string_view, int64_t, int64_t)>();

        npu_quant_update_scatter__handle.call(self_meta, indices_meta, updates_meta, quant_scales_meta,
                                              quant_zero_points_meta, reduce, axis, quant_axis);
    }
    ...
    // Grab the dispatcher entry corresponding out-of-place variant of the mutable op
    static auto npu_quant_update_scatter_handle = c10::Dispatcher::singleton()
        // specify namespace::op_name, op_overload_name
        .findSchemaOrThrow("npu::npu_quant_update_scatter", "")
        // specify the C++ schema of the corresponding out-of-place op
    ...
    {
        at::AutoDispatchSkipFunctionalize func_guard;
        tmp_output = npu_quant_update_scatter_handle.call(self_unwrap, indices_unwrap, updates_unwrap,
                                                          quant_scales_unwrap, quant_zero_points_unwrap, reduce, axis,
                                                          quant_axis);
    }
...
TORCH_LIBRARY_IMPL(npu, Functionalize, m)
{
    m.impl("npu_quant_update_scatter_", npu_quant_update_scatter__functionalization);
}
