...
c10::intrusive_ptr<NPUGeneratorState> NPUGeneratorState::clone() {
  return c10::make_intrusive<NPUGeneratorState>(
      seed_, philox_offset_per_thread_);
}
...
void NPUGeneratorCaptureState::setup_for_replay(
    uint64_t seed,
    uint64_t philox_offset) {
  TORCH_INTERNAL_ASSERT(
      is_initialized(), "Capture state should be initialized before replay.");
...
NPUGeneratorCaptureState* NPUGeneratorState::get_capture_state(
 ...
 */
void NPUGeneratorState::replay_prologue(
    c10_npu::CaptureId_t capture_id,
...
NPUGeneratorImpl::NPUGeneratorImpl(
    c10::DeviceIndex device_index,
    c10::intrusive_ptr<NPUGeneratorState> state)
    : c10::
          GeneratorImpl{c10::Device(c10::DeviceType::PrivateUse1, device_index), c10::DispatchKeySet(c10::DispatchKey::PrivateUse1)},
...
void NPUGeneratorImpl::set_current_seed(uint64_t seed) {
  if (C10_LIKELY(!c10_npu::currentStreamCaptureId().has_value())) {
    ...
    TORCH_CHECK(
        state_->seed_ == seed,
        "NPUGeneratorImpl::set_current_seed can be called during stream capture only if new seed is the same as the original seed.");
  }
}
...
void NPUGeneratorImpl::set_offset(uint64_t offset) {
  c10_npu::assertNotCapturing(
      "Cannot call NPUGeneratorImpl::set_offset while in capture mode");
...
uint64_t NPUGeneratorImpl::get_offset() const {
  c10_npu::assertNotCapturing(
      "Cannot call NPUGeneratorImpl::get_offset while in capture mode");
...
uint64_t NPUGeneratorImpl::seed() {
  c10_npu::assertNotCapturing(
      "Cannot call NPUGeneratorImpl::seed while in capture mode");
 ...
 * the generator.
 */
void NPUGeneratorImpl::graphsafe_set_state(
    const c10::intrusive_ptr<GeneratorImpl>& gen) {
  c10::intrusive_ptr<NPUGeneratorImpl> npu_gen =
      c10::dynamic_intrusive_pointer_cast<NPUGeneratorImpl>(gen);
 ...
 */
c10::intrusive_ptr<c10::GeneratorImpl> NPUGeneratorImpl::graphsafe_get_state()
    const {
  auto gen =
      c10::make_intrusive<NPUGeneratorImpl>(this->device().index(), state_);
  return gen;
}
...
void NPUGeneratorImpl::set_secondary_stream_capture_state(
    bool secondary_stream_capture_state) {
...
NPUGeneratorImpl* NPUGeneratorImpl::clone_impl() const {
  c10_npu::assertNotCapturing(
      "Cannot call NPUGeneratorImpl::clone_impl while in capture mode");
