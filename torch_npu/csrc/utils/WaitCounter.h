...
public:
    ...
    virtual intptr_t start(std::chrono::steady_clock::time_point now) noexcept = 0;
    virtual void stop(std::chrono::steady_clock::time_point now, intptr_t ctx) noexcept = 0;
};
...
public:
    ...
    virtual std::unique_ptr<WaitCounterBackendIf> create(std::string_view key) noexcept = 0;
};

C10_API void registerWaitCounterBackend(std::unique_ptr<WaitCounterBackendFactoryIf>);

C10_API std::vector<std::shared_ptr<WaitCounterBackendFactoryIf>> getRegisteredWaitCounterBackends();
}  // namespace detail
...
public:
    ...
    class WaitGuard {
    public:
        WaitGuard(WaitGuard &&other) noexcept : handle_{std::exchange(other.handle_, {})}, ctxs_{std::move(other.ctxs_)}
        {
        }
    ...
    private:
        WaitGuard(WaitCounterHandle &handle, SmallVector<intptr_t> &&ctxs) : handle_{&handle}, ctxs_{std::move(ctxs)} {}
...
#define STATIC_SCOPED_WAIT_COUNTER(_name) auto C10_ANONYMOUS_VARIABLE(SCOPE_GUARD) = STATIC_WAIT_COUNTER(_name).start();
