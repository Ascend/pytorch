...
static std::vector<std::string> TORCH_HCCL_BLOCKING_WAIT = {"TORCH_HCCL_BLOCKING_WAIT", "HCCL_BLOCKING_WAIT"};
...
static std::vector<std::string> TORCH_HCCL_DUMP_ON_TIMEOUT = {"TORCH_HCCL_DUMP_ON_TIMEOUT"};
...
static std::vector<std::string> TORCH_HCCL_ENABLE_MONITORING = {"TORCH_HCCL_ENABLE_MONITORING"};
...
static std::vector<std::string> TORCH_HCCL_TRACE_BUFFER_SIZE = {"TORCH_HCCL_TRACE_BUFFER_SIZE"};

// Control how much extra time we will wait for dumping the debugging info
// before we exit and throws timeout exception.
static std::vector<std::string> TORCH_HCCL_WAIT_TIMEOUT_DUMP_MILSEC = {"TORCH_HCCL_WAIT_TIMEOUT_DUMP_MILSEC"};

// Control the watchdog heartbeat timeout period after which the monitoring
// thread will abort the process.
static std::vector<std::string> TORCH_HCCL_HEARTBEAT_TIMEOUT_SEC = {"TORCH_HCCL_HEARTBEAT_TIMEOUT_SEC"};
...
static std::vector<std::string> TORCH_HCCL_COORD_CHECK_MILSEC = {"TORCH_HCCL_COORD_CHECK_MILSEC"};

// Control whether to always use high priority streams
static std::vector<std::string> TORCH_HCCL_HIGH_PRIORITY = {"TORCH_HCCL_HIGH_PRIORITY"};
...
struct DumpPipe {
  DumpPipe(int rank, const std::string& fileStem, int traceBufferSize) {
    ...
    TORCH_CHECK(unlink(filename.c_str()) != -1 || errno == ENOENT, "Error removing existing named pipe ", filename);
    TORCH_CHECK(mkfifo(filename.c_str(), 0666) != -1, "Error creating named pipe ", filename);
    fd_ = open(filename.c_str(), O_RDONLY | O_NONBLOCK);
    LOG(INFO) << "Pipe file " << filename << " has been opened, write to it to trigger HCCL Debug Dump.";
...
class C10_NPU_API ProcessGroupHCCL : public c10d::Backend {
 public:
  class WorkHCCL : public c10d::Work, public std::enable_shared_from_this<WorkHCCL> {
   ...
   protected:
    ...
    // for tests.
    virtual std::exception_ptr checkForHCCLErrors(const std::vector<std::shared_ptr<HCCLComm>>& hcclComms) const;

    friend std::ostream& operator<<(std::ostream& output, const WorkHCCL& workHCCL);
  ...
  class Watchdog {
   public:
    ...
    int getSignalSrcRank(c10::intrusive_ptr<c10d::Store>& store, const std::string& signal);
 ...
 protected:
  ...
  void broadcastMasterID(HcclRootInfo* hcclID, bool isSingleP2POp, const std::string& devicesKey, int p2pRank);

  // All-gathers scalable root infos from the selected roots through the process-group store.
  void allgatherScalableRootInfos(int rootIdx, HcclRootInfo* rootInfo, std::vector<HcclRootInfo>& rootInfoList);
  ...
  virtual std::exception_ptr checkForHCCLErrors(const std::vector<std::shared_ptr<HCCLComm>>& hcclComms);
 ...
 private:
  ...
  static std::string getMstxHcclMsg(
...
// Group.
TORCH_API std::string dump_hccl_trace(bool includeCollectives, bool includeStackTraces, bool onlyActive);

// Dumps the HCCL comm traces and additional information about the Process
// Group in JSON formatted string.
// We don't include stack traces in JSON format as it is far too much data.
TORCH_API std::string dump_hccl_trace_json(bool includeCollectives, bool onlyActive);
