#include "torch_npu/csrc/distributed/FlightRecorder.hpp"
...
#include "third_party/nlohmann/include/nlohmann/json.hpp"
...
void DebugInfoWriter::write(const std::string &trace)
{
    ...
    if (!file.is_open()) {
        LOG(ERROR) << "Error opening file for writing HCCLPG debug info: " << filename_;
        return;
    }
...
{
    if (writer_ == nullptr) {
        std::string fileNamePrefix = c10d::getCvarString({"TORCH_HCCL_DEBUG_INFO_TEMP_FILE"}, "/tmp/hccl_trace_rank_");
...
{
    TORCH_CHECK_WITH(DistBackendError, hasWriterRegistered_.load() == false, "debugInfoWriter already registered", PTA_ERROR(ErrCode::INTERNAL));
...
std::optional<size_t> FlightRecorder::record(size_t pg_id, const std::tuple<std::string, std::string> &pg_name,
                                             size_t collective_seq_id, size_t p2p_seq_id, size_t op_id,
                                             std::string profiling_name, const std::vector<at::Tensor> &inputs,
                                             const std::vector<at::Tensor> &outputs, Event *start, Event *end,
                                             std::chrono::milliseconds timeout_ms,
                                             std::shared_ptr<ProcessGroupStatus> pg_status, bool isP2P)
{
    if (!enabled_) {
        return std::nullopt;
    }
...
void FlightRecorder::retire_id(std::optional<size_t> id, bool compute_duration)
{
    ...
    if (can_compute_duration) {
        ...
        // npuEventDuration() can hang, and we need to acquire the lock before we
...
const c10::List<c10::IValue> FlightRecorder::getCollectiveTrace(bool includeStacktraces, bool onlyActive)
{
...
const std::map<std::string, std::map<std::string, std::string>> FlightRecorder::getPgConfigJson()
{
...
const std::map<std::string, std::map<std::string, std::string>> FlightRecorder::getPgStatusJson()
{
    std::map<std::string, std::map<std::string, std::string>> result;
    for (const auto &[pg_id, status] : all_pg_status_) {
...
std::string FlightRecorder::dump_json(
    const std::optional<std::unordered_map<std::string, std::unordered_map<std::string, std::string>>> &hcclDumpMap,
    bool includeCollectives, bool onlyActive)
{
...
std::string FlightRecorder::dump(
    const std::optional<std::unordered_map<std::string, std::unordered_map<std::string, std::string>>> &hcclDumpMap,
    bool includeCollectives, bool includeStackTraces, bool onlyActive)
{
...
float getDurationFromEvent(c10_npu::NPUEvent &hcclStartEvent, c10_npu::NPUEvent &hcclEndEvent)
{
    TORCH_CHECK(hcclEndEvent.query(), "getDuration can only be called after work is succeeded.", PTA_ERROR(ErrCode::INTERNAL))
