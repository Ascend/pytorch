...
#define TORCH_NPU_LOAD_FUNCTION(funcName) TORCH_NPU_REGISTER_FUNCTION(libhccl, funcName)
...
extern HcclResult hcclSend(
    ...
    HcclComm comm,
    aclrtStream stream) {
  using HcclSendFunc = HcclResult (*)(void*, uint64_t, HcclDataType, uint32_t, HcclComm, aclrtStream);
...
extern HcclResult hcclRecv(
    ...
    HcclComm comm,
    aclrtStream stream) {
  using HcclRecvFunc = HcclResult (*)(void*, uint64_t, HcclDataType, uint32_t, HcclComm, aclrtStream);
...
extern HcclResult hcclCommInitAll(uint32_t ndev, int32_t* devices, HcclComm* comms) {
  using HcclCommInitAllFunc = HcclResult (*)(uint32_t, int32_t*, HcclComm*);
...
extern HcclResult hcclAllReduce(
    ...
    HcclComm comm,
    aclrtStream stream) {
  using HcclAllReduceFunc = HcclResult (*)(void*, void*, uint64_t, HcclDataType, HcclReduceOp, HcclComm, aclrtStream);
...
extern HcclResult hcclCommInitRootInfo(uint32_t nRanks, const HcclRootInfo* rootInfo, uint32_t rank, HcclComm* comm) {
  using HcclCommInitRootInfoFunc = HcclResult (*)(uint32_t, const HcclRootInfo*, uint32_t, HcclComm*);
...
extern HcclResult hcclAlltoAllV(
  ...
  auto ret = func(sendBuf, sendCounts, sdispls, sendType, recvBuf, recvCounts, rdispls, recvType, comm, stream);
  return ret;
}
...
extern HcclResult hcclAllGatherV(
  ...
  using HcclAllGatherVFunc =
      HcclResult (*)(const void*, uint64_t, const void*, const void*, const void*, HcclDataType, HcclComm, aclrtStream);
...
extern HcclResult hcclReduceScatterV(
  ...
  using HcclReduceScatterVFunc = HcclResult (*)(
      const void*, const void*, const void*, const void*, uint64_t, HcclDataType, HcclReduceOp, HcclComm, aclrtStream);
...
HcclResult hcclBatchIsendIrecv(void* sendRecvInfo, uint32_t itemNum, HcclComm comm, aclrtStream stream) {
  using HcclBatchIsendIrecvVFunc = HcclResult (*)(void*, uint32_t, HcclComm, aclrtStream);
...
HcclResult hcclAlltoAll(
  ...
  using HcclAlltoAllFunc =
      HcclResult (*)(const void*, uint64_t, HcclDataType, const void*, uint64_t, HcclDataType, HcclComm, aclrtStream);
  ...
  auto ret = func(sendBuf, sendCount, sendType, recvBuf, recvCount, recvType, comm, stream);
  return ret;
}
...
bool hcclCommInitRootInfoConfigExist() {
  ...
  c10::call_once(flag, [&]() {
    auto func = TORCH_NPU_GET_FUNC(HcclCommInitRootInfoConfig) if (func != nullptr) {
      exist = true;
    }
  });
  return exist;
...
bool hcclAllGatherVExist() {
  ...
  static bool exist = false;
  c10::call_once(flag, [&]() {
    auto func = TORCH_NPU_GET_FUNC(HcclAllGatherV) const auto soc_version = c10_npu::GetSocVersion();
    const bool is_atlas_a3 =
...
bool hcclReduceScatterVExist() {
  ...
  static bool exist = false;
  c10::call_once(flag, [&]() {
    auto func = TORCH_NPU_GET_FUNC(HcclReduceScatterV) const auto soc_version = c10_npu::GetSocVersion();
    const bool is_atlas_a3 =
        (soc_version >= c10_npu::SocVersion::Ascend910_9391) && (soc_version < c10_npu::SocVersion::Ascend950);
    if (func != nullptr &&
        (((soc_version >= c10_npu::SocVersion::Ascend310P1) && (soc_version < c10_npu::SocVersion::Ascend310B1)) ||
         is_atlas_a3 || (soc_version == c10_npu::SocVersion::Ascend950))) {
      exist = true;
    }
  });
  return exist;
...
HcclResult hcclCommInitRootInfoConfig(
    ...
    HcclComm* comm) {
  using HcclCommInitRootInfoConfigFunc =
      HcclResult (*)(uint32_t, const HcclRootInfo*, uint32_t, HcclCommConfig*, HcclComm*);
...
HcclResult hcclCommInitRootInfoScalable(
    ...
    HcclComm* comm) {
  using HcclCommInitRootInfoScalableFunc =
      HcclResult (*)(uint32_t, uint32_t, const HcclRootInfo*, uint32_t, uint32_t, const HcclCommConfig*, HcclComm*);
...
bool hcclCommInitClusterInfoConfigExist() {
  const static bool isClusterInitExist = []() -> bool {
    auto func = TORCH_NPU_GET_FUNC(HcclCommInitClusterInfoConfig) return func != nullptr;
  }();
...
bool hcclCreateSubCommConfigExist() {
  const static bool isCreateSubCommExist = []() -> bool {
    auto func = TORCH_NPU_GET_FUNC(HcclCreateSubCommConfig) return func != nullptr;
  }();
...
HcclResult hcclCreateSubCommConfig(
    ...
    HcclCommConfig* config,
    HcclComm* subComm) {
  using HcclCreateSubCommConfigFunc =
...
bool hcclCommWorkingDevNicSetExist() {
  const static bool isHcclCommWorkingDevNicSetExist = []() -> bool {
    auto func = TORCH_NPU_GET_FUNC(HcclCommWorkingDevNicSet) return func != nullptr;
  }();
