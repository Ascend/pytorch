...
std::unique_ptr<TransportRegistration> makeShmTransport() {
  ...
  return std::make_unique<TransportRegistration>(TransportRegistration{std::move(context), kShmTransportPriority, ""});
}
...
std::unique_ptr<ChannelRegistration> makeMultiplexedUvChannel() {
  ...
  return std::make_unique<ChannelRegistration>(ChannelRegistration{std::move(context), kMultiplexedUvChannelPriority});
}
...
void TensorPipeAgent::checkAndSetStaticGroup(const c10::intrusive_ptr<::c10d::Store>& store) {
  ...
  std::vector<uint8_t> isStaticGroupVec(
      (uint8_t*)isStaticGroupStr.c_str(), (uint8_t*)isStaticGroupStr.c_str() + isStaticGroupStr.length());
...
void TensorPipeAgent::pipeRead(
  ...
  pipe->readDescriptor([this, fn{std::move(fn)}, pipe](
                           const tensorpipe_npu::Error& error, tensorpipe_npu::Descriptor tpDescriptor) mutable {
    if (error) {
    ...
    pipe->read(
        ...
        [tpDescriptor{std::move(tpDescriptor)},
         tpBuffers{std::make_shared<TensorpipeReadBuffers>(std::move(tpBuffers))},
         fn{std::move(fn)},
...
void TensorPipeAgent::pipeWrite(
  ...
  pipe->write(
      std::move(tpMessage),
      [tpBuffers{std::make_shared<TensorpipeWriteBuffers>(std::move(tpBuffers))},
       fn{std::move(fn)},
...
void TensorPipeAgent::sendCompletedResponseMessage(
  ...
  if (!rpcAgentRunning_.load()) {
    LOG(WARNING) << "RPC agent for " << workerInfo_.name_ << " won't send response to request #" << messageId << " to "
  ...
  if (!futureResponseMessage.hasError()) {
    ...
    pipeWrite(
        ...
        [this, pipe, messageId](const tensorpipe_npu::Error& error) {
          ...
          VLOG(1) << "RPC agent for " << workerInfo_.name_ << " done sending response to request #" << messageId
                  << " to " << pipe->getRemoteName();
        });
  } else {
    pipeWrite(
        pipe,
        createExceptionResponse(futureResponseMessage.tryRetrieveErrorMessage(), messageId),
        {},
        std::move(streams),
        [this, pipe, messageId](const tensorpipe_npu::Error& error) {
          if (error) {
  ...
  decltype(connectedPipes_)::iterator it;
  {
    ...
    if (it == connectedPipes_.end()) {
      ...
      std::tie(it, std::ignore) = connectedPipes_.emplace(
          ...
          std::forward_as_tuple(context_->connect(url, tensorpipe_npu::PipeOptions().remoteName(toWorkerInfo.name_))));
    }
  }
      ...
      [this, &clientPipe, messageId](const tensorpipe_npu::Error& error) mutable {
        ...
        pipeRead(
            ...
            [this, &clientPipe](
              ...
              VLOG(1) << "RPC agent for " << workerInfo_.name_ << " received response #" << messageId << " from "
                      << clientPipe.pipe_->getRemoteName();
              ...
              std::shared_ptr<AtomicJitFuture> futureResponseMessage;
              {
                ...
                TORCH_INTERNAL_ASSERT(
                    !clientPipe.inError_, "Shouldn't be in error state", DIST_ERROR(ErrCode::INTERNAL));
              ...
              if (responseMessage->type() == MessageType::EXCEPTION) {
                ...
                markFutureAsComplete(std::move(futureResponseMessage), std::move(responseMessage), std::move(streams));
              }
            });
      });
...
void TensorPipeAgent::pollTimeoutRpcs() {
  while (rpcAgentRunning_.load()) {
    ...
    for (auto& timeoutMetadata : timedOutFutures) {
      std::string errorMsg = fmt::format(kRpcTimeoutErrorStr, timeoutMetadata.timeout.count());
...
void TensorPipeAgent::markFutureAsComplete(
  ...
  if (!atomicFuture->isComplete.test_and_set()) {
    ...
    threadPool_.run([this,
      ...
      VLOG(1) << "TensorpipeAgent::respond set deciveID=" << c10_npu::NpuSysCtrl::GetInstance().InitializedDeviceID()
              << "pid=" << getpid() << " thread_id=" << std::this_thread::get_id();
      c10_npu::SetDevice(c10_npu::NpuSysCtrl::GetInstance().InitializedDeviceID());
...
std::vector<c10::Device> TensorPipeAgent::getDevicesForRemote(const std::string& remoteName, const Message& message)
    const {
  ...
  if (iter == deviceMaps.end()) {
    for (const auto& t : message.tensors()) {
      TORCH_CHECK(
          t.device().is_cpu(), errStr, ", but found tensor on device: ", t.device(), DIST_ERROR(ErrCode::PARAM));
    }
    return {};
  } else {
