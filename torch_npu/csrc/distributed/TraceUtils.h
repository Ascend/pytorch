...
inline bool traceUpdate(c10::intrusive_ptr<c10d::Store> &store, const std::string &key, uint64_t seq,
                        const std::string &col)
{
    ...
    try {
        ...
        LOG(ERROR) << "Store is down while updating #" << seq << " with key " << key;
        return false;
    }
    return true;
}
...
using TraceMap = std::map<uint64_t, std::map<int, std::pair<std::string, TraceDebugEvent>>>;
...
inline std::string ranksFromTrace(const std::vector<std::pair<int, std::string>> &items)
{
    std::string ranks;
    for (auto &p : items) {
        if (ranks.empty()) {
...
{
    return c10::str("\n\t - To our best knowledge, ranks [", ranksToString(missingRanks),
...
inline std::string analyzeLaggingRanks(const TraceMap &traceMap)
{
    ...
    if (!startRanks.empty()) {
        report += c10::str("\n\t   - [", ranksToString(startRanks), "] joined but didn't finish collective #", lagSeq,
                           " (count from 1)");
    }
    if (!endRanks.empty()) {
        report += c10::str("\n\t     [", ranksToString(endRanks), "] finished collective #", lagSeq,
                           ", but didn't join collective #", lagSeq + 1, " (count from 1)");
    }
    return report;
}
...
inline std::string dumpSnapshot(TraceMap &traceMap)
{
    ...
    for (auto &tracePair : traceMap) {
        ...
        std::map<int, std::pair<std::string, TraceDebugEvent>> &subMap = tracePair.second;
        ...
        if (!collectivesStart.empty()) {
            ...
            for (auto &mapPair : collectivesStart) {
                report += c10::str("\n\t     [", ranksToString(mapPair.second), "] started ", mapPair.first);
            }
        }
        if (!collectivesEnd.empty()) {
            report += c10::str("\n\t   #", seq, " finished ranks:");
            for (auto &mapPair : collectivesEnd) {
                report += c10::str("\n\t     [", ranksToString(mapPair.second), "] finished ", mapPair.first);
            }
        }
    }
    return report;
...
inline bool parseTraceValue(c10::intrusive_ptr<c10d::Store> &store, const std::string &key, uint64_t &seq,
                            std::string &col)
{
    try {
...
inline std::string retrieveDesyncReport(c10::intrusive_ptr<c10d::Store> &store, const std::string &pgName, int myRank,
                                        int worldSize)
{
    std::string report;
    ...
    TORCH_INTERNAL_ASSERT(!missingRanks.empty() || !traceMap.empty(),
                          "Trace shouldn't be empty while enabled GLOO_ASYNC_TIMEOUT_DEBUG");
    TORCH_INTERNAL_ASSERT(!thisCol.empty(), "Timeout rank [", myRank,
                          "] must have collective tracking iteam in c10::Store trace");
    TORCH_INTERNAL_ASSERT(traceMap[thisSeq][myRank].second == kEventStart, "Timeout rank [", myRank,
                          "] last trace item must be kEventStart. thisSeq = ", thisSeq, ", col = ", thisCol);
...
inline std::string get_python_cpp_trace()
{
    ...
    std::shared_ptr<torch::CapturedTraceback> tb = torch::CapturedTraceback::gather(true, true, true); // python, script, cpp
