...
void checkSupportedDataType(at_npu::lccl::LcclDataType type, std::string functionName) {
  ...
  TORCH_CHECK(
      ...
      "LCCL " + functionName + ": Unsupported data type ",
      getLcclDataTypeSerialString(type),
...
at_npu::lccl::LcclReduceOp getLcclReduceOp(const c10d::ReduceOp reduceOp, at::Tensor& input) {
  ...
  if (unsupportedOp.find(reduceOp) != unsupportedOp.end()) {
    TORCH_CHECK(
        false, "Cannot use ReduceOp." + unsupportedOp[reduceOp] + " with LCCL", DIST_ERROR(ErrCode::NOT_SUPPORT));
...
std::vector<at::Tensor> FlattenForScatterGather(
  ...
  for (auto i = size_t{}; i < num_devices; ++i) {
    ...
    if (tensor_lists[i].front().get_device() != other[i].get_device()) {
      TORCH_CHECK(
          false,
          "Corresponding input/output tensors to scatter/gather must all on the same device",
          DIST_ERROR(ErrCode::PARAM));
    }
