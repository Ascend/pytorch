...
namespace {
using MemPoolSet =
    std::unordered_set<std::tuple<c10_npu::MempoolId_t, bool>, c10::hash<std::tuple<c10_npu::MempoolId_t, bool>>>;
...
uint64_t getNumelForHCCL(const at::Tensor& self) {
  if (!at_npu::native::FormatHelper::IsBaseFormatType(self)) {
    if (self.storage().data_ptr().get() != self.data_ptr()) {
      TORCH_CHECK(
          false, "For a tensor of internal format, it's storage_offset must be 0", DIST_ERROR(ErrCode::NOT_SUPPORT));
    }
...
HcclReduceOp getHcclReduceOp(const c10d::ReduceOp reduceOp, at::Tensor& input) {
  ...
  if (reduceOp == c10d::ReduceOp::PREMUL_SUM) {
    TORCH_CHECK(
        input.scalar_type() == at::kHalf || input.scalar_type() == at::kFloat || input.scalar_type() == at::kBFloat16 ||
  ...
  if (unsupportedOp.find(reduceOp) != unsupportedOp.end()) {
    TORCH_CHECK(
        false, "Cannot use ReduceOp." + unsupportedOp[reduceOp] + " with HCCL", DIST_ERROR(ErrCode::NOT_SUPPORT));
...
void checkHcclCommConfigValid(const HcclCommConfig* config) {
  if (strlen(config->hcclCommName) > 0) {
    TORCH_CHECK(
        ...
        "The current version of CANN does not support the hcclCommName:",
        config->hcclCommName,
        DIST_ERROR(ErrCode::NOT_SUPPORT));
  }
}
...
std::unordered_map<std::string, std::string> checkEnvVarOrLogWarning() {
  ...
  if (local_rank_env == nullptr) {
    map["enable"] = "false";
    TORCH_NPU_WARN_ONCE(
        "Environment variable 'LOCAL_RANK' is not set. And HCCL_ZERO_COPY will not enable.",
  ...
  if (global_rank_env == nullptr) {
    map["enable"] = "false";
    TORCH_NPU_WARN_ONCE(
        "Environment variable 'RANK' is not set. And HCCL_ZERO_COPY will not enable.",
  ...
  if (nodes_rank_env == nullptr) {
    map["enable"] = "false";
    TORCH_NPU_WARN_ONCE(
        "Environment variable 'GROUP_RANK' is not set. And HCCL_ZERO_COPY will not enable.",
  ...
  if (local_world_size_env == nullptr) {
    map["enable"] = "false";
    TORCH_NPU_WARN_ONCE(
        "Environment variable 'LOCAL_WORLD_SIZE' is not set. And HCCL_ZERO_COPY will not enable.",
...
void fill_equal_split_sizes_when_empty(std::vector<int64_t>& split_sizes, at::Tensor tensor, int group_size) {
  ...
  TORCH_CHECK(
      group_size > 0, "Invalid group size within current process group", group_size, DIST_ERROR(ErrCode::PARAM));
  TORCH_CHECK(
...
void checkAndMakePath(const char* path, std::string errormessage) {
  ...
  if (!is_dir) { // Not a directory, return false.
    if (!ec) { // ec.value() == 0 means the path may not exist or may be a regular file.
      TORCH_NPU_HCCL_LOGI("checkAndMakePath path is not a directory or path is not exist, path=%s", path);
    } else { // ec.value() != 0 indicates other issues, such as inaccessible path or path too long.
      TORCH_NPU_HCCL_LOGW(
          "checkAndMakePath is_directory failed: %s (errno=%d), path=%s", ec.message().c_str(), ec.value(), path);
      std::cout << "[checkAndMakePath] is_directory failed: " << ec.message() << " (errno=" << ec.value()
                << "), path=" << path << std::endl;
    }
  }
  ...
  if (access_ret != 0) { // No write permission.
    TORCH_NPU_HCCL_LOGI(
        "checkAndMakePath access W_OK failed: path=%s, ret=%d, errno=%d, errmsg=%s",
        path,
        access_ret,
        errno,
        std::strerror(errno));
  ...
  if (is_dir && access_ret == 0) { // It is a directory and has write permission. Return directly; no need to create
                                   // directory or verify permissions later.
    return;
  }
  ...
  if (created) {
    ...
    TORCH_NPU_HCCL_LOGI(
        "checkAndMakePath create_directories: path=%s, created=%d, ec.value=%d, ec.message=%s",
        path,
        created,
        ec.value(),
        ec.message().c_str());
    std::cout << "[checkAndMakePath] create_directories: path=" << path << ", created=" << created
              << ", ec.value=" << ec.value() << ", ec.message=" << ec.message() << std::endl;
    // Grant permissions.
    std::error_code permEc;
    fs::permissions(
        dir, fs::perms::owner_all | fs::perms::group_read | fs::perms::group_exec, permEc); // Grant 750 permissions.
    if (!permEc) { // Permission grant succeeded, log it.
      TORCH_NPU_HCCL_LOGI(
          "checkAndMakePath permissions: path=%s, permEc.value=%d, permEc.message=%s",
          path,
          permEc.value(),
          permEc.message().c_str());
    } else { // Permission grant failed, log it.
      TORCH_NPU_HCCL_LOGW(
          "checkAndMakePath permissions failed: %s (errno=%d), path=%s",
          permEc.message().c_str(),
          permEc.value(),
          path);
      std::cout << "[checkAndMakePath] permissions failed: " << permEc.message() << " (errno=" << permEc.value()
                << "), path=" << path << std::endl;
    }
  } else { // return False
    if (!ec) { // ec.value() == 0 means the directory already exists; no need to create it.
      ...
      if (access(path, W_OK) != 0) {
        TORCH_NPU_HCCL_LOGW(
            "checkAndMakePath final access W_OK failed: %s (errno=%d), path=%s", std::strerror(errno), errno, path);
        std::cout << "[checkAndMakePath] final access W_OK failed: " << std::strerror(errno) << " (errno=" << errno
                  << "), path=" << path << std::endl;
      }
    } else { // ec.value() != 0 means creation failed, possibly due to permission issues or the path not being a
             // directory. Log the error.
      TORCH_NPU_HCCL_LOGW(
          "checkAndMakePath create_directories failed: %s (errno=%d), path=%s", ec.message().c_str(), ec.value(), path);
      std::cout << "[checkAndMakePath] create_directories failed: " << ec.message() << " (errno=" << ec.value()
                << "), path=" << path << std::endl;
    }
  }
}
...
void createFile(const char* path) {
  ...
  if (fd == -1) {
    throw std::runtime_error(
        "Create file failed. Please check whether input file is valid." + DIST_ERROR(ErrCode::NOT_FOUND));
  }
  close(fd);
}
...
std::string dump_hccl_trace(bool includeCollectives, bool includeStackTraces, bool onlyActive) {
  return HCCLTraceBuffer::get()->dump(c10::nullopt, includeCollectives, includeStackTraces, onlyActive);
}

std::string dump_hccl_trace_json(bool includeCollectives, bool onlyActive) {
  return HCCLTraceBuffer::get()->dump_json(c10::nullopt, includeCollectives, onlyActive);
}
...
static c10d::control_plane::RegisterHandler dumpHcclHandler{
    ...
    [](const c10d::control_plane::Request& req, c10d::control_plane::Response& res) {
      ...
      std::unordered_map<std::string, bool> processedParams = {
          {includeCollectivesStr, true}, {includeStackTracesStr, true}, {onlyActiveStr, false}};
      ...
      for (const auto& [paramName, paramValue] : params) {
        ...
        if (it != processedParams.end()) {
          ...
          if (paramValue == "true") {
            ...
            res.setStatus(400);
            res.setContent("Invalid value for " + paramName + " valid values are true or false", "text/plain");
            return;
          }
        }
      }
      if (validParamCount < params.size()) {
        res.setStatus(400);
        res.setContent("Invalid parameters - unexpected param passed in", "text/plain");
        return;
      }
      res.setContent(
          dump_hccl_trace(
...
static c10d::control_plane::RegisterHandler jsonDumpHcclHandler{
    ...
    [](const c10d::control_plane::Request& req, c10d::control_plane::Response& res) {
      ...
      std::unordered_map<std::string, bool> processedParams = {{includeCollectivesStr, true}, {onlyActiveStr, false}};
      ...
      res.setStatus(200);
      res.setContent(
          dump_hccl_trace_json(processedParams[includeCollectivesStr], processedParams[onlyActiveStr]),
          "application/json");
    }};

c10::optional<std::function<void(std::function<void(const std::string&)>)>>& get_cpp_trace_dumper() {
  static c10::optional<std::function<void(std::function<void(const std::string&)>)>> dumper(c10::nullopt);
  return dumper;
}
...
ProcessGroupHCCL::WorkHCCL::WorkHCCL(
  ...
  if (desyncDebug || c10_npu::option::OptionsManager::CheckStatusSaveEnable() ||
      ProcessGroupHCCL::monitorThreadEnabled_.load()) {
...
void ProcessGroupHCCL::WorkHCCL::checkAndSetException() {
  ...
  if (exception_) {
    TORCH_NPU_HCCL_LOGE(
        "[Rank %d], found async exception when checking for HCCL errors: %s",
        rank_,
...
std::chrono::milliseconds GetDispatchTimeout() noexcept {
  ...
  if (hccl_exec_timeout > 0) {
    if (static_cast<uint32_t>(hccl_exec_timeout) < dispatchTimeout_ + dispatchoffset &&
        static_cast<uint32_t>(hccl_exec_timeout) > mindispatchTimeout_ + dispatchoffset) {
...
void ProcessGroupHCCL::WorkHCCL::checkDispatch() {
  if (!*is_dispatched && !is_reported) {
    ...
    TORCH_NPU_HCCL_LOGW(
        "Process group work %s, seq_num %u dispatch success. This warning log can be ignored.",
        opTypeToString(opType_).c_str(),
        seq_);
    is_reported = false;
  }
...
void ProcessGroupHCCL::WorkHCCL::synchronize() {
  ...
  if (c10d::allow_inflight_collective_as_graph_input()) {
    c10d::unregister_work(c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>::unsafe_reclaim_from_nonowning(this));
  }
}
  ...
  if (!recorded_inputs_.empty()) {
    ...
    for (auto i = 0; i < recorded_inputs_.size(); ++i) {
      ...
      if (storage) {
        ...
        c10_npu::NPUCachingAllocator::eraseStreamWithBlockPtr(
            recorded_block_ptr_for_inputs_[i], recorded_inputs_[i].second, static_cast<void*>(this));
      }
    }
  }
...
ProcessGroupHCCL::ProcessGroupHCCL(
  ...
  std::call_once(kOpWaitTimeoutInitFlag, [&]() {
    if (hccl_event_timeout > 0) {
      ...
      if (hccl_event_timeout <= hccl_exec_timeout) {
        TORCH_NPU_WARN_ONCE(
            "The value of HCCL_EVENT_TIMEOUT:",
            hccl_event_timeout,
            " is less than or equal to the value of HCCL_EXEC_TIMEOUT:",
            hccl_exec_timeout,
            ".");
      } else if (hccl_exec_timeout == 0) {
        TORCH_NPU_WARN_ONCE(
            "The value of HCCL_EXEC_TIMEOUT was set to 0(never timeout), so it is bigger than the value of HCCL_EVENT_TIMEOUT:",
            hccl_event_timeout,
            ".");
      }
  ...
  if (blockingWait_) {
    if (asyncErrorHandling_ != NoHandling || desyncDebug_) {
      ...
      desyncDebug_ = false;
      LOG(INFO) << logPrefix() << "TORCH_HCCL_BLOCKING_WAIT is enabled, NO watchdog thread is created.";
    }
  } else {
    if (desyncDebug_ && asyncErrorHandling_ == NoHandling) {
      LOG(INFO) << "[Rank " << rank_ << "] TORCH_HCCL_DESYNC_DEBUG and TORCH_HCCL_ASYNC_ERROR_HANDLING "
    ...
    if ((options_->timeout).count() != DEFAULT_TIMEOUT) {
      if ((options_->timeout).count() <= hccl_exec_timeout * 1000) {
        TORCH_NPU_WARN(
            ...
            "ms(which is set by init_process_group) is less than or equal to HCCL execution timeout ",
            hccl_exec_timeout * 1000,
            "ms! The plog may not be recorded.");
      } else if (hccl_exec_timeout == 0) {
        TORCH_NPU_WARN(
            "The HCCL execution timeout was set to 0(never timeout), so it is bigger than watchdog timeout ",
            (options_->timeout).count(),
            "ms which is set by init_process_group! The plog may not be recorded. You can disable watchdog by 'export TORCH_HCCL_ASYNC_ERROR_HANDLING=0'.");
      }
    } else {
      if (hccl_exec_timeout == 0) {
  ...
  if (options_->global_ranks_in_group.empty()) {
    ...
    if (c10_npu::option::OptionsManager::IsHcclZeroCopyEnable() &&
        c10_npu::NPUCachingAllocator::checkConfigExpandableSegments()) {
      TORCH_NPU_HCCL_LOGI(
          "Set the HCCL_ZERO_COPY environment variable in ExpandableSegments. Try to enable the HCCL_ZERO_COPY feature.");
      ...
      if (envMap["enable"] == "true") {
        ...
        TORCH_NPU_HCCL_LOGI(
            "Because the environment variables are not fully configured, the HCCL_ZERO_COPY feature cannot be enabled.");
      }
    } else {
      TORCH_NPU_HCCL_LOGI(
          "The IsHcclZeroCopyEnable function return %d, the checkConfigExpandableSegments function return %d.",
          c10_npu::option::OptionsManager::IsHcclZeroCopyEnable(),
          c10_npu::NPUCachingAllocator::checkConfigExpandableSegments());
    }
  }
...
void abortCommsFromMap(
  ...
  for (auto& it : hcclCommsMap) {
    ...
    if (abortReason.has_value()) {
      LOG(INFO) << "[Rank " << rank << "] Destroyed " << hcclComms.size() << "communicators on ASCEND device "
                << devName << " for reason: " << *abortReason;
    } else {
      LOG(INFO) << "[Rank " << rank << "] Destroyed " << hcclComms.size() << "communicators on ASCEND device "
                << devName;
    }
...
void ProcessGroupHCCL::waitForFutureOrTimeout(
  ...
  if (status == std::future_status::ready) {
    ...
    try {
      bool result = fut.get();
      if (result) {
        LOG(INFO) << logPrefix() << "future is successfully executed for: " << futDescription;
      }
    } catch (const std::exception& e) {
      errorMsg = c10::str(logPrefix(), "Exception thrown when waitng for future ", futDescription, ": ", e.what());
      LOG(ERROR) << errorMsg;
    } catch (...) {
      errorMsg = c10::str(logPrefix(), "Unknown exception thrown when waitng for future ", futDescription);
      LOG(ERROR) << errorMsg;
    }
  } else {
...
void ProcessGroupHCCL::shutdown() {
  ...
  std::vector<c10_npu::NPUStream> hcclStreamsToSync;
  {
    ...
    for (const auto& it : hcclStreams_) {
      hcclStreamsToSync.insert(hcclStreamsToSync.end(), it.second.begin(), it.second.end());
    }
  }
  ...
  if (hcclStreamsToSync.empty()) {
    LOG(INFO) << logPrefix() << "Skip shutdown stream synchronization because no HCCL streams were created.";
  } else {
...
ProcessGroupHCCL::~ProcessGroupHCCL() {
  ...
  if (hcclHeartbeatMonitorThread_.joinable()) {
    hcclHeartbeatMonitorThread_.join();
    LOG(INFO) << logPrefix() << "ProcessGroupHCCL heart beat monitor thread joined.";
  }
#endif
  {
...
bool ProcessGroupHCCL::dumpDebuggingInfo() {
  ...
  if (futStatus != std::future_status::ready) {
    ...
    LOG(ERROR) << "Could not acquire GIL within 3000 ms when dump python traceback, possible GIL induced hang";
  }
  ...
  if (hcclTraceBufferSize_ > 0) {
    ...
    LOG(ERROR) << logPrefix() << "ProcessGroupHCCL dumping hccl trace to " << writer.getWriterTarget();
...
void ProcessGroupHCCL::dumpTraceAndResetStatus() {
  ...
  // local disk)
  std::future<bool> asyncDebugDump = std::async(std::launch::async, [this]() { return this->dumpDebuggingInfo(); });

  // wait for the dump until timeout
  waitForFutureOrTimeout(
      asyncDebugDump, std::chrono::milliseconds(waitTimeoutDumpInMilSec_), "Flight recorder dump in heartbeatMonitor");
...
void ProcessGroupHCCL::heartbeatMonitor() {
    ...
    if (checkDumpSignal) {
      ...
      if (computeDeltaMS(lastWorkListUpdateTime_, currentTime) >= kWatchdogThreadSleepMillis &&
        ...
        try {
          ...
          LOG(ERROR) << logPrefix() << "Failed to get exception dump flag from the global store." << e.what();
          dumpTraceAndResetStatus();
        }
        if (checkExceptionDump) {
          ...
          try {
            ...
            LOG(ERROR) << logPrefix() << "Failed to get timeout rank ID from the global store." << e.what();
          }
          errorMsg = c10::str(
              logPrefix(),
  ...
  if (cpp_dumper.has_value()) {
    ...
    cpp_dumper.value()([](const std::string& line) { LOG(ERROR) << line; });
  }
  ...
  if ((terminateProcessGroup_.load() || collectiveDebugInfoMode_.load() || shouldDump_.load()) &&
  ...
  if (!terminateHeartbeatMonitorThread_.load()) {
    ...
    if (monitorThreadEnabled_.load()) {
      ...
      LOG(ERROR) << "PGHCCL Monitor Thread is disabled, but would have killed this job:\n" << finalExitMsg;
    }
  }
}
#endif
...
void ProcessGroupHCCL::Watchdog::start() {
  TORCH_CHECK(!hcclCommWatchdogThread_.joinable(), "Watchdog thread already started");
...
void ProcessGroupHCCL::Watchdog::run() {
  ...
  try {
    ...
    const auto exitMsg = c10::str("[Rank ", rank_, "] HCCL watchdog thread terminated with exception: ", e.what());
    LOG(ERROR) << exitMsg;
    ...
    const auto exitMsg = c10::str("[Rank ", rank_, "] HCCL watchdog thread terminated with exception: unknown");
    LOG(ERROR) << exitMsg;
    if (c10_npu::option::OptionsManager::CheckStatusSaveEnable()) {
...
int ProcessGroupHCCL::Watchdog::getSignalSrcRank(c10::intrusive_ptr<c10d::Store>& store, const std::string& signal) {
  ...
  try {
    ...
    LOG(WARNING) << pg_->logPrefix() << "Failed to check the signal " << signal << " on TCPStore, " << e.what();
  }
  if (!signalExists) {
    return srcRank;
  }
  ...
  try {
    ...
    LOG(ERROR) << pg_->logPrefix() << "Failed to get source rank of the signal " << signal << " from TCPStore."
               << e.what();
  }
  TORCH_CHECK_WITH(DistBackendError, vec.size() == sizeof(int), "Invalid size for the timeout rank ID");
  ...
  for (const auto& [name, hcclComms] : devHCCLCommMap_) {
    ...
    if (exception_ptr) {
      ...
      if (reportedErrorComms_.find(name) == reportedErrorComms_.end()) {
        ...
        TORCH_NPU_HCCL_LOGE(
            "[Rank %d] checkHcclComms found HcclComms vector %s got ERROR via HcclGetCommAsyncError : %s",
            rank_,
            name.c_str(),
            getExceptionMsgFromExceptionPtr(exception_ptr).c_str());
...
void ProcessGroupHCCL::Watchdog::runLoop() {
  ...
  while (!pg_->terminateProcessGroup_.load()) {
    ...
    if (status_save_enable) {
      ...
      checkAndMakePath(
          status_save_path.c_str(),
          "Failed to prepare HCCL status save directory. Please verify the path is accessible and writable.");
      timenow = std::chrono::steady_clock::now();
      recordflag =
          (std::chrono::duration_cast<std::chrono::milliseconds>(timenow - lastrecordtime).count() >
           (c10_npu::option::OptionsManager::GetStatusSaveInterval() * kThousandMillis));
    }

    {
      ...
      pg_->workMetaListCV_.wait_for(lock, std::chrono::milliseconds(kWatchdogThreadSleepMillis), [&]() -> bool {
        return pg_->terminateProcessGroup_.load();
      });
        ...
        if (pg_->dumpOnException_ && exec_timeout) {
          try {
            ...
            auto vec = std::vector<uint8_t>(
                reinterpret_cast<uint8_t*>(&rank), reinterpret_cast<uint8_t*>(&rank) + sizeof(rank));
            pg_->globalStore_->set(std::string(EXCEPTION_DUMP), vec);
            if (!ProcessGroupHCCL::shouldDump_.load()) {
              LOG(ERROR) << pg_->logPrefix() << "First watchdog exec timeout to set the dump signal.";
            }
            ProcessGroupHCCL::shouldDump_.store(true);
          } catch (const std::exception& e) {
            LOG(ERROR) << pg_->logPrefix() << "Failed to set exec timeout dump signal in tcpstore. "
                       << "Error: " << e.what();
          }
        }
        ...
        if (work.exception()) {
          ...
          if (pg_->dumpOnException_) {
            try {
              ...
              pg_->globalStore_->set(std::string(EXCEPTION_DUMP), vec);
              if (!ProcessGroupHCCL::shouldDump_.load()) {
                LOG(ERROR) << pg_->logPrefix() << "First watchdog to set the dump signal.";
              }
              ProcessGroupHCCL::shouldDump_.store(true);
              std::this_thread::sleep_for(std::chrono::seconds(pg_->heartbeatTimeoutInSec_));
            } catch (const std::exception& e) {
              LOG(ERROR) << pg_->logPrefix() << "Failed to set dump signal in tcpstore. "
                         << "Error: " << e.what();
            }
          }
...
void ProcessGroupHCCL::allgatherScalableRootInfos(
  ...
  TORCH_CHECK(numRoots > 0, "The scalable HCCL root info list must not be empty.", DIST_ERROR(ErrCode::PARAM));
  ...
  if (rootIdx >= 0) {
    TORCH_CHECK(
        rootInfo != nullptr, "The scalable HCCL root info must not be null on a root rank.", DIST_ERROR(ErrCode::PTR));
...
void ProcessGroupHCCL::recordDataVol(
  ...
  TORCH_NPU_HCCL_LOGD(
      "Record data volume for HCCL op %s, data volume %s, current rank %d.", opName.c_str(), dataVol.c_str(), currRank);
...
bool ProcessGroupHCCL::recordHcclStatus(const std::string path, bool end, bool error) {
  try {
    if (!options_->global_ranks_in_group.empty() &&
        !error) { // If it is a communication subgroup and no error, do not log.
      return true;
    }
    // Log errors for communication subgroups as well; always log for global communication groups regardless of error
    // status.
    ...
    if (StatusOutput_.empty()) {
      TORCH_NPU_HCCL_LOGI("recordHcclStatus return false: StatusOutput_ is empty");
      return false;
    }
    ...
    std::stringstream fileName;
    fileName << "torch_hccl_status-" << std::to_string(global_rank) << "_" << master_addr << "_"
             << std::to_string(deviceId_) << "_";
    ...
    createFile(tmp_file_path.c_str());
    {
      ...
      if (!outfile.good()) { // Write failed this time; print a log message.
        TORCH_NPU_HCCL_LOGW("Write status file %s failed.", tmp_file_path.c_str());
        TORCH_NPU_HCCL_LOGW("Status content: %s", result_str.c_str());
    ...
    if (std::rename(tmp_file_path.c_str(), out_file_path.c_str()) != 0) {
      ...
      TORCH_NPU_HCCL_LOGW(
          "Rename status file %s -> %s failed: %s (errno=%d).",
          tmp_file_path.c_str(),
          out_file_path.c_str(),
          std::strerror(renameErr),
          renameErr);
      std::cout << "[recordHcclStatus] Rename status file " << tmp_file_path << " -> " << out_file_path
                << " failed: " << std::strerror(renameErr) << " (errno=" << renameErr << ")" << std::endl;
      std::remove(tmp_file_path.c_str());
      return false;
    }
    return true;
  } catch (const std::exception& e) {
    TORCH_NPU_HCCL_LOGW("recordHcclStatus failed: %s", e.what());
    std::cout << "[recordHcclStatus] failed: " << e.what() << std::endl;
    return false;
  } catch (...) {
    TORCH_NPU_HCCL_LOGW("recordHcclStatus failed: unknown exception.");
...
void ProcessGroupHCCL::recordComm(
    ...
    std::vector<std::shared_ptr<HCCLComm>>& hcclComms) {
  TORCH_NPU_HCCL_LOGD(
      "Record HCCL comm, filename %s, opName %s, currRank %d.", filename.c_str(), opName.c_str(), currRank);
  std::ofstream outfile;
  std::string commName = getHcclCommNameWithoutInit(hcclComms);
  if (isFileExists(filename)) {
    try {
      ...
      throw std::runtime_error(
          "Open shared directory failed. Please check whether file is valid." + DIST_ERROR(ErrCode::UNAVAIL));
    }
  } else {
...
c10_npu::NPUStream ProcessGroupHCCL::getHcclNPUStream(const at::Device& device) {
  ...
  if (stream) {
    TORCH_NPU_HCCL_LOGD(
        "HCCL use the same stream with bufferName = %s, device_index = %d, stream id = %lu",
        bufferName.c_str(),
        device.index(),
        stream->id());
    return stream.value();
  ...
  TORCH_NPU_HCCL_LOGD(
      "HCCL use alloc new stream with bufferName = %s, device_index = %d, stream id = %lu. result = %d",
      bufferName.c_str(),
      device.index(),
      newStream.id(),
      result);
...
void ProcessGroupHCCL::createHCCLCommOrigin(
  ...
  if (commType == HcclCommType::DEFAULT && c10_npu::option::OptionsManager::IsScalableRootInfoEnable()) {
    if (!IsCompatibleSoc()) {
      ...
      TORCH_CHECK(ranksPerRoot > 0, "TORCH_HCCL_RANKS_PER_ROOT must be greater than 0.", DIST_ERROR(ErrCode::VALUE));
  ...
  for (size_t i = 0; i < devices.size(); ++i) {
    ...
    switch (commType) {
      ...
      default:
        throw std::runtime_error(
            "create/get the HCCL Communicator failed for comm type:" + std::to_string(static_cast<int>(commType)) +
            DIST_ERROR(ErrCode::PARAM));
    }
  ...
  auto timeElapsed = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
  TORCH_NPU_HCCL_LOGI(
      "Create hccl comm by hcclCommInitRootInfoConfig success, group id is %s, commType is %d, use %d ms.",
      options_->group_id.c_str(),
      static_cast<int>(commType),
      timeElapsed.count());
}
...
bool ProcessGroupHCCL::createHCCLCommEx(
  ...
  if (c10_npu::option::OptionsManager::GetHCCLConnectTimeout() < 300) {
    TORCH_NPU_WARN_ONCE(
        "When creating an HCCL process group using the RANK_TABLE_FILE method, the connection may time out. ",
  ...
  if (options_->global_ranks_in_group.empty() && commType == HcclCommType::DEFAULT) {
    ...
    auto timeElapsed = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    TORCH_NPU_HCCL_LOGI(
        "Create global hccl comm with ranktable success, take %d milliseconds", static_cast<int>(timeElapsed.count()));
    return true;
  }

  // sub process group
  return createHCCLCommSub(devicesKey, devices, commType, commConfig, hcclComms, streamVal, p2pRank);
}
...
bool ProcessGroupHCCL::createHCCLCommSub(
  ...
  if (!isOrigin) {
    ...
    globalHcclComm = global_->getHcclCommByDevicesFromCache(devices);
    if (!globalHcclComm) {
  ...
  for (size_t i = 0; i < devices.size(); ++i) {
    ...
    if (commType == HcclCommType::P2P) {
      ...
      if (options_->global_ranks_in_group.empty()) {
        ...
        TORCH_CHECK(
            highRank < options_->global_ranks_in_group.size(),
      ...
      std::string p2pName =
          "group" + options_->group_id + "_p2p_" + std::to_string(lowRank) + "_" + std::to_string(highRank);
      if (strlen(commConfig->hcclCommName) > 0) {
#ifndef BUILD_LIBTORCH
        torch_npu::toolkit::profiler::Utils::safe_strcpy_s(
            commConfig->hcclCommName, p2pName.c_str(), COMM_NAME_MAX_LENGTH);
#else
        strncpy(commConfig->hcclCommName, p2pName.c_str(), COMM_NAME_MAX_LENGTH - 1);
        commConfig->hcclCommName[COMM_NAME_MAX_LENGTH - 1] = '\0';
#endif
      }
      if ((!isOrigin) && strlen(commConfig->hcclUdi) > 0) {
#ifndef BUILD_LIBTORCH
      ...
      subComm = HCCLComm::createSubHcclComm(
          globalHcclComm, numRanks, options_->global_ranks_in_group.data(), hcclid, rank, commConfig);
    }
    if (subComm == nullptr) {
      TORCH_NPU_HCCL_LOGI(
          "Create sub hccl comm by hcclCreateSubCommConfig failed, group id is %s, subCommId is %llu, devicesKey is %s, switch to original interface.",
          options_->group_id.c_str(),
          hcclid,
          devicesKey.c_str());
      return false;
  ...
  TORCH_NPU_HCCL_LOGI(
      "Create sub hccl comm by hcclCreateSubCommConfig success, group id is %s, subCommId is %llu, devicesKey is %s, use %d ms.",
      options_->group_id.c_str(),
      hcclid,
      devicesKey.c_str(),
      static_cast<int>(subTimeElapsed.count()));
  return true;
}
...
void ProcessGroupHCCL::createHCCLCommForZeroCopy(
  ...
  TORCH_NPU_HCCL_LOGI(
      "Rank %s create process group  HCCL communicator for hccl zero copy", envMap["global_rank"].c_str());
  ...
  if (envMap["local_rank"] == localRootRank) {
    auto vec = std::vector<uint8_t>(
        reinterpret_cast<uint8_t*>(hcclID_), reinterpret_cast<uint8_t*>(hcclID_) + HCCL_ROOT_INFO_BYTES);
    store_->set(storeKey, vec);
  } else {
    try {
      auto vec = store_->get(storeKey);
...
std::vector<std::shared_ptr<HCCLComm>>& ProcessGroupHCCL::createHCCLComm(
  ...
  TORCH_NPU_HCCL_LOGI(
      "Create HCCL comm, devicesKey %s, commType %d, p2pRank %d.", devicesKey.c_str(), commType, p2pRank);
  ...
  for (const auto i : c10::irange(hcclActiveGroupCounter_)) {
    (void)i;
    auto hccl_call = [this]() -> HcclResult { return hcclGroupEnd(); };
  ...
  for (const auto i : c10::irange(hcclActiveGroupCounter_)) {
    (void)i;
    auto hccl_call = [this]() -> HcclResult { return hcclGroupStart(); };
...
int64_t ProcessGroupHCCL::getCollStreamId(at::Device device) {
  ...
  if (hcclStreams_.find(key) == hcclStreams_.end() || hcclStreams_[key].empty()) {
    bool force_high = c10d::getCvarBool(TORCH_HCCL_HIGH_PRIORITY, false);
    auto streamVal = c10_npu::getStreamFromPool(options_->is_high_priority_stream || force_high, device.index());
...
int64_t ProcessGroupHCCL::getP2PStreamId(at::Device device, int peer, int is_batched) {
  std::string key;
  if (is_batched == 1) {
...
static void cacheAllocatorRegisterHook(const c10_npu::NPUCachingAllocator::TraceEntry& te) {
  ...
  for (auto& it : hcclCommMemPoolMap) {
    ...
    auto found =
        std::find_if(memPools.begin(), memPools.end(), [&](const auto& p) { return std::get<0>(p) == te.mempool_; });
    if (found != memPools.end()) {
      // NOLINTNEXTLINE(performance-no-int-to-ptr)
      comm->registerSegment(
          reinterpret_cast<void*>(te.addr_), te.size_, /*errorOnRereg*/ false, /*symmetric*/ std::get<1>(*found));
    }
  }
}
...
void ProcessGroupHCCL::deregisterMemPool(c10_npu::MemPool* pool) {
      ...
      DIST_ERROR(ErrCode::UNAVAIL));
  {
    ...
    if (iter != hcclCommMemPoolMap.end()) {
      ...
      auto found =
          std::find_if(memPools.begin(), memPools.end(), [&](const auto& p) { return std::get<0>(p) == pool->id(); });
      TORCH_CHECK(
          found != memPools.end(),
...
void ProcessGroupHCCL::windowRegisterAndExchange(int64_t windowSize, std::vector<uint32_t>& peerRanks) {
  ...
  TORCH_CHECK(
      !c10_npu::option::OptionsManager::IsHcclZeroCopyEnable(),
      "Window memory register unsupported set HCCL_ZERO_COPY=1",
      DIST_ERROR(ErrCode::UNAVAIL));
...
void ProcessGroupHCCL::assignTimeoutToWork(
    const c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>& work,
...
void check_npu_tensors_same_device(const std::vector<at::Tensor>& tensors) {
  ...
  for (const auto& t : tensors) {
    ...
    TORCH_CHECK(t.scalar_type() == first.scalar_type(), "Tensors must have identical type", DIST_ERROR(ErrCode::TYPE));

    TORCH_CHECK(
        t.is_non_overlapping_and_dense(), "Tensors must be non-overlapping and dense", DIST_ERROR(ErrCode::TYPE));

    TORCH_CHECK(t.get_device() == first.get_device(), "Tensors must be on same NPU device", DIST_ERROR(ErrCode::TYPE));
    if (!at_npu::native::FormatHelper::IsBaseFormatType(t) && (t.storage().data_ptr().get() != t.data_ptr())) {
      TORCH_CHECK(
          false, "For a tensor of internal format, it's storage_offset must be 0", DIST_ERROR(ErrCode::NOT_SUPPORT));
    }
...
void check_npu_single_tensor(const at::Tensor& tensor) {
  ...
  if (!at_npu::native::FormatHelper::IsBaseFormatType(tensor) &&
      (tensor.storage().data_ptr().get() != tensor.data_ptr())) {
    TORCH_CHECK(
        false, "For a tensor of internal format, it's storage_offset must be 0", DIST_ERROR(ErrCode::NOT_SUPPORT));
  }
}
...
void ProcessGroupHCCL::resumeHcclComm(int device_id) {
  ...
  TORCH_NPU_HCCL_LOGI(
      "resumeHcclComm success, group id is %s, device_id is %d.", options_->group_id.c_str(), device_id);
}
...
bool ProcessGroupHCCL::setSwitchNicComm(
    int rankid,
    int nranks,
    std::vector<uint32_t>& ranks,
    std::vector<bool>& useBackup) {
  ...
  auto key = getKeyFromDevices(devices);
  {
    ...
    if (devHCCLCommMap_.find(key) != devHCCLCommMap_.end()) {
      ...
      for (auto& hcclComm : hcclComms) {
        ...
        bool result =
            setCommWorkingDevNic(comm, nranks, ranks, useBackup, rankid, hcclComm->hcclCommType, hcclComm->p2pPeer);
        if (!result) {
          return false;
        }
      }
...
void ProcessGroupHCCL::setHcclCommName(const std::string& hccl_comm_name) {
  ...
  TORCH_CHECK(
      nameSize > 0 && nameSize < COMM_NAME_MAX_LENGTH,
      "The length of the name must be between 1 and ",
      COMM_NAME_MAX_LENGTH - 1,
      ", Invalid hcclCommName:",
      hccl_comm_name,
      DIST_ERROR(ErrCode::VALUE));
...
#ifndef BUILD_LIBTORCH
    torch_npu::toolkit::profiler::Utils::safe_strcpy_s(config.hcclCommName, hcclCommName.c_str(), COMM_NAME_MAX_LENGTH);
#else
...
void ProcessGroupHCCL::silenceCheck(at::Tensor& input, c10d::OpType opType) {
  ...
  op_plugin::_npu_silent_check_v2(
      val,
      input,
      silenceCheckCache_[opType].second,
      silenceCheckCache_[opType].first,
      min_steps,
      c10_npu::option::OptionsManager::GetSilenceUpperThresh().first,
      c10_npu::option::OptionsManager::GetSilenceSigmaThresh().first,
      c10_npu::option::OptionsManager::GetSilenceUpperThresh().second,
      c10_npu::option::OptionsManager::GetSilenceSigmaThresh().second,
...
HcclCommConfig ProcessGroupHCCL::createHcclCommConfigWithOptions() {
  ...
  if (options_->hccl_config.find("group_name") != options_->hccl_config.end()) {
    if (std::holds_alternative<std::string>(options_->hccl_config["group_name"])) {
      ...
      if (hcclGroupName.length() >= UDI_MAX_LENGTH) {
        ...
        TORCH_NPU_WARN(
            "The length of group_name has exceeded the limit UDI_MAX_LENGTH which will be truncated to UDI_MAX_LENGTH - 1.");
      }
  ...
  if (options_->hccl_config.find("hccl_sym_win_max_mem_size_per_rank") != options_->hccl_config.end()) {
    if (std::holds_alternative<uint64_t>(options_->hccl_config["hccl_sym_win_max_mem_size_per_rank"])) {
      config.hcclSymWinMaxMemSizePerRank =
          std::get<uint64_t>(options_->hccl_config["hccl_sym_win_max_mem_size_per_rank"]);
    } else if (std::holds_alternative<uint32_t>(options_->hccl_config["hccl_sym_win_max_mem_size_per_rank"])) {
      config.hcclSymWinMaxMemSizePerRank =
          static_cast<uint64_t>(std::get<uint32_t>(options_->hccl_config["hccl_sym_win_max_mem_size_per_rank"]));
    } else {
  ...
  if (options_->hccl_config.find("hccl_algo") != options_->hccl_config.end()) {
    if (std::holds_alternative<std::string>(options_->hccl_config["hccl_algo"])) {
      ...
      if (length >= HCCL_COMM_ALGO_MAX_LENGTH) {
        ...
        TORCH_NPU_WARN(
            "The length of hccl_algo has exceeded the limit HCCL_COMM_ALGO_MAX_LENGTH(1600) which will be truncated to HCCL_COMM_ALGO_MAX_LENGTH - 1.");
      }
  ...
  if (options_->hccl_config.find("hccl_retry_enable") != options_->hccl_config.end()) {
    if (std::holds_alternative<std::string>(options_->hccl_config["hccl_retry_enable"])) {
      ...
      if (length >= HCCL_COMM_RETRY_ENABLE_MAX_LENGTH) {
        ...
        TORCH_NPU_WARN(
            "The length of hccl_retry_enable has exceeded the limit HCCL_COMM_RETRY_ENABLE_MAX_LENGTH(50) which will be truncated to HCCL_COMM_RETRY_ENABLE_MAX_LENGTH - 1.");
      }
  ...
  if (options_->hccl_config.find("hccl_retry_params") != options_->hccl_config.end()) {
    if (std::holds_alternative<std::string>(options_->hccl_config["hccl_retry_params"])) {
      ...
      if (length >= HCCL_COMM_RETRY_PARAMS_MAX_LENGTH) {
        ...
        TORCH_NPU_WARN(
            "The length of hccl_retry_params has exceeded the limit HCCL_COMM_RETRY_PARAMS_MAX_LENGTH(128) which will be truncated to HCCL_COMM_RETRY_PARAMS_MAX_LENGTH - 1.");
      }
  ...
  if (options_->hccl_config.find("hccl_buffer_name") != options_->hccl_config.end()) {
    if (std::holds_alternative<std::string>(options_->hccl_config["hccl_buffer_name"])) {
      ...
      if (length >= BUFFER_NAME_MAX_LENGTH) {
        ...
        TORCH_NPU_WARN(
            "The length of hccl_buffer_name has exceeded the limit BUFFER_NAME_MAX_LENGTH(128) which will be truncated to BUFFER_NAME_MAX_LENGTH - 1.");
      }
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::collective(
    ...
    if (perfdumppath.empty()) {
      ...
      if (isFileExists(path_temp)) {
        ...
        try {
          ...
          throw std::runtime_error(
              "Open shared directory failed. Please check whether perfdumppath is valid." +
              DIST_ERROR(ErrCode::NOT_FOUND));
        }
...

  {
    for (const auto i : c10::irange(inputs.size())) {
      ...
      HCCL_CHECK_ERROR(
          fn(inputs[i], outputs[i], hcclComms[i]->getHcclComm(), hcclStream, work->is_dispatched),
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    ...
    TORCH_NPU_HCCL_LOGI(
        "Event: record hccl work is successfully executed, event=%p", (*(work->hcclEndEvents_))[i].event());
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::collectiveCoalesced(
  ...
  if (coalescing_state_ & CoalActive) {
    ...
    if (coalescedComm_ == nullptr) {
      ...
      TORCH_CHECK(coalescedComm_ == hcclComms[0], "Expecting same communicator across coalesced P2P operations.");
    }
  }
...

  {
    for (const auto i : c10::irange(inputs.size())) {
      ...
      hcclUs startut = std::chrono::steady_clock::now();
      HCCL_CHECK_ERROR(
          fn(inputs[i], outputs[i], hcclComms[0]->getHcclComm(), hcclStream, work->is_dispatched),
          opTypeToString(opType).c_str());
      if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() == c10_npu::option::ERASE_RECORD_STREAM) {
        work->recorded_outputs_.push_back(std::make_pair(outputs[i].storage().getWeakStorageImpl(), hcclStream));
      } else if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() == c10_npu::option::AVOID_RECORD_STREAM) {
  ...
  TORCH_NPU_HCCL_LOGI(
      "Event: record hccl work is successfully executed, event=%p", (*(work->hcclEndEvents_))[0].event());
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::pointToPoint(
  ...
  }
  {
    for (const auto i : c10::irange(tensors.size())) {
      ...
      HCCL_CHECK_ERROR(
          fn(tensors[i], hcclComms[i]->getHcclComm(), hcclStream, is_dispatched, p2pTargetRank),
          opTypeToString(opType).c_str());
      if (coalescing_state_) {
        auto hccl_call = [this]() -> HcclResult { return hcclGroupEnd(); };
        at_npu::native::OpCommand::RunOpApiV3("hcclGroupEnd", hccl_call);
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::allreduce(
  ...
  return collective(
      ...
      [&](at::Tensor& input,
        ...
        auto hccl_call =
            [inputDataPtr, outputDataPtr, numel, hcclType, hcclReduceOp, comm, stream, is_dispatched]() -> int {
#ifndef BUILD_LIBTORCH
          torch_npu::profiler::MstxRange range(
              getMstxHcclMsg("HcclAllreduce", numel, hcclType, comm, stream.id(), -1, -1),
              stream.stream(false),
          ...
          auto hccl_result =
              hcclAllReduce(inputDataPtr, outputDataPtr, numel, hcclType, hcclReduceOp, comm, stream.stream(false));
          *is_dispatched = true;
          return hccl_result;
        };
      ...
      [&](std::vector<c10_npu::NPUStream>& hcclStreams, c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>&) {
        ...
        if (opts.reduceOp == c10d::ReduceOp::PREMUL_SUM) {
          ...
          const auto* preMulSupplement =
              reinterpret_cast<c10d::NCCLPreMulSumSupplement*>(opts.reduceOp.supplement_.get());
      ...
      [&](at::Tensor& input,
        ...
        for (size_t i = 0; i < remote_rank_list.size(); ++i) {
          if (remote_rank_list[i] < 0 || remote_rank_list[i] > std::numeric_limits<uint32_t>::max()) {
            throw std::runtime_error(
                "Value at index " + std::to_string(i) + " (" + std::to_string(remote_rank_list[i]) +
        ...
        auto hccl_call = [tensor_ptr_list,
          ...
          for (size_t i = 0; i < op_type.size(); ++i) {
            ...
            sendRecvInfo[i] =
                HcclSendRecvItem{currType, tensor_ptr_list[i], numel_list[i], type_list[i], remote_rank_list_cast[i]};
          }
#ifndef BUILD_LIBTORCH
          torch_npu::profiler::MstxRange range(
              getMstxHcclMsg(
                  "HcclBatchSendRecv", sendRecvInfo[0].count, sendRecvInfo[0].dataType, comm, stream.id(), -1, -1),
              stream.stream(false),
              torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
          if (c10_npu::is_core_control_enabled()) {
      ...
      [&](std::vector<c10_npu::NPUStream>& hcclStreams, c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>& work) {
        ...
        for (size_t i = 1; i < tensors.size(); ++i) {
          if (mode == c10_npu::option::AVOID_RECORD_STREAM) {
            work->stashed_for_allocator_safety_->stash(tensors[i]);
          } else {
            c10_npu::NPUCachingAllocator::recordStream(tensors[i].storage().data_ptr(), hcclStreams[0]);
...
#ifndef BUILD_LIBTORCH
          torch_npu::profiler::MstxRange range(
              getMstxHcclMsg("HcclBroadcast", numel, hcclType, comm, stream.id(), -1, -1),
              stream.stream(false),
              torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
          if (c10_npu::is_core_control_enabled()) {
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::reduce(
  ...
  return collective(
      ...
      [&](at::Tensor& input,
        ...
        auto hccl_call =
            [inputDataPtr, outputDataPtr, numel, hcclType, reduceOp, rank, comm, stream, is_dispatched]() -> int {
#ifndef BUILD_LIBTORCH
          torch_npu::profiler::MstxRange range(
              getMstxHcclMsg("HcclReduce", numel, hcclType, comm, stream.id(), -1, -1),
              stream.stream(false),
              torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
          if (c10_npu::is_core_control_enabled()) {
          ...
          auto hccl_result =
              hcclReduce(inputDataPtr, outputDataPtr, numel, hcclType, reduceOp, rank, comm, stream.stream(false));
          *is_dispatched = true;
          return hccl_result;
        };
...
at::Tensor ProcessGroupHCCL::byte_alignment(at::Tensor& tensors) const {
  ...
  int64_t byte_add =
      (tensor_byte % ADDRESS_ALIGNMENT_BYTE == 0) ? 0 : (ADDRESS_ALIGNMENT_BYTE - tensor_byte % ADDRESS_ALIGNMENT_BYTE);
  ...
  if (same_size) {
...
#ifndef BUILD_LIBTORCH
            torch_npu::profiler::MstxRange range(
                getMstxHcclMsg("HcclAllGather", numel, hcclType, comm, stream.id(), -1, -1),
                stream.stream(false),
                torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
            if (c10_npu::is_core_control_enabled()) {
          ...
          for (const auto i : c10::irange(outputTensors.size())) {
            ...
            for (const auto j : c10::irange(outputTensors[0].size())) {
              ...
              if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
                ...
                c10_npu::NPUCachingAllocator::recordStream(outputTensors[i][j].storage().data_ptr(), hcclStreams[i]);
    ...
    for (const auto i : c10::irange(num_reduces)) {
      ...
      auto broadcastOpts = c10d::BroadcastOptions{
          static_cast<int64_t>(i / num_devices), static_cast<int64_t>(i % num_devices), opts.timeout};
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::_allgather_base(
  ...
  if (inputTensor.numel() * size_ != outputTensor.numel()) {
    TORCH_CHECK(
        false, "output tensor size must be equal to world_size times input tensor size", DIST_ERROR(ErrCode::PARAM));
  }
  std::vector<at::Tensor> inputTensors = {inputTensor};
  std::vector<at::Tensor> outputTensors = {outputTensor};
  check_npu_tensors_different_devices(inputTensors);
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::reduce_scatter(
  ...
  if (same_size) {
...
#ifndef BUILD_LIBTORCH
            torch_npu::profiler::MstxRange range(
                getMstxHcclMsg("HcclReduceScatter", numel, hcclType, comm, stream.id(), -1, -1),
                stream.stream(false),
                torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
            if (c10_npu::is_core_control_enabled()) {
        ...
        [&](std::vector<c10_npu::NPUStream>& hcclStreams, c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>& work) {
          ...
          for (const auto i : c10::irange(outputTensors.size())) {
            ...
            if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() == c10_npu::option::AVOID_RECORD_STREAM) {
              ...
              c10_npu::NPUCachingAllocator::recordStream(outputTensors[i].storage().data_ptr(), hcclStreams[i]);
    ...
    for (const auto i : c10::irange(num_reduces)) {
      ...
      auto reduceOpts =
          c10d::ReduceOptions{opts.reduceOp, static_cast<int64_t>(i), static_cast<int64_t>(0), opts.timeout};
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::_reduce_scatter_base(
  ...
  if (inputTensor.numel() != outputTensor.numel() * size_) {
    TORCH_CHECK(
        false, "input tensor must be the same size as output size times world size", DIST_ERROR(ErrCode::PARAM));
  }
  ...
  return collective(
      ...
      [&](at::Tensor& input,
        ...
        auto hccl_call =
          ...
          auto hccl_result =
              hcclReduceScatter(inputDataPtr, outputDataPtr, numel, hcclType, hcclReduceOp, comm, stream.stream(false));
          *is_dispatched = true;
          return hccl_result;
        };
        at_npu::native::OpCommand::RunOpApiV3("HcclReduceScatter", hccl_call, false, &stream);
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::barrier(const c10d::BarrierOptions& opts) {
  ...
  for (auto& device : devices) {
    ...
    barrierTensors.push_back(at::ones({1}, at::TensorOptions().device(c10::DeviceType::PrivateUse1).dtype(at::kFloat)));
  }
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::endCoalescing(c10d::OpType optype) {
  ...
  TORCH_CHECK(
      coalescedDevice_.index() >= 0, "Something went wrong. Did you call end_coalescing before start_coalescing?");
  ...
  auto& hcclStreams = hcclStreams_[key];
  c10_npu::NPUStream& hcclStream = hcclStreams[0];
  auto opProfilerTitle =
      optype != c10d::OpType::COALESCED ? "hccl:" + opTypeToString(optype) + "_coalesced" : "hccl:coalesced";

  // Create Work object
  c10_npu::CaptureStatus capture_status = c10_npu::currentStreamCaptureStatusMayInitCtx();
  bool enqueue = (coalescing_state_) && capture_status == c10_npu::CaptureStatus::None;
  auto work = initWork(std::vector<c10::Device>{device}, rank_, optype, opProfilerTitle.c_str(), {}, {}, enqueue);
...
#ifndef BUILD_LIBTORCH
                  torch_npu::profiler::MstxRange range(
                      getMstxHcclMsg("HcclRecv", numel, hcclType, comm, stream.id(), static_cast<uint32_t>(r), -1),
                      stream.stream(false),
                      torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
                  auto hccl_result =
                      hcclRecv(outputDataPtr, numel, hcclType, static_cast<uint32_t>(r), comm, stream.stream(false));
                  *is_dispatched = true;
                  return hccl_result;
                };
...
#ifndef BUILD_LIBTORCH
              torch_npu::profiler::MstxRange range(
                  getMstxHcclMsg("HcclSend", numel, hcclType, comm, stream.id(), -1, static_cast<uint32_t>(root)),
                  stream.stream(false),
                  torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
              auto hccl_result =
                  hcclSend(inputDataPtr, numel, hcclType, static_cast<uint32_t>(root), comm, stream.stream(false));
              *is_dispatched = true;
              return hccl_result;
            };
      ...
      [this, opts, use_compatible_impl, inputTensors, is_compatible_soc](
        ...
        if (use_compatible_impl && is_compatible_soc) {
...
#ifndef BUILD_LIBTORCH
                    torch_npu::profiler::MstxRange range(
                        getMstxHcclMsg("HcclSend", numel, hcclType, comm, stream.id(), -1, static_cast<uint32_t>(r)),
                        stream.stream(false),
                        torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
                    auto hccl_result =
                        hcclSend(inputDataPtr, numel, hcclType, static_cast<uint32_t>(r), comm, stream.stream(false));
                    *is_dispatched = true;
                    return hccl_result;
                  };
                  at_npu::native::OpCommand::RunOpApiV3("HcclSend", hccl_call, false, &stream);
...
#ifndef BUILD_LIBTORCH
                torch_npu::profiler::MstxRange range(
                    getMstxHcclMsg("HcclRecv", numel, hcclType, comm, stream.id(), static_cast<uint32_t>(root), -1),
                    stream.stream(false),
                    torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
                auto hccl_result =
                    hcclRecv(outputDataPtr, numel, hcclType, static_cast<uint32_t>(root), comm, stream.stream(false));
                *is_dispatched = true;
                return hccl_result;
              };
              at_npu::native::OpCommand::RunOpApiV3("HcclRecv", hccl_call, false, &stream);
          ...
          auto hccl_call = [inputDataPtr, outputDataPtr, numel, hcclType, root, comm, stream, is_dispatched]() -> int {
#ifndef BUILD_LIBTORCH
            torch_npu::profiler::MstxRange range(
                getMstxHcclMsg("HcclScatter", numel, hcclType, comm, stream.id(), -1, -1),
                stream.stream(false),
                torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
            if (c10_npu::is_core_control_enabled()) {
            ...
            auto hccl_result =
                hcclScatter(inputDataPtr, outputDataPtr, numel, hcclType, root, comm, stream.stream(false));
            *is_dispatched = true;
            return hccl_result;
          };
  ...
  auto ret = pointToPoint(
      ...
      [&](at::Tensor& input,
        ...
        auto hccl_call = [inputDataPtr, numel, hcclType, dst_rank, comm, stream, is_dispatched]() -> int {
#ifndef BUILD_LIBTORCH
          torch_npu::profiler::MstxRange range(
              getMstxHcclMsg("HcclSend", numel, hcclType, comm, stream.id(), -1, dst_rank),
              stream.stream(false),
              torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
          if (c10_npu::is_core_control_enabled()) {
          ...
          auto hccl_result =
              hcclSend(inputDataPtr, numel, hcclType, static_cast<uint32_t>(dst_rank), comm, stream.stream(false));
          *is_dispatched = true;
          return hccl_result;
        };
        at_npu::native::OpCommand::RunOpApiV3("HcclSend", hccl_call, false, &stream);
  ...
  auto ret = pointToPoint(
      ...
      [&](at::Tensor& output,
        ...
        auto hccl_call = [outputDataPtr, numel, hcclType, src_rank, comm, stream, is_dispatched]() -> int {
#ifndef BUILD_LIBTORCH
          torch_npu::profiler::MstxRange range(
              getMstxHcclMsg("HcclRecv", numel, hcclType, comm, stream.id(), src_rank, -1),
              stream.stream(false),
              torch_npu::profiler::DOMAIN_COMMUNICATION);
#endif
          if (c10_npu::is_core_control_enabled()) {
          ...
          auto hccl_result =
              hcclRecv(outputDataPtr, numel, hcclType, static_cast<uint32_t>(src_rank), comm, stream.stream(false));
          *is_dispatched = true;
          return hccl_result;
        };
        at_npu::native::OpCommand::RunOpApiV3("HcclRecv", hccl_call, false, &stream);
        ...
        [&](at::Tensor& input,
          ...
          auto hccl_call = [inputDataPtr,
            ...
            torch_npu::profiler::MstxRange range(
                getMstxHcclMsg(
                    "HcclAlltoAllV",
                    static_cast<uint64_t>(inputCounts.size()),
                    inputhcclDataType,
                    comm,
                    stream.id(),
                    -1,
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::alltoall(
    ...
    const c10d::AllToAllOptions& opts) {
  TORCH_CHECK(
      output_tensors.size() == size_,
      "the size of output_tensors and worldsize must equal",
      DIST_ERROR(ErrCode::PARAM));
  TORCH_CHECK(
      output_tensors.size() == input_tensors.size(),
      "the size of input_tensors and output_tensors must equal",
      DIST_ERROR(ErrCode::PARAM));
        ...
        if (use_compatible_impl && is_compatible_soc) {
          ...
          auto hccl_call = [inputDataPtr,
            ...
            torch_npu::profiler::MstxRange range(
                getMstxHcclMsg(
                    "HcclAlltoAllV",
                    static_cast<uint64_t>(input_counts.size()),
                    inputhcclDataType,
                    comm,
                    stream.id(),
                    -1,
