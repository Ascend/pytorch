...
void ProcessGroupHCCL::Watchdog::runLoop() {
  ...
  while (!pg_->terminateProcessGroup_.load()) {
    if (status_save_enable) {
      checkAndMakePath(
          status_save_path.c_str(), "Open shared directory failed. Please check whether input path is valid.");
...
void ProcessGroupHCCL::recordDataVol(
  ...
  try {
    ...
    throw std::runtime_error(
        "Open shared directory failed. Please check whether input path is valid." + DIST_ERROR(ErrCode::NOT_FOUND));
  }
