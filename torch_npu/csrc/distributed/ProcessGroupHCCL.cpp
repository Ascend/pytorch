...
void checkSupportedDataType(HcclDataType type, std::string functionName)
{
    ...
    static std::set<HcclDataType> a5ExtraDataTypes = {
        HCCL_DATA_TYPE_UINT64,
        HCCL_DATA_TYPE_FP64};
    bool is_atlas_a5 = c10_npu::GetSocVersion() >= c10_npu::SocVersion::Ascend950;
    bool supported = (supportedDataTypes.count(type) != 0) ||
        (is_atlas_a5 && a5ExtraDataTypes.count(type) != 0);
    TORCH_CHECK(
        supported,
    ...
    std::vector<at::Tensor>& outputTensors)
{
    if (tensor_lists.size() != outputTensors.size()) {
        TORCH_CHECK(false, "Tensor list operands to reduce_scatter must have the same length",
    ...
    for (const auto i : c10::irange(num_devices)) {
        TORCH_CHECK(!tensor_lists[i].empty(),
            "Tensor list operands to reduce_scatter must be non-empty",
            DIST_ERROR(ErrCode::PARAM));
        if (tensor_lists[i].front().get_device() != outputTensors[i].get_device()) {
            TORCH_CHECK(false, "Corresponding input/output tensors to reduce_scatter must all reside"
                " on the same device", DIST_ERROR(ErrCode::PARAM));
        }
        flattened[i] = c10d::newLikeFlat(tensor_lists, i);
    }
