...
void syncStreams(
    ...
    std::vector<c10_npu::NPUEvent>& lcclEvents,
...
std::vector<at_npu::lccl::LcclComm>& ProcessGroupLCCL::getLCCLComm(
  ...
  if (devicesKey.empty()) {
    throw std::runtime_error(
        "Not able to create/get the lccll Communicator since "
...
c10::intrusive_ptr<c10d::Work> ProcessGroupLCCL::collective(
...
c10::intrusive_ptr<c10d::Work> ProcessGroupLCCL::allreduce(
  ...
  return collective(
      ...
      [&](at::Tensor& input, at::Tensor& output, at_npu::lccl::LcclComm comm, c10_npu::NPUStream& stream) {
        ...
        auto lccl_call = [inputDataPtr, outputDataPtr, numel, lcclType, lcclReduceOp, stream, comm]() -> int {
          auto lccl_result = at_npu::lccl::LcclAllReduce(
              inputDataPtr, outputDataPtr, numel, lcclType, lcclReduceOp, comm, stream.stream(false));
          return lccl_result;
        };
        at_npu::native::OpCommand::RunOpApiV3("LcclAllreduce", lccl_call, false, &stream);
        return 0;
      },
      c10d::OpType::ALLREDUCE);
}

c10::intrusive_ptr<c10d::Work> ProcessGroupLCCL::allgather(
    ...
    return collective(
        ...
        [&](at::Tensor& input, at::Tensor& output, at_npu::lccl::LcclComm comm, c10_npu::NPUStream& stream) {
          ...
          auto lccl_call = [inputDataPtr, outputDataPtr, numel, lcclType, comm, stream]() -> int {
            auto lccl_result =
                at_npu::lccl::LcclAllGather(inputDataPtr, outputDataPtr, numel, lcclType, comm, stream.stream(false));
            return lccl_result;
          };
          at_npu::native::OpCommand::RunOpApiV3("LcclAllgather", lccl_call, false, &stream);
          return 0;
        },
        [&](std::vector<c10_npu::NPUStream>&, c10::intrusive_ptr<ProcessGroupLCCL::WorkLCCL>&) {},
        [&](std::vector<c10_npu::NPUStream>& lcclStreams, c10::intrusive_ptr<ProcessGroupLCCL::WorkLCCL>& work) {
          ...
          for (const auto i : c10::irange(outputTensors.size())) {
            ...
            for (const auto j : c10::irange(outputTensors[0].size())) {
              // See [Sync Streams].
              c10_npu::NPUCachingAllocator::recordStream(outputTensors[i][j].storage().data_ptr(), lcclStreams[i]);
...
c10::intrusive_ptr<c10d::Work> ProcessGroupLCCL::broadcast(
    std::vector<at::Tensor>& tensors,
  ...
  return collective(
      ...
      [&](at::Tensor& input, at::Tensor& output, at_npu::lccl::LcclComm comm, c10_npu::NPUStream& stream) {
        ...
        at_npu::native::OpCommand::RunOpApiV3("LcclBroadcast", lccl_call, false, &stream);
        return 0;
      },
      c10d::OpType::BROADCAST);
}

c10::intrusive_ptr<c10d::Work> ProcessGroupLCCL::reduce_scatter(
  ...
  if (same_size) {
    ...
    return collective(
        ...
        [&](at::Tensor& input, at::Tensor& output, at_npu::lccl::LcclComm comm, c10_npu::NPUStream& stream) {
          ...
          auto lcclReduceOp = getLcclReduceOp(opts.reduceOp, input);
          auto lccl_call = [inputDataPtr, outputDataPtr, numel, lcclType, lcclReduceOp, stream, comm]() -> int {
            auto lccl_result = at_npu::lccl::LcclReduceScatter(
                inputDataPtr, outputDataPtr, numel, lcclType, lcclReduceOp, comm, stream.stream(false));
            return lccl_result;
          };
          at_npu::native::OpCommand::RunOpApiV3("LcclReduceScatter", lccl_call, false, &stream);
          return 0;
        },
        [&](std::vector<c10_npu::NPUStream>& lcclStreams, c10::intrusive_ptr<ProcessGroupLCCL::WorkLCCL>& work) {
          ...
          for (const auto i : c10::irange(inputTensors.size())) {
            ...
            for (const auto j : c10::irange(inputTensors[0].size())) {
              // See [Sync Streams].
              c10_npu::NPUCachingAllocator::recordStream(inputTensors[i][j].storage().data_ptr(), lcclStreams[i]);
