...
Proxy::Proxy(
    ...
    : localServer_{std::make_unique<torch_npu::ParallelStoreServer>(
          localSocketPath,
...
StoreMessage Proxy::HandleLocalServerMessage(const int& fd, const torch_npu::StoreMessage& message) noexcept {
  ...
  if (ret < 0) {
    LOG(ERROR) << "write data to server (" << host_ << ":" << port_ << ") failed" << errno << ":" << strerror(errno);
  }
...
void Proxy::WriteData(const int& fd, std::vector<uint8_t>& buf, int64_t& unpackSize) noexcept {
  ...
  while (offset < unpackSize) {
    ...
    if (ret <= 0) {
      LOG(ERROR) << "proxy write buf data failed. fd: " << fd << "host:" << host_ << "port:" << port_ << "ret:" << ret
  ...
  while (running_) {
    ...
    while (!StoreMessagePacker::Full(responseBuf)) {
      ...
      if (ret < 0) {
        ...
        if (running_) {
          LOG(ERROR) << "proxy read data from server failed. fd: " << tcpClient_->GetSocketFd() << "host:" << host_
