...
// Write HCCL debug info to local disk or any storage users define.
...
struct FlightRecorder {
    ...
    std::optional<size_t> record(size_t pg_id, const std::tuple<std::string, std::string> &pg_name,
                                 size_t collective_seq_id, size_t p2p_seq_id, size_t op_id, std::string profiling_name,
                                 const std::vector<at::Tensor> &inputs, const std::vector<at::Tensor> &outputs,
                                 Event *start, Event *end, std::chrono::milliseconds timeout_ms,
  ...
  watchdog thread, which is currently a place we call CANN APIs which may hang,
