...
uint64_t computeFlops(
  ...
  if (op_name == kConv2dOp) {
    if (extra_args.find(kInputSize) == extra_args.end() ||
      ...
      TORCH_NPU_WARN(
          "Calculating flops for aten::conv2d requires groups, padding, stride, dilation, input_size, "
    ...
    if (!padding_ref.isIntList() || !stride_ref.isIntList() ||
      ...
      TORCH_NPU_WARN(
          "Failed to compute flops for op aten::conv2d because it requires padding, stride, and "
          "dilation values.");
      return 0;
    }
