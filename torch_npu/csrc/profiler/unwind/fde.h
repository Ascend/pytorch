...
#include "torch_npu/csrc/profiler/unwind/action.h"
#include "torch_npu/csrc/profiler/unwind/lexer.h"
...
template <bool LOG = false> struct FDE {
    FDE(void* data, const char* library_name, uint64_t load_bias) : library_name_(library_name), load_bias_(load_bias)
    {
        Lexer L(data);
        ...
        TORCH_INTERNAL_ASSERT(version == 1 || version == 3, "non-1 version for CIE");
    ...
    {
        out_ = &out;
        out << "FDE(augmentation_string=" << augmentation_string_ << ", low_pc=" << (void*)(low_pc_ - load_bias_)
            << ",high_pc=" << (void*)(high_pc_ - load_bias_) << ",code_alignment_factor=" << code_alignment_factor_
            << ", data_alignment_factor=" << data_alignment_factor_ << ", ra_register_=" << ra_register_ << ")\n";
        readUpTo(high_pc_);
        out_ = &std::cout;
    }
    ...
    TableState readUpTo(uint64_t addr)
    {
        ...
        if (LOG) {
            (*out_) << "readUpTo " << (void*)addr << " for " << library_name_ << " at " << (void*)load_bias_ << "\n";
        }
