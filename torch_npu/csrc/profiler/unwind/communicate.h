...
#include "torch_npu/csrc/profiler/unwind/unwind_error.h"
...
struct Communicate {
    Communicate(const char* command, const char** args)
    {
        ...
        if (pid < 0) {
            ...
            close(outpipe_[1]);
            close(errpipe_[1]);
            outbuf_.reset(new __gnu_cxx::stdio_filebuf<char>(inpipe_[1], std::ios::out));
            inbuf_.reset(new __gnu_cxx::stdio_filebuf<char>(outpipe_[0], std::ios::in));
            errbuf_.reset(new __gnu_cxx::stdio_filebuf<char>(errpipe_[0], std::ios::in));
