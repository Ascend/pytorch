...
{
    TORCH_CHECK(!active_, "Cannot clear state while PythonTracer is active.", PROF_ERROR(ErrCode::PARAM));
    event_count_ = 0;
...
{
    pybind11::gil_scoped_acquire gil;
    TORCH_CHECK(PyType_Ready(&TraceContextType) == 0, PROF_ERROR(ErrCode::PARAM));
    registerFunctions(
        &PythonTracer::call
    );
}
