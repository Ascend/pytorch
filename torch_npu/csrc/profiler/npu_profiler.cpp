...
struct NpuObserverContext : public at::ObserverContext {
  explicit NpuObserverContext(std::unique_ptr<torch_npu::toolkit::profiler::OpRangeData> data)
      : data_(std::move(data)) {}
  std::unique_ptr<torch_npu::toolkit::profiler::OpRangeData> data_;
};

struct NpuProfilerThreadLocalState : public ProfilerStateBase {
  explicit NpuProfilerThreadLocalState(const NpuProfilerConfig& config, std::set<NpuActivityType> activities)
  ...
  std::unique_ptr<NpuObserverContext> newOpEvent(const at::RecordFunction& fn) {
    return std::make_unique<NpuObserverContext>(std::make_unique<torch_npu::toolkit::profiler::OpRangeData>(
...
static void parseInputShapesAndDtypes(const at::RecordFunction& fn, OpRangeData* data, bool report_details) {
      ...
      at::RecordFunctionCallback(
          [](const at::RecordFunction& fn) -> std::unique_ptr<at::ObserverContext> {
            ...
            if ((C10_UNLIKELY(config.report_input_shapes))) {
              bool report_details =
                  config.report_input_shapes && (config.with_stack || config.with_modules) && config.profile_memory;
...
void reportMarkDataToNpuProfiler(uint32_t category, const std::string& msg, uint64_t correlation_id) {
  ProfilerMgr::GetInstance()->UploadWithLock(std::make_unique<torch_npu::toolkit::profiler::OpMarkData>(
      static_cast<int64_t>(Utils::GetClockTime()), category, correlation_id, Utils::GetTid(), Utils::GetPid(), msg));
}
