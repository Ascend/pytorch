...
struct NpuProfilerThreadLocalState : public ProfilerStateBase {
  explicit NpuProfilerThreadLocalState(
      ...
      : ProfilerStateBase(ProfilerConfig(ProfilerState::CPU)),
        npu_config_(config),
  ...
  bool tracePython() {
    return (config_.with_stack || config_.with_modules) &&
        activities_.count(NpuActivityType::CPU);
  }
  ...
  void reportMemoryUsage(
    ...
    if (config_.profile_memory &&
        ProfilerMgr::GetInstance()->ReportEnable().load(
            std::memory_order_relaxed)) {
      ProfilerMgr::GetInstance()->UploadWithLock(
          std::make_unique<torch_npu::toolkit::profiler::MemoryData>(
...
void initNpuProfiler(
  ...
  TORCH_CHECK(
      !realPath.empty(), "Invalid path", path, PROF_ERROR(ErrCode::PARAM));
...
static void parseInputShapesAndDtypes(
...
static void registerCallback(
  ...
  TORCH_INTERNAL_ASSERT(
      registeration_state_ptr,
      "Expected profiler state set",
      PROF_ERROR(ErrCode::PTR));
      ...
      at::RecordFunctionCallback(
          [](const at::RecordFunction& fn)
            ...
            if (C10_UNLIKELY(
              ...
              cs = cs.empty() ? torch::profiler::impl::prepareCallstack(
          ...
          [](const at::RecordFunction& fn, at::ObserverContext* ctx_ptr) {
            ...
            TORCH_INTERNAL_ASSERT(
                npu_ctx_ptr != nullptr, PROF_ERROR(ErrCode::PTR));
            ...
            if (ProfilerMgr::GetInstance()->ReportEnable().load(
                    std::memory_order_relaxed)) {
...
void enableProfilerInChildThread(const NpuProfilerConfig& config) {
  ...
  auto state =
      std::make_shared<NpuProfilerThreadLocalState>(config, activities);
  if (c10::ThreadLocalDebugInfo::get(c10::DebugInfoKind::PROFILER_STATE) !=
      nullptr) {
...
void disableProfilerInChildThread() {
  ...
  if (state_ptr == nullptr) {
    ASCEND_LOGE(
        "Can't disable Ascend Pytorch CPU Profiler when it's not running.");
    return;
  }
...
void reportMarkDataToNpuProfiler(
  ...
  ProfilerMgr::GetInstance()->UploadWithLock(
      std::make_unique<torch_npu::toolkit::profiler::OpMarkData>(
          static_cast<int64_t>(Utils::GetClockTime()),
          category,
          correlation_id,
          Utils::GetTid(),
...
void reportMemoryDataToNpuProfiler(const MemoryUsage& data) {
  ...
  if (c10_npu::acl::IsExistRtGetStreamId()) {
    ...
    if (c10_npu::acl::AclrtStreamGetId(data.stream, &stream_ptr) !=
        ACL_ERROR_NONE) {
      stream_id = -1;
    } else {
