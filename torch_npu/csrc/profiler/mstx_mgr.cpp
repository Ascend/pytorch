    ...
    static bool isEnable = []() -> bool {
        ...
        try {
            ...
            if (sampleConfigJson.contains("mstxDomainInclude") && sampleConfigJson["mstxDomainInclude"].is_string()) {
                mgr->domain_include_ =
                    Utils::SplitString(sampleConfigJson["mstxDomainInclude"].get<std::string>(), ",");
            }
            if (sampleConfigJson.contains("mstxDomainExclude") && sampleConfigJson["mstxDomainExclude"].is_string()) {
                mgr->domain_exclude_ =
                    Utils::SplitString(sampleConfigJson["mstxDomainExclude"].get<std::string>(), ",");
            }
...
bool MstxMgr::isMstxTxDomainEnable(const std::string &domainName)
{
    ...
    // if domain_include_ is empty and domain_exclude_ is not empty, disable domains in domain_exclude_.
