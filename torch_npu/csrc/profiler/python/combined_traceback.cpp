...
struct PythonTraceback : public CapturedTraceback::Python {
    ...
    void appendSymbolized(const std::vector<CapturedTraceback::PyFrame>& to_symbolize,
            ...
            if (stack_frames_for_code.ptr()) {
                ...
                if (!extra.is_none()) {
                    for (py::handle h : extra) {
                        result.tracebacks.back().push_back(result.all_frames.size());
                        result.all_frames.emplace_back(unwind::Frame{py::cast<std::string>(h[filename_s]),
...
{
    torch::CapturedTraceback::addPythonUnwinder(new PythonTraceback());
}
