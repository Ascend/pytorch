...
namespace {
const static char* VERSION = "1.11.0\0";
...
void FeatureMgr::FormatFeatureList(size_t size, void* featuresData)
{
    ...
    while ((features != nullptr) && (i < size)) {
        ...
        if (it == NAME_TABLE.end()) {
            NPU_LOGW("Do not support feature: %s, log is: %s", features->featureName, features->info.infoLog);
            continue;
        }
...
bool FeatureMgr::IsSupportFeature(FeatureType featureName)
{
    ...
    if (fmkIt == FMK_FEATURES.end() || profIt == profFeatures_.end()) {
        NPU_LOGW("FMW or CANN do not support this feature type is: %d.", featureName);
        return false;
    }
