...
#include <torch_npu/csrc/inductor/aoti_package/npu_model_package_loader.h>
...
std::string create_temp_dir()
{
...
#else
    // 1. 获取基础路径
    const char* base_path = std::getenv("AOTIMODEL_PATH");
    std::string template_path;
    if (base_path != nullptr && base_path[0] != '\0') {
        template_path = base_path; // 使用环境变量路径
    } else {
        template_path = "/tmp";    // 默认路径
    }

    // 2. 规范化路径格式（确保末尾有/）
    if (template_path.back() != '/') {
        template_path += '/';
    }
    template_path += "XXXXXX"; // 添加随机目录模板

    // 3. 创建可修改的字符缓冲区
    std::vector<char> buffer(template_path.begin(), template_path.end());
    buffer.push_back('\0'); // 确保以空字符结尾
    ...
    // 5. 返回生成的目录路径
    std::string res_path = std::string(buffer.data());
    std::cout << "[AOTINDUCTOR] create temp dir: " << res_path << std::endl;
    return res_path;
#endif
}
    ...
    const std::string& consts_filename)
{
    ...
    if (file_exists(serialized_weights_path)) {
        ...
        std::vector<char> padding(SIZE_16KB - so_size % SIZE_16KB, ' ');
...
void NPUAOTIModelPackageLoader::load_metadata(const std::string& cpp_filename)
{
...
NPUAOTIModelPackageLoader::NPUAOTIModelPackageLoader(
    const std::string& model_package_path)
    : NPUAOTIModelPackageLoader(model_package_path, "model") {}
    ...
    for (uint32_t i = 0; i < zip_archive.m_total_files; i++) {
        ...
        if (filename_str.length() >= model_directory.length() &&
            ...
            if (!recursive_mkdir(parent_path.c_str())) {
                std::ostringstream oss;
                oss << "Failed to create directory " << parent_path
...
AOTIModelContainerRunner* NPUAOTIModelPackageLoader::get_runner()
{
    return runner_.get();
}

std::vector<at::Tensor> NPUAOTIModelPackageLoader::run(
...
std::unordered_map<std::string, std::string> NPUAOTIModelPackageLoader::get_metadata()
{
    return metadata_;
}

std::vector<std::string> NPUAOTIModelPackageLoader::get_call_spec()
{
    return runner_->get_call_spec();
}

void NPUAOTIModelPackageLoader::load_constants(
...
std::vector<std::string> NPUAOTIModelPackageLoader::get_constant_fqns()
{
    std::unordered_map<std::string, std::string> constant_name_to_fqn =
        runner_->getConstantNamesToOriginalFQNs();
