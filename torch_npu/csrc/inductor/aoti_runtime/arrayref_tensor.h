...
public:
    ...
    template <typename Container, typename = std::enable_if_t<
        std::is_same_v<std::remove_const_t<decltype(std::declval<Container>().data())>, T *>>>
        /* implicit */
    ...
    template <typename A>
    /* implicit */
    MiniArrayRef(const std::vector<T, A> &Vec) : Data(Vec.data()), Length(Vec.size())
    {
    ...
    template <size_t N>
    /* implicit */
    constexpr MiniArrayRef(std::array<T, N> &Arr) : Data(Arr.data()), Length(N)
    {}
...
public:
    ...
    AtenTensorHandle expensiveCopyToTensor() const
    {
        ...
        AOTI_TORCH_ERROR_CODE_CHECK(aoti_torch_empty_strided(sizes_.size(), sizes_.data(), strides_.data(),
            aoti_torch_dtype<std::remove_const_t<T>>(), device_type_, device_idx_, &result));
    ...
    {
        AtenTensorHandle result = nullptr;
        AOTI_TORCH_ERROR_CODE_CHECK(aoti_torch_create_tensor_from_blob_npu_v2(data(), sizes_.size(), sizes_.data(),
            strides_.data(), 0, aoti_torch_dtype<std::remove_const_t<T>>(), device_type_, device_idx_, &result,
...
static_assert(sizeof(ArrayRefTensor<int>) ==
    3 * sizeof(MiniIntArrayRef) + 3 * sizeof(int32_t) + (alignof(ArrayRefTensor<int>) > 4 ? sizeof(int32_t) : 0),
    ...
    const int64_t *sizes_ptr, const int64_t *strides_ptr, int64_t storage_offset)
{
    ...
    return ArrayRefTensor<T>(MiniArrayRef<T>(self.data() + storage_offset, self.numel() - storage_offset),
        MiniArrayRef<const int64_t>(sizes_ptr, ndim), MiniArrayRef<const int64_t>(strides_ptr, ndim),
        self.device_type(), self.device_idx());
}
...
template <typename T>
inline const ArrayRefTensor<T> &unwrap_raii_handle_if_needed(const ArrayRefTensor<T> &tensor)
{
    return tensor;
}

template <typename T>
inline ArrayRefTensor<T> &unwrap_raii_handle_if_needed(ArrayRefTensor<T> &tensor)
{
    return tensor;
}

template <typename T>
inline const ArrayRefTensor<T> &wrap_with_raii_handle_if_needed(const ArrayRefTensor<T> &tensor)
{
    return tensor;
}

template <typename T>
inline ArrayRefTensor<T> &wrap_with_raii_handle_if_needed(ArrayRefTensor<T> &tensor)
{
    return tensor;
}

template <typename T>
inline RAIIAtenTensorHandle expensive_copy_to_tensor_if_needed(const ArrayRefTensor<T> &tensor)
{
    return tensor.expensiveCopyToTensor();
}

inline AtenTensorHandle expensive_copy_to_tensor_if_needed(AtenTensorHandle handle)
{
    return handle;
}
...
template <typename T>
RAIIAtenTensorHandle copy_arrayref_tensor_to_tensor(const ArrayRefTensor<T> &art)
{
...
template <typename T>
RAIIAtenTensorHandle borrow_arrayref_tensor_as_tensor(const ArrayRefTensor<T> &art)
{
