...
AOTI_NOINLINE static void throw_exception(const char* call, const char* file, int64_t line)
{
    std::stringstream ss;
...
{
    AOTI_TORCH_ERROR_CODE_CHECK(aoti_torch_delete_tensor_object(reinterpret_cast<AtenTensorHandle>(ptr)));
}
...
public:
    ...
    RAIIAtenTensorHandle(AtenTensorHandle handle) : handle_(handle, delete_tensor_object) {}

    ~RAIIAtenTensorHandle() { handle_.reset(); }
    ...
    operator AtenTensorHandle() const { return handle_.get(); }

    AtenTensorHandle release() { return handle_.release(); }

    AtenTensorHandle get() const { return handle_.get(); }
    ...
    {
        void* result = nullptr;
        AOTI_TORCH_ERROR_CODE_CHECK(aoti_torch_get_data_ptr(handle_.get(), &result));
        return result;
    }
...
public:
    ...
    MaybeOwningAtenTensorHandle& operator=(const MaybeOwningAtenTensorHandle& other) = delete;

    // Move constructor and move assignment operator
    MaybeOwningAtenTensorHandle(MaybeOwningAtenTensorHandle&& other) = default;
    MaybeOwningAtenTensorHandle& operator=(MaybeOwningAtenTensorHandle&& other) = default;

    // Steal the ownership from another RAIIAtenTensorHandle using std::move
    MaybeOwningAtenTensorHandle(RAIIAtenTensorHandle&& other) : raii_handle_(std::move(other))
    {
    ...
    MaybeOwningAtenTensorHandle(AtenTensorHandle handle) : raii_handle_(handle) { handle_ = raii_handle_.get(); }
    ...
    // Return a raw AtenTensorHandle to be used by aoti_torch functions
    // Note: this function does NOT transfer the ownership of the handle
    operator AtenTensorHandle() const { return handle_; }
    ...
    AtenTensorHandle get() const { return handle_; }
    ...
    {
        int64_t storage_offset = 0;
        AOTI_TORCH_ERROR_CODE_CHECK(aoti_torch_get_storage_offset(handle_, &storage_offset));
        return storage_offset;
    }
...
inline std::vector<RAIIAtenTensorHandle> steal_from_raw_handles_to_raii_handles(AtenTensorHandle* handles, size_t size)
{
...
inline AtenTensorHandle reinterpret_tensor_wrapper(AtenTensorHandle self, int64_t ndim, const int64_t* sizes_ptr,
                                                   const int64_t* strides_ptr, int64_t storage_offset)
{
    AtenTensorHandle result = nullptr;
    AOTI_TORCH_ERROR_CODE_CHECK(
        aoti_torch__reinterpret_tensor(self, ndim, sizes_ptr, strides_ptr, storage_offset, &result));
    return result;
}
...
inline AtenTensorHandle unwrap_raii_handle_if_needed(const RAIIAtenTensorHandle& handle) { return handle.get(); }

inline RAIIAtenTensorHandle wrap_with_raii_handle_if_needed(AtenTensorHandle handle)
{
...
public:
    ...
    AtenTensorHandle tensor() const { return handle_; }
...
inline void* get_data_ptr_wrapper(const ConstantHandle& constant) { return constant.data_ptr(); }

inline const ConstantHandle& unwrap_raii_handle_if_needed(const ConstantHandle& handle) { return handle; }

// Shouldn't be called.
inline AtenTensorHandle wrap_with_raii_handle_if_needed(const ConstantHandle& handle) = delete;
...
template <class T> T& temporary_reference(T&& t) { return t; }

#define CACHE_TORCH_DTYPE(typename) static auto cached_torch_dtype_##typename = aoti_torch_dtype_##typename()

#define CACHE_TORCH_DEVICE(device) static auto cached_torch_device_type_##device = aoti_torch_device_type_##device()

#define CACHE_TORCH_LAYOUT(layout) static auto cached_torch_layout_##layout = aoti_torch_layout_##layout()

#define CACHE_TORCH_MEMORY_FORMAT(format)                                                                              \
    static auto cached_torch_memory_format_##format = aoti_torch_memory_format_##format()
