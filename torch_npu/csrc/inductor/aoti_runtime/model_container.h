...
public:
    AOTInductorModelContainer(size_t num_models, const std::string& device_str,
                              const std::optional<std::string>& cubin_dir = std::nullopt)
    {
        ...
        for (size_t i = 0; i < num_models; ++i) {
            models_.push_back(AOTInductorModel::Create(constants_map_, constants_array_, device_str, cubin_dir));
             ...
             DeviceStreamType stream, AOTIProxyExecutorHandle proxy_executor)
    {
        ...
        ConstantState& const_folded = use_secondary_ ? constant_folded_secondary_ : constant_folded_;
        if (const_folded == ConstantState::INITIALIZED) {
            ...
            if (const_folded == ConstantState::INITIALIZED) {
                auto folded_const_map = model->run_const_fold(stream, proxy_executor, true);
                update_constant_buffer(std::move(folded_const_map), false, false);
                const_folded = ConstantState::FOLDED;
            }
            constants_folding_lk.unlock();
            model_lk.lock();
        } else if (const_folded != ConstantState::FOLDED) {
            throw std::runtime_error("Unknown constant state: " + toStringConstantState(constant_folded_));
        }
    ...
    void run_const_fold(bool inactive_buffer, DeviceStreamType stream, AOTIProxyExecutorHandle proxy_executor)
    {
        ...
        if (!inactive_buffer) {
            ...
            // const run.
            auto constants_map = get_constants_map(true);
            auto constants_array = get_constants_array(true);

            try {
                model->update_constants_map(constants_map, false);
                model->update_constants_array(constants_array);

                auto folded_const_map = model->run_const_fold(stream, proxy_executor);
                update_constant_buffer(std::move(folded_const_map), true, false);

                constants_map = get_constants_map(false);
                constants_array = get_constants_array(false);
                model->update_constants_map(constants_map, false);
                model->update_constants_array(constants_array);
                const_folded = ConstantState::FOLDED;
            } catch (...) {
    ...
    void assert_all_constants(const std::unordered_map<std::string, AtenTensorHandle>& constants_map)
    {
        ...
        for (size_t idx = 0; idx < num_constants; idx++) {
            ...
            if (it == constants_map.end()) {
                ...
                throw std::runtime_error(std::string("Cannot find constants ") + constant_name +
    ...
    void update_constant_buffer(std::unordered_map<std::string, AtenTensorHandle>&& constants_map, bool use_inactive,
                                bool validate_full_update)
    {
        ...
        for (size_t idx = 0; idx < num_constants; idx++) {
            ...
            constants_map_to_update->insert_or_assign(constant_name, RAIIAtenTensorHandle(tensor));
        }
        // Update the inactive constant array.
        update_array_from_map(get_constants_array(use_inactive), constants_map_to_update);
    }

    // This function updates the buffer for storing constants.
    void update_constant_buffer(const std::unordered_map<std::string, AtenTensorHandle>& constants_map,
                                bool use_inactive, bool validate_full_update, bool user_managed = false)
    {
        if (this->num_models() == 0) {
            throw std::runtime_error("No model available in container!");
        }
        ...
        for (size_t idx = 0; idx < num_constants; idx++) {
            ...
            auto* constants_blob_ptr = static_cast<uint8_t*>(get_constant_blob_ptr(use_inactive));
            ...
            constants_map_to_update->insert_or_assign(constant_name, RAIIAtenTensorHandle(tensor_handle));
        }
        // Update the inactive constant array.
        update_array_from_map(get_constants_array(use_inactive), constants_map_to_update);
    }

    void update_array_from_map(const std::shared_ptr<std::vector<ConstantHandle> >& constants_array,
                               const std::shared_ptr<ConstantMap>& constants_map)
    {
        auto num_constants = models_[0]->num_constants();
        for (size_t idx = 0; idx < num_constants; idx++) {
            if (constants_map->find(models_[0]->constant_name(static_cast<int64_t>(idx))) != constants_map->end()) {
                constants_array->at(idx) =
                    ConstantHandle(constants_map->find(models_[0]->constant_name(static_cast<int64_t>(idx)))->second);
            }
        }
    }
    ...
    void free_inactive_constant_buffer()
    {
        ...
        std::shared_ptr<ConstantMap> to_free_map = use_secondary_ ? constants_map_ : constants_map_secondary_;
    ...
    size_t num_inputs() const { return input_names_.size(); }

    size_t num_outputs() const { return output_names_.size(); }

    const char* input_name(size_t idx) const { return input_names_.at(idx).c_str(); }

    const char* output_name(size_t idx) const { return output_names_.at(idx).c_str(); }

    size_t num_models() const { return models_.size(); }

    const char* get_in_spec() const { return in_spec_; }

    const char* get_out_spec() const { return out_spec_; }
...
private:
    ...
    {
#if defined(USE_NPU)
        return RAII_npuMalloc(blob_size_);
#else
    ...
    {
        if ((get_inactive && use_secondary_) || (!get_inactive && !use_secondary_)) {
            if (!constant_blob_) {
    ...
    std::shared_ptr<std::vector<ConstantHandle> > get_constants_array(bool get_inactive)
    {
        if ((get_inactive && use_secondary_) || (!get_inactive && !use_secondary_)) {
            ...
            if (!constants_array_secondary_) {
                constants_array_secondary_ =
                    std::make_shared<std::vector<ConstantHandle> >(models_[0]->num_constants());
            }
    ...
    void reclaim_finished_models(std::unique_lock<std::mutex>& lk)
    {
        ...
        auto it = std::partition(pending_models_.begin(), pending_models_.end(),
                                 [](AOTInductorModel* m) { return !m->is_finished(); });
#else
        // push finished model instances to the end of pending_models_
        auto it = std::stable_partition(pending_models_.begin(), pending_models_.end(),
                                        [](AOTInductorModel* m) { return !m->is_finished(); });
#endif
