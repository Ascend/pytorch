...
AOTIModelContainerRunnerNpu::AOTIModelContainerRunnerNpu(const std::string &model_so_path, size_t num_models,
    ...
    : AOTIModelContainerRunner(model_so_path, num_models, device_str, cubin_dir)
{
...
std::vector<at::Tensor> AOTIModelContainerRunnerNpu::run(const std::vector<at::Tensor> &inputs, void *stream_handle)
{
    init_proxy_executor();
    c10_npu::NPUStream npu_stream = c10_npu::getCurrentNPUStream();
    return AOTIModelContainerRunner::run(inputs, reinterpret_cast<AOTInductorStreamHandle>(npu_stream.stream()));
}

std::vector<at::Tensor> AOTIModelContainerRunnerNpu::run_with_npu_stream(std::vector<at::Tensor> &inputs,
