...
namespace {
at::Tensor* tensor_handle_to_tensor_pointer(AtenTensorHandle handle) { return reinterpret_cast<at::Tensor*>(handle); }
} // namespace

namespace torch::aot_inductor {

void OSSProxyExecutor::prefill_stack_with_static_arguments(size_t index, const at::TypePtr& schema_arg_type,
                                                           const nlohmann::json& serialized_arg, OSSOpKernel& op_kernel)
{
    ...
    switch (schema_arg_type->kind()) {
        case c10::TypeKind::TensorType: {
            TORCH_CHECK(serialized_arg_type == "as_tensor", "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_tensor for argument ", index, " but got ",
                        serialized_arg_type);
            dynamic_args.emplace_back(index, DynamicArgType::TensorType, 1);
            break;
        }
        case c10::TypeKind::IntType: {
            TORCH_CHECK(serialized_arg_type == "as_int", "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_int for argument ", index, " but got ",
                        serialized_arg_type);
            dynamic_args.emplace_back(index, DynamicArgType::IntType, 1);
            break;
        }
        case c10::TypeKind::SymIntType: {
            TORCH_CHECK(serialized_arg_type == "as_int" || serialized_arg_type == "as_sym_int",
                        "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_int or as_sym_int for "
                        "argument ",
                        index, " but got ", serialized_arg_type);
            dynamic_args.emplace_back(index, DynamicArgType::IntType, 1);
            break;
        }
        case c10::TypeKind::FloatType: {
            TORCH_CHECK(serialized_arg_type == "as_float", "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_float for argument ", index, " but got ",
                        serialized_arg_type);
            stack.at(index) = serialized_arg_val.get<double>();
            break;
        }
        case c10::TypeKind::BoolType: {
            TORCH_CHECK(serialized_arg_type == "as_bool", "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_bool for argument ", index, " but got ",
                        serialized_arg_type);
        ...
        case c10::TypeKind::NumberType: {
            if (serialized_arg_type == "as_int") {
                ...
                TORCH_CHECK(false, "Expected extern kernel ", op_kernel.target_,
                            " to have a scalar input for argument ", index, " but got ", serialized_arg_type);
            }
            break;
        }
        case c10::TypeKind::StringType: {
            TORCH_CHECK(serialized_arg_type == "as_string", "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_string for argument ", index, " but got ",
                        serialized_arg_type);
            stack.at(index) = serialized_arg_val.get<std::string>();
            break;
        }
        case c10::TypeKind::ScalarTypeType: {
            TORCH_CHECK(serialized_arg_type == "as_scalar_type", "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_scalar_type for argument ", index, " but got ",
                        serialized_arg_type);
            stack.at(index) = serialized_arg_val.get<c10::ScalarType>();
            break;
        }
        case c10::TypeKind::MemoryFormatType: {
            TORCH_CHECK(serialized_arg_type == "as_memory_format", "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_memory_format for argument ", index, " but got ",
                        serialized_arg_type);
            stack.at(index) = serialized_arg_val.get<c10::MemoryFormat>();
            break;
        }
        case c10::TypeKind::LayoutType: {
            TORCH_CHECK(serialized_arg_type == "as_layout", "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_layout for argument ", index, " but got ",
                        serialized_arg_type);
            stack.at(index) = serialized_arg_val.get<c10::Layout>();
            break;
        }
        case c10::TypeKind::DeviceObjType: {
            TORCH_CHECK(serialized_arg_type == "as_device", "Expected extern kernel ", op_kernel.target_,
                        " to have serialized argument type as_device for argument ", index, " but got ",
                        serialized_arg_type);

            std::string device_string = serialized_arg_val["type"].get<std::string>();
            if (serialized_arg_val.contains("index") && serialized_arg_val["index"].is_number()) {
            ...
            if (device != *device_) {
                VLOG(1) << "ProxyExecutor is using " << *device_ << " for " << op_kernel.target_ << " argument #"
                        << index << ", which is different from the one serialized in thrift: " << device
        ...
        case c10::TypeKind::ListType: {
            if (schema_arg_type->isSubtypeOf(at::ListType::ofTensors())) {
                TORCH_CHECK(serialized_arg_type == "as_tensors", "Expected extern kernel ", op_kernel.target_,
                            " to have serialized argument type as_tensors for argument ", index, " but got ",
                            serialized_arg_type);
                TORCH_CHECK(serialized_arg_type == "as_tensors");
                dynamic_args.emplace_back(index, DynamicArgType::ListTensorType, serialized_arg_val.size());
            } else if (schema_arg_type->isSubtypeOf(at::ListType::ofInts())) {
                TORCH_CHECK(serialized_arg_type == "as_ints", "Expected extern kernel ", op_kernel.target_,
                            " to have serialized argument type as_ints for argument ", index, " but got ",
                            serialized_arg_type);
                dynamic_args.emplace_back(index, DynamicArgType::ListIntType, serialized_arg_val.size());
            } else if (schema_arg_type->isSubtypeOf(at::ListType::ofSymInts())) {
                TORCH_CHECK(serialized_arg_type == "as_ints" || serialized_arg_type == "as_sym_ints",
                            "Expected extern kernel ", op_kernel.target_,
                            " to have serialized argument type as_ints or as_sym_ints "
                            "for argument ",
                            index, " but got ", serialized_arg_type);
                dynamic_args.emplace_back(index, DynamicArgType::ListIntType, serialized_arg_val.size());
            } else if (schema_arg_type->isSubtypeOf(at::ListType::ofFloats())) {
                TORCH_CHECK(serialized_arg_type == "as_floats", "Expected extern kernel ", op_kernel.target_,
                            " to have serialized argument type as_floats for argument ", index, " but got ",
                            serialized_arg_type);
                ...
                TORCH_CHECK(serialized_arg_type == "as_bools", "Expected extern kernel ", op_kernel.target_,
                            " to have serialized argument type as_bools for argument ", index, " but got ",
                            serialized_arg_type);
                std::vector<bool> ret;
                for (const auto& arg : serialized_arg_val) {
                ...
                if (serialized_arg_type == "as_ints") {
                    ...
                    TORCH_CHECK(false, "Expected extern kernel ", op_kernel.target_,
                                " to have a List[Scalar] input for argument ", index, " but got ", serialized_arg_type);
                }
            } else if (schema_arg_type->isSubtypeOf(at::ListType::ofOptionalTensors())) {
                if (serialized_arg_type == "as_optional_tensors") {
                    std::vector<std::string> list_item_types;
                    for (const auto& arg : serialized_arg_val) {
                        list_item_types.push_back(arg.begin().key());
                    }
                    dynamic_args.emplace_back(index, DynamicArgType::ListOptionalTensorType, serialized_arg_val.size(),
                                              list_item_types);
                } else if (serialized_arg_type == "as_tensors") {
                    dynamic_args.emplace_back(index, DynamicArgType::ListTensorType, serialized_arg_val.size());
                } else {
                    TORCH_CHECK(false, "Expected extern kernel ", op_kernel.target_,
                                " to have a Tensor?[] input for argument ", index, " but got ", serialized_arg_type);
                }
            } else if (schema_arg_type->isSubtypeOf(at::ListType::ofStrings())) {
                TORCH_CHECK(serialized_arg_type == "as_strings", "Expected extern kernel ", op_kernel.target_,
                            " to have serialized argument type as_strings for argument ", index, " but got ",
                            serialized_arg_type);
                ...
                TORCH_CHECK(false, "NYI: Unsupported list type ", serialized_arg_type, " for extern kernel ",
                            op_kernel.target_, " argument ", index);
            }
            break;
        }
        case c10::TypeKind::OptionalType: {
            auto inner_type = schema_arg_type->castRaw<at::OptionalType>()->getElementType();
            ...
            if (serialized_arg_type == "as_none") {
                ...
                prefill_stack_with_static_arguments(index, inner_type, serialized_arg, op_kernel);
            }
            break;
        }
        default:
            TORCH_CHECK(false, "Unsupported input type ", serialized_arg_type, " for extern kernel ", op_kernel.target_,
                        " argument ", index);
    }
}

// Populates op_kernel.stack_, op_kernel.dynamic_args_
void OSSProxyExecutor::get_input_info_from_serialized(const std::vector<c10::Argument>& schema_args,
                                                      const nlohmann::json& serialized_node, OSSOpKernel& op_kernel)
{
    ...
    for (const auto& named_argument : serialized_node["inputs"]) {
        ...
        for (size_t index = 0; index < schema_args.size(); index++) {
            ...
            if (schema_arg.name() == name) {
                prefill_stack_with_static_arguments(index, schema_arg.real_type(), arg, op_kernel);
                filled[index] = true;
                break;
            }
        }
...
void OSSProxyExecutor::get_output_info_from_serialized(const std::vector<c10::Argument>& schema_returns,
                                                       const nlohmann::json& serialized_node, OSSOpKernel& op_kernel)
{
    std::vector<OSSDynamicArg>& outputs = op_kernel.outputs_;

    TORCH_CHECK(schema_returns.size() == serialized_node["outputs"].size(), "Serialized node doesn't match operator ",
                serialized_node["target"], "'s schema outputs.");
    ...
    for (const auto& serialized_output : serialized_node["outputs"]) {
        ...
        switch (schema_return_type->kind()) {
            case c10::TypeKind::TensorType: {
                TORCH_CHECK(serialized_output_type == "as_tensor", "Expected extern kernel ", serialized_node["target"],
                            " to have serialized output type as_tensor, ", " but got ", serialized_output_type);
                outputs.emplace_back(output_index, DynamicArgType::TensorType, 1);
                break;
            }
            case c10::TypeKind::ListType: {
                if (schema_return_type->isSubtypeOf(at::ListType::ofTensors())) {
                    TORCH_CHECK(serialized_output_type == "as_tensors", "Expected extern kernel ",
                                serialized_node["target"], " to have serialized output type as_tensors, ", " but got ",
                                serialized_output_type);
                    outputs.emplace_back(output_index, DynamicArgType::ListTensorType, serialized_output_val.size());
                } else {
                    TORCH_CHECK(false, "Unsupported return list type ", schema_return_type->repr_str());
                }
                break;
            }
            default: {
                TORCH_CHECK(false, "Unsupported return type ", schema_return_type->repr_str(), " for extern kernel ",
                            op_kernel.target_);
            }
        }
...
OSSProxyExecutor::OSSProxyExecutor(const std::string& json_path, bool is_cpu)
{
    ...
    for (auto const& serialized_extern_node : json_obj["nodes"]) {
        ...
        c10::OperatorHandle op_handle =
            c10::Dispatcher::singleton().findSchemaOrThrow(opName.c_str(), overloadName.c_str());
...
void OSSProxyExecutor::call_function(int extern_node_index, int num_ints, int64_t* flatten_int_args, int num_tensors,
                                     AtenTensorHandle* flatten_tensor_args)
{
    TORCH_CHECK(extern_node_index < static_cast<int>(op_kernels_.size()), "Invalid extern node index");
    ...
    for (auto& dynamic_arg : dynamic_args) {
        ...
        switch (dynamic_arg_type) {
            case DynamicArgType::TensorType: {
                at::Tensor* tensor = tensor_handle_to_tensor_pointer(flatten_tensor_args[tensor_id++]);
            ...
            case DynamicArgType::ListOptionalTensorType: {
                ...
                TORCH_CHECK(list_item_types.has_value(),
    ...
    TORCH_CHECK(tensor_id == num_tensors - num_output_tensors,
                "Mismatch between tensors consumed and num of input tensor, got "
                "tensor_id = .",
                tensor_id, ", expected num = ", num_tensors - num_output_tensors);
    TORCH_CHECK(int_id == num_ints, "Mismatch between ints consumed and num_ints, got int_id = ", int_id,
                ", num_ints = ", num_ints);
    ...
    for (const auto& schema_return : schema_returns) {
        ...
        } else if (schema_return.type()->kind() == c10::TypeKind::ListType &&
    ...
    TORCH_CHECK(tensor_id == num_tensors,
                "Mismatch between tensors consumed and num_tensors, got tensor_id = ", tensor_id,
                ", expected num = ", num_tensors);
}
