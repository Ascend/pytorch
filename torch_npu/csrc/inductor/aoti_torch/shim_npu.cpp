    ...
    *ret_new_tensor = torch::aot_inductor::new_tensor_handle(
        ...
        (data != nullptr)
            ? at_npu::native::from_blob(
                  data, sizes, strides, storage_offset, options, device)
...
AOTITorchError aoti_torch_create_npu_guard(
  ...
  AOTI_TORCH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    c10_npu::NPUGuard* guard =
        new c10_npu::NPUGuard(static_cast<c10::DeviceIndex>(device_index));
...
AOTITorchError aoti_torch_npu_guard_set_index(
    ...
    int32_t device_index) {
  AOTI_TORCH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    reinterpret_cast<c10_npu::NPUGuard*>(guard)->set_index(
        static_cast<c10::DeviceIndex>(device_index));
  });
}
...
AOTITorchError aoti_torch_create_npu_stream_guard(
  ...
  AOTI_TORCH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    ...
    auto* guard =
        new c10_npu::NPUStreamGuard(static_cast<c10::Stream>(managed_stream));
...
AOTITorchError aoti_torch_get_current_npu_stream(
  ...
  AOTI_TORCH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    *ret_stream =
        reinterpret_cast<void*>(c10_npu::getCurrentNPUStream(
                                    static_cast<c10::DeviceIndex>(device_index))
                                    .stream());
  });
}
