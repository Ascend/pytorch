    ...
    *ret_new_tensor = nullptr;
    if (layout == static_cast<int32_t>(at::kMkldnn)) {
      ...
      auto err = aoti_torch_create_tensor_from_blob_npu(
          data, ndim, sizes_ptr, strides_ptr, storage_offset, dtype, device_type, device_index, ret_new_tensor);
...
AOTITorchError aoti_torch_create_npu_stream_guard(void* stream, int32_t device_index, NPUStreamGuardHandle* ret_guard) {
  AOTI_TORCH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    ...
    auto managed_stream =
        c10_npu::getNPUStreamFromManagedAclrtStream(raw_stream, static_cast<c10::DeviceIndex>(device_index));
...
AOTITorchError aoti_torch_get_current_npu_stream(int32_t device_index, void** ret_stream) {
  AOTI_TORCH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    *ret_stream =
        reinterpret_cast<void*>(c10_npu::getCurrentNPUStream(static_cast<c10::DeviceIndex>(device_index)).stream());
  });
}
...
AOTITorchError aoti_torch_npu_caching_allocator_raw_alloc(uint64_t nbytes, void** ret_ptr) {
  AOTI_TORCH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    ...
    *ret_ptr = c10_npu::NPUCachingAllocator::raw_alloc(nbytes);
    ...
    if (*ret_ptr == nullptr) {
      TORCH_CHECK(false, "Failed to allocate ", nbytes, " bytes from NPU caching allocator");
    }
  });
}
