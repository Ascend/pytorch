...
int64_t ShapeOpStrategyBase::FindClosestGear(
    int64_t cur_size,
...
uint64_t ShapeOpStrategyBase::EncodeTransformStep(int op, int index, int64_t original_size, int dimension) {
  /*
   ...
   ◦ op: padding or split
   ◦ index: the position of the tensor being operated on in the current step
   within the list ◦ original_size: the size of the specified dimension before
   the operation ◦ dimension: the dimension being operated on ◦ The composition
...
void ShapeOpStrategyBase::DecodeTransformStep(
    uint64_t operation,
    int& op,
    int& index,
...
void BSShapeOpStrategy::InitializeCore(
  ...
  TORCH_CHECK(
      gears.size() <= MAX_GEARS_NUM,
      "Number of batch size gears (",
      gears.size(),
...
void SeqShapeOpStrategy::InitializeCore(
    std::vector<int64_t>& gears,
  ...
  TORCH_CHECK(
      gears.size() <= MAX_GEARS_NUM,
      "Number of sequence length gears (",
      gears.size(),
      ") exceeds maximum supported (",
      MAX_GEARS_NUM,
      ").");
...
void DefaultBSShapeOp::GenerateExpectedRes(
...
/*
• input: {{res1_1, res2_1...resn_1}, {res1_2, res2_2...resn_2}...{res1_m,
res2_m...resn_m}}
...
void DefaultSeqShapeOp::TransformValidate(std::vector<at::Tensor>& inputs) {
  ...
  for (size_t i = 0; i < m_indices.size(); i++) {
    TORCH_CHECK(
        m_dimensions[i] < inputs[m_indices[i]].dim(),
        "The dimension exceeds the actual tensor's dimension range, check inputs or update the dimension config.");
    TORCH_CHECK(
        m_max_gear >= inputs[m_indices[i]].size(m_dimensions[i]),
...
void NPUShapeHandling::Initialize(
    ShapeType type,
    std::vector<int64_t>& gears,
    std::vector<int>& dimensions,
    std::vector<int>& indices,
...
void NPUShapeHandling::GenerateGears(
    int64_t min_size,
    int64_t max_size,
    ShapePolicy policy,
    std::vector<int64_t>& gears) {
