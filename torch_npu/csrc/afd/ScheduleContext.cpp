  ...
  for (uint32_t session_id = 0; session_id < context_.common.session_num; ++session_id) {
    for (uint32_t micro_batch_id = 0; micro_batch_id < context_.common.micro_batch_num; ++micro_batch_id) {
      ...
      *tmp_buf_int++ = 0;
      // layer_id
      *tmp_buf_int++ = 0;
      for (uint32_t idx = 0; idx < context_.common.micro_batch_size * context_.common.selected_expert_num; ++idx) {
        // expert_id
...
uint32_t ScheduleContextHolder::InitFfn() {
  ...
  if (ffn_window_size_ <= token_info_aligned_size) {
    ASCEND_LOGE(
        "ffn_window_size=%lu must be > token_info_aligned_size=%lu.", ffn_window_size_, token_info_aligned_size);
    return kFailure;
  }
  ...
  context_.ffn.expert_ids_buf_size = sizeof(int32_t) * context_.common.session_num * context_.common.micro_batch_size *
...
uint32_t ScheduleContextHolder::InitAttention() {
  ...
  auto ret = aclrtMemset(
      reinterpret_cast<void*>(static_cast<uintptr_t>(context_.attention.token_info_buf)),
      token_info_size,
      '\0',
      token_info_size);
  if (ret != ACL_ERROR_NONE) {
...
uint32_t ScheduleContextHolder::AllocAndAssignDevMem() {
  ...
  at::Tensor host_tensor = at::from_blob(&context_, context_shape, at::TensorOptions().dtype(torch::kInt8));
...
bool ScheduleContextHolder::CheckParams() const {
  ...
  if ((context_.common.attn_to_ffn_token_size % kBufAlignSize) != 0U) {
    ASCEND_LOGE(
        "attn_to_ffn_token_size[%lu] must be align with %lu.", context_.common.attn_to_ffn_token_size, kBufAlignSize);
    return false;
  }
  if ((context_.common.ffn_to_attn_token_size % kBufAlignSize) != 0U) {
    ASCEND_LOGE(
        "ffn_to_attn_token_size[%lu] must be align with %lu.", context_.common.ffn_to_attn_token_size, kBufAlignSize);
    return false;
  }
...
uint32_t ScheduleContextHolder::GetScheduleContextFromDev(ScheduleContext& context) const {
  ...
  at::Tensor host_tensor =
      at::from_blob(&context, {static_cast<int64_t>(sizeof(context))}, at::TensorOptions().dtype(torch::kInt8));
...
std::string ScheduleContextHolder::ToString(const ScheduleContext& context) {
  ...
  if (context.common.schedule_mode == kScheduleModeFfn) {
    ss << ", ffn info: token_info_buf=" << context.ffn.token_info_buf
       ...
       << ", token_data_buf_size=" << context.ffn.token_data_buf_size << ", polling_index=" << context.ffn.polling_index
       << ", layer_ids_buf=" << context.ffn.layer_ids_buf << ", layer_ids_buf_size=" << context.ffn.layer_ids_buf_size
