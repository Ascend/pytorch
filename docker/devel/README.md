...
> 镜像中预装的 PyTorch CPU 版本默认为 v2.7.1 分支对应的版本（`torch==2.7.1`）。可通过构建参数 `TORCH_VERSION` 指定其他版本（如 `2.7.1`），详见 [构建参数参考](#构建参数参考)。
...
| `TORCH_VERSION` | `2.7.1` | PyTorch 版本，格式 `x.x.x`（如 `2.7.1`） | all |
| `DEVTOOLSET_VERSION` | `11` | GCC toolset 版本 | builder |
...
| `--torch-version VER` | PyTorch 版本，格式 `x.x.x`（如 `2.7.1`） |
