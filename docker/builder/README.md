...
> 镜像中预装的 PyTorch CPU 版本默认为 master 分支对应的 nightly 版本（`torch==2.13.0.dev20260521+cpu`）。可通过构建参数 `TORCH_VERSION` 指定稳定版本（如 `2.7.1`），详见 [构建参数参考](#构建参数参考)。
...
| `TORCH_VERSION` | `2.13.0.dev20260521` | PyTorch 版本，格式 `x.x.x`（如 `2.7.1`）或 dev 版本（如 `2.13.0.dev20260521`）；dev 版本从 nightly 源安装 | all |
...
| `--torch-version VER` | PyTorch 版本，格式 `x.x.x`（如 `2.7.1`）或 dev 版本（如 `2.13.0.dev20260521`，默认） |
