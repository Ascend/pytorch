...
> 镜像中预装的 PyTorch CPU 版本默认为 v2.12.0 分支对应的版本（`torch==2.12.0`）。可通过构建参数 `TORCH_VERSION` 指定其他版本（如 `2.7.1`），详见 [构建参数参考](#构建参数参考)。
...
| `TORCH_VERSION` | `2.12.0` | PyTorch 版本，格式 `x.x.x`（如 `2.12.0`） | all |
...
| `--torch-version VER` | PyTorch 版本，格式 `x.x.x`（如 `2.12.0`） |
