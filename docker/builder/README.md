...
本目录提供 Dockerfile 及构建脚本，用于生成 Ascend NPU 平台的 PyTorch 编译镜像。
...
> `builder.sh` 为 **Linux 一键部署脚本**，自动检测架构（X86/ARM）、构建镜像并启动容器。Windows 环境请见下方 [Windows 环境使用说明](#windows-环境使用说明)。

```Shell
cd pytorch/docker/builder
bash builder.sh
```

脚本自动检测当前架构（X86/ARM），构建镜像并启动容器。容器启动后，执行以下命令进入编译：
...
Windows 上 `builder.sh` 依赖的 bash 通常不可用（WSL 入口 `bash.exe` 默认指向 `docker-desktop` 分发，非完整 Linux），且 MSYS 路径转换会导致 `docker run -v` 挂载失败。建议直接用 PowerShell 等价命令。

### 1. 构建镜像

```PowerShell
cd pytorch\docker\builder\X86    # ARM 机器用 ARM 子目录

# 默认：builder 镜像（编译环境，不含 CANN）
docker build -t manylinux-builder:v1 `
    --target builder `
    --build-arg PY_VERSION=3.9 `
    .

# dev 镜像（运行环境，含 CANN，基于 builder）
docker build -t manylinux-builder:v1 `
    --target dev `
    --build-arg PY_VERSION=3.9 `
...
```PowerShell
docker rm -f torch-npu-builder 2>$null
docker run -d --rm `
    --name torch-npu-builder `
    -v /path/to/pytorch:/home/pytorch `
    -e PY_VERSION=3.9 `
...
> 挂载路径使用 Windows 风格（`E:\...`），Docker Desktop for Windows 会自动转换；切勿使用 MSYS 风格（`/e/...`）。

## builder.sh 参数说明

| 参数 | 说明 |
|------|------|
| `-p, --python VERSION` | Python 版本：3.9、3.10、3.11、3.12、3.13（默认：3.9） |
...
```Shell
bash builder.sh                                          # 默认：Python 3.9，不含 CANN
bash builder.sh -p 3.10                                 # 使用 Python 3.10
...
> **关于 CANN 版本与 release train**：OBS 上的 CANN `.run` 包按 release train 分目录存放（如 `CANN%209.1.T1`、`CANN%209.0.0`），目录名无法由版本号自动推导。因此当 `--cann-version` 与默认值 `9.1.0_beta.1` 不同时，必须同时通过 `--cann-release-train` 指定对应的发布版本号，否则脚本会报错退出。请到 [昇腾官网下载页](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/softwareinst/instg/instg_0008.html?OS=openEuler&InstallType=local) 查找目标版本对应的目录名。

## 手动构建

### 仅编译环境（builder 镜像，不含 CANN）
...
Dockerfile 仅安装指定 Python 版本对应的依赖，减少构建时间。
...
docker build -t manylinux-builder:v1 \
    --target dev \
    --build-arg PY_VERSION=3.9 \
    --build-arg CANN_RELEASE_TRAIN=CANN%209.1.T1 \
    .
```
...
| ARG | 默认值 | 说明 | 适用阶段 |
|-----|--------|------|---------|
| `PY_VERSION` | `3.9` | Python 版本，仅安装对应版本依赖 | all |
...
### 场景二：运行 torch_npu（dev 镜像，含 CANN）
...
> `import torch_npu` 需要 CANN 运行时库；若镜像未用 `--cann` 构建，请在含 CANN 的环境中验证，或重新用 `bash builder.sh --cann` 构建镜像。
> `device_count()` 另需宿主机已安装 NPU 驱动。
