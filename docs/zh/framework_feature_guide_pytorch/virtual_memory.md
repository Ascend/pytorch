...
虚拟内存管理机制主要是将地址和内存的概念分离。通过配置环境变量PYTORCH\_NPU\_ALLOC\_CONF或修改torch.npu.memory.\_set\_allocator\_settings接口的“expandable\_segments”属性值，可开启虚拟内存管理机制。该机制使PyTorch管理虚拟内存和物理内存的映射，并允许多次申请连续内存。通过构建可扩展的内存段并动态调整内存块大小，可以有效减少内存碎片的产生。对于有较多内存碎片的模型，可以降低设备内存占用率。
...
- 设置环境变量PYTORCH\_NPU\_ALLOC\_CONF=expandable\_segments:<value\>，此环境变量使用详情请参考《环境变量》中的“[PYTORCH\_NPU\_ALLOC\_CONF](../environment_variable_reference/PYTORCH_NPU_ALLOC_CONF.md)”章节。
