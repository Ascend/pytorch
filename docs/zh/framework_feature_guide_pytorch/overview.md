...
|特性类别|特性名称|特性说明|
|--|--|--|
|内存资源优化|[虚拟内存](虚拟内存.md)|动态调整内存块的大小，减少内存碎片的产生。|
|内存资源优化|[内存快照]()|支持训练过程中内存溢出时生成设备内存快照。|
|内存资源优化|[自定义内存分配器]()|从so文件加载自定义NPU内存分配器。|
|内存资源优化|[多流内存复用]()|多流情况下，优化内存使用情况，提高内存复用率。|
|内存资源优化|[内存共享（IPC）]()|支持跨进程共享内存，有效减少内存消耗。|
|内存资源优化|[Stream级TaskQueue并行下发]()|每个Stream会初始化独立的TaskQueue和对应的Dequeue线程，实现真正的二级流水并行下发机制。|
