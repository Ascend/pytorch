...
|torch.distributed.tensor.placement_types._StridedShard|是<br>暂不支持<term>Ascend 950DT</term>|支持bf16，fp16，fp32，fp64，uint8，int8，int32，int64，bool|
|torch.distributed.tensor.experimental.context_parallel|是<br>暂不支持<term>Ascend 950DT</term>|仅支持NPU fused SDPA路径；q/k/v仅支持BNSD布局；暂不支持pse、padding_mask、prefix、actual_seq_qlen、actual_seq_kvlen、sink以及任意非causal attention mask；load balance要求causal attention；暂不支持通过torch.compile编译为计算图|
