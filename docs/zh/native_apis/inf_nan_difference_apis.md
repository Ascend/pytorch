...
在torch_npu中，部分API对`inf`/`nan`特殊值的处理行为与PyTorch原生实现存在差异，可能导致计算结果不一致。本文档主要列出两类差异场景：异常输入（输入包含`inf`/`nan`）时的行为差异，以及数值溢出时输出`inf`/`nan`的差异。

**异常输入处理有差异的API清单**

当输入Tensor中包含`inf`/`nan`值时，torch_npu与PyTorch原生的输出结果存在差异，具体API列表请参见[表1](#输入包含inf-nan时处理与原生不一致的API列表)。
