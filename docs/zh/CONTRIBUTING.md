...
在您第一次向 PyTorch 社区提交代码之前，需要签署 [CLA](https://clasign.osinfra.cn/sign-cla/690ca9ddf91c03dee6082ab1/individual)。
...
- **软件安装**（`installation_guide/`）：环境准备、源码编译、pip 安装等说明。
- **快速入门**（`quick_start/`）：快速上手教程。
- **原生API**（`native_apis/`）：各版本 PyTorch 原生 API 支持情况。
- **框架特性**（`framework_feature_guide_pytorch/`）：NPU 图模式、Inductor、内存优化等特性说明。
- **环境变量**（`environment_variable_reference/`）：NPU 相关环境变量说明。
- **故障处理**（`troubleshooting/`）：常见问题及错误码分析。
