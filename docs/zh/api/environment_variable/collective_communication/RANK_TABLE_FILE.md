...
> [!NOTE]
>
> 配置RANK\_TABLE\_FILE场景下，执行模型分布式训练时如果出现“RuntimeError: The Inner Error ...”的报错，建议将HCCL\_CONNECT\_TIMEOUT的超时时间适当增大，避免rank table场景下无协商导致的建链超时问题，具体请参考[在进行模型分布式训练时遇到报错“RuntimeError: The Inner Error ...”](../appendix/runtimeerror_Inner_Error.md)。
...
> - 配置的文件路径存在，但配置信息有误时，不会通过默认的协商流程进行集合通信域建链，而是在实际通信时会抛出相应的报错。
