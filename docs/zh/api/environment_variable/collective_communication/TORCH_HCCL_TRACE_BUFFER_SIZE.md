...
- 配置为“0”或负数时：关闭Flight Recorder记录功能，不记录任何事件，导出dump时也不包含trace内容。
...
该变量对应PyTorch的`TORCH_FR_BUFFER_SIZE`，PyTorch同时兼容[`TORCH_NCCL_TRACE_BUFFER_SIZE`](https://docs.pytorch.org/tutorials/unstable/flight_recorder_tutorial.html#enabling-flight-recorder)名称，PyTorch默认值为2000。
