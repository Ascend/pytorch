...
本表集中展示环境变量文档中32项具有PyTorch对应变量或相关配置的对照说明，按对应关系分组。配置取值、使用约束和支持型号请参见各变量详情；完整变量范围请参见[环境变量列表](env_variable_list.md)。
...
| [TORCHINDUCTOR_WORKER_SUPPRESS_LOGGING](inductor/TORCHINDUCTOR_WORKER_SUPPRESS_LOGGING.md) | `TORCHINDUCTOR_WORKER_SUPPRESS_LOGGING`（环境变量） | 变量名称和行为一致，用于控制是否输出Inductor worker子进程的日志。 |
