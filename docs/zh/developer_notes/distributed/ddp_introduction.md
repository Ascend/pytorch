...
DDP 的核心机制依靠`torch.distributed`提供的集合通信能力，通过`AllReduce`操作在所有进程间同步梯度。每个进程持有模型的一份完整副本，处理不同的输入数据分片，在反向传播时梯度自动同步，保证多卡训练的效果与单卡训练效果一致，显著提升了训练吞吐量。在实现层面，DDP 为模型中的每个参数注册了 autograd 钩子。反向传播时这些钩子触发梯度同步，确保各进程梯度一致后再进行参数更新。
