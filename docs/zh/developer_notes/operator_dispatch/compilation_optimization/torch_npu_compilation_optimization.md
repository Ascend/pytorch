...
>
> <!-- npu="950" id1 -->
> <term>Ascend 950DT系列产品</term>暂不支持编译优化（TorchNPU）。
> <!-- end id1 -->
...
1. 依赖安装。
    ...
    TorchNPU推荐在容器里进行编译，具体请参考[源码安装](../../../installation_guide/building_from_source.md)。以下描述均以TorchNPU v2.7.1版为例。
...
3. 根据需要的优化类型进行相应编译参数设置并进行编译，LTO和PGO优化可以单独使用，也可以叠加使用，当前TorchNPU已支持基于毕昇编译器的编译优化选项。
    ...
    > [!NOTE]
    > 
    > 使用毕昇编译TorchNPU之前需要先参照[PyTorch编译优化](pytorch_compilation_optimization.md)，用毕昇重新编译PyTorch并安装，然后再编译TorchNPU。
