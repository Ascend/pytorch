...
当开启点对点通信使用独立通信域功能时，每一个通信域都会额外占用P2P\_HCCL\_BUFFSIZE大小的缓存区。若集群网络中存在较多的通信域，此缓存区占用量就会增多，可能存在影响模型数据正常存放的风险，此种场景下，可通过此环境变量减少点对点通信域占用的缓存区大小；若业务的模型数据量较小，但点对点通信数据量较大，则可通过此环境变量增大点对点通信域占用的缓存区大小，提升点对点通信效率。
...
-   若之前未对P2P创建独立通信域，配置该环境变量后，会对P2P独立创建通信域，若模型侧存在send和recv下发间隔时间长的场景，会出现超时的可能，需要将HCCL\_CONNECT\_TIMEOUT的时间配置得更长，建议配置值为600s，具体配置值需依据模型脚本实际情况，HCCL\_CONNECT\_TIMEOUT具体可参考《CANN 环境变量参考》中的“HCCL\_CONNECT\_TIMEOUT”章节。
