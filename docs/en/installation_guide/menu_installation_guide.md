# Software Installation Guide

- [Installation Instructions](installation_description.md)
- [Preparing for Installation](preparing_installation.md)
- [Installing PyTorch](installing_PyTorch.md)
  - [Method 1: Offline Installation (Whl)](installation_via_binary_package.md)
  - [Method 2: Installation from Source Code](compilation_installation_using_source_code.md)

- [(Optional) Installing Extension Modules](Optional_installing_extension_modules.md)
  - [Building libtorch_npu](building_libtorch_npu.md)
  - [Installing Apex Module](https://gitcode.com/Ascend/apex/blob/master/docs/en/installing_apex.md)
  - [Installing torchvision](installing_torchvision.md)

- [Uninstallation](uninstallation.md)
- [Upgrade](upgrade.md)
- [References](references.md)
  - [Configuring pip Source](configuring_pip_source.md)
  - [Installing GCC 11.2.0](installing_gcc_11-2-0.md)
  - [Installing CMake 3.18.4](installing_cmake_3-18-4.md)

- [Appendixes](appendixes.md)
  - [torch.libs/libopenblasp-r0-56e95da7.3.24.so Link Error or Missing libgfortran](torch-libs-libopenblasp-r0-56e95da7-3-24-so_not_exist.md)