...
In multi-threaded multi-stream delivery scenarios, you are advised to use this feature when Dequeue is blocked. Using it in other scenarios is not recommended.

## Usage Guide

The `PER_STREAM_QUEUE` environment variable controls whether the one-`task_queue`-per-stream operator delivery queue mode is enabled.

- When configured as "0", the one-`task_queue`-per-stream operator delivery queue mode is disabled.
- When configured as "1", the one-`task_queue`-per-stream operator delivery queue mode is enabled.
...
- This feature does not support [process-level online recovery](https://gitcode.com/Ascend/mind-cluster/blob/branch_v26.1.0/docs/en/scheduling/04_usage/04_resumable_training/03_configuration/02_configuring_fault_handling_policies.md) scenarios.
