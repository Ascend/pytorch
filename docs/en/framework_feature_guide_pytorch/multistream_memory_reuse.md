...
![figure2](../figures/enabling_multi_stream_memory_reuse.png)  

## Use Scenario

In LLM distributed training scenarios, consider using this feature to avoid out-of-bounds memory access and improve the cross-stream memory reuse rate.

## Usage Guide

Use the environment variable `MULTI_STREAM_MEMORY_REUSE` to control whether multi-stream memory reuse is enabled.

- 0: Disable memory reuse.
- 1: Enable memory reuse. It erases the previous `recordStream` marks based on the `eraseStream` method to ensure memory reuse, holds a weak reference to the tensor, and does not extend the lifecycle of the tensor.
- 2: Enable memory reuse. It does not execute the `recordStream` marks to ensure memory reuse, holds a strong reference to the tensor, and may extend the lifecycle of the tensor. This value is currently not recommended for use.
- 3: Enable memory reuse. Based on the value "1", it further optimizes reuse and can erase the `recordStream` marks in scenarios where tensors are released early.
