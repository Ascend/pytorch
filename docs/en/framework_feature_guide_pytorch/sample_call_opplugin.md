...
2. Adapt the code of the custom operator using the following methods. Select a method based on the actual situation.
   - Perform structured operator adaptation in `op_plugin_functions.yaml`.

      > [!NOTE]  
      > `npu_add_custom` is a forward interface. Since the corresponding adaptation code has a simple structure, you can use structured adaptation to automatically generate the adaptation code.
   ...
   - General operator adaptation.
      ...
      2. After completing the operator adaptation, the complete `AddCustomKernelNpuOpApi.cpp` file is as follows.
          ...
          { 
              ...
              // The format of the first input parameter is aclnn+Optype, and the subsequent parameters are the inputs and outputs respectively
...
3. Implement auxiliary operator adaptation.

   1. Add the operator interface documentation in the following file.

      In `codegen/templates/_op_plugin_docs.py`, add the following content:
      ...
      Performs a custom addition operation on two tensors. It supports scaling the second input tensor by a coefficient before adding it. The FakeTensor mode is supported.

      Parameter description
      self (Tensor) - First input tensor.
      other (Tensor) - Second input tensor. Its shape must be broadcastable with the shape of self.
      alpha (float, default value 1) - Scaling coefficient applied to other. The computation is self + alpha * other.
   ...
   2. The public interfaces must be configured in the following documents.

      - Add the following entry to `test/allowlist_for_publicAPI.json`:
      ...
      - Add the following entry to `test/core_tests/torch_npu_OpApi_schema_all.json`:
   ...
   3. Register the meta implementation of the operator interface in the following file.
      ...
      Add the following entry to `op_plugin/python/meta/_meta_registrations.py`:

        ```python
        @impl(m, "npu_add_custom")
        def npu_add_custom_meta(self, other, alpha=1):
            # Simulate the computation logic of the custom addition, used in the FakeTensor mode
...
1. Compile and install the TorchNPU plugin. You are advised to compile in a container scenario. For details, see the "Method 1 (Recommended): Container Scenario" section in the "[Method 2: Source Code Installation](../installation_guide/compilation_installation_using_source_code.md)" chapter of the *Software Installation* guide.

2. After the preceding development process is complete, run the developer test script to verify that the basic functions work normally.

   1. In the `test/test_custom_ops` directory, create the developer test file `test_npu_add_custom.py` and add the following content:
   ...
   2. Run the following command to verify that the new operator functions correctly:
      > [!CAUTION]  
      >
      > Do not run this script in the root directory of the TorchNPU repository. Otherwise, an error may occur indicating that `torch_npu._C` cannot be found.
