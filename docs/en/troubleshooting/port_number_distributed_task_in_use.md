...
After the running distributed task is interrupted, residual processes remain, or processes with the same port number exist in the environment, causing the port number to be occupied, normal communication to fail, and the distributed task to fail.
