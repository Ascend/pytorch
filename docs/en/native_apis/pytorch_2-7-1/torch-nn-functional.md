...
|torch.nn.functional.max_pool2d|Yes|Supports bf16, fp16, fp32<br>By setting torch_npu.npu.use_compatible_impl(True), memory consistency is aligned with the community interface of the same name|
|torch.nn.functional.max_pool3d|Yes|Supports bf16, fp16, fp32<br>The value of dilation is currently only supported as 1 or (1,1,1) on NPU<br>When return_indices is True, the data type of the returned argmax is int32|
...
|torch.nn.functional.fractional_max_pool2d|Yes|May Fallback to CPU execution|
...
|torch.nn.functional.hardswish|Yes|Supports fp16, fp32<br>May Fallback to CPU execution|
...
|torch.nn.functional.silu|Yes|Supports bf16, fp16, fp32|
|torch.nn.functional.mish|Yes|Supports fp16, fp32|
|torch.nn.functional.batch_norm|Yes|Supports fp16, fp32<br>weight and bias only support one-dimensional scenarios<br>The shape of bias is 1-dimensional, with a length equal to the length of the channel axis in the input parameter|
...
|torch.nn.functional.one_hot|Yes|Supports int64|
...
|torch.nn.functional.poisson_nll_loss|Yes|Supports bf16, fp16, fp32, int64<br>May Fallback to CPU execution|
...
|torch.nn.functional.kl_div|Yes|Supports bf16, fp16, fp32<br>Currently, the log_target parameter only supports False<br>Currently, target does not support gradient computation|
|torch.nn.functional.l1_loss|Yes|Supports bf16, fp16, fp32, int64|
|torch.nn.functional.mse_loss|Yes|Supports bf16, fp16, fp32, fp64, uint8, int8, int16, int32, int64, complex64, complex128|
...
|torch.nn.functional.nll_loss|Yes|Supports fp32<br>Each element value in target should be greater than or equal to 0 and less than the number of classes in input|
