...
|torch.cuda.current_stream|torch_npu.npu.current_stream|Yes|When device is not set, calling this interface will implicitly initialize the current device (default card 0)|
|torch.cuda.default_stream|torch_npu.npu.default_stream|Yes|When device is not set, calling this interface will implicitly initialize the current device (default card 0)|
...
|torch.cuda.get_device_properties|torch_npu.npu.get_device_properties|Yes|Only supports the name, total_memory, L2_cache_size, cube_core_num, and vector_core_num attributes. All other attributes supported on native CUDA return empty fields|
