...
<!-- md-trans-meta sourceCommit=e6dd39e7131a89f72cf49d80d53002e4cc645bbf translatedAt=2026-07-09T08:36:03.492Z pushedAt=2026-07-09T08:44:08.342Z -->
...
| torch.optim.Adam | Yes | Supports bf16, fp16, fp32<br>When the optimizer is launched with foreach enabled (foreach=None or foreach=True), if there are too many parameter groups being optimized, the characteristics of the foreach operator may lead to performance degradation. In this case, it is recommended to set foreach=False<br>May fall back to CPU execution in some cases |
...
|torch.optim.NAdam|Yes|Supports bf16, fp16, fp32<br>When the optimizer has foreach enabled (foreach=None or foreach=True), if there are too many parameter groups being optimized, the characteristics of the foreach operator may lead to performance degradation. In such cases, it is recommended to set foreach=False|
...
|torch.optim.RAdam|Yes|Supports bf16, fp16, fp32<br>When the optimizer has foreach enabled (foreach=None or foreach=True), if there are too many parameter groups being optimized, the characteristics of the foreach operator may lead to performance degradation. In such cases, it is recommended to set foreach=False|
...
|torch.optim.RMSprop|Yes|Supports bf16, fp16, fp32<br>When the optimizer has foreach enabled (foreach=None or foreach=True), if there are too many parameter groups being optimized, the characteristics of the foreach operator may lead to performance degradation. In such cases, it is recommended to set foreach=False|
...
|torch.optim.SGD|Yes|Supports bf16, fp16, fp32<br>When the optimizer has foreach enabled (foreach=None or foreach=True), if there are too many parameter groups being optimized, the characteristics of the foreach operator may lead to performance degradation. In such cases, it is recommended to set foreach=False|
