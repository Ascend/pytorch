...
|--|--|--|
|torch.distributed.fsdp.FullyShardedDataParallel|Yes|Supports bf16, fp16, fp32<br>When using FSDP on Ascend NPU, it is recommended to pass the device parameter `device_id=torch.device("npu:0")`|
