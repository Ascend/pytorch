...
<!-- md-trans-meta sourceCommit=e6dd39e7131a89f72cf49d80d53002e4cc645bbf translatedAt=2026-07-09T08:36:23.708Z pushedAt=2026-07-09T08:44:08.343Z -->
...
|torch.set_default_tensor_type|Yes|Does not support passing torch.npu.DtypeTensor type|
|torch.numel|Yes|-|
...
|torch.range|Yes|-|
|torch.linspace|Yes|Supports bf16, fp16, fp32, fp64, int16, int32, int64, bool, complex64, complex128<br>Creates a 1-D vector with sequence size equal to steps|
...
|torch.tile|Yes|Supports bf16, fp16, fp32, fp64, uint8, int8, int16, int32, int64, bool, complex64, complex128<br>If the length of the input argument dims is less than the length of input.shape, 1s will be automatically prepended to dims to align its length with input.shape. The padded dims must satisfy the following restrictions:<br>- When repeating along the first axis is required, repeat operations are allowed on at most 4 dimensions simultaneously (i.e., the Number of Elements Greater Than 1 in dims ≤ 4). For example: torch.tile(input, [2, 3, 4, 5, 6]) is Not Supported, while torch.tile(input, [2, 3, 1, 5, 6]) is supported.<br>- When repeating along the first axis is not required, repeat operations are allowed on at most 3 dimensions simultaneously (i.e., the Number of Elements Greater Than 1 in dims ≤ 3). For example: torch.tile(input, [1, 3, 4, 5, 6]) is Not Supported, while torch.tile(input, [1, 3, 1, 5, 6]) is supported.<br>- If Backpropagation is performed, the sum of the Number of Dimensions of the input tensor and the Number of Elements Greater Than 1 in the input argument dims must not exceed 8|
...
|torch.where|Yes|Supports bf16, fp16, fp32, fp64, uint8, int8, int16, int32, int64, bool<br>Does not support 8-dimensional shapes|
|torch.Generator|Yes|-|
...
|torch.norm|Yes|Supports bf16, fp16, fp32, fp64<br>When parameter p is negative, the computation result may have precision errors<br>When parameter dim specifies an axis whose dimension value in the input tensor shape is 1, the computation result may have precision errors|
...
|torch.stft|Yes|Supports fp32, fp64, complex64, complex128<br>If the operator times out, use the official interface set_op_execute_time_out to configure and increase the timeout threshold to extend the judgment time|
...
|torch.histc|Yes|Supports fp16, fp32<br>When the input tensor value falls on the boundary between counting intervals, whether it is counted in the left interval or the right interval may have errors|
...
|torch.use_deterministic_algorithms|Yes|When both HCCL_DETERMINISTIC and torch.use_deterministic_algorithms are set, if HCCL_DETERMINISTIC enables determinism, HCCL interfaces use deterministic mode; otherwise, HCCL determinism is controlled by the torch.use_deterministic_algorithms interface|
...
|torch.compile|Yes|Backend supports npugraphs; overall functionality is consistent with backend="cudagraphs"|
