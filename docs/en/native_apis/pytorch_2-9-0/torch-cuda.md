...
|torch.cuda.current_stream|torch_npu.npu.current_stream|Yes|When no device is set, calling this interface implicitly initializes the current device (device 0 by default)|
|torch.cuda.default_stream|torch_npu.npu.default_stream|Yes|When no device is set, calling this interface implicitly initializes the current device (device 0 by default)|
...
|torch.cuda.graph_pool_handle|torch.npu.graph_pool_handle|Yes|Currently only supports inference scenarios, not training scenarios|
|torch.cuda.CUDAGraph|torch.npu.NPUGraph|Yes|Currently only supports inference scenarios, not training scenarios|
|torch.cuda.CUDAGraph.capture_begin|torch.npu.NPUGraph.capture_begin|Yes|Currently only supports inference scenarios, not training scenarios|
|torch.cuda.CUDAGraph.capture_end|torch.npu.NPUGraph.capture_end|Yes|Currently only supports inference scenarios, not training scenarios|
|torch.cuda.CUDAGraph.debug_dump|torch.npu.NPUGraph.debug_dump|Yes|Currently only supports inference scenarios, not training scenarios<br>The exported file content is in JSON format|
|torch.cuda.CUDAGraph.pool|torch.npu.NPUGraph.pool|Yes|Currently only supports inference scenarios, not training scenarios|
|torch.cuda.CUDAGraph.replay|torch.npu.NPUGraph.replay|Yes|Currently only supports inference scenarios, not training scenarios|
|torch.cuda.CUDAGraph.reset|torch.npu.NPUGraph.reset|Yes|Currently only supports inference scenarios, not training scenarios|
|torch.cuda.graph|torch.npu.graph|Yes|Currently only supports inference scenarios, not training scenarios|
|torch.cuda.make_graphed_callables|torch.npu.make_graphed_callables|Yes|Currently only supports inference scenarios, not training scenarios|
