...
|torch.distributed.tensor.experimental.context_parallel|Yes|Only supports the NPU fused SDPA path; q/k/v only supports BNSD layout; pse, padding_mask, prefix, actual_seq_qlen, actual_seq_kvlen, sink, and any non-causal attention mask are not supported; load balance requires causal attention|
