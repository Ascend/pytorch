...
<!-- md-trans-meta sourceCommit=e6dd39e7131a89f72cf49d80d53002e4cc645bbf translatedAt=2026-07-09T08:30:27.422Z pushedAt=2026-07-09T08:44:08.269Z -->

> [!NOTE]  
> If the API "Supported" is "Yes" and "Restrictions and Notes" is "-", it means the API support is consistent with the native API.<br>
> When using supported CUDA APIs, you need to replace `cuda` in the API name with the NPU-adapted name. Replace `torch.cuda.` with `torch_npu.npu.` or `torch.npu.`. Both `torch_npu.npu.` and `torch.npu.` calling methods have the same functionality. Examples are as follows:
>
...
|torch.cuda.current_stream|torch_npu.npu.current_stream|Yes|When no device is set, calling this API implicitly initializes the current device (default card 0)|
|torch.cuda.default_stream|torch_npu.npu.default_stream|Yes|When no device is set, calling this API implicitly initializes the current device (default card 0)|
...
|torch.cuda.get_device_capability|torch_npu.npu.get_device_capability|Yes|The return value of `torch_npu.npu.get_device_capability()` is configured via the environment variable `TORCH_NPU_DEVICE_CAPABILITY`. This is only for compatibility with native PyTorch and does not represent the actual capabilities of the NPU hardware|
|torch.cuda.get_device_name|torch_npu.npu.get_device_name|Yes|-|
|torch.cuda.get_device_properties|torch_npu.npu.get_device_properties|Yes|Only the `name`, `total_memory`, `L2_cache_size`, `cube_core_num`, and `vector_core_num` attributes are supported. All other attributes supported on native CUDA return empty fields|
...
|torch.cuda.graph_pool_handle|torch.npu.graph_pool_handle|Yes|Currently only supports inference scenario, not training scenario|
|torch.cuda.CUDAGraph|torch.npu.NPUGraph|Yes|Currently only supports inference scenario, not training scenario|
|torch.cuda.CUDAGraph.capture_begin|torch.npu.NPUGraph.capture_begin|Yes|Currently only supports inference scenario, not training scenario|
|torch.cuda.CUDAGraph.capture_end|torch.npu.NPUGraph.capture_end|Yes|Currently only supports inference scenario, not training scenario|
|torch.cuda.CUDAGraph.debug_dump|torch.npu.NPUGraph.debug_dump|Yes|Currently only supports inference scenario, not training scenario<br>The exported file content is in JSON format|
|torch.cuda.CUDAGraph.pool|torch.npu.NPUGraph.pool|Yes|Currently only supports inference scenario, not training scenario|
|torch.cuda.CUDAGraph.replay|torch.npu.NPUGraph.replay|Yes|Currently only supports inference scenario, not training scenario|
|torch.cuda.CUDAGraph.reset|torch.npu.NPUGraph.reset|Yes|Currently only supports inference scenario, not training scenario|
|torch.cuda.graph|torch.npu.graph|Yes|Currently only supports inference scenario, not training scenario|
|torch.cuda.make_graphed_callables|torch.npu.make_graphed_callables|Yes|Currently only supports inference scenario, not training scenario|
