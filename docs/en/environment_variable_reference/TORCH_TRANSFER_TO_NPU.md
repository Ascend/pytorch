...
Enable the transfer\_to\_npu feature:

```bash
export TORCH_TRANSFER_TO_NPU=1
```

Disable the transfer\_to\_npu feature:
...
- For more constraints related to transfer\_to\_npu, refer to [(Recommended) Automatic Migration](https://gitcode.com/Ascend/ModelZoo-PyTorch/blob/master/PyTorch/docs/en/model_migration/auto_migration.md).
