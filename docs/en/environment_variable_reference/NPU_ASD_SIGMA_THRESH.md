...
This environment variable configures the relative threshold of the feature value detection function. The format is an integer data pair, with a minimum value of 3.

The algorithm detects the jump amplitude of feature values. The first element controls the first-level threshold. When the jump amplitude exceeds the first-level threshold, training is terminated and an alarm is reported. The second element controls the second-level threshold. When the jump amplitude exceeds the second-level threshold and `ASCEND_GLOBAL_LOG_LEVEL` is set to `0`, `1`, or `2`, a Warning-level log is printed as an advance warning. Decreasing the threshold can detect abnormal data with smaller fluctuations and increase the detection rate, while increasing the threshold has the opposite effect.
