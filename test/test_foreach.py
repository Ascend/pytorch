...
from torch.testing._internal.common_device_type import \
    (instantiate_device_type_tests, dtypes, onlyPRIVATEUSE1, ops, OpDTypes)
...
class ForeachFuncWrapper:
    ...
    def __call__(self, inputs, is_npu, is_fastpath, **kwargs):
        actual = None
...
class TestForeach(TestCase):
        ...
        *,
        ...
        if zero_size:
            with InplaceForeachVersionBumpCheck(self, inputs[0]) if op.is_inplace else nullcontext():
                op(inputs, self.is_npu, is_fastpath, zero_size=zero_size)
                return

        ref_inputs = [[t.clone().detach() for t in inputs[0]], inputs[1]] if is_inplace else inputs
        try:
            with InplaceForeachVersionBumpCheck(self, inputs[0]) if op.is_inplace else nullcontext():
                actual = op(inputs, self.is_npu, is_fastpath, zero_size=zero_size)
        ...
        if alpha is not None and not scalar_self_arg:
            ...
            try:
                ...
                with InplaceForeachVersionBumpCheck(self, inputs[0]) if op.is_inplace else nullcontext():
                    actual = op(inputs, self.is_npu, is_fastpath, **op_kwargs)
            except RuntimeError as e:
                with self.assertRaisesRegex(type(e), re.escape(str(e))):
    ...
    def test_binary_op(self, device, dtype, op, is_fastpath):
        ...
        for i, sample in enumerate(op.sample_inputs(device, dtype, noncontiguous=not is_fastpath)):
            ...
            if op.supports_autograd and dtype in floating_types() and not zero_size:
                ...
                try:
                    sum(
                        wrapped_op([tensors, rhs_arg], is_npu=False, is_fastpath=False, zero_size=zero_size)
                    ).mean().backward()
                except RuntimeError:
                    ...
                    inplace_op([tensors, rhs_arg], is_npu=False, is_fastpath=False, zero_size=zero_size)
            ...
            if (
                ...
                if op.supports_autograd and dtype == torch.float32 and not zero_size:
                    ...
                    ref_tensors, ref_rhs_arg = clone(tensors), clone(rhs_arg)
                    sum(wrapped_op(
                        [rhs_arg, tensors], is_npu=False, is_fastpath=False, zero_size=False
                    )).mean().backward()
            ...
            if is_fastpath and isinstance(values, list) and not zero_size:
                ...
                for is_inplace, op_, ref_ in ((False, wrapped_op, ref), (True, inplace_op, inplace_ref)):
                    ...
                    if self.is_npu:
                        self._pointwise_test(
                            op_, ref_, inputs, is_fastpath and not disable_fastpath, is_inplace,
                            values=tensor_values.npu(),
                            custom_values_err="Expected scalars to be on CPU, got npu:0 instead.",
                            zero_size=False,
                        )
                    self._pointwise_test(
                        op_, ref_, inputs, is_fastpath and not disable_fastpath, is_inplace,
    ...
    def _pointwise_test(
        ...
        *,
        ...
        if zero_size:
            op(inputs, self.is_npu, is_fastpath, **kwargs)
            return
        ...
        try:
            with (InplaceForeachVersionBumpCheck(self, inputs[0]) if is_inplace else nullcontext()):
                actual = op(inputs, self.is_npu, is_fastpath, **kwargs)
        except RuntimeError as e:
            with self.assertRaisesRegex(type(e), re.escape(str(e))):
                ref(ref_inputs)
        else:
        ...
        if values is not None:
            try:
                actual = op(inputs + [values], self.is_npu, is_fastpath, **kwargs)
            except RuntimeError as e:
    ...
    def _inplace_unary_test(self, inplace, inplace_ref, inputs, is_fastpath, **kwargs):
        ...
        try:
            with InplaceForeachVersionBumpCheck(self, inputs[0]):
                inplace(inputs, self.is_npu, is_fastpath, **kwargs)
        except RuntimeError as e:
            with self.assertRaisesRegex(type(e), re.escape(str(e))):
        ...
        for sample in samples:
            ...
            if zero_size:
                if not op.has_no_out_of_place:
                    wrapped_op(inputs, self.is_npu, is_fastpath and not disable_fastpath, zero_size=zero_size)
                inplace_op(inputs, self.is_npu, is_fastpath and not disable_fastpath, zero_size=zero_size)
                continue
            inputs = [sample.input]
    ...
    def test_reduce_op(self, device, dtype, op, is_fastpath):
        for sample in op.sample_inputs(device, dtype, noncontiguous=not is_fastpath):
            ...
            self.assertEqual(
                ref(inputs, ord=ord),
                wrapped_op(
                    inputs, self.is_npu, is_fastpath and not disable_fastpath, ord=ord,
                    zero_size=zero_size,
                ),
            )
            if op.supports_autograd and dtype in floating_types() and not zero_size:
    ...
    def test_binary_op_list_error_cases(self, device, dtype, op):
        ...
        # different devices
        if self.device_type == "npu" and torch.npu.device_count() > 1:
            tensor1 = torch.zeros(10, 10, device="npu:0", dtype=dtype)
            tensor2 = torch.ones(10, 10, device="npu:1", dtype=dtype)
    ...
    @unittest.skipIf(not torch.npu.is_available(), "NPU not found")
    @ops(
        filter(lambda op: not op.has_no_out_of_place, foreach_binary_op_db),
        dtypes=OpDTypes.supported,
    )
    ...
    # but tensors of the same index are on the same device, e.g., ['npu', 'cpu].
    @onlyPRIVATEUSE1
    @ops(foreach_unary_op_db)
    ...
    def test_binary_op_tensors_on_different_devices(self, device, dtype, op):
        # `tensors1`: ['npu', 'cpu']
        # `tensors2`: ['npu', 'cpu']
        _npu_tensors = list(op.sample_inputs(device, dtype, num_input_tensors=[2], same_size=True))[0].input
        _cpu_tensors = list(op.sample_inputs("cpu", dtype, num_input_tensors=[2], same_size=True))[0].input
        tensors1, tensors2 = list(zip(_npu_tensors, _cpu_tensors))
    ...
    def test_pointwise_op_tensors_on_different_devices(self, device, dtype, op):
        ...
        tensors1, tensors2, tensors3 = list(zip(_npu_tensors, _cpu_tensors))

        foreach_op, foreach_op_, native_op = op.method_variant, op.inplace_variant, op.ref
        actual = foreach_op(tensors1, tensors2, tensors3)
        expected = [native_op(*_npu_tensors), native_op(*_cpu_tensors)]
        self.assertEqual(expected, actual)
    ...
    def test_foreach_l2_large_value_input(self, device, dtype, op):
        ...
        actual = fn(inputs, is_npu=True, is_fastpath=True, ord=ord, zero_size=False)
    ...
    def test_lerp(self, device, dtype, op, is_fastpath):
        for sample in op.sample_inputs(device, dtype, noncontiguous=not is_fastpath):
            ...
            if dtype in integral_types() or dtype == torch.bool:
                with self.assertRaises(RuntimeError):
                    wrapped_op(inputs, self.is_npu, is_fastpath, **kwargs)
                return
            actual = wrapped_op(inputs, self.is_npu, is_fastpath, **kwargs)
            ...
            with InplaceForeachVersionBumpCheck(self, inplace_inputs[0]):
                inplace_actual = inplace_op(inplace_inputs, self.is_npu, is_fastpath, **kwargs)
    ...
    def test_foreach_reduce_large_input(self, device, dtype, op):
        ...
        self.assertEqual(
            ref(inputs, ord=ord),
            wrapped_op(inputs, self.is_npu, not disable_fastpath, ord=ord, zero_size=False),
        )
    ...
    def test_outplace_with_invalid_grads(self, device, dtype, op):
        ...
        (out1, out2) = func([sample.input, *sample.args], is_npu=False, is_fastpath=False, **sample.kwargs)
    ...
    def test_lifetime_of_grad_fn_when_result_is_saved(self, device, dtype, op):
        ...
        def get_ref(func, sample):
            ...
            out = func((sample.input, *sample.args), is_npu=False, is_fastpath=False, **sample.kwargs)
            foo = Foo()
    ...
    @unittest.skipIf(not (torch.npu.is_available() and torch.npu.device_count() > 1), "requires multiple GPUs")
    def test_tensors_grouping(self):
        ...
        list1 = [
            torch.tensor(
                i,
                device=torch.device("npu", random.randint(0, num_devices - 1)),
    ...
    def test_0dim_tensor_overload_exception(self):
        ...
        tensors = [make_tensor((2, 2), dtype=torch.float, device="npu") for _ in range(2)]

        with self.assertRaisesRegex(RuntimeError, "scalar tensor expected to be 0 dim but"):
            torch._foreach_mul(tensors, torch.tensor([1.0, 1.0], device="npu"))
        ...
        tensors = [make_tensor((2, 2), dtype=torch.float, device=d) for d in ("cpu", "npu")]
        with self.assertRaisesRegex(RuntimeError, "scalar tensor expected to be 0 dim but"):
            torch._foreach_mul(tensors, torch.tensor([1.0, 1.0], device="npu"))
...
instantiate_device_type_tests(TestForeach, globals(), only_for='privateuse1')
