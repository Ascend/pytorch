...
class ForeachFuncWrapper:
    ...
    def __call__(self, inputs, is_npu, expect_fastpath, **kwargs):
        actual = None
...
class TestForeach(TestCase):
    ...
    def test_all_zero_size_tensors_do_not_launch_kernel(self, device, dtype, op):
        ...
        for sample in op.sample_zero_size_inputs(device, dtype):
            if op.supports_out:
                wrapped_op((sample.input, *sample.args), is_npu=self.is_npu, expect_fastpath=True, zero_size=True)
            with InplaceForeachVersionBumpCheck(self, sample.input):
                inplace_op((sample.input, *sample.args), is_npu=self.is_npu, expect_fastpath=True, zero_size=True)
    ...
    def test_parity(self, device, dtype, op, noncontiguous, inplace):
        ...
        for sample in op.sample_inputs(device, dtype, noncontiguous=noncontiguous):
            ...
            try:
                with ctxmgr:
                    actual = func([sample.input, *sample.args], self.is_npu, expect_fastpath, **sample.kwargs)
        ...
        *,
        ...
        try:
            with InplaceForeachVersionBumpCheck(self, inputs[0]) if op.is_inplace else nullcontext():
                actual = op(inputs, self.is_npu, is_fastpath)
        ...
        for i, sample in enumerate(op.sample_inputs(device, dtype, noncontiguous=not is_fastpath)):
            ...
            if isinstance(rhs_arg, Number) and not scalar_self_arg_test_complete:
                ...
                if op.supports_autograd and dtype == torch.float32:
                    ...
                    sum(wrapped_op(
                        [rhs_arg, tensors], is_npu=False, expect_fastpath=False
                    )).mean().backward()
    ...
    def test_binary_op_tensors_on_different_devices(self, device, dtype, op):
        # `tensors1`: ['npu', 'cpu']
        # `tensors2`: ['npu', 'cpu']
        _npu_tensors = next(iter(op.sample_inputs(device, dtype, num_input_tensors=[2], same_size=True))).input
    ...
    def test_foreach_l2_large_value_input(self, device, dtype, op):
        ...
        actual = fn(inputs, is_npu=True, expect_fastpath=True, ord=ord, zero_size=False)
    ...
    def test_big_num_tensors(self, device, dtype, op, use_cuda_graph):
        ...
        for ord in (1, 2, math.inf):
            if not use_cuda_graph:
                actual = fn(inputs=[tensorlist], is_npu=True, expect_fastpath=True, ord=ord, zero_size=False)
            else:
    ...
    def test_outplace_with_invalid_grads(self, device, dtype, op):
        ...
        (out1, out2) = func([sample.input, *sample.args], is_npu=False, expect_fastpath=False, **sample.kwargs)
    ...
    def test_lifetime_of_grad_fn_when_result_is_saved(self, device, dtype, op):
        ...
        def get_ref(func, sample):
            ...
            out = func((sample.input, *sample.args), is_npu=False, expect_fastpath=False, **sample.kwargs)
            foo = Foo()
    ...
    @unittest.skipIf(not (torch.npu.is_available() and torch.npu.device_count() > 1), "multi-NPU not supported")
    ...
    def test_0dim_tensor_overload_cpu_ok(self):
        tensors = [torch.ones((), device="npu", dtype=torch.float32) for _ in range(2)]
    ...
    def test_0dim_tensor_overload_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "scalar tensor expected to be 0 dim but"):
            torch._foreach_add(tensors, torch.tensor([1.0, 1.0], device="npu"))
    ...
    def test_foreach_copy_with_multi_dtypes(self, device, dtype, op):
        ...
        for sample in op.sample_inputs(device, dtype, noncontiguous=False):
            for src_dtype in floating_types_and(torch.half, torch.bfloat16):
                ...
                out = foreach_copy_((self_tensors, src_tensors), is_npu=True, expect_fastpath=True)
