...
from torch.testing._internal.common_device_type import (
    instantiate_device_type_tests, onlyCPU, dtypes, skipLazy, skipMeta, skipXLA)
from torch.testing._internal.common_dtype import (
    all_types_and_complex_and, complex_types, all_types_and, floating_and_complex_types_and,
)


# replace this with make_tensor() in common_utils.py
...
# refactor tests to avoid this function
...
class TestViewOps(TestCase):
    ...
    def test_view_dtype_new(self, device, dtype):
        dtypes_t = {value : key for (key, value) in numpy_to_torch_dtype_dict.items()}
        del dtypes_t[torch.bool]
        ...
        for a in generate_inputs():
            ...
            for view_dtype, np_view_dtype in dtypes_t.items():
        ...
        # Remove this when autograd support is added
        if dtype.is_floating_point or dtype.is_complex:
    ...
    def test_view_as_complex(self, device):
        def fn(contiguous_input=True, dim0=0, dim1=1):
            ...
            input_t = self._do_transpose(t, contiguous_input, dim0, dim1)

            if input_t.size()[-1] != 2:
                self.assertRaisesRegex(
                    RuntimeError, "Tensor must have a last dimension of size 2",
                    lambda: torch.view_as_complex(input_t))
                return

            if input_t.stride()[-1] != 1:
                self.assertRaisesRegex(
                    RuntimeError, "Tensor must have a last dimension with stride 1",
                    lambda: torch.view_as_complex(input_t))
                return

            res = torch.view_as_complex(input_t)
    ...
    def test_view_as_real(self, device, dtype):
        def fn(contiguous_input=True):
            ...
            input_t = self._do_transpose(t, contiguous_input)
            res = torch.view_as_real(input_t)
            self.assertEqual(res[:, :, 0], input_t.real)
            self.assertEqual(res[:, :, 1], input_t.imag)
            self.assertTrue(self.is_view_of(t, res))
    ...
    @suppress_warnings
    def test_set_real_imag(self, device, dtypes_t):
        x = torch.randn(10, dtype=dtypes_t[0], device=device)

        new_real = _make_tensor((10,), dtypes_t[1], device)
        new_imag = _make_tensor((10,), dtypes_t[1], device)
    ...
    # opinfo this or move to unbind's test suite
    ...
    # Fix this test for LTC. There is an interaction with dynamic shapes here that is broken,
        ...
        def test(x, prepro_fn, size, strides, offset=None):
            ...
            with torch.no_grad():
                ...
                if max_offset >= len(y.storage()):
                    raise RuntimeError("test case resizes storage")
...
class TestOldViewOps(TestCase):
    ...
    # this should be refactored into the view ops test suite
    ...
    def test_reshape(self, device):
        ...
        # fix these once we have multi-dimensional empty tensors
    ...
    # is resize best put in test_view_ops?
                ...
                for random_negative in [True, False]:
                    for src_dim, dst_dim in permutations(range(nd), r=2):
                        ...
                        try:
                            ...
                            print(f"partial_map do not have the key {fn}")
