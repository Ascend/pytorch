...
class TestUtilityFuns(_BaseTestCase):
    ...
    def test_is_in_onnx_export(self):
        ...
        f = io.BytesIO()
        try:
            torch.onnx.export(MyModule(), x, f, opset_version=self.opset_version, dynamo=False)
        except ValueError:
    ...
    def test_constant_fold_lstm(self):
        class GruNet(torch.nn.Module):
            ...
            def forward(self, input_, initial_state):
                return self.mygru(input_, initial_state)

        GLOBALS.export_onnx_opset_version = self.opset_version
        GLOBALS.operator_export_type = OperatorExportTypes.ONNX
        input_ = torch.randn(5, 3, 7)
    ...
    def test_verbose(self):
        ...
        def is_model_stripped(f, verbose=None):
            if verbose is None:
                ...
                torch.onnx.export(
                    MyModule(), x, f, verbose=verbose, opset_version=self.opset_version, dynamo=False
                )
    ...
    def test_error_on_data_parallel(self):
        ...
        with self.assertRaisesRegex(
            ...
            torch.onnx.export(model, x, f, opset_version=self.opset_version, dynamo=False)
    ...
    def test_sequence_dim(self):
        ...
        f = io.BytesIO()
        y = torch.randn(2, 3)
        torch.onnx.export(script_model, (x, y), f, opset_version=self.opset_version, dynamo=False)
        onnx_model = onnx.load(io.BytesIO(f.getvalue()))
        loop_output_value_info_proto = onnx_model.graph.output[0]
        ref_value_info_proto = onnx.helper.make_tensor_sequence_value_info(
    ...
    def test_export_does_not_fail_on_frozen_scripted_module(self):
        ...
        torch.onnx.export(module, (x,), io.BytesIO(), opset_version=self.opset_version, dynamo=False)
    ...
    # See pytorch/pytorch/pull/109759 for an example. The exception that
    ...
    def test_aten_fallthrough(self):
        ...
        iter_ = graph.nodes()
        self.assertEqual(next(iter_).kind(), "aten::erfc")
    ...
    def test_custom_op_fallthrough(self):
        ...
        class FooModel(torch.nn.Module):
            def forward(self, input_, other):
                # Calling custom op
                return torch.ops.custom_namespace.custom_op(input_, other)
        ...
        iter_ = graph.nodes()
        self.assertEqual(next(iter_).kind(), "custom_namespace::custom_op")
    ...
    def test_onnx_fallthrough(self):
        ...
        iter_ = graph.nodes()
        self.assertEqual(next(iter_).kind(), "aten::digamma")
    ...
    def test_prim_fallthrough(self):
        ...
        iter_ = graph.nodes()
        self.assertEqual(next(iter_).kind(), "prim::ListConstruct")

    def test_custom_layer_tuple(self):
        class CustomFunction(torch.autograd.Function):
            @staticmethod
            def symbolic(g, input_):
                return g.op("CustomNamespace::Custom", input_, outputs=2)
        ...
        class Custom(torch.nn.Module):
            def forward(self, input_):
                return CustomFunction.apply(input_)
        ...
        iter_ = graph.nodes()
        self.assertEqual(next(iter_).kind(), "CustomNamespace::Custom")
    ...
    def test_autograd_onnx_fallthrough(self):
        ...
        iter_ = graph.nodes()
        self.assertEqual(next(iter_).kind(), "prim::PythonOp")
    ...
    def test_autograd_module_name(self):
        ...
        class Custom(torch.nn.Module):
            def forward(self, input_):
                return CustomFunction.apply(input_) + CustomFunction2.apply(input_)
        ...
        iter_ = graph.nodes()
        autograd1 = next(iter_)
        autograd2 = next(iter_)
    ...
    def test_onnx_node_naming(self):
        ...
        torch.onnx.export(module, torch.ones(1, 10), f, output_names=["y"], dynamo=False)
        onnx_model = onnx.load(io.BytesIO(f.getvalue()))
        for n in onnx_model.graph.node:
            self.assertIn(n.name, ref_node_names)

        torch.onnx.export(
            torch.jit.script(module), torch.ones(1, 10), f, output_names=["y"], dynamo=False
        )
        onnx_model = onnx.load(io.BytesIO(f.getvalue()))
        for n in onnx_model.graph.node:
            self.assertIn(n.name, ref_node_names)
    ...
    def test_deduplicate_initializers_diff_devices(self):
        ...
        f = io.BytesIO()
        torch.onnx.export(Model(), (x, y), f, opset_version=self.opset_version, dynamo=False)
        graph = onnx.load(io.BytesIO(f.getvalue()))
    ...
    def test_bad_symbolic_registration(self):
        ...
        self.assertExpectedRaisesInline(
            ...
            (
                ...
                f"library, please file an issue at {issue_url} to "
                "report this bug."
            ),
        )
