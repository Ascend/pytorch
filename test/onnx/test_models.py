...
class TestModels(pytorch_test_common.ExportTestCase):
    ...
    def test_ops(self):
        ...
        self.exportTest(to_deivce(DummyNet()), to_deivce(x))
    ...
    def test_concat(self):
        ...
        inputs = ((to_deivce(input_a), to_deivce(input_b)),)
        self.exportTest(to_deivce(ConcatNet()), inputs)
    ...
    def test_srresnet(self):
        ...
        self.exportTest(
            to_deivce(SRResNet(rescale_factor=4, n_filters=64, n_blocks=8)), to_deivce(x)
        )

    @skipIfNoLapack
    def test_super_resolution(self):
        x = Variable(torch.randn(BATCH_SIZE, 1, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(SuperResolutionNet(upscale_factor=3)), to_deivce(x), atol=1e-6)

    def test_alexnet(self):
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(alexnet()), to_deivce(x))

    def test_mnist(self):
        x = Variable(torch.randn(BATCH_SIZE, 1, 28, 28).fill_(1.0))
        self.exportTest(to_deivce(MNIST()), to_deivce(x))

    @unittest.skip("This model takes too much memory")
    def test_vgg16(self):
        # VGG 16-layer model (configuration "D")
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(vgg16()), to_deivce(x))

    @unittest.skip("This model takes too much memory")
    def test_vgg16_bn(self):
        # VGG 16-layer model (configuration "D") with batch normalization
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(vgg16_bn()), to_deivce(x))

    @unittest.skip("This model takes too much memory")
    def test_vgg19(self):
        # VGG 19-layer model (configuration "E")
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(vgg19()), to_deivce(x))

    @unittest.skip("This model takes too much memory")
    def test_vgg19_bn(self):
        # VGG 19-layer model (configuration "E") with batch normalization
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(vgg19_bn()), to_deivce(x))

    def test_resnet(self):
        # ResNet50 model
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(resnet50()), to_deivce(x), atol=1e-6)
    ...
    def test_inception(self):
        ...
        self.exportTest(to_deivce(inception_v3()), to_deivce(x), acceptable_error_percentage=0.01)
    ...
    def test_squeezenet(self):
        ...
        self.exportTest(to_deivce(sqnet_v1_0), to_deivce(x))

        # SqueezeNet 1.1 has 2.4x less computation and slightly fewer params
        # than SqueezeNet 1.0, without sacrificing accuracy.
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        sqnet_v1_1 = SqueezeNet(version=1.1)
        self.exportTest(to_deivce(sqnet_v1_1), to_deivce(x))

    def test_densenet(self):
        # Densenet-121 model
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(densenet121()), to_deivce(x), rtol=1e-2, atol=1e-5)
    ...
    def test_dcgan_netD(self):
        ...
        self.exportTest(to_deivce(netD), to_deivce(input_))
    ...
    def test_dcgan_netG(self):
        ...
        self.exportTest(to_deivce(netG), to_deivce(input_))

    @skipIfUnsupportedMinOpsetVersion(10)
    def test_fake_quant(self):
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(FakeQuantNet()), to_deivce(x))
    ...
    def test_qat_resnet_pertensor(self):
        ...
        self.exportTest(to_deivce(qat_resnet50), to_deivce(x))
    ...
    def test_googlenet(self):
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(googlenet()), to_deivce(x), rtol=1e-3, atol=1e-5)

    def test_mnasnet(self):
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(mnasnet1_0()), to_deivce(x), rtol=1e-3, atol=1e-5)

    def test_mobilenet(self):
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(mobilenet_v2()), to_deivce(x), rtol=1e-3, atol=1e-5)

    @skipScriptTest()  # prim_data
    def test_shufflenet(self):
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(to_deivce(shufflenet_v2_x1_0()), to_deivce(x), rtol=1e-3, atol=1e-5)

    @skipIfUnsupportedMinOpsetVersion(11)
    def test_fcn(self):
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(
            to_deivce(fcn_resnet101(weights=None, weights_backbone=None)),
            to_deivce(x),
            rtol=1e-3,
            atol=1e-5,
        )
    ...
    def test_deeplab(self):
        x = Variable(torch.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
        self.exportTest(
            to_deivce(deeplabv3_resnet101(weights=None, weights_backbone=None)),
            to_deivce(x),
            rtol=1e-3,
            atol=1e-5,
        )
    ...
    def test_r3d_18_video(self):
        ...
        self.exportTest(to_deivce(r3d_18()), to_deivce(x), rtol=1e-3, atol=1e-5)

    def test_mc3_18_video(self):
        x = Variable(torch.randn(1, 3, 4, 112, 112).fill_(1.0))
        self.exportTest(to_deivce(mc3_18()), to_deivce(x), rtol=1e-3, atol=1e-5)

    def test_r2plus1d_18_video(self):
        x = Variable(torch.randn(1, 3, 4, 112, 112).fill_(1.0))
        self.exportTest(to_deivce(r2plus1d_18()), to_deivce(x), rtol=1e-3, atol=1e-5)
