...
class TestDynamoWithONNXRuntime(onnx_test_common._TestONNXRuntime):
    ...
    def test_get_ort_device_type(self):
        self.assertEqual(
            torch.onnx._internal.onnxruntime._get_ort_device_type("npu"),
    ...
    def _assert_dynamic_input_and_output_shapes_in_all_onnx_models(self, backend):
        for (
            ...
            for onnx_session_info in onnx_session_infos:
                ...
                for input_ in onnx_session_info.input_value_infos:
                    if hasattr(input_.type, "tensor_type") and hasattr(
                        input_.type.tensor_type, "shape"
                    ):
    ...
    @skipIfNNModuleInlined("pytorch/pytorch/issues/129456")
    ...
    @unittest.skipIf(not torch.npu.is_available(), "No NPU to run mix devicei nputs")
    def test_mix_device_inputs(self):
        data = torch.randn(4, 8, device="npu")
