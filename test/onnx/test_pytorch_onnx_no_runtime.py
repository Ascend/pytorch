...
def export_to_onnx(
    ...
    **torch_onnx_export_kwargs,
    ...
    Args:
        ...
        inputs: model input with same format as `torch.onnx.export(..,args,...)`
...
class TestONNXExport(pytorch_test_common.ExportTestCase):
    ...
    def test_export_dynamic_slice(self):
        ...
        input_ = torch.rand(3, 4, 5)

        torch.onnx.export_to_pretty_string(
            DynamicSliceExportMod(), (input_,), opset_version=10
        )
    ...
    def test_onnx_checker_invalid_graph(self):
        ...
        def symbolic_custom_invalid_add(g, input_, other, alpha=None):
            return g.op("Add", input_, other, invalid_attr_i=1)
    ...
    def test_fp8_export(self, fp8_dtype: torch.dtype):
        ...
        }  # From onnx/blob/main/onnx/onnx.proto3#L512-L521
        loaded_model = onnx.load_from_string(f.getvalue())
        self.assertEqual(
...
class TestQuantizeEagerONNXExport(common_utils.TestCase):
    def _test_lower_graph_impl(self, model, data):
        ...
        def _export_to_onnx(model, input_, input_names):
            traced = torch.jit.trace(model, input_)
            buf = io.BytesIO()
    ...
    def test_composed_layer_norm_small_eps_fp16_keep_double(self):
        ...
        N, C = 8, 4
        model = Net(C).npu().half()
        x = torch.randn(N, C).npu().half()
        f = io.BytesIO()
