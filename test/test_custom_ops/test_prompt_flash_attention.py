...
class TestPromptFlashAttention(TestCase):
    ...
    def supported_op_exec(self, query, past_key, past_value, head_dim, hidden_size):
        attn_weights1 = torch.matmul(query, past_key.transpose(2, 3)) / 0.0078125
        attn_weights1 = torch.max(attn_weights1, torch.full((1, 1), torch.finfo(attn_weights1.dtype).min, device=attn_weights1.device))
        attn_weights1 = torch.nn.functional.softmax(attn_weights1, dim=-1, dtype=torch.float32).to(query.dtype)
    ...
    def custom_op_exec(self, q, k, v, head_dim):
        scale = 1 / 0.0078125
        return torch_npu.npu_prompt_flash_attention(q, k, v, num_heads=32, input_layout="BSH", scale_value=scale)
    ...
    def test_npu_prompt_flash_attention(self, device="npu"):
        ...
        custom_output = self.custom_op_exec(q_FA, k_FA, v_FA, head_dim)
