...
@unittest.skipIf(IS_ASCEND950,
    "Ascend950 copy behavior differs; see TestCopyFormatAscend950")
        ...
        for fmt_name, fmt in FORMAT_INFO.items():
            ...
            for other_name, other in FORMAT_INFO.items():
                ...
                self.assertTrue(torch.equal(dst_d2d.cpu(), src_cpu),
                    f"d2d dst={fmt_name} src={other_name}")
                self.assertEqual(torch_npu.get_npu_format(dst_d2d), fmt_before,
                    f"d2d dst={fmt_name} format changed")
...
class TestCopyFormatAscend950(TestCase):
    ...
    def test_copy_nz_d2h(self):
        for dt in D2H_TEST_DTYPES:
            ...
            self.assertIn(torch_npu.get_npu_format(src_nz), NZ_FORMAT_FAMILY,
