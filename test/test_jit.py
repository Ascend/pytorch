...
from jit.test_npu import TestCUDA  # noqa: F401
...
class TestJitProfiler(JitTestCase):
    ...
    ytorch/pytorch/issues/91483 in which test_profiler was flaky and failed in the
...
class TestJit(JitTestCase):
    ...
    def test_restore_device_cuda(self):
        ...
        m = MyModule()
        m.npu(torch.npu.device_count() - 1)
        cuda_device_str = 'npu:' + str(torch.npu.device_count() - 1)

        self.assertTrue(m.p0.is_npu)
        self.assertTrue(m.b0.is_npu)
        ...
        input = torch.rand(2, 3).npu(torch.npu.device_count() - 1)
    ...
    def test_restore_shared_storage_on_cuda(self):
        ...
        m = Foo()
        m2 = self.getExportImportCopy(m, map_location=torch.device('npu:0'))
        self.assertEqual(tuple(m.parameters()), tuple(m2.parameters()))
        self.assertEqual(tuple(m.buffers()), tuple(m2.buffers()))
        self.assertTrue(m2.p0.is_npu)
        self.assertTrue(m2.b0.is_npu)
    ...
    def test_peephole_optimize_shape_ops(self):
        ...
        def test_device():
            @torch.jit.script
            def func_1(x):
                if x.device == torch.device('npu:0'):
                    a = 0
                else:
                    a = 1
                return a
    ...
    def test_arg_configurations(self):
        ...
        if torch.npu.is_available():
            ...
            if torch.npu.device_count() > 1:
                x_cuda_1 = Variable(x.data.npu(1))
                configurations += [
                    (x_cuda_1,),
                    ([x_cuda, x_cuda_1],),
                ]
...
graph(%Ra, %Rb):
  return (%Ra)""", graph)
    ...
    def test_dropout_cuda(self):
        ...
        x = torch.ones(1000, 1000).npu().requires_grad_()
    ...
    def test_device_not_equal(self):
        ...
        def compare_device(x: torch.device):
            return x != torch.device("npu:0")

        def compare_two_device(x: torch.device, y: torch.device):
            return x != y

        self.checkScript(compare_device, (torch.device("npu:0"),))
        self.checkScript(compare_two_device, (torch.device("npu:0"), torch.device("npu:1"), ))
    ...
    @unittest.skipIf(True, "TODO: re-enable with pytorch/pull/29339")
    ...
    def test_pow_scalar_backward_cuda(self):
        ...
        with enable_profiling_mode_for_profiling_tests():
            for dtype in [torch.float, torch.double]:
                ...
                a = torch.rand(1, requires_grad=True, device='npu', dtype=dtype)
    ...
    def test_scriptmodule_releases_tensors_cuda(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            def test(backward=False):
                x = torch.randn(3, 3, dtype=torch.double, device='npu', requires_grad=True)
                y = torch.randn(3, 3, dtype=torch.double, device='npu', requires_grad=True)
        ...
        def checkMath(func_name, num_args, is_float=True, ret_type="float", debug=False, vals=None, args_type=None):
            ...
            for a, b in vals:
                ...
                if res_python != res_script:
                    ...
                    # math.pow() behavior has changed in 3.11, see docs.python.org/3/library/math.html#math.pow
...
dedent """
    ...
    def test_tensor_to(self):
        ...
        def s(t, to_str, non_blocking=None, device=None, cuda=None):
            ...
            cuda = "npu" if cuda is None else cuda
        ...
        def test_copy_behavior(t, non_blocking=False):
            ...
            if t.device.type == 'npu':
                if t.device.index == -1:
                    devices.append(f'npu:{torch.npu.current_device()}')
                elif t.device.index == torch.npu.current_device():
                    devices.append('npu')
        ...
        if torch.npu.is_available():
            for non_blocking in [True, False]:
                for cuda in ['npu', 'npu:0' if torch.npu.device_count() == 1 else 'npu:1']:
    ...
    def test_device_kwarg(self):
        ...
        def f():
            return device(type='npu'), torch.device(type='cpu')
        self.checkScript(f, ())
    ...
    def test_script_module_export_tensor_cuda(self):
        class M(torch.jit.ScriptModule):
            ...
            def __init__(self):
                super().__init__()
                self.param = torch.nn.Parameter(torch.zeros((5, 5), device='npu:0').random_())
        ...
        # check to make sure the storage wasn't resized
        self.assertTrue(m_orig.param.storage().size() == 25)
        self.assertTrue(m_import.foo().device == torch.device('npu:0'))
        self.assertEqual(m_orig.foo(), m_import.foo())
        self.assertTrue(m_orig.foo().dtype == m_import.foo().dtype)
    ...
    def test_batch_norm_inference_backward_cuda(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            hw = 3
            # Initialize param and input values
            x_init = torch.randn(batch, c, hw, hw, dtype=torch.float).npu()
            grad = torch.randn(batch, c, hw, hw, dtype=torch.float).npu()

            training = False
            affine = True
            track_running_stats = True

            module = torch.jit.script(MyBatchNorm(c, affine, track_running_stats)).npu()
            ref_module = MyBatchNorm(c, affine, track_running_stats).npu()
            module.eval()
            ref_module.eval()
    ...
    def test_autodiff_complex(self):
        ...
        x = torch.randn(128, 16, dtype=torch.float32, device='npu:0')
        y = torch.randn(128, 16, dtype=torch.float32, device='npu:0')
        W = torch.randn(16, 1, dtype=torch.float32, device='npu:0', requires_grad=True)
        W.data /= 4
    ...
    @unittest.skip('pytorch/pytorch/issues/9595')
    ...
    def test_scriptmodule_multi_head_attn_cuda(self):
        ...
        sl = 3
        bs = 2
        model = MyModule(embed_dim, num_heads).npu()
        q = torch.randn(sl, bs, embed_dim, device="npu")
        kv = torch.randn(sl, bs, embed_dim, device="npu")
    ...
    def test_weak_cuda(self):
        ...
        m = M()
        m.npu()
        out = m(torch.ones(5, 5, 5).npu())
        self.assertTrue(out[0].is_npu)
    ...
    def get_pickle_values(self):
        return (('dict', {"I": "am", "a test": "test"}, Dict[str, str]),
                ...
                ('another_device', torch.device('npu:1'), torch.device))
