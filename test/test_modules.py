...
from torch.testing._internal.common_device_type import (
    instantiate_device_type_tests, onlyCPU, onlyPRIVATEUSE1, toleranceOverride, tol, skipMeta)
...
class TestModule(TestCase):
        ...
        for module_input_device, module_input_cpu in zip(module_inputs_device, module_inputs_cpu):
            ...
            with freeze_rng_state():
                ...
                if torch.npu.device_count() >= 2:
                    ...
                    self._assert_module_parameters_and_buffer_are(m, torch.device("npu:1"), dtype)
    ...
    def _traverse_obj(self, obj, func):
        if isinstance(obj, (tuple, list)):
            return type(obj)(self._traverse_obj(obj_ele, func) for obj_ele in obj)
        elif isgenerator(obj):
            return tuple(self._traverse_obj(obj_ele, func) for obj_ele in obj)
        elif isinstance(obj, dict):
            return {name: self._traverse_obj(obj_ele, func) for name, obj_ele in obj.items()}
    ...
    def test_non_contiguous_tensors(self, device, dtype, module_info, training):
        ...
        def _can_be_noncontiguous(obj):
            if isinstance(obj, (tuple, list)):
                return any(_can_be_noncontiguous(obj_ele) for obj_ele in obj)
            elif isinstance(obj, dict):
                return any(_can_be_noncontiguous(obj_ele) for obj_ele in obj.values())
    ...
    def test_cpu_gpu_parity(self, device, dtype, module_info, training):
        ...
        def _to_device(obj):
            if isinstance(obj, torch.Tensor):
                ...
                return tuple(_to_device(obj_ele) for obj_ele in obj)
            elif isinstance(obj, dict):
                return {key: _to_device(obj_ele) for key, obj_ele in obj.items()}
            else:
                return deepcopy(obj)
    ...
    def test_memory_format(self, device, dtype, module_info, training):
        is_sm86or80 = device.startswith("npu")
        ...
        def _check_dims(obj, n):
            if isinstance(obj, torch.Tensor):
                ...
                return any(_check_dims(obj_ele, n) for obj_ele in obj)
            else:
                return False
...
instantiate_device_type_tests(TestModule, globals(), allow_mps=True, only_for='privateuse1')
