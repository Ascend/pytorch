...
class DistMatrixOpsTest(DTensorTestBase):
    ...
    def test_addmm(self):
        ...
        tensor_to_shard = torch.randn(12, 8).npu()
        mat1 = distribute_tensor(tensor_to_shard, device_mesh, shard_spec)
        tensor_to_replicate = torch.randn(8, 4).npu()
        mat2 = distribute_tensor(tensor_to_replicate, device_mesh, replica_spec)
        input_tensor = torch.randn(4).npu()
        input1 = distribute_tensor(input_tensor, device_mesh, replica_spec)

        dist_res = torch.addmm(input1, mat1, mat2)
    ...
    def test_mm(self):
        ...
        t1 = torch.randn(12, 8, requires_grad=True, device=self.device_type)
        t2 = torch.randn(8, 16, requires_grad=True, device=self.device_type)
    ...
    def test_baddbmm(self):
        device_mesh = DeviceMesh(self.device_type, list(range(self.world_size)))
        tensor = torch.rand(4, 4, 8, requires_grad=True)
        batch_1 = torch.rand(4, 4, 8, requires_grad=True)
        ...
        def test_placement_comb(
            ...
            self.assertFalse(torch.isnan(local_result).any())
            self.assertFalse(torch.isnan(dist_local_res).any())
