...
class ScheduleTest(MultiProcContinousTest):
    ...
    def test_schedule_with_native_zero_bubble(self, ScheduleClass):
        ...
        stages = [
            ...
            for stage_module, stage_idx in zip(stage_modules, rank_stages.get(self.rank))
        ]
    ...
    def test_non_symmetric_stage_ids(self, schedule_class, use_new_runtime):
        ...
        stage_index_to_group_rank = {
            value: key
            for key, values in rank_stages.items()
            for value in values
        }
...
if __name__ == "__main__":
    ...
    if not (
        ...
        print(
            "c10d HCCL not available or not enough GPUs, skipping tests",
            file=sys.stderr,
        )
        sys.exit(0)
