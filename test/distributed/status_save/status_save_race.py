...
def main():
    ...
    dist.init_process_group(
        backend='hccl', rank=rank, world_size=2,timeout=datetime.timedelta(seconds=30))
    ...
    sub_groups = [dist.new_group(ranks=[0, 1], backend='hccl',
                                                timeout=datetime.timedelta(seconds=30)) for _ in range(3)]
