...
class OptionsTest(TestCase):
    ...
    def _test_all_reduce_with_options(cls, rank, ranks, world_size, input1):
        ...
        test_case.assertEqual(default_pg.options.hccl_config, hccl_config1, 
                         "Once Options are set for a ProcessGroupHCCL, later changes to Options won't affect that ProcessGroupHCCL.")
    ...
    def _test_options_wrong_type(cls, rank, ranks, world_size, input1):
        options = torch._C._distributed_c10d.ProcessGroup.Options("hccl")
        test_case = TestCase()
        with test_case.assertRaisesRegex(AssertionError, 
                                    "Expected pg_options argument to be of type torch_npu._C._distributed_c10d.ProcessGroupHCCL.Options"):
            OptionsTest._init_dist_hccl(rank, options, world_size)
