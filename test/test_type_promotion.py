...
from torch.testing._internal.common_device_type import (instantiate_device_type_tests, onlyPRIVATEUSE1,
...
class TestTypePromotion(TestCase):
    ...
    def test_complex_half(self, device):
        ...
        # with tensor
        tensor_dtypes = all_types_and_complex_and(torch.chalf, torch.half, torch.bfloat16, torch.bool)
    ...
    def test_alternate_result(self, device):
        ...
        tensor_o = torch.tensor([0, 0, 0, 0], dtype=torch.long, device=device)
        self.assertRaisesRegex(RuntimeError,
                               "can't be cast to",
                               lambda: torch.add(x, x, out=tensor_o))
    ...
    def test_many_promotions(self, device):
        ...
        # supported dtype
        dtypes1 = get_all_math_dtypes('npu')
    ...
    def test_complex_assertraises(self, device):
        ...
        for op in comparison_ops:
            ...
            tensor_dtypes = get_all_dtypes(include_half=is_npu,
                                    include_bfloat16=False, include_bool=False,
                                    include_complex32=True)

            for dt1, dt2 in itertools.product(tensor_dtypes, tensor_dtypes):
                if (dt1.is_complex or dt2.is_complex) and not (op["name"] == "eq" or op["name"] == "ne"):
    ...
    def _run_all_tests_for_sparse_op(self, op_name, device, tensor_dtypes):
        for dtype1, dtype2 in itertools.product(tensor_dtypes, tensor_dtypes):
    ...
    def test_sparse_add(self, device):
        self._run_all_tests_for_sparse_op('add', device,
                                          tensor_dtypes=get_all_math_dtypes(device))
    ...
    def test_sparse_div(self, device):
        self._run_all_tests_for_sparse_op('div', device,
                                          tensor_dtypes=(torch.float32, torch.float64,
    ...
    def test_cat_different_dtypes(self, device):
        tensor_dtypes = all_types_and_complex_and(torch.half, torch.bool)
        for x_dtype, y_dtype in itertools.product(tensor_dtypes, tensor_dtypes):
    ...
    def test_cat_out_different_dtypes(self, device):
        tensor_dtypes = all_types_and_complex_and(torch.half)
        for x_dtype, y_dtype, out_dtype in itertools.product(tensor_dtypes, tensor_dtypes, tensor_dtypes):
    ...
    def test_clamp_type_promotion(self, device, dtypes):
        ...
        min_t = make_tensor_t((S,), dtype1)
        max_t = make_tensor_t((S,), dtype2)
        mins = (min_t, min_t[0], min_t[0].item())
        maxs = (max_t, max_t[0], max_t[0].item())
        inp = make_tensor_t((S,), dtype0)
        for min_v, max_v in itertools.product(mins, maxs):
            ...
            def expected_type(inp, max_t, min_t):
                arg1, arg2 = max_t, min_t
                if isinstance(max_t, torch.Tensor) and max_t.ndim == 0:
                    # first do a maybe dimensional boundary
                    arg1, arg2 = min_t, max_t
