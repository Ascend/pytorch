...
class TestLRScheduler(TestCase):
    ...
    def test_no_cyclic_references(self):
        ...
        optim_sgd = SGD([param], lr=0.5)
        scheduler = LambdaLR(optim_sgd, lambda epoch: 1.0)
        del scheduler

        self.assertTrue(
            len(gc.get_referrers(optim_sgd)) == 0,
    ...
    def test_no_cyclic_references_in_step(self):
        ...
        def run():
            ...
            optim_sgd = SGD(params=[param], lr=0.5)
            scheduler = LambdaLR(optim_sgd, lambda epoch: 1.0)
            param.sum().backward()
            optim_sgd.step()
            scheduler.step()
    ...
    def _test_lr_is_constant_for_constant_epoch(self, scheduler):
        ...
        for _ in range(10):
            ...
            lrs.append(self.opt.param_groups[0]["lr"])
        self.assertEqual(min(lrs), max(lrs))
