...
class TestRngprimsPhiloxRandDefault(TestUtils):
    def op_calc(self, x):
        size = list(x.shape)
        ...
        rand, new_offset = torch.ops.rngprims.philox_rand.default(
            size,           # SymInt[]
            seed,           # Tensor
            offset,         # Tensor
            None,           # stride
...
instantiate_parametrized_tests(TestRngprimsPhiloxRandDefault)
