...
class TestFoldReducePass(TestUtils):
    ...
    def test_sum_all_dims_size_1_should_fold(self, shape, dtype):
        """All reduce dimensions are 1 → should be folded"""
        ...
        self.assertEqual(len(sum_nodes), 0, "All reduce dimensions are 1 → should be folded")
    ...
    def test_sum_partial_dims_not_1_no_fold(self, shape, dtype):
        """Partial dim is not 1 → not folded"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        fold_reduce(gm.graph)
        sum_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.sum.dim_IntList]
        self.assertEqual(len(sum_nodes), 0, "Partial dim is not 1 → not folded")
        self.assertEqual(model(x), gm(x), "Partial dim is not 1 → not folded")


    @parametrize('shape', [(2, 1, 1, 8)])
    def test_sum_with_keepdim_true_should_fold(self, shape):
        """Keepdim=True and all dim=1 → should be folded"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        sum_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.sum.dim_IntList]
        self.assertEqual(len(sum_nodes), 0, "Keepdim=True and all dim=1 → should be folded")


    @parametrize('shape', [(3, 1)])
    def test_sum_scalar_dim_should_fold(self, shape):
        """Dim is scalar (int) instead of list → Handle correctly"""
        class M(torch.nn.Module):
            def forward(self, x):
                reduced = torch.ops.aten.sum.dim_IntList(x, 1, keepdim=False)
                return reduced
        ...
        fold_reduce(gm.graph)
        sum_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.sum.dim_IntList]
        self.assertEqual(len(sum_nodes), 0, "Dim is scalar (int) instead of list → Handle correctly")


    @parametrize('shape', [(2, 1, 1)])
    def test_multi_sum_chain_should_all_fold(self, shape):
        """Multi layer continuous sum → should be fully folded"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        sum_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.sum.dim_IntList]
        self.assertEqual(len(sum_nodes), 0, "Multi layer continuous sum → should be fully folded")


    @parametrize('shape', [(1, 64)])
    def test_sum_with_multiple_downstream_users(self, shape):
        """After the sum is folded, multiple downstream users correctly point to the original input"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        sum_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.sum.dim_IntList]
        self.assertEqual(len(sum_nodes), 0, "After the sum is folded, multiple downstream users correctly point to the original input")

        self.assertEqual(model(x), gm(x), "After the sum is folded, multiple downstream users correctly point to the original input")


    def test_no_sum_node_no_change(self):
        """No sum node → pass without modifying the graph"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        fold_reduce(gm.graph)
        self.assertEqual(orig_graph, str(gm.graph), "No sum node → pass without modifying the graph")


    @parametrize('shape', [(4, 4)])
    def test_sum_without_shape_info_no_fold(self, shape):
        """Enter no shape → Do not fold"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        sum_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.sum.dim_IntList]
        self.assertEqual(len(sum_nodes), 1, "Enter no shape → Do not fold")
