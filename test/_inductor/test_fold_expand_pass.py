...
class TestFoldExpandPass(TestUtils):
    ...
    def test_expand_same_shape_should_fold(self, shape, dtype):
        """ The target shape is identical to the input → should be folded and deleted """
        ...
        self.assertEqual(len(expand_nodes), 0, "The target shape is identical to the input → should be folded and deleted")
    ...
    def test_expand_with_minus_one_should_fold(self, shape, dtype):
        """The target shape contains -1 (broadcast dimension), which is equivalent to folding"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        expand_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.expand.default]
        self.assertEqual(len(expand_nodes), 0, "The target shape contains -1 (broadcast dimension), which is equivalent to folding")


    @parametrize('shape', [(2, 3, 1)])
    @parametrize('dtype', ['float32'])
    def test_expand_different_shape_no_fold(self, shape, dtype):
        """The target shape is not equivalent to the input → no folding"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        expand_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.expand.default]
        self.assertEqual(len(expand_nodes), 1, "The target shape is not equivalent to the input → no folding")


    @parametrize('shape', [(1, 8)])
    def test_multi_expand_chain_should_all_fold(self, shape):
        """Multi layer continuous expansion → should be fully folded"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        expand_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.expand.default]
        self.assertEqual(len(expand_nodes), 0, "Multi layer continuous expansion → should be fully folded")


    @parametrize('shape', [(4, 4)])
    def test_expand_with_multiple_downstream_users(self, shape):
        """After the expand is folded, multiple downstream users correctly point to the original input"""
        class M(torch.nn.Module):
            def forward(self, x):
    ...
    def test_no_expand_node_no_change(self):
        """no expand node → pass dosenot change graph"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        fold_expand(gm.graph)
        self.assertEqual(orig_graph, str(gm.graph), "do not modify graph due to no expand node")


    @parametrize('shape', [(2, 3)])
    def test_expand_without_shape_info_no_fold(self, shape):
        """no shape info → no fold"""
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        expand_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.expand.default]
        self.assertEqual(len(expand_nodes), 1, "shouldn't be folded due to lack of shape info")


    @parametrize('shape', [(1, 5)])
    def test_expand_non_list_shape_no_fold(self, shape):
        """shape not list → no fold"""
        class M(torch.nn.Module):
            def forward(self, x):
                # input not list (like tuple)
        ...
        expand_nodes = [n for n in gm.graph.nodes if n.target == torch.ops.aten.expand.default]
        self.assertEqual(len(expand_nodes), 1, "shouldn't be folded due to shape type is not list")
