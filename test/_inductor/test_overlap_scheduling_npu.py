...
class TestComputeNodeClassification(TestCase):
    ...
    def test_registered_compute_ops_and_upstream_fallback(self):
        packets = {
            ...
            "npu_fused_matmul": object(),
            "npu_fusion_attention_v3": object(),
            "npu_fusion_attention_grad_v3": object(),
            "npu_fusion_attention_v3_unsafe": object(),
            "npu_fusion_attention_grad_v3_unsafe": object(),
        }
...
class TestEstimatorAndScheduleGuards(TestCase):
    ...
    def test_gather_estimator_uses_pytorch_2_13_dict_result(self):
        first = object()
        second = object()
        upstream = mock.Mock(return_value={first: 1.0})

        def gather(gm, enable_fusion_regions=False):
            return upstream(gm, enable_fusion_regions=enable_fusion_regions)

        wrapped = npu_overlap._build_npu_gather_node_runtime_estimations(gather)
        gm = object()
        region = object()
        with (
            mock.patch.object(
                ...
                return_value={first: region},
            ) as build_regions,
            mock.patch.object(
                dvm_fusion_regions,
                "estimate_dvm_fused_node_costs",
                return_value={second: 2.0},
            ) as estimate_costs,
        ):
            result = wrapped(gm, enable_fusion_regions=True)

        self.assertEqual(result, {first: 1.0, second: 2.0})
        upstream.assert_called_once_with(gm, enable_fusion_regions=False)
