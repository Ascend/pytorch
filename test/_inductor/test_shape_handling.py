...
shape_options = {
    ...
    "shape_handling_configs": [
        ...
        },
        {
            ...
            # 该维度在tensor中的下标，与indices一一对应；单个dimension表示所有tensor使用相同下标
            "dimensions": [1, 1],
            # "indices": [0, 1],         # 需要处理的tensor下标(默认为所有tensor)(Optional)
            "value": 0.0,              # padding时填充的值, 默认为0.0(Optional)
            "gears": [],               # 自定义档位信息(Optional)
...
class TestInductorWrapperShapeHandlingOptions(TestCase):
    def test_shape_handling_options_are_registered(self):
        config = torch._inductor.config.get_config_copy()

        self.assertFalse(config["enable_shape_handling"])
        self.assertEqual(config["shape_handling_configs"], [])
        self.assertIsNone(config["shape_handling_dict"])

    def test_apply_options_patches_shape_handling(self):
        with mock.patch.object(torch_npu._inductor, "patch_shape_handling") as patch:
        ...
        self.assertTrue(wrapper.config["enable_shape_handling"])
        self.assertEqual(
            wrapper.config["shape_handling_configs"],
            shape_options["shape_handling_configs"],
        )

    def test_apply_options_does_not_patch_when_disabled(self):
        with mock.patch.object(torch_npu._inductor, "patch_shape_handling") as patch:
            torch._TorchCompileInductorWrapper(
                mode=None,
                options={"enable_shape_handling": False},
                dynamic=False,
            )
...
instantiate_parametrized_tests(TestInductorWrapperShapeHandlingOptions)
