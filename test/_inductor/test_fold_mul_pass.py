...
class TestFoldMulPass(TestUtils):
    ...
    def test_mul_by_one_int_fold(self, shape):
        """ X * 1 (int literal) should be folded """
        class M(torch.nn.Module):
            def forward(self, x):
                y = torch.mul(x, 1)
                return y.mean()
    ...
    def test_mul_one_chain_fold(self, shape):
        """ Chain * 1.0/* 1 should be fully folded """
        class M(torch.nn.Module):
            def forward(self, x):
                a = torch.mul(x, 1.0)
                b = torch.mul(a, 1)
        ...
        self.assertEqual(len(mul_nodes), 0, "All * 1 should be folded")


    @parametrize('shape', [(6, 6)])
    def test_mul_non_one_no_fold(self, shape):
        """ Multiplying by a non-zero value should not be folded """
        class M(torch.nn.Module):
            def forward(self, x):
                y = torch.mul(x, 2.0)
        ...
        self.assertEqual(before, after, "Multiplying by a non-zero value should not be folded")


    @parametrize('shape', [(1, 64)])
    def test_mul_one_multi_users(self, shape):
        """ After mul * 1, there are multiple downstream users who should be replaced correctly """
        class M(torch.nn.Module):
            def forward(self, x):
                y = torch.mul(x, 1.0)
        ...
        self.assertEqual(m(t), gm(t), "After mul * 1, there are multiple downstream users who should be replaced correctly")


    def test_no_mul_op_no_change(self):
        """ There is no mul in the picture, pass should not modify the picture """
        class M(torch.nn.Module):
            def forward(self, x):
        ...
        fold_four_op_pass(gm.graph)
        self.assertEqual(orig_str, str(gm.graph), "There is no mul in the picture, pass should not modify the picture")
