    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt >= MAX_RETRIES:
                break
            logger.info("%s failed (attempt %d/%d): %s", label, attempt, MAX_RETRIES, exc)
...
def log_release_info() -> None:
    try:
        ...
        logger.warning("Failed to read release_info.yaml: %s", exc)
...
def install_akg_wheels(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        if args.download_only:
            raise
        logger.warning("AKG wheel installation failed (%s); continuing with MLIR fallback tests", exc)
        return False
    finally:
