...
def test_opensora_cases_model_9_inference():
    ...
    torch.testing.assert_close(ref, calc, equal_nan=True, rtol=1e-4, atol=1e-4)
...
def test_opensora_cases_model_11_inference():
    ...
    for i in range(len(ref)):
        print(ref[i])
        torch.testing.assert_close(ref[i], calc[i], equal_nan=True, rtol=1e-4, atol=1e-4)
...
def test_opensora_cases_model_16_forward():
    ...
    for i in range(len(ref)):
        print("i=", i)
        torch.testing.assert_close(ref[i], calc[i], equal_nan=True, rtol=1e-3, atol=1e-3)

if __name__ == '__main__':
    test_opensora_cases_model_9_inference()
    test_opensora_cases_model_14_forward()
