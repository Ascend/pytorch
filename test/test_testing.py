    ...
    def test_get_supported_dtypes(self, device):
        ...
        for op in ops_to_test:
            ...
            if self.device_type == 'cpu':
                ...
                dtypes_ = op.dtypesIfCUDA

            self.assertTrue(set(dtypes_) == set(dynamic_dtypes))
            self.assertTrue(set(dtypes_) == set(dynamic_dispatch.dispatch_fn()))
...
class TestMakeTensor(TestCase):
    ...
    def test_requires_grad(self, dtype, device, requires_grad):
        make_tensor_partial = functools.partial(
    ...
    def test_low_high_outside_valid_range(self, dtype, device):
        make_tensor_partial = functools.partial(torch.testing.make_tensor, dtype=dtype, device=device)
        ...
        with self.assertRaisesRegex(ValueError, ""):
            ...
            make_tensor_partial(low=low, high=high)

        with self.assertRaisesRegex(ValueError, ""):
            make_tensor_partial(low=highest_inclusive * 2, high=highest_inclusive * 4)
...
class TestTestParametrizationDeviceType(TestCase):
    def test_unparametrized_names(self, device):
        ...
        instantiate_device_type_tests(TestParametrized, locals())
...
class TestImports(TestCase):
    ...
    def test_circular_dependencies(self) -> None:
        ...
        Prevents regression reported in pytorch issues 77441 """
    ...
    def test_not_import_sympy(self) -> None:
        ...
        self.assertEqual(out.strip(), "True",
                         ...
                         "delving-into-what-happens-when-you-import-torch 1589\n\n"
