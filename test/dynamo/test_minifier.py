...
"""
    ...
    def test_after_dynamo_npu_compile_error(self):
        self._test_after_dynamo(
            "npu:0", "relu_compile_error_TESTING_ONLY", "ReluCompileError"
        )

    @requires_npu()
    def test_after_dynamo_npu_runtime_error(self):
        self._test_after_dynamo(
            "npu:0", "relu_runtime_error_TESTING_ONLY", "ReluRuntimeError"
        )

    @requires_npu()
    def test_after_dynamo_npu_accuracy_error(self):
        self._test_after_dynamo(
            "npu:0", "relu_accuracy_error_TESTING_ONLY", "AccuracyError"
        )
...
"""
    ...
    def test_after_dynamo_npu_compile_backend_passes(self):
        self._test_after_dynamo_backend_passes(
            "npu:0", "relu_compile_error_TESTING_ONLY"
        )

    @requires_npu()
    def test_after_dynamo_npu_runtime_backend_passes(self):
        self._test_after_dynamo_backend_passes(
            "npu:0", "relu_runtime_error_TESTING_ONLY"
        )

    @requires_npu()
    def test_after_dynamo_npu_accuracy_backend_passes(self):
        self._test_after_dynamo_backend_passes(
            "npu:0", "relu_accuracy_error_TESTING_ONLY"
        )
...
class CpuNpuModule(torch.nn.Module):
    def __init__(self):
        ...
        self.p_x = torch.nn.Parameter(torch.randn(20, 20).npu())
        self.p_y = torch.nn.Parameter(torch.randn(20, 20))
        self.register_buffer("b_x", torch.ones(20, 20).npu())
...
def inner(x1, y1):
    x2 = torch.randn(20, 20).npu()
    y2 = torch.randn(20, 20)
    x3, y3 = mod(x1 + x2, y1 + y2)
    return torch.relu(x3.cpu() + y3)

inner(torch.randn(20, 20).npu(), torch.randn(20, 20))
"""
...
class Repro(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.G__mod___m_x = Linear(in_features=20, out_features=20, bias=True).npu()
