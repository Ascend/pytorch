...
def npubackend_schedule_fn(a):
    # official func is inductor_schedule_fn
    output = a.add(torch.ones(1000, 1000, device="npu:0"))
    return output
...
class LoggingTests(LoggingTestCase):
    ...
    def test_schedule(self, records):
        fn_opt = torch._dynamo.optimize(npu_backend)(npubackend_schedule_fn)
        fn_opt(torch.ones(1000, 1000, device="npu:0"))
...
LoweringException: AssertionError:
  ...
  args[0]: TensorBox(StorageBox(
    ...
    def test_ddp_graphs(self, records):
        ...
        ddp_model = torch._dynamo.optimize(npu_backend)(
            DDP(ToyModel().to("npu:0"), device_ids=[0], bucket_cap_mb=4)
        )

        ddp_model(torch.randn(1024, 1024, device="npu:0"))
    ...
    def test_multiline_format(self, records):
        ...
        for record in records:
            ...
            for l_str in r.splitlines():
                self.assertIn("[INFO]", l_str)
