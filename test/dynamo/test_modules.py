...
class ModuleDict(torch.nn.Module):
    ...
    def forward(self, x):
        # do for later(future PR): handle more logic
        x = self.layers["0"](x)
        return x
...
class LazyLayerWithNamedTupleInput(LazyModuleMixin, torch.nn.Module):
    ...
    def initialize_parameters(self, input0):
        with torch.no_grad():
            self._param = torch.nn.Parameter(
                torch.empty(input0.x["a"][0].shape).fill_(0.5)
            )
...
class LazyLayerWithListInput(LazyModuleMixin, torch.nn.Module):
    ...
    def initialize_parameters(self, input0):
        with torch.no_grad():
            self._param = torch.nn.Parameter(torch.empty(input0[0].shape).fill_(0.5))
...
class ModuleGuardNameIsValid(torch.nn.ModuleDict):
    ...
    def __init__(self):
        super().__init__()
        for i in range(2):
            self.add_module("l_@yer-%d" % (i + 1), BasicModule())
...
class NNModuleTests(torch._dynamo.test_case.TestCase):
        ...
        def run():
            ...
            class TensorProxy(torch.Tensor):
                ...
                def __torch_function__(cls, func, types, args=(), kwargs=None):
                    ...
                    # do for later(future PR): support writes as well
                    counter + 1
                    return super().__torch_function__(func, types, args, kwargs)
    ...
    def test_lazy_module1(self):
        ...
        def test_torch_static():
            input0 = torch.ones(*input_shape)
            return module(input0)  # fully materialized
    ...
    def test_lazy_module3(self):
        ...
        ref = m(x)
        self.assertTrue(torch.allclose(ref, res))
        # move to npu and second iteration
        m = m.to("npu:0")
        x = x.to("npu:0")
        res = opt_m(x)
        ref = m(x)
...
class OptimizedModuleTest(torch._dynamo.test_case.TestCase):
    ...
    def test_no_guard_on_torch_nn_modules(self):
        # See pytorch/pytorch/issues/110048
