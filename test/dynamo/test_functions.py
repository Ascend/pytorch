...
class FunctionTests(torch._dynamo.test_case.TestCase):
    ...
    def test_callable_class(self):
        ...
        for f in [fn1, fn2]:
            self.assertEqual(f(input0, NotCallableClass()), input0 + 1)
            self.assertEqual(
                f(input0, CallableClass()), input0 if f is fn1 else input0 * 2
            )

            # passing tensor and scalars
            self.assertEqual(f(input0, 1), input0 + 1)
            self.assertEqual(f(input0, 1.1), input0 + 1)
            self.assertEqual(f(input0, True), input0 + 1)
            self.assertEqual(f(input0, input0), input0 + 1)
    ...
    def test_cublas_allow_tf32(x):
        if torch.backends.npu.matmul.allow_tf32:
            return x.sin() + 1
    ...
    def test_get_device_properties_tensor_device(a):
        x = a.to("npu:0")
        prop = torch.npu.get_device_properties(x.device)
        if prop.major == 8:
