...
class ActivationCheckpointingViaTagsTests(torch._dynamo.test_case.TestCase):
    ...
    def _compare_orig_and_checkpointed_fns(
        ...
        compiled_orig_fn = torch.compile(
            orig_fn, fullgraph=fullgraph, backend="npu"
        )
        ...
        compiled_checkpointed_fn = torch.compile(
            checkpointed_fn, fullgraph=fullgraph, backend="npu"
        )
    ...
    def test_tags_module(self):
        ...
        x = torch.randn(10, 10, device="npu:0", requires_grad=True)
    ...
    def test_compile_selective_checkpoint_gemm_only(self):
        ...
        x = torch.randn(4, 4, requires_grad=True, device="npu:0")
        y = torch.randn(4, 4, requires_grad=True, device="npu:0")
    ...
    def test_compile_selective_checkpoint_tensor_subclass(self):
        ...
        rand_tensor = torch.randn(4, 4, requires_grad=True, device="npu:0")
    ...
    def test_autocast_flash_attention(self):
        ...
        with torch.npu.amp.autocast():
            x = torch.randn(4, 2, 16, 32, device="npu:0", requires_grad=True)
            y = torch.randn(4, 2, 16, 32, device="npu:0", requires_grad=True)
            z = torch.randn(4, 2, 16, 32, device="npu:0", requires_grad=True)
            args = (x, y, z)
    ...
    def test_pattern_matcher(self):
        ...
        dtype = torch.float16
        args1 = [
            torch.randn(tensor_shape, device="npu:0", dtype=dtype, requires_grad=True),
            torch.randn(tensor_shape, device="npu:0", dtype=dtype, requires_grad=True),
            torch.randn(tensor_shape, device="npu:0", dtype=dtype, requires_grad=True),
        ]
