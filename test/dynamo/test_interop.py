...
class InteropTests(torch._dynamo.test_case.TestCase):
    ...
    def test_vmap_in_graph(self):
        ...
        fn_opt = torch.compile(fn1, backend=cnts, fullgraph=True)
        fn_opt_traceable = torch.compile(traceable(fn1), backend=cnts, fullgraph=True)

        self.assertEqual(fn1(x), fn_opt(x))
