...
class TestSDPA(torch._dynamo.test_case.TestCase):
    ...
    def test_returns_SDPAParams(self):
        with allow_in_graph_sdpa_params():
            ...
            m = torch.randn(10)
            output = fn(q, k, v, m)
            self.assertTrue(isinstance(output, SDPAParams))
            self.assert_ref_equals_params(output, SDPAParams(q, k, v, m, 0.1, True))
    ...
    def test_intermediate_attr_access_SDPAParams(self):
        with allow_in_graph_sdpa_params():
            ...
            v = torch.randn(10)
            m = torch.randn(10)
            _, output, _ = fn(q, k, v, m)
            expected = SDPAParams(q, k, v, m, 0.1, True)
            self.assert_ref_equals_params(output, expected)
            self.assertEqual(counter.frame_count, 1)
