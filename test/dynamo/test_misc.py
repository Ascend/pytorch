...
class MiscTests(torch._dynamo.test_case.TestCase):
    ...
    def test_matmul1(self):
        ...
        # do for later(jansel): FX doesn't support this, should add upstream support
    ...
    def test_list_mul(self):
        ...
        # Do for later: the captured frame here is a bit goofy, because we don't
    ...
    def test_setattr_mutation3(self):
        # do for later(jansel): dead code eliminate the object creation
        class MyObj:
            def __init__(self, x):
                super().__init__()
                self.a = x + 1
...
{str(chr(10)).join(' ' * 4 + 'x' + str(i) + ' = 1' for i in range(1 << 9))}
    ...
    def test_tensor_is_contiguous(self):
        def fn(x):
            input1 = torch.randn((1, 16, 1, 1))
            weight = torch.randn((8, 16, 3, 3))
            weight = weight.to(memory_format=x)
            output = torch.conv2d(input1, weight, None, (2, 1), (1, 1), (1, 1), 1)
    ...
    def test_python_slice(self):
        def f1(input1):
            y = 0
            for i, x in enumerate(input1[2:], 1):
                y = y + x
            return y

        def f2(input2):
            y = 0
            for i, x in enumerate(input2.shape[2:], 1):
                y = y + x
            return y
    ...
    @unittest.skip("see the issue: 99726")
    def test_cross_entropy_loss_fancy_ctor1(self):
        ...
        input1 = rand_3_5
        dynamo_output = opt_loss(input1, target)
    ...
    @pytest.mark.xfail(reason="see the issue: 106207")
    ...
    def test_error_on_nested_fx_trace(self):
        ...
        self.assertTrue(same(optimized(input1), real))
    ...
    def test_no_error_on_nested_fx_trace(self):
        ...
        # should not error
        gm = torch.fx.symbolic_trace(optimized)
        self.assertTrue(same(gm(input1), real))
    ...
    def test_tagging_tensors_mix_used_unused_structure(self):
        def pre_attention_state_ops(input1, mems, state):
            lc_key = state[0]
            lc_val = state[1]
            bar = []
            for i in range(0, 4):
...
""",
    ...
    def test_compile_profiler(self):
        ...
        with CompileProfiler() as prof:
            ...
            base_checker().check("Recompile Reasons").check("'forward'").check(
                "tensor 'L['input1']' size mismatch at index 0. expected 2, actual 3"
            ).check(
                "tensor 'L['input1']' size mismatch at index 0. expected 3, actual 4"
            ).run(
                prof.report()
            )
