...
class XSoftmax(torch.autograd.Function):
    ...
    @staticmethod
    def forward(self, input0, mask, dim):
        self.dim = dim
        rmask = ~(mask.to(torch.bool))
        output = input0.masked_fill(rmask, torch.tensor(torch.finfo(input0.dtype).min))
...
class ReproTests(torch._dynamo.test_case.TestCase):
    ...
    def _reformer(self, nopython):
        input0 = torch.randn([1, 64, 256])
        model = ReformerEncoder()
        torch.manual_seed(1337)
        correct = copy.deepcopy(model)(input0)
        cnt = torch._dynamo.testing.CompileCounter()
        torch.manual_seed(1337)
        opt_model = torch._dynamo.optimize(cnt, nopython=nopython)(model)
        self.assertTrue(same(opt_model(input0), correct))
        return cnt
    ...
    def test_sub_alpha_scalar_repro(self):
        ...
        f(torch.ones(2, device="npu:0", dtype=torch.float64))

    # See pytorch/pytorch/issues/113010
    ...
    # See pytorch/pytorch/issues/109053
    ...
    # See pytorch/pytorch/issues/90552
    ...
    # See pytorch/pytorch/issues/97745
    ...
    # See pytorch/pytorch/issues/111603
    ...
    def test_hf_t5_forward(self):
        input0 = torch.randn([1, 2048, 512])
        model = PartialT5()
        correct = model(input0)
        cnt = torch._dynamo.testing.CompileCounter()
    ...
    # see: See pytorch/pytorch/issues/80067
    ...
    def test_maml_item_capture(self):
        ...
        # else:
        #     self.assertExpectedInline(cnt.frame_count, """3""")
        # do for later(jansel): figure out why op count depends on imports
    ...
    def test_rng_state(self):
        ...
        try:
            ...
            # See pytorch/pytorch/pull/87490
    ...
    # See pytorch/pytorch/issues/100067
    ...
    def test_recursive_map(self):
        # See pytorch/torchdynamo/issues/132
    ...
    # See pytorch/torchdynamo/issues/1446
    ...
    def test_abc_setattr(self):
        ...
        # do for later: does not ensure ABC classes are correctly inferred as ClassVariables
    ...
    def test_out_none(self):
        # See pytorch/pytorch/issues/92814
        def fn(input0):
            return torch.nn.functional.normalize(input0, dim=0, out=None)
    ...
    def test_optimized_deepcopy(self):
        # See pytorch/pytorch/pull/88629
    ...
    def test_tensor_data_kwarg(self):
        # See pytorch/pytorch/issues/96278
        def f():
    ...
    def test_norm_dtype(self):
        ...
        args = [((2, 50, 256), (1, 256, 1), torch.float16, "npu:0", False)]
        args = [
            rand_strided(sh, st, dt, dev).requires_grad_(rg)
            for (sh, st, dt, dev, rg) in args
        ]
    ...
    def test_exception_in_dynamo_handling(self):
        ...
        # See pytorch/pytorch/pull/96488
    ...
    def test_generator_dealloc(self):
        ...
        # do for later: This test is pretty delicate.  To test if it's actually doing
    ...
    def test_error_return_without_exception_set(self):
        # See pytorch/pytorch/issues/93781
        @torch.compile
        def f():
    ...
    def common_merge_criteria_processor_list(self, list_cls, fullgraph):
        ...
        l2 = list_cls([])
        input0 = torch.randn(16)
        result = f(input0, l1, l2)
        self.assertEqual(result, l1(input0))
        self.assertEqual(cnt.frame_count, 1)
        self.assertEqual(cnt.op_count, 2)

        cnt.clear()
        l3 = list_cls([torch.nn.SiLU()])
        expected = l3(l1(input0))
        result = f(input0, l1, l3)
    ...
    def test_restricted_list_subclass3(self):
        ...
        y = torch.randn(10)
        l_ = CustomList2([x, y])
        self.assertIs(fn(l_, l_), l_)
        self.assertEqual(len(l_), 7)
        self.assertIs(l_[0], x)
        self.assertIs(l_[1], y)
        self.assertIs(l_[2], x)
        self.assertIs(l_[3], y)
        self.assertEqual(l_[4], x + 1)
        self.assertIs(l_[5], l_[4])
        self.assertEqual(l_[6], y + 2)
    ...
    def test_list_aliasing(self):
        ...
        x = torch.randn(10)
        l_ = [x]
        self.assertIs(fn(l_), l_)
        self.assertEqual(len(l_), 2)
        self.assertIs(l_[0], x)
        self.assertEqual(l_[1], torch.sin(x))
        self.assertEqual(cnt.frame_count, 1)
        self.assertEqual(cnt.op_count, 1)
    ...
    def test_ephemeral_module(self):
        ...
        class ReLUSquaredActivation(nn.Module):
            def forward(self, input0):
                relu_applied = torch.nn.functional.relu(input0)
    ...
    def test_maybe_multiply_symint(self):
        # See pytorch/pytorch/issues/97346
    ...
    def test_hf_xsoftmax_inference(self):
        def fn(input0, mask):
            return XSoftmax.apply(input0 + 1, mask, 1) + 2
    ...
    def test_hf_xsoftmax_training(self):
        ...
        def fn(input0, mask):
            return XSoftmax.apply(input0, mask, 1)
    ...
    def test_guard_default_device(self):
        try:
            ...
            self.assertEqual(f().device.type, "npu")
            self.assertEqual(counter.frame_count, 1)
    ...
    def test_list_self_reference(self):
        # Issue - See pytorch/pytorch/issues/100150
        root = []
    ...
    # Test for See pytorch/pytorch/issues/103132
    @torch._dynamo.config.patch("assume_static_by_default", False)
    ...
    def test_call_finally_python_3_8(self):
        # Issue - See pytorch/pytorch/issues/97811
        def make_fn(g):
            def fn():
                while True:
                    try:
    ...
    def test_dynamic_shapes_double_not_equal(self):
        # See pytorch/pytorch/issues/113393
        def fn(x):
            if x.size() != (5, 1, 2, 3):
                return x.cos()
            return x.sin()
    ...
    def test_user_defined_object_callable(self):
        # See pytorch/pytorch/issues/114019
        class MyCallable:
            def __call__(self, x):
                return x + 1
    ...
    # See pytorch/pytorch/issues/118799
    ...
    def test_tensor_set_data(self):
        # See pytorch/pytorch/issues/113030
        def func1(x, y):
            x.data = y
            x.add_(1)
            return x
