...
class XSoftmax(torch.autograd.Function):
    ...
    @staticmethod
    def forward(self, input1, mask, dim):
        self.dim = dim
        rmask = ~(mask.to(torch.bool))
        output = input1.masked_fill(rmask, torch.tensor(torch.finfo(input1.dtype).min))
...
class ReproTests(torch._dynamo.test_case.TestCase):
    ...
    def _reformer(self, nopython):
        input1 = torch.randn([1, 64, 256])
        model = ReformerEncoder()
        torch.manual_seed(1337)
        correct = copy.deepcopy(model)(input1)
        cnt = torch._dynamo.testing.CompileCounter()
        torch.manual_seed(1337)
        opt_model = torch._dynamo.optimize(cnt, nopython=nopython)(model)
        self.assertTrue(same(opt_model(input1), correct))
        return cnt
    ...
    def test_hf_t5_forward(self):
        input1 = torch.randn([1, 2048, 512])
        model = PartialT5()
        correct = model(input1)
        cnt = torch._dynamo.testing.CompileCounter()
    ...
    # see pytorch issue 103620
    @expectedFailureDynamic
    def test_chunk_reformer_ff(self):
        input1 = torch.randn([1, 4096, 256])
    ...
    def test_out_none(self):
        # see pytorch/issues/92814
        def fn(input1):
            return torch.nn.functional.normalize(input1, dim=0, out=None)
    ...
    def test_reformer_remove_unused_args(self):
        ...
        # torchdynamo/issues/1925).  So long
    ...
    def test_ephemeral_module(self):
        ...
        class ReLUSquaredActivation(nn.Module):
            def forward(self, input1):
                relu_applied = torch.nn.functional.relu(input1)
    ...
    def test_hf_xsoftmax_inference(self):
        def fn(input1, mask):
            return XSoftmax.apply(input1 + 1, mask, 1) + 2
    ...
    def test_hf_xsoftmax_training(self):
        ...
        def fn(input1, mask):
            return XSoftmax.apply(input1, mask, 1)
