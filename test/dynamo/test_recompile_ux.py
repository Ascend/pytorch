...
class RecompileUxTests(torch._dynamo.test_case.TestCase):
    ...
    def test_nvfuser_guards(self):
        ...
        a = torch.rand(3, 4, 5, device="npu:0")
        b = torch.rand(3, 4, 5, device="npu:0")
        b_v = torch.rand(3, 5, 4, device="npu:0").view(3, 4, 5)
        b_p = torch.rand(3, 5, 4, device="npu:0").permute(0, 2, 1)
        c = torch.rand(3, 4, 5, device="npu:0")
        compile_counter = torch._dynamo.testing.CompileCounter()
