...
class RecompileUxTests(torch._dynamo.test_case.TestCase):
    # for later (whc) dynamo actually recompiles one more time than the cache limit
    cache_limit = 1
    ...
    def test_loop_torture(self):
        ...
        # for later (whc) these checks fail on py37.  Why?
    ...
    def test_verbose_tensor_check(self):
        ...
        def cache_fail_test(cached_input, missed_input, expected_failure):
            # for later (whc) maybe its hacky to have a 'test within a test' but this seemed convenient
            torch._dynamo.reset()
