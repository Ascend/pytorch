...
class TorchCompileTriggerTests(unittest.TestCase):
    ...
    def test_shape_handling_initializes_after_backend_selection(self):
        self.run_in_subprocess(
            ...
            def fake_setup(actual_options):
                actual_options = dict(actual_options)
                events.append(("setup", actual_options))
    ...
    def test_dynamo_patch_inventory_is_preserved(self):
        self.run_in_subprocess(
            ...
            # PyTorch 2.10 handles torch.Event (and its npu subclass) natively in
            # VariableBuilder._wrap, so torch_npu no longer patches it.
            assert VariableBuilder._wrap.__module__ == "torch._dynamo.variables.builder"
