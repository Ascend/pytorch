...
def _triton_compile_works():
    """Check whether triton code generation succeeds in the current environment.

    The CI ACL headers may reference symbols (e.g. aclmdlRICondHandle) that are
    absent from the installed ACL version, causing every inductor/triton
    compilation to fail.  Tests that exercise the real inductor backend should
    be skipped when this pre-existing environment issue is present.
    """
    ...
    probe = textwrap.dedent(
        ...
        assert bool(torch.allclose(actual, fn(x)))
        """
    )
...
_TRITON_COMPILE_OK = _triton_compile_works()
...
class TorchCompileTriggerTests(unittest.TestCase):
    ...
    @unittest.skipUnless(_TRITON_COMPILE_OK, "inductor/triton compile unavailable (ACL header issue)")
    ...
    def test_fsdp_patch_when_imported_before_torch_npu(self):
        self.run_in_subprocess(
            ...
            from torch_npu.distributed.fsdp._add_fsdp_patch import (
                _patched_fsdp_param_group_init,
            )
            from torch_npu.npu.amp.sharded_grad_scaler import _ShardedGradScaler

            assert callable(torch_npu.distributed.fsdp.fully_shard)
    ...
    def test_inductor_patch_inventory_is_preserved(self):
        self.run_in_subprocess(
            ...
            # RNG/decomposition patches are installed when the deferred
            # compiler setup is first triggered, not while importing torch_npu.
