...
class AotAutogradFallbackTests(torch._dynamo.test_case.TestCase):
    ...
    def test_arg_dupe_via_dynamo_recompiles_many_args_param_non_tensor_arg(self):
        class F1(torch.nn.Module):
            ...
            def forward(self, a, b, c, d, *args):
                ...
                c.trunc_()
                d.trunc_()
                e, f = args if len(args) >= 2 else (1, 1)
        ...
        f = torch._dynamo.optimize(cc, guard_fail_fn=guard_fail_fn)(F1())
        f(a1, a1, a1, a1, 2, 2)
        f(a2, b2, b2, b2, 2, 2)
        self.assertEqual(cc.frame_count, 2)
    ...
    def test_arg_dupe_via_dynamo_recompiles_many_with_global(self):
        ...
        f = torch._dynamo.optimize(cc, guard_fail_fn=guard_fail_fn)(F2())
        f(a1, a1, a1, a1, 2, 2)
        f(a2, b2, b2, b2, 2, 2)
        self.assertEqual(cc.frame_count, 2)
        self.assertExpectedInline(failure_reason, """L['a'] is L['b']""")
    ...
    def test_arg_dupe_via_dynamo_recompiles_many_args_param_non_tensor_arg_list(self):
        class F3(torch.nn.Module):
            ...
            def forward(self, a, b, c, d, *args):
                ...
                c.trunc_()
                d.trunc_()
                e, f = args if len(args) >= 2 else ([1, 1, 1], [1, 1, 1])
        ...
        f = torch._dynamo.optimize(cc, guard_fail_fn=guard_fail_fn)(F3())
        f(a1, a1, a1, a1, [3, 2, 1], [4, 5, 6])
        f(a2, b2, b2, b2, [3, 2, 1], [4, 5, 6])
        self.assertEqual(cc.frame_count, 2)
        self.assertExpectedInline(failure_reason, """L['a'] is L['b']""")
        ...
        f = torch._dynamo.optimize(cc, guard_fail_fn=guard_fail_fn)(F3())
        f(a3, b3, c3, c3, [3, 2, 1], [4, 5, 6])
        f(a4, b4, c4, d4, [3, 2, 1], [4, 5, 6])
        self.assertEqual(cc.frame_count, 2)
    ...
    @expectedFailureDynamic  # see torch issues 103539
