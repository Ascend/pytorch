...
class OptimizerTests(torch._dynamo.test_case.TestCase):
    ...
    # calling the closure
    # mlazos: future work (re-enable once we have latest pytorch with FakeTensor fix #497)
    # test_lbfgs = make_test(
    #    torch.optim.LBFGS, exp_frame_cnt=3, closure=lambda: model(ipt).sum()
    # )
...
class End2EndTests(torch._dynamo.test_case.TestCase):
    ...
    def test_state_dict(self):
        ...
        def _test_state_dict(weight, bias, ipt=torch.ones([10, 10])):
