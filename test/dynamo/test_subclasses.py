...
class SubclassTests(torch._dynamo.test_case.TestCase):
    ...
    def test_no_call_to_new(self):
        ...
        with torch._dynamo.config.patch(
            ...
            input0 = torch.ones(2, 2).as_subclass(BadNewTorchFunction)
    ...
    def test_base_torch_function_tracing(self):
        ...
        input0 = torch.ones(2, 2).as_subclass(BaseTorchFunction)
        out = fn(input0)
        out_opt = compile_full_eager(fn)(input0)
    ...
    def test_type_check(self, comparison, input_type):
        with torch._dynamo.config.patch("traceable_tensor_subclasses", {DummyNDim}):
            ...
            input0 = torch.ones(2, 2).as_subclass(input_type)
            exp_res = fn(input0)
            act_res = torch.compile(backend="eager", fullgraph=True)(fn)(input0)
    ...
    def test_wrapper_subclass_guards_on_inner_tensor(self):
        ...
        class DoubleSizeMaybeAddGeThreeTensor(torch.Tensor):
            ...
            def __new__(cls, inner):
                ...
                return torch.Tensor._make_wrapper_subclass(
                    # do for later: right now, _make_wrapper_subclass's dynamic shape interaction is not great.
...
class TestNestedTensor(torch._dynamo.test_case.TestCase):
    ...
    # do for later: cannot parametrize this test class with device for some reason
    def _test_autograd(self, backend):
        a = torch.randn(2, 3, requires_grad=True, dtype=torch.float64)
        b = torch.randn(3, 3, requires_grad=True, dtype=torch.float64)
        c = torch.randn(4, 3, requires_grad=True, dtype=torch.float64)
        nt = torch.nested.as_nested_tensor([a, b, c], layout=torch.jagged)
        # do for later: Switch to public API when it exists
