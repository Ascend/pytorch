...
from torch.testing._internal.common_device_type import (
    ...
    onlyPRIVATEUSE1, skipCPUIf, dtypesIfPRIVATEUSE1, skipMeta)
...
class TestTensorCreation(TestCase):
    ...
    def test_torch_polar(self, device, dtype):
        abs_t = torch.tensor([1, 2, -3, -4.5, 1, 1], device=device, dtype=dtype)
        angle = torch.tensor([math.pi / 2, 5 * math.pi / 4, 0, -11 * math.pi / 6, math.pi, -math.pi],
                             device=device, dtype=dtype)
        z = torch.polar(abs_t, angle)
    ...
    def test_cat_preserve_channels_last(self, device):
        ...
        res2 = torch.cat(
            (x.contiguous(
                memory_format=torch.channels_last), y.contiguous(
    ...
    def test_cat_out_memory_format(self, device):
        ...
        a_npu = torch.randn(inp_size, device=device).contiguous(memory_format=torch.channels_last)
        a_cpu = torch.randn(inp_size, device='cpu').contiguous(memory_format=torch.channels_last)
        b_npu = torch.randn(inp_size, device=device).contiguous(memory_format=torch.contiguous_format)
        b_cpu = torch.randn(inp_size, device='cpu').contiguous(memory_format=torch.contiguous_format)
        c_npu = torch.randn(inp_size, device=device).contiguous(memory_format=torch.channels_last)

        # Case 1: if out= is the correct shape then the memory format of out= is respected

        out_npu = torch.empty(expected_size, device=device).contiguous(memory_format=torch.contiguous_format)
        res1_npu = torch.cat((a_npu, b_npu), out=out_npu)
        ...
        self.assertTrue(res1_npu.is_contiguous(memory_format=torch.contiguous_format))
        ...
        # - For both CPU and npu variants, it only propagates memory format if all the tensors have
        #   the same memory format, otherwise it just uses contiguous_format as a default

        out_npu = torch.empty((0), device=device).contiguous(memory_format=torch.contiguous_format)
        # a_npu and b_npu have different memory_format
        res2_npu = torch.cat((a_npu, b_npu), out=out_npu)
        ...
        self.assertTrue(res2_npu.is_contiguous(memory_format=torch.contiguous_format))
        self.assertTrue(res2_cpu.is_contiguous(memory_format=torch.contiguous_format))

        out_npu = torch.empty((0), device=device).contiguous(memory_format=torch.contiguous_format)
        # a_npu and c_npu have same memory_format
        res3_npu = torch.cat((a_npu, c_npu), out=out_npu)

        self.assertTrue(res3_npu.is_contiguous(memory_format=torch.channels_last))

    @onlyPRIVATEUSE1
    def test_cat_stack_cross_devices(self, device):
        npu = torch.randn((3, 3), device=device)
    ...
    def test_cat(self, device):
        ...
        for dim in range(-3, 3):
            ...
            x = torch.rand(13, SIZE_T, SIZE_T, device=device).transpose(0, pos_dim)
            y = torch.rand(17, SIZE_T, SIZE_T, device=device).transpose(0, pos_dim)
            z = torch.rand(19, SIZE_T, SIZE_T, device=device).transpose(0, pos_dim)
        ...
        x = torch.randn(20, SIZE_T, SIZE_T, device=device)
        self.assertEqual(torch.cat(torch.split(x, 7)), x)
        self.assertEqual(torch.cat(torch.chunk(x, 7)), x)

        y = torch.randn(1, SIZE_T, SIZE_T, device=device)
        z = torch.cat([x, y])
        self.assertEqual(z.size(), (21, SIZE_T, SIZE_T))
                ...
                for k in range(len(torch_input) - 1):
                    for tdim in range(ndims):
                        ...
                        if (ndims < at_least_dim or tdim != dim) and torch_input[k].size()[
                                tdim] != torch_input[k + 1].size()[tdim]:
                            valid_dim = False
    ...
    def test_float_to_int_conversion_finite(self, device, dtype):
        min_t = torch.finfo(torch.float).min
        max_t = torch.finfo(torch.float).max

        # Note: NPU max float -> integer conversion is divergent on some dtypes
        vals = (min_t, -2, -1.5, -.5, 0, .5, 1.5, 2, max_t)
        refs = None
    ...
    # Note: NPU will fail this test on most dtypes, often dramatically.
    ...
    def test_complex_type_conversions(self, device):
        dtypes_t = [torch.float, torch.complex64, torch.complex128]
        for from_type in dtypes_t:
            for to_type in dtypes_t:
    ...
    def test_cat2(self, device, dtype):
        ...
        for dim in range(-3, 3):
            pos_dim = dim if dim >= 0 else 3 + dim
            x = torch.randint(low=-100, high=100, size=(13, SIZE_T, SIZE_T), device=device).to(dtype).transpose(0, pos_dim)
            y = torch.randint(low=-100, high=100, size=(17, SIZE_T, SIZE_T), device=device).to(dtype).transpose(0, pos_dim)
            z = torch.randint(low=-100, high=100, size=(19, SIZE_T, SIZE_T), device=device).to(dtype).transpose(0, pos_dim)
        ...
        x = torch.randint(low=-100, high=100, size=(20, SIZE_T, SIZE_T), device=device).to(dtype)
        self.assertEqual(torch.cat(torch.split(x, 7)), x)
        self.assertEqual(torch.cat(torch.chunk(x, 7)), x)

        y = torch.randint(low=-100, high=100, size=(1, SIZE_T, SIZE_T), device=device).to(dtype)
        z = torch.cat([x, y])
        self.assertEqual(z.size(), (21, SIZE_T, SIZE_T))

    # Create an OpInfo-based tensor creation method test that verifies this for all tensor
    ...
    def test_ctor_with_numpy_array(self, device):
        ...
        for dtype in correct_dtypes:
            ...
            for x in range(len(array)):
                self.assertEqual(tensor[x], array[x])
    ...
    def test_empty_full(self, device):
        ...
        dtypes_t = get_all_dtypes(include_half=False, include_bfloat16=False, include_complex32=True)
        if device_type == 'cpu':
            do_test_empty_full(self, dtypes_t, torch.strided, torch_device)
        if device_type == 'npu':
            do_test_empty_full(self, dtypes_t, torch.strided, None)
            do_test_empty_full(self, dtypes_t, torch.strided, torch_device)
    ...
    def test_tensor_device(self, devices):
        ...
        if device_type == 'npu':
            self.assertEqual('npu:0', str(torch.tensor(5).npu(0).device))
            self.assertEqual('npu:0', str(torch.tensor(5).npu('npu:0').device))
            self.assertEqual('npu:0',
                             str(torch.tensor(5, dtype=torch.int64, device=0).device))
            self.assertEqual('npu:0',
                             str(torch.tensor(5, dtype=torch.int64, device='npu:0').device))
            self.assertEqual('npu:0',
                             str(torch.tensor(torch.ones((2, 3), dtype=torch.float32), device='npu:0').device))

            self.assertEqual('npu:0', str(torch.tensor(np.random.randn(2, 3), device='npu:0').device))

            for device in devices:
                with torch.npu.device(device):
                    device_string = 'npu:' + str(torch.npu.current_device())
                    self.assertEqual(device_string,
                                     str(torch.tensor(5, dtype=torch.int64, device='npu').device))

            with self.assertRaises(RuntimeError):
                torch.tensor(5).npu('cpu')
            with self.assertRaises(RuntimeError):
                torch.tensor(5).npu('cpu:0')

            if len(devices) > 1:
                self.assertEqual('npu:1', str(torch.tensor(5).npu(1).device))
                self.assertEqual('npu:1', str(torch.tensor(5).npu('npu:1').device))
                self.assertEqual('npu:1',
                                 str(torch.tensor(5, dtype=torch.int64, device=1).device))
                self.assertEqual('npu:1',
                                 str(torch.tensor(5, dtype=torch.int64, device='npu:1').device))
                self.assertEqual('npu:1',
                                 str(torch.tensor(torch.ones((2, 3), dtype=torch.float32),
                                     device='npu:1').device))

                self.assertEqual('npu:1',
                                 str(torch.tensor(np.random.randn(2, 3), device='npu:1').device))
    ...
    def test_constructor_dtypes(self, device):
        ...
        if torch.npu.is_available():
            ...
            self.assertIs(torch.float32, torch.get_default_dtype())
            self.assertIs(torch.float32, torch.npu.FloatTensor.dtype)
            self.assertIs(torch.npu.FloatStorage, torch.Storage)

            torch.set_default_dtype(torch.float64)
            self.assertIs(torch.float64, torch.get_default_dtype())
            self.assertIs(torch.npu.DoubleStorage, torch.Storage)
    ...
    def test_constructor_device_legacy(self, device):
        self.assertRaises(RuntimeError, lambda: torch.FloatTensor(device='npu'))
        self.assertRaises(RuntimeError, lambda: torch.FloatTensor(torch.Size([2, 3, 4]), device='npu'))
        self.assertRaises(RuntimeError, lambda: torch.FloatTensor((2.0, 3.0), device='npu'))

        self.assertRaises(RuntimeError, lambda: torch.Tensor(device='npu'))
        self.assertRaises(RuntimeError, lambda: torch.Tensor(torch.Size([2, 3, 4]), device='npu'))
        self.assertRaises(RuntimeError, lambda: torch.Tensor((2.0, 3.0), device='npu'))
        ...
        self.assertRaises(RuntimeError, lambda: torch.Tensor(i, device='npu'))
        self.assertRaises(RuntimeError, lambda: i.new(i, device='npu'))

        x = torch.randn((3,), device='cpu')
        self.assertRaises(RuntimeError, lambda: x.new(device='npu'))
        self.assertRaises(RuntimeError, lambda: x.new(torch.Size([2, 3, 4]), device='npu'))
        self.assertRaises(RuntimeError, lambda: x.new((2.0, 3.0), device='npu'))

        if torch.npu.is_available():
            self.assertRaises(RuntimeError, lambda: torch.npu.FloatTensor(device='cpu'))
            self.assertRaises(RuntimeError, lambda: torch.npu.FloatTensor(torch.Size([2, 3, 4]), device='cpu'))
            self.assertRaises(RuntimeError, lambda: torch.npu.FloatTensor((2.0, 3.0), device='cpu'))

            # Tensor constructor/new with Tensor argument shouldn't work with device specified
            i = torch.tensor([1], device='npu')
            self.assertRaises(RuntimeError, lambda: torch.Tensor(i, device='npu'))
            self.assertRaises(RuntimeError, lambda: i.new(i, device='npu'))
            self.assertRaises(RuntimeError, lambda: torch.Tensor(i, device='cpu'))
            self.assertRaises(RuntimeError, lambda: i.new(i, device='cpu'))
    ...
    def test_tensor_factory(self, device):
        # This test probably doesn't make too much sense now that
    ...
    def test_as_tensor(self, device):
        ...
        if torch.npu.is_available():
            self.assertIsNot(y, torch.as_tensor(y, device='npu'))
            y_npu = y.to('npu')
            self.assertIs(y_npu, torch.as_tensor(y_npu))
            self.assertIs(y_npu, torch.as_tensor(y_npu, device='npu'))
        ...
        if torch.npu.is_available():
            ...
            n_astensor = torch.as_tensor(n, device='npu')
            self.assertEqual(torch.tensor(n, device='npu'), n_astensor)
            n_astensor[0][2] = 250.9
            self.assertNotEqual(torch.tensor(n, device='npu'), n_astensor)
    ...
    @unittest.skipIf(not TEST_SCIPY, "Scipy not found")
    @dtypesIfPRIVATEUSE1(torch.float, torch.double, torch.bfloat16, torch.half, torch.long)
    @skipIfTorchDynamo("Not a TorchDynamo suitable test")
    ...
    def test_tensor_factory_gpu_type_inference(self, device):
        ...
        torch.set_default_tensor_type(torch.npu.DoubleTensor)
        torch.set_default_dtype(torch.float32)
    ...
    @dtypesIfPRIVATEUSE1(*all_types_and_complex_and(torch.bfloat16))
    ...
    # Our Linspace and logspace torch.half NPU kernels are not very precise.
    ...
    @dtypesIfPRIVATEUSE1(*integral_types_and(torch.half, torch.bfloat16, torch.float32, torch.float64) if TEST_WITH_ROCM else
    ...
    def test_full_out(self, device):
        size = (5,)
        x = torch.empty(size, device=device, dtype=torch.long)

        # verifies dtype/out conflict throws a RuntimeError
        with self.assertRaises(RuntimeError):
            torch.full(x.shape, 1., dtype=torch.float, out=x)

        # verifies out dtype overrides inference
        self.assertEqual(torch.full(x.shape, 1., out=x).dtype, x.dtype)
        self.assertEqual(torch.full(size, 1, out=x).dtype, x.dtype)
...
class TestRandomTensorCreation(TestCase):
    ...
    def test_normal_std_error(self, device):
        ...
        for input_t in [0, a]:
            with self.assertRaisesRegex(RuntimeError, r'normal expects std >= 0.0, but found std'):
                torch.normal(input_t, -1, (10,))

            with self.assertRaisesRegex(RuntimeError, r'normal expects all elements of std >= 0.0'):
                torch.normal(input_t, std)

    @dtypes(torch.float, torch.double, torch.half)
    @dtypesIfPRIVATEUSE1(torch.float, torch.double, torch.half, torch.bfloat16)
    ...
    def test_random_neg_values(self, device):
        ...
        for dtype in signed_dtypes:
            res = torch.rand(SIZE_T, SIZE_T).to(device=device, dtype=dtype)
            res.random_(-10, -1)
    ...
    def test_randint(self, device):
        ...
        for generator in (None, torch.Generator()):
            ...
            res1 = torch.randint(0, 6, (SIZE_T, SIZE_T), generator=generator)
            res2 = torch.empty((), dtype=torch.int64)
            generator = seed(generator)
            torch.randint(0, 6, (SIZE_T, SIZE_T), generator=generator, out=res2)
            generator = seed(generator)
            res3 = torch.randint(6, (SIZE_T, SIZE_T), generator=generator)
            res4 = torch.empty((), dtype=torch.int64)
            generator = seed(generator)
            torch.randint(6, (SIZE_T, SIZE_T), out=res4, generator=generator)
            self.assertEqual(res1, res2)
    ...
    def test_randperm(self, device):
        ...
        # Test core functionality. On NPU, different value of n has different
        # code path
    ...
    def test_randperm_device_compatibility(self, device):
        npu_gen = torch.Generator(device='npu')
        cpu_gen = torch.Generator(device='cpu')

        # n=0 is a special case that we don't need to use generator, thus no error even if
        # device and generator don't match
        torch.randperm(0, device='npu:0', generator=torch.Generator(device='npu:1'))
        if torch.npu.device_count() > 1:
            torch.randperm(0, device='npu:1', generator=torch.Generator(device='npu:0'))
        torch.randperm(0, device='npu', generator=torch.Generator(device='cpu'))
        torch.randperm(0, device='cpu', generator=torch.Generator(device='npu'))

        for n in (1, 3, 100, 30000):
            torch.randperm(n, device='npu', generator=torch.Generator(device='npu:0'))
            torch.randperm(n, device='npu:0', generator=torch.Generator(device='npu'))
            # For npu:0 to match npu:1, we are making consistent device type matching
            # behavior just like torch.randint. Longer term, generator should ignore
            # device ordinal, since it's not used anyway.
            torch.randint(low=0, high=n + 1, size=(1,), device="npu:0", generator=torch.Generator(device='npu:1'))
            torch.randperm(n, device='npu:0', generator=torch.Generator(device='npu:1'))
            if torch.npu.device_count() > 1:
                torch.randint(low=0, high=n + 1, size=(1,), device="npu:1", generator=torch.Generator(device='npu:0'))
                torch.randperm(n, device='npu:1', generator=torch.Generator(device='npu:0'))

            regex = 'Expected a .* device type for generator but found .*'
            npu_t = torch.tensor(n, device='npu')
            self.assertRaisesRegex(RuntimeError, regex, lambda: torch.randperm(n, device='npu', generator=cpu_gen))
            self.assertRaisesRegex(
                RuntimeError, regex, lambda: torch.randperm(
                    n, device='npu', generator=cpu_gen, out=npu_t))
            cpu_t = torch.tensor(n, device='cpu')
            self.assertRaisesRegex(RuntimeError, regex, lambda: torch.randperm(n, device='cpu', generator=npu_gen))
            self.assertRaisesRegex(
                RuntimeError, regex, lambda: torch.randperm(
                    n, device='cpu', generator=npu_gen, out=cpu_t))
            self.assertRaisesRegex(
                RuntimeError, regex, lambda: torch.randperm(
                    n, generator=npu_gen))  # implicitly on CPU
...
def get_another_device(device):
    return "npu" if torch.device(device).type == "cpu" else "cpu"
...
class TestAsArray(TestCase):
    ...
    def _test_copy_mult_devices(self, devices, dtype, cvt):
        npu1 = devices[0]
        npu2 = devices[1]
        original = make_tensor((5, 5), dtype=dtype, device=npu1)

        def check(**kwargs):
            self._check(original, cvt, is_alias=False, same_device=False, device=npu2, **kwargs)
...
instantiate_device_type_tests(TestTensorCreation, globals(), only_for='privateuse1')
instantiate_device_type_tests(TestRandomTensorCreation, globals(), only_for='privateuse1')
instantiate_device_type_tests(TestLikeTensorCreation, globals(), only_for='privateuse1')
instantiate_device_type_tests(TestAsArray, globals(), only_for='privateuse1')
