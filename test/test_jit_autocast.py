...
TEST_BFLOAT16 = TEST_NPU and torch.npu.is_bf16_supported()
...
class TestAutocast(JitTestCase):
    def setUp(self):
        ...
        if TEST_NPU:
            self.a_fp16 = torch.rand((2, 2), dtype=torch.float16, device='npu')
            self.b_fp16 = torch.rand((2, 2), dtype=torch.float16, device='npu')
            self.c_fp16 = torch.rand((2, 2), dtype=torch.float16, device='npu')
            self.d_fp16 = torch.rand((2, 2), dtype=torch.float16, device='npu')
            self.a_fp32 = torch.rand((2, 2), dtype=torch.float32, device='npu')
            self.b_fp32 = torch.rand((2, 2), dtype=torch.float32, device='npu')
            self.c_fp32 = torch.rand((2, 2), dtype=torch.float32, device='npu')
            self.d_fp32 = torch.rand((2, 2), dtype=torch.float32, device='npu')
    ...
    @unittest.skipIf(not TEST_NPU, "No npu")
    ...
    @unittest.skipIf(not TEST_NPU or not TEST_BFLOAT16, "No npu bfloat16 support")
    ...
    def test_script_module(self):
        class TestModule(torch.nn.Module):
            ...
            def forward(self, input_):
                with autocast(enabled=True):
                    output = self.weight.mv(input_)
                    output = self.linear(output)
                    return output

        scripted_module = torch.jit.script(TestModule(2, 3)).npu()
        inputs = torch.rand(3, dtype=torch.float32, device='npu')
    ...
    def test_autocast_api(self):
        ...
        def t_autocast_cuda(x, y):
            with torch.autocast("npu", dtype=torch.half):
                return torch.mm(x, y)
        ...
        x = torch.randn(5, 5, device="npu", dtype=torch.float32)
        y = torch.randn(5, 5, device="npu", dtype=torch.float32)
    ...
    def test_autocast_mixed_dtypes(self):
        ...
        def t(cpu0, cpu1, cuda0, cuda1):
            with torch.autocast("cpu", torch.bfloat16):
                with torch.autocast("npu", torch.float16):
        ...
        cuda0 = torch.randn(5, 5, device="npu", dtype=torch.float32)
        cuda1 = torch.randn(5, 5, device="npu", dtype=torch.float32)
    ...
    def test_autocast_autodiff(self):
        ...
        jit_t = torch.jit.script(t)
        t0 = torch.randn(5, 5, device="npu", dtype=torch.float32).requires_grad_()
        t1 = torch.randn(5, 5, device="npu", dtype=torch.float32).requires_grad_()
    ...
    def test_jit_freeze_autocast_basic(self):
        ...
        x = torch.rand((3, 4), dtype=torch.float).npu()
        y = torch.rand((4, 5), dtype=torch.float).npu()
    ...
    def test_jit_freeze_autocast_constants(self):
        class TestModule(torch.nn.Module):
            def __init__(self):
                super().__init__()
                self.x = torch.rand((3, 4), dtype=torch.float).npu()
    ...
    @unittest.skipIf(TEST_NPU, "CPU-only test")
    ...
    def test_jit_autocast_softmax_gpu(self):
        ...
        fn_s = torch.jit.script(fn)
        x = torch.rand((2, 2), dtype=torch.half).npu()
        fn_s(x)
        y = fn_s(x)
...
class TestJitTraceAutocast(JitTestCase):
    ...
    @unittest.skipIf(not TEST_NPU, "No npu")
    def test_script_autocast_npu(self):
        def fn(x):
