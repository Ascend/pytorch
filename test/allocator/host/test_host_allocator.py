...
class TestHostCachingAllocator(TestCase):
    ...
    def test_allocate_with_block_cut(self):
        ...
        self.assertEqual(torch_npu.npu.pin_memory_allocated(), 64 * 1024 * 1024 + 512)
        self.assertEqual(torch_npu.npu.pin_memory_reserved(), 80 * 1024 * 1024)

        # 释放64M内存，内存归还内存池，物理内存不变，使用量为0
        memory_64m = None
        self.assertEqual(torch_npu.npu.pin_memory_reserved(), 80 * 1024 * 1024)
        self.assertEqual(torch_npu.npu.pin_memory_allocated(), 0)
        ...
        self.assertEqual(torch_npu.npu.pin_memory_allocated(), (4 + 8 + 16) * 1024 * 1024 + 512 * 3)
        self.assertEqual(torch_npu.npu.pin_memory_reserved(), 80 * 1024 * 1024)
        # 内存块归还内存池, 执行empty_pin_memory_cache释放所有物理内存防止用例间相互影响
        memory_4m = None
        memory_8m = None
        memory_16m = None
        torch_npu.npu.empty_pin_memory_cache()
        self.assertEqual(torch_npu.npu.pin_memory_allocated(), 0)
        self.assertEqual(torch_npu.npu.pin_memory_reserved(), 0)
    ...
    def test_allocate_with_block_merge(self):
        ...
        memory_40m1 = None
        memory_40m2 = None
        self.assertEqual(torch_npu.npu.pin_memory_reserved(), 100 * 1024 * 1024)
    ...
    def tearDownClass(cls):
        instance = cls()
        instance.assertEqual(torch_npu.npu.pin_memory_stats().get("allocated_bytes.peak", 0), 80 * 1024 * 1024 + 512 * 2)
        instance.assertEqual(torch_npu.npu.pin_memory_stats().get("allocated_bytes.current", 0), 0)
        instance.assertEqual(torch_npu.npu.pin_memory_stats().get("reserved_bytes.peak", 0), 100 * 1024 * 1024)
        instance.assertEqual(torch_npu.npu.pin_memory_stats().get("reserved_bytes.current", 0), 0)
