...
class TestHostCachingAllocator(TestCase):
    ...
    def test_allocate_with_block_cut(self):
        ...
        self.assertEqual(torch_npu.npu.host_memory_stats().get(ACTIVE_BYTES_CURRENT, 0), 64 * 1024 * 1024 + 512)
        self.assertEqual(torch_npu.npu.host_memory_stats().get(ALLOCATED_BYTES_CURRENT, 0), 80 * 1024 * 1024)

        # 释放64M内存，内存归还内存池，物理内存不变，使用量为0
        memory_64m = None
        self.assertEqual(torch_npu.npu.host_memory_stats().get(ALLOCATED_BYTES_CURRENT, 0), 80 * 1024 * 1024)
        self.assertEqual(torch_npu.npu.host_memory_stats().get(ACTIVE_BYTES_CURRENT, 0), 0)
        ...
        self.assertEqual(torch_npu.npu.host_memory_stats().get(ACTIVE_BYTES_CURRENT, 0), (4 + 8 + 16) * 1024 * 1024 + 512 * 3)
        self.assertEqual(torch_npu.npu.host_memory_stats().get(ALLOCATED_BYTES_CURRENT, 0), 80 * 1024 * 1024)
    ...
    def test_allocate_with_block_merge(self):
        ...
        memory_40m1 = None
        memory_40m2 = None
        self.assertEqual(torch_npu.npu.host_memory_stats().get(ALLOCATED_BYTES_CURRENT, 0), 100 * 1024 * 1024)
    ...
    def tearDownClass(cls):
        instance = cls()
        instance.assertEqual(torch_npu.npu.host_memory_stats().get("active_bytes.peak", 0), 80 * 1024 * 1024 + 512 * 2)
        instance.assertEqual(torch_npu.npu.host_memory_stats().get(ACTIVE_BYTES_CURRENT, 0), 0)
        instance.assertEqual(torch_npu.npu.host_memory_stats().get("allocated_bytes.peak", 0), 100 * 1024 * 1024)
