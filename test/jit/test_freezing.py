...
TEST_CUDA = torch_npu.npu.is_available()
TEST_ROCM = torch_npu.npu.is_available() and torch.version.hip is not None
...
class TestFreezing(JitTestCase):
    ...
    def test_freeze_module_no_forward(self):
        ...
        self.assertEqual(m_s.foo(input_), m_f.foo(input_))
    ...
    def test_freeze_module_with_user_preserved_method(self):
        ...
        self.assertFalse(fm.hasattr("b"))
        input_ = torch.randn(2, 2)
        expected = m.forward(input_)
        out = fm.forward(input_)
        self.assertEqual(out, expected)
    ...
    def test_freeze_non_module_class_getattr(self):
        class BoxCoder:
            ...
            def decode(self, input_):
                return input_ * self.bbox_xform_clip
        ...
        class MyModule(torch.nn.Module):
            ...
            def forward(self, input_):
                return self.box_coder.decode(input_)
...
class TestFrozenOptimizations(JitTestCase):
    ...
    @unittest.skipIf(not TEST_CUDA, "Optimization currently only run for NPU")
    def test_conv_bn_folding_autocast_scenario_cuda(self):
        # NPU conv takes input tensors which must all be the same dtype,
        ...
        mod_eager = ConvBN(3, 32, kernel_size=3, stride=2).npu().eval()
        scripted_mod = torch.jit.script(mod_eager)
        scripted_mod = torch.jit.freeze(scripted_mod)
        FileCheck().check("conv").check_not("aten::batch_norm").run(scripted_mod.graph)
        ...
        x = torch.rand((3, 3, 32, 32), dtype=torch.half).npu()
    ...
    def test_conv_mul_add_bn(self):
        ...
        input_ = torch.randn(8, 3, 64, 64)
        model = Conv_Mul_Add_Bn(3, 32, kernel_size=3, stride=1).eval()

        with torch.no_grad():
            result = model(input_)
            traced_model = torch.jit.trace(model, input_).eval()
            traced_model = torch.jit.freeze(traced_model)
            tresult = traced_model(input_)
    ...
    def test_linear_bn_folding_autocast_scenario_cuda(self):
        ...
        for modules, tracing, track_stats in product(module_pairs, use_tracing, bn_running_stats):
            ...
            mod_eager = LinearBN(32, 32).npu().eval()
            ...
            x = torch.rand(inps, dtype=torch.half).npu()
    ...
    def check_linear_optimizations(self, eager_mod, orig_linears, new_linears, test_vals):
        for is_cuda in [False, True]:
            if is_cuda:
                mod_to_device = eager_mod.npu()
                test_vals_to_device = [t.npu() if isinstance(t, torch.Tensor) else t for t in test_vals]
            else:
    ...
    def test_freeze_remove_dropout(self):
        ...
        output_s = mod.forward(input_)
        output_f = frozen_mod.forward(input_)
        self.assertEqual(output_s, output_f)
