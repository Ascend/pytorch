...
# Check if multiple NPU's are available
TEST_MULTIGPU = TEST_CUDA and torch.npu.device_count() >= 2

# If NPU is not available, then do not run the tests
if not TEST_CUDA:
...
# If NPU is available, then initialize the npu context and check
# if there is memory available to allocate for LARGE Tensors.
if TEST_CUDA:
    torch.ones(1).npu()  # initialize npu context
...
class TestCUDA(JitTestCase):
    ...
    def test_streams_and_events(self):
        ...
        # when the stream is none for `with torch.npu.stream`
        @torch.jit.script
        ...
        # when the device is none for `with torch.npu.device`
        @torch.jit.script
        ...
        # Check if a NPU JIT stream is created
        ...
        # This test has been adapted from the eager mode tests available at test/test_npu.py
        @torch.jit.script
        ...
        # Test a simple NPU event. Test if the NPU event was created successfully
        @torch.jit.script
        ...
        # and events. This test is adapted from eager mode tests for NPU. Please refer
        # test/test_npu.py
        @torch.jit.script
        ...
        # Test if a scripted module with npu streams can be saved, loaded and executed
    ...
    # Make sure that npu._exchange_device doesn't get DCE'ed
    ...
    # Make sure that npu._maybe_exchange_device doesn't get DCE'ed
    @unittest.skipIf(not TEST_CUDA, "Cuda not available")
