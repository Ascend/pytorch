    ...
    def test_has_attr(self):
        ...
        class Mod(torch.nn.Module):
            ...
            def forward(self):
                ...
                data = torch.jit.annotate(List[int], [])
                for mod in self.mods:
                    data.append(int(hasattr(mod, "a")))
                    data.append(int(hasattr(mod, "b")))
...
class TestTensorBuiltins(JitTestCase):
    ...
    def test_tensor_subscript_assign_device(self):
        def fn6(x):
            a = torch.zeros_like(x, dtype=torch.float32, device="npu")
            a[torch.tensor(0)] = 2
            return a

        self.checkScript(fn6, (torch.zeros(2, dtype=torch.float32, device="npu"),))
