...
from torch.testing._internal.jit_utils import JitTestCase, enable_cpu_fuser, \
    _tmp_donotuse_dont_inline_everything, _trace, make_global
from torch.testing._internal.common_cuda import with_tf32_off
from torch import Tensor


RUN_CUDA = torch.npu.is_available()
RUN_CUDA_MULTI_GPU = RUN_CUDA and torch.npu.device_count() > 1
...
class TestTracer(JitTestCase):
    ...
    def test_large_nbr_kernel_args(self):
        ...
        input_ = torch.rand(batch_size, seq_len, input_size)

        torch.npu.set_device(0)
        rec = rec.npu()
        input_ = input_.npu()

        traced_rec = torch.jit.trace(rec, (input_))
    ...
    def test_trace_tensor_factory(self):
        def run(**kwargs):
            ...
            input_ = torch.ones(2, 3, **input_kwargs)
            self.checkTrace(fn, (input_,), inputs_require_grads=inputs_require_grads)
            # check we recorded 'ones' and did not just record a constant
            tfn = torch.jit.trace(fn, input_)
    ...
    @unittest.skipIf(not RUN_CUDA, "calls .npu()")
    # By default, on Ampere or later NPUs, nn.Linear computes float tensors at TF32 precision.
    ...
    def test_traced_module_cuda(self):
        ...
        # Type casts
        linear_submodule.npu()
        traced_model.float().npu()
        cuda_out = traced_model(x.float().npu())
        traced_model.cpu()
    ...
    @unittest.skipIf(not RUN_CUDA, "uses npu")
    def test_type_same_device(self):
        ...
        a = Model()
        b = torch.jit.trace(a, example_inputs=(torch.ones([1], device=torch.device("npu")),))
    ...
    def test_trace_save(self):
        ...
        def check(func):
            with TemporaryFileName() as fname:
                ...
                input_ = torch.randn(2, 2)
                self.assertEqual(func(input_), loaded(input_))
    ...
    def test_trace_checker_dropout_notrain(self):
        ...
        self.assertEqual(foo(input_), input_)
    ...
    def test_tracing_hooks(self):
        ...
        def test_hook(is_post_hook, hook, fc):
            ...
            self.assertEqual(input_, eager_input)
            self.assertEqual(output, eager_out)

        def hook_no_return(mod, input_, output):
            input_[0].add_(1)
            output.sub_(1)
        ...
        def hook_return(mod, input_, output):
            input_[0].add_(1)
            return output - 3
...
class TestMixTracingScripting(JitTestCase):
    ...
    def test_trace_mixed_by_script_with_dict_output(self):
        @torch.jit.script
        def return_dict(input_: torch.Tensor) -> Dict[str, torch.Tensor]:
            return {"foo": input_ + 1}

        class TraceModule(torch.nn.Module):
            def forward(self, input_):
                dict_ = return_dict(input_)
                return dict_["foo"] + dict_["foo"]
    ...
    def test_script_inline_trace_multiple_args(self):
        class M(torch.nn.Module):
            def forward(self, input_, input2_):
                return input_ + input2_
    ...
    def test_trace_script_returning_complex_dict(self):
        ...
        class ChecksDict(torch.nn.Module):
            def forward(self, input_: Dict[str, Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]):
                v = input_["1000"]
                return v[1] + 1
        ...
        traced_module = torch.jit.trace(eager_module, inputs1)
        self.assertEqual(traced_module(inputs1), eager_module(inputs1))
        self.assertEqual(traced_module(inputs2), eager_module(inputs2))
    ...
    def test_traced_module_contains_scripted_interface_types(self):
        ...
        class TopModule(torch.nn.Module):
            ...
            def forward(self, input_: torch.Tensor):
                return self.sub1(input_) + self.sub2(input_)
