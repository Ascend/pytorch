...
class TestModels(JitTestCase):
    ...
    def _test_dcgan_models(self, device, check_export_import=True):
        ...
        class DCGANDiscriminator(nn.Module):
            ...
            def forward(self, input_):
                return self.main(input_).view(-1, 1).squeeze(1)
    ...
    def test_dcgan_models_cuda(self):
        # Note: Can sometimes fail with low precision if run with float dtype
        with set_default_dtype(torch.double):
            self._test_dcgan_models(self, device='npu', check_export_import=False)
    ...
    def _test_neural_style(self, device, check_export_import=True):
        ...
        class ResidualBlock(torch.nn.Module):
            """ResidualBlock
            introduced in: abs 1512.03385
            recommended architecture: torch.ch/blog/2016/02/04/resnets.html
            """
    ...
    def test_neural_style_cuda(self):
        self._test_neural_style(self, device='npu', check_export_import=False)
    ...
    def test_mnist_cuda(self):
        self._test_mnist(self, device='npu', check_export_import=False)

    @unittest.skipIf(not RUN_CUDA, "no NPU")
    def test_mnist_training_leaks_no_memory_cuda(self):
        net = MnistNet().npu()
        # MnistNet uses dropout, don't check its trace
        traced_net = torch.jit.trace(net, [torch.randn(5, 1, 28, 28, device='npu')],
                                     check_trace=False)

        def train(iters):
            for _ in range(iters):
                # Get some fake data
                inp = torch.randn(5, 1, 28, 28, device='npu')
                out = traced_net(inp)
    ...
    def test_reinforcement_learning_cuda(self):
        self._test_reinforcement_learning(self, device='npu', test_export_import=False)
    ...
    def _test_snli(self, device, check_export_import=True):
        class Bottle(nn.Module):
            ...
            def forward(self, input_):
                ...
                size = input_.size()[:2]
                out = super().forward(input_.view(size[0] * size[1], -1))
    ...
    def test_snli_cuda(self):
        self._test_snli(self, device='npu', check_export_import=False)
    ...
    def test_super_resolution_cuda(self):
        self._test_super_resolution(self, device='npu', check_export_import=False)
    ...
    def test_time_sequence_prediction(self):
        class Sequence(torch.jit.ScriptModule):
            ...
            def forward(self, input_):
                ...
                a, b, c, d = input_.chunk(input_.size(1), dim=1)
        ...
        class Traced(nn.Module):
            ...
            def forward(self, input_):
                return self.seq.forward(input_)
    ...
    def test_vae_cuda(self):
        self._test_vae(self, device='npu', check_export_import=False)
    ...
    def test_script_module_trace_resnet18(self):
        ...
        input_ = torch.randn(1, 3, 224, 224, requires_grad=True)
        output_orig = m_orig(input_)
        output_orig.sum().backward()
        grad_orig = input_.grad.clone()
        input_.grad.zero_()

        output_import = m_import(input_)
        output_import.sum().backward()
        grad_import = input_.grad.clone()
    ...
    def test_script_module_script_resnet(self):
        ...
        input_ = torch.randn(1, 3, 224, 224, requires_grad=True)
        output_orig = resnet18(input_)
        output_orig.sum().backward()
        grad_orig = input_.grad.clone()
        input_.grad.zero_()
        output_import = resnet18_imported(input_)
        output_import.sum().backward()
        grad_import = input_.grad.clone()
