...
class TestList(JitTestCase):
    def test_list_bool_conversion(self):
        def if_predicate(data: List[int]):
            if data:
                s = 0
                for n in data:
                    s += n
        ...
        def while_predicate(data: List[int]):
            s = 0
        ...
        def ternary_predicate(data: List[int]):
            return "non-empty" if data else "empty"
    ...
    def test_comprehensions_basic(self):
        def comp(data: List[int]) -> List[int]:
    ...
    def test_comprehensions_basic_float(self):
        def comp(data: List[float]) -> List[float]:
    ...
    def test_extend_list_mutable(self):
        ...
        for data in [[], [torch.rand(2)], [torch.rand(2), torch.rand(2), torch.rand(2)]]:
            for r in [[], [torch.rand(2)], [torch.rand(2), torch.rand(2), torch.rand(2)]]:
                self.assertEqual(extend_list(data, r), data + r)
    ...
    def test_copy_list_mutable(self):
        ...
        for data in [[], [torch.rand(2)], [torch.rand(2), torch.rand(2), torch.rand(2)]]:
            self.assertEqual(copy_list(data), data)
    ...
    def test_to_list_gpu(self):
        """NPU tests for Tensor.tolist() function."""
        def to_list_bool_1D(x: torch.Tensor) -> List[bool]:
            li = torch.jit.annotate(List[bool], x.tolist())
            return li
        ...
        self.checkScript(to_list_bool_1D, (torch.tensor(
            [True, False, True, False], dtype=torch.bool).npu(),))
        self.checkScript(to_list_int_1D, (torch.tensor(
            [1, 2, 3, 4], dtype=torch.long).npu(),))
        self.checkScript(to_list_float_1D, (torch.randn(
            5, dtype=torch.double).npu(),))
...
class TestDict(JitTestCase):
    ...
    def test_type_annotation_missing_contained_type(self):
        ...
        def fn_with_comment(input_: Dict) -> Any:
            return input_

        # This function uses Python3 style type annotations.
        def annotated_fn(input_: Dict) -> Any:
            return input_
...
class TestScriptList(JitTestCase):
    ...
    """
    def _script_list_add(self, data: torch._C.ScriptList, e: int):
        """
    ...
    def test_repr(self):
        ...
        Test the __repr__ method.
        """
        self._compare_eager_and_script(lambda data: repr(data), [1])
    ...
    def test_bool(self):
        ...
        """
        self._compare_eager_and_script(lambda data: bool(data), [1])
        self._compare_eager_and_script(lambda data: bool(data), [])
    ...
    def test_getitem(self):
        ...
        self._compare_eager_and_script(lambda x: x[1], data)
        self._compare_eager_and_script(lambda x: x[3], data)
        self._compare_eager_and_script(lambda x: x[-1], data)

        # Test slicing.
        self._compare_eager_and_script(lambda x: x[1:3], data)
        self._compare_eager_and_script(lambda x: x[:], data)
        self._compare_eager_and_script(lambda x: x[1:], data)
        self._compare_eager_and_script(lambda x: x[:2], data)
        self._compare_eager_and_script(lambda x: x[-1], data)
        self._compare_eager_and_script(lambda x: x[-1::-1], data)

        # Test errors.
        self._compare_eager_and_script(lambda x: x[5], data)
        self._compare_eager_and_script(lambda x: x[-7], data)
        self._compare_eager_and_script(lambda x: x["key"], data)
    ...
    def test_len(self):
        ...
        Test len() builtin function.
        """
        self._compare_eager_and_script(lambda x: len(x), [1, 2, 3, 4])
        self._compare_eager_and_script(lambda x: len(x), [])

    def test_count(self):
        """
        Test count method.
        """
        self._compare_eager_and_script(lambda x: x.count(3), [1, 2, 3, 3])
    ...
    def test_remove(self):
        ...
        Test remove method.
        """
        self._compare_eager_and_script(lambda x: x.remove(1), [1, 2, 3])
        self._compare_eager_and_script(lambda x: x.remove(10), [1, 2, 3])
    ...
    def test_append(self):
        ...
        Test append method.
        """
        self._compare_eager_and_script(lambda x: x.append(1), [4, 3, 2])
    ...
    @skipIfTorchDynamo("torchdynamo issues 1991")
    def test_clear(self):
        """
        Test clear.
        """
        self._compare_eager_and_script(lambda x: x.clear(), [4, 3, 2])
    ...
    def test_insert(self):
        ...
        self._compare_eager_and_script(lambda x: x.insert(3, 3), data.copy())
        self._compare_eager_and_script(lambda x: x.insert(0, 3), data.copy())
        self._compare_eager_and_script(lambda x: x.insert(-2, 3), data)
    ...
    def test_pop(self):
        ...
        # Test normal cases.
        self._compare_eager_and_script(lambda x: x.pop(), data.copy())
        self._compare_eager_and_script(lambda x: x.pop(2), data.copy())
        self._compare_eager_and_script(lambda x: x.pop(-3), data.copy())

        # Test error cases.
        self._compare_eager_and_script(lambda x: x.pop(10), data)
