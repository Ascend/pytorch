...
RUN_CUDA_HALF = torch.npu.is_available()
...
class TestMisc(JitTestCase):
    ...
    def test_hacked_twin(self):
        ...
        input_, index, value, = gen_data()
        input1, index1, value1, = gen_data()
        out1 = torch.ops.aten.index_put.hacked_twin(input_, [index], value, accumulate=False)
        out2 = torch.index_put(input1, [index1], value1, accumulate=False)
        self.assertEqual(out1, out2)

        torch.ops.aten.index_put_.hacked_twin(input_, [index], value, accumulate=False)
        torch.index_put_(input1, [index1], value1, accumulate=False)
        self.assertEqual(input_, input1)
    ...
    def test_unsafe_hacked_twin(self):
        ...
        input_, index, value, = gen_data()
        input1, index1, value1, = gen_data()
        out1 = torch.ops.aten._unsafe_index_put.hacked_twin(input_, [index], value, accumulate=False)
        out2 = torch.index_put(input1, [index1], value1, accumulate=False)
        self.assertEqual(out1, out2)

        torch.ops.aten._unsafe_index.Tensor_hacked_twin(input_, [index])
        torch.index_put(input1, [index1], value1, accumulate=False)
        self.assertEqual(input_, input1)

        def index_put_fn(input_x, index_x, value_x):
            return torch.ops.aten._unsafe_index_put(input_x, [index_x], value_x, accumulate=False)
        ...
        def index_fn(input_x, index_x, value_x):
            return torch.ops.aten._unsafe_index_put(input_x, [index_x], value_x, accumulate=False)
    ...
    def test_legacy_tensor_constructor(self):
        ...
        jit_sc = torch.jit.script(empty_overload)()
        eager[:] = 1
        jit_sc[:] = 1
        self.assertEqual(eager, jit_sc)
    ...
    @unittest.skipIf(not RUN_CUDA_HALF, "need NPU half support")
