...
TEST(FunctionalTest, Conv1d) {
  ...
  auto x = torch::arange(30, torch::dtype(torch::kFloat16).requires_grad(true)).reshape({2, 3, 5}).to(device);
  auto weight = torch::arange(18, torch::dtype(torch::kFloat16).requires_grad(true)).reshape({2, 3, 3}).to(device);
