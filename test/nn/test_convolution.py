...
from torch.testing._internal.common_device_type import instantiate_device_type_tests, dtypes, \
    dtypesIfPRIVATEUSE1, precisionOverride, skipCUDAIfNoCudnn, skipCUDAIfCudnnVersionLessThan, onlyPRIVATEUSE1, onlyCPU, \
    skipCUDAIfRocm, skipCUDAIfRocmVersionLessThan, onlyNativeDeviceTypes, largeTensorTest, skipMeta, \
...
class TestConvolutionNN(NNTestCase):
    ...
    def test_invalid_conv1d(self):
        for dtype in [torch.half, torch.bfloat16, torch.float, torch.double, torch.cfloat, torch.cdouble]:
            ...
            input1 = torch.randn(1, 3, 4).to(dtype)
    ...
    def test_invalid_conv2d(self):
        for dtype in [torch.half, torch.bfloat16, torch.float, torch.double, torch.cfloat, torch.cdouble]:
            ...
            input1 = torch.empty(1, 1, 4, 4).to(dtype)
            self.assertRaises(RuntimeError, lambda: module(input1))

            module = nn.Conv2d(in_channels=3, out_channels=33, kernel_size=10, stride=1, bias=True)
            input1 = torch.randn(1, 3, 1, 1)
            with self.assertRaisesRegex(RuntimeError,
            ...
            input1 = torch.randn(1, 3, 4, 4).to(dtype)
            with self.assertRaisesRegex(RuntimeError, 'non-positive stride is not supported'):
                module(input1)
    ...
    def test_invalid_conv3d(self):
        for dtype in [torch.half, torch.bfloat16, torch.float, torch.double, torch.cfloat, torch.cdouble]:
            ...
            input1 = torch.empty(1, 1, 4, 4, 4).to(dtype)
            self.assertRaises(RuntimeError, lambda: module(input1))

            # Negative stride check
            module = torch.nn.Conv3d(1, 1, kernel_size=3, stride=-2)
            input1 = torch.empty(1, 1, 4, 4, 4)
            with self.assertRaisesRegex(RuntimeError, 'non-positive stride is not supported'):
                module(input1)
    ...
    def test_thnn_conv_strided_padded_dilated(self):
        for convfn, dims, transposed in (
            ...
            for stride, padding, dilation in (
                ...
                inputs = torch.randn(inp_shape, dtype=torch.double, device="npu", requires_grad=True)
                weight = torch.randn(weight_shape, dtype=torch.double, device="npu", requires_grad=True)
                bias = torch.randn(2, dtype=torch.double, device="npu", requires_grad=True)
    ...
    def test_Conv2d_inconsistent_types_on_GPU_without_cudnn(self):
        ...
        weights = torch.randn(1, 1, 3, 3, dtype=torch.double, device="npu")
        bias = torch.randn(1, dtype=torch.double, device="npu")
    ...
    def test_Conv2d_1x1(self):
        ...
        input1 = torch.randn(1, in_channels, 5, 5, requires_grad=True, dtype=torch.double)
        for enabled in (False, True):
            with torch.backends.mkldnn.flags(enabled=enabled):
                gradcheck(F.conv2d, (input1, mod.weight))
        ...
        for dtype in [torch.half, torch.bfloat16, torch.double, torch.float]:
            ...
            for invalid_dims, module in zip(invalid_input_dims, modules):
                for dims in invalid_dims:
                    input1 = torch.empty(torch.Size((3, ) * dims))
                    self.assertRaises(RuntimeError, lambda: module(input1))

    def test_conv_shapecheck(self):
        def test(should_raise, module, input_size, dtype):
            input1 = torch.empty(3, *input_size).to(dtype)
            if should_raise:
                self.assertRaises(RuntimeError, lambda: module(input1))
            else:
    ...
    def test_ConvTranspose2d_half_cublas_gemm(self):
        with torch.backends.cudnn.flags(enabled=False):
            inputs = torch.randn(1, 1, 16, 16, device='npu', dtype=torch.half)
            deconv = nn.ConvTranspose2d(
                1, 1, 3, stride=2, padding=1, output_padding=1).npu().half()
    ...
    def test_grouped_conv_cudnn_nhwc_support(self):
        ...
        input1 = torch.randn((16, 16, 8, 8), dtype=torch.float16, device="npu").to(memory_format=torch.channels_last)
        weight = torch.randn((8, 4, 3, 3), dtype=torch.float16, device="npu").to(memory_format=torch.channels_last)
        out = torch.convolution(input1, weight, None, (1, 1), (1, 1), (1, 1), False, (0, 0), 4)
        input1 = torch.randn((16, 8, 8, 8), dtype=torch.float16, device="npu").to(memory_format=torch.channels_last)
        out_transpose = torch.convolution(input1, weight, None, (1, 1), (1, 1), (1, 1), True, (0, 0), 4)
    ...
    def test_conv_cudnn_memory_layout_dominance(self):
        ...
        input1 = torch.randint(1, 10, (2, 8, 4, 4), dtype=torch.float32, device="npu", requires_grad=True)
        conv = nn.Conv2d(8, 4, 3).npu().float()
    ...
    def test_cudnn_noncontiguous_weight(self):
        ...
        # passed to cuDNN
        input1 = torch.tensor([1, 1, 1], dtype=torch.double, device="npu").view(1, 1, 3)
        weights1 = torch.tensor([1], dtype=torch.double, device="npu").expand(1, 1, 2)
        weights2 = torch.tensor([1], dtype=torch.double, device="npu").expand(1, 1, 2).contiguous()
        self.assertEqual(F.conv1d(input1, weights1, bias=None, stride=2, dilation=2),
                         F.conv1d(input1, weights2, bias=None, stride=2, dilation=2))
        ...
        for kern, inp_size in [(3, 6), (3, 7), (4, 9)]:
            for batch, stride, padding, chan_in, chan_out, dilation in \
                ...
                for has_bias in [True, False]:
                    ...
                    input1 = torch.randn(input_shape, requires_grad=True)
                    weight = torch.randn(weight_shape, requires_grad=True)
                    if has_bias:
                        bias = torch.randn([chan_out], requires_grad=True)
                    output = func_forward(input1, weight, stride=stride, padding=padding, dilation=dilation, bias=bias)

                    gradient_o = torch.randn(output.shape)
                    gradient_w = torch.autograd.grad(output, input1 if (gradient == 'input') else weight, gradient_o)

                    self.assertEqual(gradient_w[0],
                                     func_backward(
                                     input_shape if (gradient == 'input') else input1,
        ...
        for kern, inp_size in [(3, 6), (3, 7), (4, 9)]:
            for batch, stride, padding, chan_in, chan_out in \
                ...
                for has_bias in [True, False]:
                    ...
                    input1 = torch.randn(input_shape, requires_grad=True, dtype=torch.float)
                    ...
                    output = torch._nnpack_spatial_convolution(
                        input1, weight, stride=stride, padding=padding, bias=bias)
                    ...
                    grads = torch.autograd.grad(output, [input1, weight], gradient_o)
                    grads_expected = torch.autograd.grad(output_expected, [input1, weight], gradient_o)
    ...
    def test_functional_grad_conv(self):
        ...
        output = F.conv1d(input1, weight, dilation=2)
        grad_output = torch.randn(output.shape)

        grad_input_autograd, grad_weight_autograd = torch.autograd.grad(output, (input1, weight), grad_output)

        grad_input_functional = torch.nn.grad.conv1d_input(input1.shape, weight, grad_output, dilation=2)
        self.assertEqual(grad_input_functional, grad_input_autograd)

        grad_weight_functional = torch.nn.grad.conv1d_weight(input1, weight.shape, grad_output, dilation=2)
        self.assertEqual(grad_weight_functional, grad_weight_autograd)

        # Conv 2D
        input1 = torch.randn(1, 1, 5, 5, requires_grad=True)
        weight = torch.randn(1, 1, 3, 3, requires_grad=True)
        output = F.conv2d(input1, weight, dilation=2)
        grad_output = torch.randn(output.shape)

        (grad_input_autograd, grad_weight_autograd) = torch.autograd.grad(output, (input1, weight), grad_output)

        grad_input_functional = torch.nn.grad.conv2d_input(input1.shape, weight, grad_output, dilation=2)
        self.assertEqual(grad_input_functional, grad_input_autograd)

        grad_weight_functional = torch.nn.grad.conv2d_weight(input1, weight.shape, grad_output, dilation=2)
        self.assertEqual(grad_weight_functional, grad_weight_autograd)

        # Conv 3D
        input1 = torch.randn(1, 1, 5, 5, 5, requires_grad=True)
        weight = torch.randn(1, 1, 3, 3, 3, requires_grad=True)
        output = F.conv3d(input1, weight, dilation=2)
        grad_output = torch.randn(output.shape)

        (grad_input_autograd, grad_weight_autograd) = torch.autograd.grad(output, (input1, weight), grad_output)

        grad_input_functional = torch.nn.grad.conv3d_input(input1.shape, weight, grad_output, dilation=2)
        self.assertEqual(grad_input_functional, grad_input_autograd)

        grad_weight_functional = torch.nn.grad.conv3d_weight(input1, weight.shape, grad_output, dilation=2)
        self.assertEqual(grad_weight_functional, grad_weight_autograd)
    ...
    def test_functional_grad_conv2d(self):
        ...
        def _test_conv2d(stride, kernel_size, groups, dilation):
            ...
            input1 = torch.empty(BATCH_SIZE, IN_CH, SPATIAL, SPATIAL).uniform_(-8.0, 8.0).requires_grad_(True)

            weight = torch.empty(OUT_CH, IN_CH // groups, kernel_size,
                                 kernel_size).uniform_(-4.0, 4.0).requires_grad_(True)

            output = F.conv2d(input1, weight,
            ...
            grad_input_functional = torch.nn.grad.conv2d_input(input1.shape, weight, grad_output,
                                                               stride=stride, padding=padding, dilation=dilation, groups=groups)
            self.assertEqual(grad_input_functional, grad_input_autograd)

            grad_weight_functional = torch.nn.grad.conv2d_weight(input1, weight.shape, grad_output,
                                                                 stride=stride, padding=padding, dilation=dilation, groups=groups)
            self.assertEqual(grad_weight_functional, grad_weight_autograd)
...
class TestConvolutionNNDeviceType(NNTestCase):
    def run_conv_double_back_test(self, kern, stride, padding, chan_in, chan_out, batch_size,
                                  inp_size, dilation, no_weight, groups=1, use_npu=False,
    ...
    def test_Conv3d_depthwise_naive_groups(self, device, dtype):
        for depth_multiplier in [1, 2]:
            ...
            output2 = m2(i2)
            output2.backward(grad_output[:, offset:].contiguous())
            is_cuda_sm86 = device.startswith("npu") and torch_npu.npu.get_device_capability(0) == (8, 6)
    ...
    def test_noncontig_conv_grad(self, device, dtype):
        ...
        input1 = torch.randn(2, 3, 10, 10, dtype=dtype, device=device, requires_grad=True)
        ...
        result = input1.grad.data.clone()
        input1.grad.data.zero_()

        output.backward(grad.contiguous())
        self.assertEqual(result, input1.grad.data, atol=dtype2prec_DONTUSE[dtype], rtol=0)
            ...
            for kern, inp_size, dilations in [(3, 5, [1, 2]), (4, 9, [1])]:
                for stride, padding, chan_in, chan_out, dilation in product([1], [2], [2], [3], dilations):
                    ...
                    result = self.run_conv_double_back_test(kern, stride,
                                                            ...
                                                            no_weight, use_npu=True, dtype=dtype)
    ...
    def test_conv_large_nosplit(self, device):
        ...
        dtype = torch.half if self.device_type == 'npu' else torch.float
    ...
    @dtypesIfPRIVATEUSE1(*floating_types_and(torch.half, *[torch.bfloat16] if AMPERE_OR_ROCM else []))
    @dtypes(torch.float)
    @torch.backends.cudnn.flags(enabled=True, benchmark=False)
    ...
    def test_conv_thnn_nhwc(self, device, dtype):
        def helper(mod, n, c, h, w, out_channels, kernel_size, dilation, groups, input_format, weight_format):
            input1 = torch.randint(-3, 3, (n, c, h, w), dtype=dtype, device=device)\
            ...
            ref_input = input1.detach().clone().contiguous().requires_grad_()
            ...
            self.assertEqual(input1.grad, ref_input.grad, exact_dtype=False)
    ...
    def test_conv_cudnn_nhwc(self, device, dtype):
        def helper(n, c, h, w, out_channels, kernel_size, groups):
            ...
            conv = nn.Conv2d(c, out_channels, kernel_size, groups=groups)\
                .to(device='npu', dtype=dtype, memory_format=torch.channels_last)
            for p in conv.parameters():
                p.data = torch.randint_like(p, -3, 3)

            # use FP64 channels-first conv as reference
            ref_input = input1.detach().clone().contiguous().double().requires_grad_()
            ref_conv = nn.Conv2d(c, out_channels, kernel_size, groups=groups)
            # load_state_dict will restore the stride & memory_layout on ref_conv.weight.
            ref_conv.load_state_dict(conv.state_dict())
            ref_conv = ref_conv.to(device='npu', dtype=torch.double, memory_format=torch.contiguous_format)
            ...
            self.assertTrue(out.is_contiguous(memory_format=torch.channels_last))
            self.assertTrue(input1.grad.is_contiguous(memory_format=torch.channels_last))
    ...
    def test_conv_cudnn_ndhwc(self, device, dtype):
        def helper(n, c, d, h, w, out_channels, kernel_size, groups):
            input1 = torch.randint(-2, 2, (n, c, d, h, w), dtype=dtype, device=device)\
                .to(memory_format=torch.channels_last_3d)
            input1.requires_grad_()
            conv = nn.Conv3d(c, out_channels, kernel_size, groups=groups)\
                .to(device='npu', dtype=dtype, memory_format=torch.channels_last_3d)
            for p in conv.parameters():
            ...
            self.assertTrue(input1.grad.is_contiguous(memory_format=torch.channels_last_3d))
    ...
    def _run_conv(self, layer, device, inp, grad, ref_conv, ref_input, ref_out,
        ...
        input1 = inp.clone().contiguous(memory_format=input_format)
        input1.resize_(input1.size(), memory_format=input_format)
    ...
    def _test_conv_cudnn_nhwc_nchw(self, layer, n, c, h, w, k, filter_size, device):
        ...
        ref_out = ref_conv(ref_input)
        grad = torch.randint(1, 10, ref_out.size(), dtype=torch.float32, device="npu")
    ...
    def test_conv_cudnn_nhwc_support(self, device, dtype):
        input1 = torch.randn((1, 16, 1, 1), dtype=dtype, device="npu", requires_grad=True)
        weight = torch.randn((8, 16, 3, 3), dtype=dtype, device="npu", requires_grad=True)
        weight = weight.to(memory_format=torch.channels_last)
        out = torch.conv2d(input1, weight, None, (2, 1), (1, 1), (1, 1), 1)
        self.assertTrue(out.is_contiguous(memory_format=torch.channels_last))
        out.sum().backward()
    ...
    def test_conv2d_no_grad(self, device, dtype):
        for batch in [1, 2, 3]:
            for groups in [1, 2, 4]:
                input1 = torch.rand(batch, groups, 8, 8, dtype=dtype, device=device)
    ...
    def test_convert_conv2d_weight_memory_format(self, device):
        input1 = torch.randint(1, 10, (2, 8, 4, 4), dtype=torch.float32, device=device)
        model = nn.Sequential(
            nn.Conv2d(8, 4, 3),
    ...
    def test_conv_double_backward_strided_with_3D_input_and_weight(self, device):
        ...
        input1 = torch.randn(2, 3, 6, device=device)
        ...
        output_padding = (0,)
        groups = 1
        output = torch.ops.aten.convolution(input1, weight, bias, stride, padding, dilation, transposed,
                                            output_padding, groups)

        ggI = torch.randn(input1.shape, device=device)
        ...
        grad_grad_output, grad_input, grad_weight = torch.ops.aten._convolution_double_backward(
            ggI, ggW, ggB, gO, weight, input1, stride, padding, dilation, transposed,
        ...
        self.assertEqual(grad_input.shape, input1.shape)
