...
from torch.testing._internal.common_utils import TestCase, run_tests, TEST_WITH_UBSAN, set_default_dtype, \
    ...
    skipIfTorchDynamo, TEST_PRIVATEUSE1
...
from torch.testing._internal.common_device_type import largeTensorTest, onlyNativeDeviceTypes, dtypes, \
    ...
    TEST_WITH_ROCM, onlyPRIVATEUSE1, dtypesIfPRIVATEUSE1
...
class TestAvgPool(TestCase):
    ...
    def test_doubletensor_avg_pool2d(self):
        n, m = 5, 8
        input1 = torch.rand(1, 1, n, m, dtype=torch.double)
        for i in range(1, n + 1):
            for j in range(1, m + 1):
                actual = torch.nn.functional.avg_pool2d(input1[0], (i, j))
                actual = actual.view(1, actual.numel())
                expected = self._avg_pool2d(input1, (i, j))
        ...
        for i in range(1, n + 1):
            for j in range(1, m + 1):
                for divisor in [1, 7, i * j]:
                    actual = F.avg_pool2d(input1[0], (i, j), divisor_override=divisor)
                    actual = actual.view(1, actual.numel())
                    expected = self._sum_pool2d(input1, (i, j)) / divisor
                    self.assertEqual(actual, expected, rtol=0, atol=1e-5)

    def test_doubletensor_avg_pool3d(self):
        h, w, d = 5, 6, 7
        input1 = torch.rand(h, w, d, dtype=torch.double)
        for i in range(1, h + 1):
            for j in range(1, w + 1):
                for k in range(1, d + 1):
                    actual = torch.nn.functional.avg_pool3d(input1.unsqueeze(0), (i, j, k))
                    actual = actual.view(1, actual.numel())
                    expected = self._avg_pool3d(input1, (i, j, k))
                    self.assertEqual(actual, expected, rtol=0, atol=1e-5)
    ...
    def test_doubletensor_avg_pool3d_with_divisor(self):
        ...
        for i in range(1, h + 1):
            for j in range(1, w + 1):
                for k in range(1, d + 1):
                    for divisor in [1, 7, i * j]:
                        actual = torch.nn.functional.avg_pool3d(input1.unsqueeze(0), (i, j, k), divisor_override=divisor)
                        actual = actual.view(1, actual.numel())
                        expected = self._sum_pool3d(input1, (i, j, k)) / divisor
                        self.assertEqual(actual, expected, rtol=0, atol=1e-5)
    ...
    def test_avg_pool1d_ceil_mode(self):
        ...
        if TEST_PRIVATEUSE1:
            y = torch.nn.functional.avg_pool1d(
                x.to('npu'), ceil_mode=True, count_include_pad=True, kernel_size=1, stride=2)
            self.assertTrue(not torch.isnan(y).any())
    ...
    def test_avg_pool2d_ceil_mode(self):
        ...
        if TEST_PRIVATEUSE1:
            y = torch.nn.functional.avg_pool2d(
                x.to('npu'), ceil_mode=True, count_include_pad=True, kernel_size=(1, 2),
                padding=(0, 1), stride=2)
            self.assertTrue(not torch.isnan(y).any())
    ...
    def test_avg_pool3d_ceil_mode(self):
        ...
        if TEST_PRIVATEUSE1:
            y = torch.nn.functional.avg_pool3d(
                x.to('npu'), ceil_mode=True, count_include_pad=True, kernel_size=(1, 2, 3), stride=2)
            self.assertTrue(not torch.isnan(y).any())
...
class TestPoolingNN(NNTestCase):
    ...
    def test_adaptive_pooling_avg_nhwc(self):
        ...
        for device in device_list:
            input1 = torch.randint(1, 10, (4, 8, 8, 8), dtype=torch.float32).to(device)
    ...
    def test_adaptive_pooling_avg_nhwc_non_contiguous(self):
        ...
        for device in device_list:
            input1 = torch.randint(1, 10, (4, 8, 8, 8), dtype=torch.float32).to(device)
            input1 = input1.contiguous(memory_format=torch.channels_last)
            input1 = input1[:, ::2, :, :].requires_grad_()
            grad = torch.randint(1, 10, (4, 8, 7, 7), dtype=torch.float32).to(device)
            grad = grad[:, ::2, :, :]
            pool = torch.nn.AdaptiveAvgPool2d((7, 7)).to(device)
    ...
    def test_adaptive_pooling_lower_precision(self):
        def _test_adaptive_pooling_lower_precision(self, device, dtype, mod, memory_format):
            input1 = torch.randint(1, 10, (3, 19, 8, 8), dtype=torch.float32)
            input1 = input1.to(device).to(memory_format=memory_format).requires_grad_()
            pool = mod((7, 7)).to(device)

            input2 = input1.detach().clone().to(dtype=dtype).requires_grad_(True)
            ...
            self.assertEqual(input1.grad, input2.grad.float(), atol=0.1, rtol=0)
    ...
    @largeTensorTest('12GB', device='npu')
    def test_adaptive_pooling_avg_nhwc_launch_config_backward(self):
        input1 = torch.randint(1, 10, (1, 32, 2 ** 17 + 1, 32), dtype=torch.float32, device="npu")
        input1 = input1.contiguous(memory_format=torch.channels_last).requires_grad_()
        grad = torch.randint(1, 10, (1, 32, 10, 32), dtype=torch.float32, device="npu")

        pool = torch.nn.AdaptiveAvgPool2d((10, 32)).npu()

        ref_input = input1.detach().clone().contiguous().requires_grad_(True)
        ref_grad = grad.detach().clone().contiguous()
        ref_pool = torch.nn.AdaptiveAvgPool2d((10, 32)).npu()
    ...
    def test_adaptive_pooling_avg_nhwc_launch_config_forward(self):
        input1 = torch.randint(1, 10, (1, 32, 16, 16), dtype=torch.float32, device="npu")
        input1 = input1.contiguous(memory_format=torch.channels_last).requires_grad_()
        pool = torch.nn.AdaptiveAvgPool2d((2 ** 17 + 1, 32)).npu()

        ref_input = input1.detach().clone().contiguous().requires_grad_(True)
        ref_pool = torch.nn.AdaptiveAvgPool2d((2 ** 17 + 1, 32)).npu()
    ...
    def test_adaptive_avg_pooling_overflow(self):
        input1 = torch.randint(-256, 256, (20, 32, 256, 256), dtype=torch.half, device='npu')
    ...
    def test_max_unpool2d_nhwc_cpu(self):
        input1 = torch.randn(2, 10, 9, 9).float().cpu()
        input1 = input1.contiguous(memory_format=torch.channels_last)
        ref_input = input1.clone().contiguous()
    ...
    def test_max_unpool(self):
        with set_default_dtype(torch.double):
            ...
            input1 = torch.randn([1, 1, 5], requires_grad=True)
            output, indices = F.max_pool1d(input1, 2, stride=2, return_indices=True)
            self.assertEqual(F.max_unpool1d(output, indices, 2, stride=2, output_size=input1.shape),
                             F.max_unpool1d(output, indices, 2, stride=2, output_size=input1.size()))
    ...
    def test_adaptive_pooling_empty_output_size(self, dtype, device):
        ...
        for fn in fns:
            with self.assertRaisesRegex(RuntimeError, error_msg):
                fn(input1, output_size).sum().backward()
...
'''
            ...
            error_msg = error_msgs.get(module_name)
    ...
    def test_adaptive_pooling_no_suppot_input(self, device, dtype):
        for numel in (2, 3):
            for pool_type in ('Max', 'Avg'):
                ...
                output_size = (2,) * numel
                module = module_cls(output_size)
                input1 = torch.randn((4,) * (numel + 1), device=device).to(dtype)
    ...
    def test_avg_pool2d_nhwc(self, device, dtype):
        def helper(n, c, h, w, kernel_size, stride=None,
            ...
            if stride is None:
                stride = kernel_size
            input1 = torch.randn(n, c, h, w, dtype=dtype, device=device)
            input1 = input1.contiguous(memory_format=torch.channels_last).requires_grad_()
        ...
        if TEST_WITH_ROCM and 'npu' in device:
    ...
    def test_max_pool3d_ndhwc(self, device, dtype):
        def helper(n, c, h, w, d, kernel_size, stride=None):
            ...
            if not batch:
                batch = 1
            input1 = torch.randn(batch, c, d, h, w, dtype=dtype, device=device)
            input1 = input1.contiguous(memory_format=torch.channels_last_3d).requires_grad_()
            if not n:
                input1 = input1.squeeze(0).detach().clone().requires_grad_()
            ...
            self.assertEqual(ind, ref_ind)
            if dtype == torch.half:
                self.assertEqual(input1.grad, ref_input.grad, atol=0.05, rtol=0.01)
            else:
                self.assertEqual(input1.grad, ref_input.grad)
    ...
    def test_max_pool_bfloat16_half(self, device, dtype):
        def helper(shape, kernel_size, stride, memory_format, dtype):
            ...
            input1 = input1.to(memory_format=memory_format).requires_grad_()
            if len(shape) == 4:
                pool = torch.nn.MaxPool2d(kernel_size, stride, return_indices=True).to(device)
            else:
                pool = torch.nn.MaxPool3d(kernel_size, stride, return_indices=True).to(device)
    ...
    def test_avg_pool2d_bfloat16(self, device):
        def helper(n, c, h, w, kernel_size, stride, memory_format):
            input1 = torch.randn(n, c, h, w, dtype=torch.float32, device=device).bfloat16()
            input1 = input1.to(memory_format=memory_format).requires_grad_()
            ...
            self.assertEqual(input1.grad.dtype, torch.bfloat16)
            self.assertEqual(out, out2.bfloat16())
            self.assertEqual(input1.grad, input2.grad.bfloat16())
    ...
    def test_adaptive_pooling_max_nhwc(self, device, dtype):
        def helper(input_size, output_plane_size, contig):
            ...
            input1 = torch.randint(1, 10, input_size, device=device, dtype=dtype)
            input1 = input1.contiguous(memory_format=channels_last)
    ...
    def test_pooling_max_nhwc(self, device, dtype):
        def helper(n, c, h, w, kernel_size, stride, padding, dilation, contig, device):
            ...
            input1 = torch.randint(1, 10, (n, c, h, w), device=device, dtype=dtype)
            input1 = input1.contiguous(memory_format=torch.channels_last)
    ...
    def _test_maxpool_indices(self, num_dim, adaptive=False, device="cpu", dtype=torch.float):
        ...
        numel = 4 ** (num_dim + 1)
        input1 = torch.arange(1, numel + 1).view(2, 2, *repeat(4, num_dim)).to(device, dtype=dtype)
        input_var = input1.clone().detach().requires_grad_()
        ...
        if num_dim != 3:
            ...
            self.assertEqual(indices.dim(), input1.dim())
        ...
        self.assertEqual(input_var.grad.data, expected_grad.view_as(input1))
    ...
    def test_maxpool_indices_no_batch_dim(self, device, dtype):
        ...
        for module, input1 in max_pool_cases:
            _, indices_no_batch = module(input1)
            _, indicies_single_batch = module(input1.unsqueeze(0))
    ...
    def test_pooling_large(self, device):
        def helper(pool):
            inp = torch.randn(2**7 + 10, 2**8, 2**8, 2**8, dtype=torch.half, device="npu")
            self.assertTrue(inp.numel() > 2**31 - 1)
            out = pool(inp)
            torch.npu.synchronize()    # asserts test finishes normally without raising errors
