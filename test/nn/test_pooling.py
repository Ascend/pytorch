...
class TestPoolingNN(NNTestCase):
    ...
    def test_adaptive_pooling_bfloat16(self):
        def _test_adaptive_pooling_bfloat16(self, device, mod, memory_format):
            ...
            input2 = input1.detach().clone().bfloat16().requires_grad_(True)
...
'''
    ...
    def test_max_pool_bfloat16(self, device):
        def helper(shape, kernel_size, stride, memory_format):
            input1 = torch.randn(shape, dtype=torch.float32, device=device).bfloat16()
    ...
    def test_pooling_large(self, device):
        def helper(pool):
            ...
            out = pool(inp)
            torch_npu.npu.synchronize()    # asserts test finishes normally without raising errors
