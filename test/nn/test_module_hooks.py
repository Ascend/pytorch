...
class TestModuleHooks(TestCase):
    ...
    def test_always_called_forward_hooks(self):
        ...
        def ctx_shutdown_hook(m, i, n):
            ctx.__exit__()

        def ctx_shutdown_failure_hook(m, i, n):
            ctx.__exit__()
            raise RuntimeError("failing in ctx shutdown")

        def throw_hook(m, i, n):
    ...
    def test_bw_hook_warning_for_non_tensor_or_tuple(self):
        ...
        self.assertEqual(counter.get('forward'), 2)
        self.assertEqual(counter.get('backward'), 0)
...
class TestModuleGlobalHooks(TestCase):
    ...
    def test_module_global_hooks(self):
        ...
        input1 = torch.ones(5, 5, requires_grad=True)
        ...
        def fw_hook(inc, h_module, input1, output):
            self.assertIsInstance(input1, tuple)
            self.assertTrue(isinstance(output, torch.Tensor))
            self.assertTrue(isinstance(h_module, module))
            self.assertEqual(input1[0], torch.ones(5, 5))
        ...
        module_2(input1)
        module_3(input1)
        self.assertEqual(counter.get('forwards'), 3)
        self.assertEqual(counter.get('backwards'), 0)

        test_bwd = nn.modules.module.register_module_backward_hook(
            lambda *args: bw_hook(1, *args))

        output_1 = module_1(input1)
        output_2 = module_2(input1)
        output_3 = module_3(input1)
        self.assertEqual(counter.get('forwards'), 6)
        self.assertEqual(counter.get('backwards'), 0)
        ...
        self.assertEqual(counter.get('forwards'), 6)
        self.assertEqual(counter.get('backwards'), 3)

        output_1.backward(torch.ones(5, 5) * 2, retain_graph=True)
        self.assertEqual(counter.get('forwards'), 6)
        self.assertEqual(counter.get('backwards'), 4)

        test2_fwd = nn.modules.module.register_module_forward_hook(lambda *args: fw_hook(2, *args))

        output = module_1(input1)
        output = module_2(input1)
        output = module_3(input1)
        self.assertEqual(counter.get('forwards'), 15)
        self.assertEqual(counter.get('backwards'), 4)

        test2_bwd = nn.modules.module.register_module_backward_hook(lambda *args: bw_hook(2, *args))

        module_1(input1).backward(torch.ones(5, 5) * 2)
        self.assertEqual(counter.get('forwards'), 18)
        self.assertEqual(counter.get('backwards'), 7)

        test2_bwd.remove()

        module_2(input1).backward(torch.ones(5, 5) * 2)
        self.assertEqual(counter.get('forwards'), 21)
        self.assertEqual(counter.get('backwards'), 8)

        test2_fwd.remove()

        module_3(input1).backward(torch.ones(5, 5) * 2)
        self.assertEqual(counter.get('forwards'), 22)
        self.assertEqual(counter.get('backwards'), 9)
    ...
    def test_module_backward_global_hook_writeable(self):
        ...
        module(input1).backward(torch.ones(5, 5))
        expected_grad = sig_x * (1 - sig_x) * 2
        self.assertEqual(input1.grad, expected_grad)
    ...
    def test_module_global_forward_preforward_hook_writeable(self):
        ...
        def forward_pre_hook1(m, input1):
            return torch.nn.functional.relu(input1[0])

        def forward_hook1(m, input1, output):
            return -output

        nn.modules.module.register_module_forward_pre_hook(forward_pre_hook1)
        nn.modules.module.register_module_forward_hook(forward_hook1)
        output = module(input1)
        expected_res = -torch.sigmoid(torch.nn.functional.relu(input1))
    ...
    def test_module_forward_preforward_hook_removable(self):
        ...
        def removable_hook(m, input1):
            nonlocal handle
            handle.remove()
            return input1

        def removable_hook_2(m, input1):
            nonlocal handle_2
            handle_2.remove()
            return input1
        ...
        output = module(input1)
        self.assertEqual(torch.sigmoid(input1), output)
    ...
    def test_module_forward_forward_hook_removable(self):
        ...
        def removable_hook(m, input1, output):
            nonlocal handle
            handle.remove()
            return output

        def removable_hook_2(m, input1, output):
            nonlocal handle_2
            handle_2.remove()
            return output
    ...
    def test_global_and_local_hooks_order(self):
        ...
        output = module(input1)
        self.assertTrue(
            local_forward_called and local_forward_pre_called and global_forward_called and global_forward_pre_called)
...
class TestModuleHookNN(NNTestCase):
    ...
    def _test_hooks(self, backward_register_fn):
        ...
        module(input1)
        module(input1)
        self.assertEqual(counter.get('forwards'), 2)
        self.assertEqual(counter.get('backwards'), 0)
        ...
        output.backward(torch.ones(5, 5) * 2, retain_graph=True)
        self.assertEqual(counter.get('forwards'), 3)
        self.assertEqual(counter.get('backwards'), 1)

        output.backward(torch.ones(5, 5) * 2, retain_graph=True)
        self.assertEqual(counter.get('forwards'), 3)
        self.assertEqual(counter.get('backwards'), 2)
        ...
        module(input1).backward(torch.ones(5, 5) * 2)
        self.assertEqual(counter.get('forwards'), 9)
        self.assertEqual(counter.get('backwards'), 5)

        test2_bwd.remove()

        module(input1).backward(torch.ones(5, 5) * 2)
        self.assertEqual(counter.get('forwards'), 12)
        self.assertEqual(counter.get('backwards'), 6)

        test2_fwd.remove()

        module(input1).backward(torch.ones(5, 5) * 2)
        self.assertEqual(counter.get('forwards'), 13)
        self.assertEqual(counter.get('backwards'), 7)
    ...
    def test_backward_hooks_interaction(self):
        ...
        self.assertEqual(cnt.get('backward_cnt'), 2)
    ...
    def test_hook_backward_writeable(self):
        module = nn.Sigmoid()
        input1 = torch.randn(5, 5, requires_grad=True)
        sig_x = torch.nn.functional.sigmoid(input1)
    ...
    def test_hook_forward_preforward_writable(self):
        ...
        module.register_forward_pre_hook(forward_pre_hook1)
        module.register_forward_hook(forward_hook1)
        output = module(input1)
        expected_res = -torch.nn.functional.sigmoid(torch.nn.functional.relu(input1))
        self.assertEqual(output, expected_res)
        output.backward(torch.ones(5, 5) * 2, retain_graph=True)
        mask = (input1 > 0)
        expected_grad = -sig_x * (1 - sig_x) * 2 * mask
