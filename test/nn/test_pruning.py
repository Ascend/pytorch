...
class TestPruningNN(NNTestCase):
    ...
    def test_rnn_pruning(self):
        ...
        assert (
            sum([isinstance(p, torch.nn.Parameter) for p in lstm._flat_weights])
            == 3
        )
        ...
        assert 'weight_ih_l0' in lstm._parameters
        assert lstm._parameters['weight_ih_l0'] is not None
        assert 'weight_ih_l0_orig' not in lstm._parameters
        assert 'weight_ih_l0' in dict(lstm.named_parameters())
        assert dict(lstm.named_parameters())['weight_ih_l0'] is not None
        assert 'weight_ih_l0_orig' not in dict(lstm.named_parameters())
