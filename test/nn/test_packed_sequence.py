...
class PackedSequenceTest(TestCase):
    ...
    def test_pack_padded_sequence(self):
        def generate_test_case(sorted_lengths, should_shuffle):
            ...
            offset = 0
            padded = torch.cat([pad(i * 100 + torch.arange(1., 5 * n + 1).view(n, 1, 5), max_length)
                                for i, n in enumerate(sorted_lengths, 1)], 1)
        ...
        for test_case, batch_first in itertools.product(test_cases, (True, False)):
            ...
            for i, n in enumerate(lengths):
                self.assertEqual(padded.grad.data[:n, i], grad_output[:n, i])
                if n < 10:
                    self.assertEqual(padded.grad.data[n:, i].abs().sum(), 0)
