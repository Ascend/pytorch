...
def pass_name_to_python_arg_parser(name):
    x = torch.empty(2, names=(name,), device="npu")
...
def create(namedshape, factory=torch.randn):
    ...
    return factory(shape, names=names).npu()
...
class TestNamedTensor(TestCase):
    ...
    def _test_name_inference(self, op, args=(), expected_names=(), device='npu',
    ...
    def test_has_names(self):
        unnamed = torch.empty(2, 3, device="npu")
        none_named = torch.empty(2, 3, names=(None, None), device="npu")
        partially_named = torch.empty(2, 3, names=('N', None), device="npu")
        fully_named = torch.empty(2, 3, names=('N', 'C'), device="npu")
    ...
    def test_py3_ellipsis(self):
        tensor = torch.randn(2, 3, 5, 7).npu()
    ...
    def test_detach(self):
        ...
        self._test_name_inference(
            Tensor.detach_,
            [torch.randn(3, requires_grad=True, names=names, device="npu")],
            names)
        self._test_name_inference(
            Tensor.detach,
            [torch.randn(3, requires_grad=True, names=names, device="npu")],
    ...
    def test_diagonal(self):
        named_tensor = torch.zeros(2, 3, 5, 7, names=list('ABCD')).npu()
    ...
    def test_no_save_support(self):
        named_tensor = torch.zeros(2, 3, names=('N', 'C')).npu()
        buf = io.BytesIO()
    ...
    def test_big_tensor_repr_has_names(self):
        ...
        check_repr(torch.randn(128, 3, 64, 64, names=('N', 'C', 'H', 'W'), device="npu"))
    ...
    def test_copy_transpose(self):
        ...
        def _test(self_names, other_names, expected_names):
            x = torch.empty(2, 5, names=self_names, device="npu")
            y = torch.empty(5, 2, device="npu").t().rename_(*other_names)
            x.copy_(y)
    ...
    def test_rename_(self):
        tensor = torch.empty(1, 1, names=('N', 'C'), device="npu")
    ...
    def test_rename_globber(self):
        scalar = torch.randn([], device="npu")
        unnamed_tensor = torch.empty(1, 1, 1, 1, device="npu")
        named_tensor = torch.empty(1, 1, 1, 1, names=('N', 'C', 'H', 'W'), device="npu")
    ...
    def test_tensor_from_lists(self):
        names = ('N', 'C')
        tensor = torch.tensor([[1]], names=names, device="npu")
    ...
    def test_tensor_from_tensor(self):
        x = torch.randn(1, 1)
        names = ('N', 'C')
        tensor = torch.tensor(x, names=names, device="npu")
        self.assertEqual(tensor.names, names)

    def test_tensor_from_named_tensor(self):
        x = torch.randn(1, 1, names=('N', 'D'), device="npu")
    ...
    def test_size(self):
        t = torch.empty(2, 3, 5, names=('N', None, 'C'), device="npu")
        ...
        with self.assertRaisesRegex(RuntimeError, 'Name \'N\' not found in '):
            torch.empty(2, 3, 4, device="npu").size('N')
    ...
    def test_autograd_smoke(self):
        x = torch.randn(3, 3, names=('N', 'D'), requires_grad=True, device="npu")
        ...
        tensor = torch.empty(1, 1, names=('N', 'D'), requires_grad=True, device="npu")
        tensor = tensor.relu()
        tensor.output_nr
        tensor.grad_fn
    ...
    def test_binary_ops(self):
        def test_basic(op):
            a = torch.empty(2, 3, names=('N', 'C'), device="npu")
            b = torch.empty(3, 2, names=('C', 'N'), device="npu")
            c = torch.empty(3, names=('C',), device="npu")
            d = torch.empty(5, names=('W',), device="npu")
        ...
        def test_wildcard(op):
            a = torch.empty(2, 3, names=('N', 'C'), device="npu")
            c = torch.empty(2, 3, names=(None, 'C'), device="npu")
            self.assertEqual(op(a, c).names, ('N', 'C'))

            b = torch.empty(2, 3, device="npu")
            self.assertEqual(op(a, b).names, ('N', 'C'))

            d = torch.empty(2, 3, names=('C', None), device="npu")
            with self.assertRaisesRegex(RuntimeError, "Misaligned"):
                op(d, c)

        def test_mixed_unnamed_named(op, is_inplace):
            named2 = torch.randn(1, 1, names=('N', 'C'), device="npu")
            unnamed1 = torch.randn(1, device="npu")
            unnamed2 = torch.randn(1, 1, device="npu")
            unnamed3 = torch.randn(1, 1, 1, device="npu")

            def compute_expected_names(tensor, other):
                self.assertTrue(tensor.has_names() ^ other.has_names())
    ...
    def test_out_fn_semantics(self):
        out_fn = torch.abs
        unnamed_tensor = torch.randn(3, 2, device="npu")
        none_named_tensor = torch.randn(3, 2, names=(None, None), device="npu")
        named_tensor = torch.randn(3, 2, names=('N', 'C'), device="npu")
        partially_named_tensor = torch.randn(3, 2, names=('N', None), device="npu")
        ...
        output = torch.randn(3, 2, device="npu")
        out_fn(unnamed_tensor, out=output)
        self.assertFalse(output.has_names())

        output = torch.randn(3, 2, names=(None, None), device="npu")
    ...
    def test_unary_propagate_names_fns(self):
        def _test(testcase, names=('N', 'D'), device='npu'):
            sizes = [2] * len(names)
    ...
    def test_flatten(self):
        tensor = torch.randn(2, 3, 5, 7, 11, names=('N', 'C', 'D', 'H', 'W'), device="npu")
    ...
    def test_flatten_nodims(self):
        tensor = torch.empty((2, 3), device="npu")
    ...
    def test_unflatten(self):
        ...
        self.assertTrue(torch.equal(
            torch.ones(4, names=('A',), device="npu").unflatten('A', (('A', 2), ('B', 2))),
            torch.ones(2, 2, names=('A', 'B'), device="npu")))
        self.assertTrue(torch.equal(
            torch.ones(4, names=('A',), device="npu").unflatten('A', [('A', 2), ('B', 2)]),
            torch.ones(2, 2, names=('A', 'B'), device="npu")))
        self.assertTrue(torch.equal(
            ...
            torch.ones(2, 2, names=('A', 'B'), device="npu")))
        self.assertTrue(torch.equal(
            torch.ones(2, 10, names=('A', 'B'), device="npu").unflatten('B', (['B1', -1],)),
            torch.ones(2, 10, names=('A', 'B1'), device="npu")))
        self.assertTrue(torch.equal(
            torch.ones(2, 3 * 4 * 5 * 6, names=('A', 'B'), device="npu")
                 .unflatten('B', (['B1', 3], ['B2', 4], ['B3', -1], ['B4', 6])),
            torch.ones(2, 3, 4, 5, 6, names=('A', 'B1', 'B2', 'B3', 'B4'), device="npu")))
        self.assertTrue(torch.equal(
            torch.ones(2, 0, names=('A', 'B'), device="npu")
                 .unflatten('B', (['B1', 3], ['B2', -1], ['B3', 4])),
            torch.ones(2, 3, 0, 4, names=('A', 'B1', 'B2', 'B3'), device="npu")))

        # test args: namedtensor, str, namedshape
        self.assertTrue(torch.equal(
            torch.ones(2, 4, names=('A', 'B'), device="npu").unflatten('B', (('B1', 2), ('B2', 2))),
            torch.ones(2, 2, 2, names=('A', 'B1', 'B2'), device="npu")))
        ...
        with self.assertRaisesRegex(TypeError, r"unflatten\(\): argument 'dim' \(position 1\) must be int, not str"):
            torch.tensor([1], names=('A',), device="npu").unflatten('A', (1, 1))
        ...
        with self.assertRaisesRegex(RuntimeError, r"input is a named tensor but no names were given for unflattened sizes"):
            torch.tensor([1], names=("A",), device="npu").unflatten(0, (1, 1))

        with self.assertRaisesRegex(RuntimeError,
                                    r"Provided sizes \[3, -1\] don't multiply up to the "
                                    r"size of dim 1 \('B': 4\) in Tensor\['A', 'B'\]"):
            torch.ones(2, 4, names=('A', 'B'), device="npu").unflatten('B', (('B1', 3), ('B2', -1)))

        with self.assertRaisesRegex(RuntimeError,
                                    r"the unspecified dimension size -1 can be any value and is ambiguous"):
            torch.ones(2, 0, names=('A', 'B'), device="npu").unflatten('B', (('B1', 0), ('B2', -1)))

        tensor = torch.randn(7, 2 * 3 * 5, 11, names=('N', 'D', 'K'), device="npu")
    ...
    def test_unsupported_op_error_msg(self):
        named = torch.randn(3, 3, names=('N', 'C'), device="npu")
        with self.assertRaisesRegex(
    ...
    # This test is failing on Python 3.12: pytorch/pytorch/issues/119464
    @unittest.skipIf(sys.version_info >= (3, 12), "Failing on python 3.12+")
    ...
    def test_no_jit_script_support(self):
        ...
        with self.assertRaisesRegex(RuntimeError, 'NYI'):
            foo(torch.randn(2, 3, names=('N', 'C'), device="npu"))
        ...
        @torch.jit.script
        def return_named_tensor(inputs):
            add_names(inputs)
            return inputs
    ...
    def test_align_tensors_two_inputs(self):
        def _test(tensor_namedshape, align_names, expected_sizes, expected_error):
            ...
            tensor = torch.empty(*tensor_sizes, names=tensor_names, device="npu")
            other = torch.empty([1] * len(align_names), names=align_names, device="npu")
    ...
    def test_align_tensors(self):
        ...
        x = torch.empty(1, 1, names=('N', 'H'), device="npu")
        y = torch.empty(2, 3, 5, names=('N', 'C', 'H'), device="npu")
        z = torch.empty(2, names=('N',), device="npu")
    ...
    def test_autograd_ignores_names(self):
        ...
        x = torch.randn(3, 3, names=('N', 'C'), requires_grad=True, device="npu")
        x.sigmoid().sum().backward()

    def test_tensor_grad_is_unnamed(self):
        x = torch.randn(3, 3, names=(None, None), requires_grad=True, device="npu")
        y = torch.randn(3, 3, names=('N', 'C'), requires_grad=True, device="npu")
        (x * y).sum().backward()
    ...
    def test_autograd_warns_named_grad(self):
        base = torch.randn(3, 3, names=('N', 'C'), device="npu")
