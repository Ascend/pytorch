...
with warnings.catch_warnings(record=True) as warns:
    with tempfile.NamedTemporaryFile() as ckpt:
        ckpt_save = torch.save(torch.nn.Module(), ckpt)
        for warn in warns:
...
class SerializationMixin:
    ...
    def test_serialize_device(self):
        device_str = ['cpu', 'cpu:0', 'npu', 'npu:0']
    ...
    def test_serialization_map_location(self):
        ...
        npu_last_map_locations = [
            f'npu:{torch_npu.npu.device_count() - 1}',
        ]
        ...
        if torch.npu.is_available():
            check_map_locations(npu_0_map_locations, torch.npu.FloatTensor, torch.device('npu', 0))
            check_map_locations(
                npu_last_map_locations,
                torch.npu.FloatTensor,
                torch.device('npu', torch.npu.device_count() - 1)
            )

    @unittest.skipIf(torch.npu.is_available(), "Testing torch.load on CPU-only machine")
    def test_load_nonexistent_device(self):
        # Setup: create a serialized file object with a 'npu:0' restore location
        # The following was generated by saving a torch.randn(2, device='npu') tensor.
        serialized = (b'\x80\x02\x8a\nl\xfc\x9cF\xf9 j\xa8P\x19.\x80\x02M\xe9'
                      ...
                      b'\x00npu:0q\x04K\x02Ntq\x05QK\x00K\x02\x85q\x06K\x01\x85q'
        ...
        error_msg = r'Attempting to deserialize object on a NPU device'
    ...
    def test_serialization_mmap_loading(self, weights_only):
        class DummyModel(torch.nn.Module):
            ...
            def forward(self, input_):
                return self.fc2(self.fc1(input_))
        ...
        input_ = torch.randn(4, 3)
        self.assertEqual(model_mmap_state_dict(input_), model_non_mmap_state_dict(input_.clone()))

    @unittest.skipIf(not torch.npu.is_available() or IS_WINDOWS,
                     "NPU is unavailable or NamedTemporaryFile on Windows")
...
instantiate_device_type_tests(TestBothSerialization, globals(), only_for='privateuse1')
