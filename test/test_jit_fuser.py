...
from torch.testing._internal.jit_utils import JitTestCase, enable_cpu_fuser, _inline_everything, warmup_backward
...
RUN_NPU_HALF = True
RUN_NPU_MULTI_NPU = RUN_NPU and torch.npu.device_count() > 1
...
class TestFuser(JitTestCase):
    ...
    def test_abs_npu(self):
        self._test_fused_abs(device="npu")
    ...
    def test_zero_element_tensors(self):
        ...
        sin = torch.zeros(0, device="npu")
        cos = torch.zeros(0, device="npu")
        inputs = [sin, cos]
        ge = self.checkScript(decode, inputs)

    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_arg_configurations_smoke_npu(self):
        def f(x, y):
            z1, z2 = (x + y).chunk(2, dim=1)
            return z1 * z2

        x = torch.randn(4, 4, dtype=torch.float, device='npu')
        y = torch.randn(4, 4, dtype=torch.float, device='npu')
    ...
    def test_broadcast_npu(self):
        ...
        inputs = [
            torch.randn(4, 4, dtype=torch.float, device='npu'),
            torch.randn(4, dtype=torch.float, device='npu'),
            torch.randn(4, dtype=torch.float, device='npu'),
        ]
    ...
    def test_npu_bfloat16(self):
        def foo(x, y):
            return (x + y).relu()
        m = torch.jit.script(foo)
        x = torch.randn(65536).npu().bfloat16()
        y = torch.randn_like(x)
        self.assertAllFused(m.graph_for(x, y))

    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    @unittest.skipIf(not RUN_NPU_HALF, "no half support")
    @unittest.skipIf(GRAPH_EXECUTOR != ProfilingMode.LEGACY, "no half support with profiling on")
    def test_npu_half(self):
        x = torch.randn(4, 4, dtype=torch.half, device='npu')
        y = torch.randn(4, 4, dtype=torch.half, device='npu')
    ...
    def test_checks_cat_inputs(self):
        ...
        x = torch.randn(2, 4, dtype=torch.float, device='npu')
        y = torch.randn(1, 4, dtype=torch.float, device='npu')

        scripted = self.checkScript(f, (x, y))
        self.assertAllFused(scripted.graph_for(x, y))

    @unittest.skipIf(not RUN_NPU, "No NPU")
    def test_remainder_npu(self):
        def cuda_rem(x, y):
            return 1 + torch.remainder(x, y) - 1

        a = torch.rand([512], dtype=torch.float).npu()
        b = torch.rand([512], dtype=torch.float).npu()
        inputs = [a, b]
    ...
    @unittest.skipIf(not RUN_NPU, "No NPU")
    def test_chunk_npu(self):
        def fn(x):
            a, b, c = x.chunk(3, 1)
            return a * b + c

        inputs = [torch.randn(10, 6, dtype=torch.float, device='npu')]
    ...
    @unittest.skipIf(not RUN_NPU, "No NPU")
    def test_chunk_correctness_npu(self):
        return self._test_chunk_correctness(self, 'npu')

    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_chunk_distributes_npu(self):
        def f(x, y):
            z1, z2 = (x + y).chunk(2, dim=1)
            return z1 * z2
    ...
    def test_chunk_motion_deduplicates_inputs(self):
        ...
        inputs = [
            torch.tensor([1.1, 1.2], device='npu', dtype=torch.float),
        ]
    ...
    @unittest.skipIf(not RUN_NPU, "No NPU")
    def test_chunk_multiple_npu(self):
        ...
        inputs = [
            torch.randn(5, 2, 3, dtype=torch.float, device='npu'),
            torch.randn(5, 6, 3, dtype=torch.float, device='npu'),
            torch.randn(10, 2, 3, dtype=torch.float, device='npu'),
            torch.randn(5, 2, 6, dtype=torch.float, device='npu'),
        ]
    ...
    def test_minmax(self):
        ...
        a = torch.randn(4, 4, dtype=torch.float, device="npu")
        b = torch.randn(4, 4, dtype=torch.float, device="npu")
        nan = torch.tensor(float('nan'), dtype=torch.float, device="npu")
    ...
    def test_clamp(self):
        ...
        a = torch.randn(4, 4, dtype=torch.float, device='npu', requires_grad=True)
        b = torch.randn(4, 4, dtype=torch.float, device='npu')
        nan = torch.tensor(float('nan'), dtype=torch.float, device='npu')
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_comparison_gt_lt_npu(self):
        x = torch.randn(4, 4, dtype=torch.float, device='npu')
        y = torch.randn(4, 4, dtype=torch.float, device='npu')
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_comparison_ge_le_npu(self):
        def f(x, y):
            mask = (x >= 0).type_as(x)
            z = x * mask + y
            mask = (x <= 0).type_as(x)
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_addcmul_npu(self):
        t = torch.randn(1, 4, dtype=torch.float, device='npu')
        t1 = torch.randn(4, 1, dtype=torch.float, device='npu')
        t2 = torch.randn(1, 4, dtype=torch.float, device='npu')
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_lerp(self):
        start = torch.randn(4, 1, dtype=torch.float, device='npu')
        end = torch.randn(1, 4, dtype=torch.float, device='npu')
        weight = torch.tensor(0.5, dtype=torch.float, device='npu')
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_concat_npu(self):
        hx = torch.randn(3, 20, dtype=torch.float, device='npu')
        cx = torch.randn(3, 20, dtype=torch.float, device='npu')
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_concat_invariant_npu(self):
        ...
        x = torch.randn(2, 2, dtype=torch.float, device='npu')
        y = torch.randn(2, 2, dtype=torch.float, device='npu')
        z = torch.randn(4, 2, dtype=torch.float, device='npu')
    ...
    def test_fuse_decompose_normalization(self):
        ...
        def test_norm_decompose(nm, in_opt_graph, not_in_opt_graph, in_fusegraph):
            model = ResLike(nm).npu()
            model_noopt = ResLike(nm).npu()
            model_noopt.load_state_dict(model.state_dict())
            x = torch.randn(2, 16, 8, 8, device='npu')
            y = torch.randn(2, 16, 8, 8, device='npu')
    ...
    def test_threshold(self):
        ...
        x = torch.tensor([-1, -0.5, 0, 1, 2, 3], device='npu')
        scripted = self.checkScript(f, (x,))
        self.assertAllFused(scripted.graph_for(x))

    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_scalar_arg_npu(self):
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    @unittest.skipIf(not RUN_NPU_MULTI_NPU, "needs non-zero device")
    @enable_cpu_fuser
    def test_fusion_reuse_multi_gpu(self):
        ...
        inputs_cuda0 = [x.npu(0) for x in inputs_cpu]
        inputs_cuda1 = [y.npu(1) for y in inputs_cpu]
    ...
    def test_kernel_cache_multi_gpu(self):
        ...
        inputs = [
            torch.randn(4, 4, dtype=torch.float),
            torch.randn(4, 4, dtype=torch.float, device='npu:0'),
            torch.randn(4, 4, dtype=torch.float, device='npu:1'),
        ]
    ...
    @unittest.skipIf(not RUN_NPU_MULTI_NPU, "needs non-zero device")
    def test_nonzero_device_npu(self):
        device = 'npu:' + str(1)
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_lstm_npu(self):
        inputs = get_lstm_inputs('npu', training=True)
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    # By default, on Ampere or later NPUs, LSTM computes float tensors at TF32 precision.
    # We want float tensors to be computed at full precision in order to use the default precision
    @with_tf32_off
    def test_lstm_concat_npu(self):
        inputs = get_lstm_inputs('npu')
        ge = self.checkTrace(LSTMCellC, inputs)
        graph = ge.graph_for(*inputs)
        FileCheck().check("FusedConcat").check_next("return").run(str(graph))

    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_lstm_gates_permutations_npu(self):
        ...
        for permutation in permutations(choices, len(choices)):
            ...
            inputs = get_lstm_inputs('npu', training=False)
    ...
    @unittest.skipIf(IS_SANDCASTLE, "NYI: fuser CPU support for Sandcastle")
    @unittest.skip("Test is flaky, see pytorch/issues/8746")
    @enable_cpu_fuser
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_milstm_npu(self):
        inputs = get_milstm_inputs('npu', training=True)
    ...
    def test_rand_npu(self):
        class M(torch.jit.ScriptModule):
            __constants__ = ['d']

            def __init__(self):
                super().__init__()
                self.d = torch.device('npu')

            @torch.jit.script_method
            def create(self, x):
                return x * x + x + torch.rand_like(x)

        x = torch.zeros([3, 4, 5], dtype=torch.float, device='npu')
        m = M()
        out1 = m.create(x)
        out2 = m.create(x)
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    @unittest.skipIf(GRAPH_EXECUTOR == ProfilingMode.LEGACY, "borked on the legacy executor")
    def test_rand_broadcast_npu(self):
        ...
        x = torch.ones(4, 4, dtype=torch.float, device='npu')
        y = torch.ones(4, dtype=torch.float, device='npu')
        out = script_f(x, y)
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_small_constant_npu(self):
    ...
    @unittest.skipIf(not RUN_NPU, "fuser requires NPU")
    def test_tensor_scalar_ops_npu(self):
        def should_fuse(x):
            z = 3.
            y = x + z
            return x * y
        ...
        inputs = [torch.randn(2, 2, dtype=torch.float, device='npu')]
        ge = self.checkScript(should_fuse, inputs)
        self.assertAllFused(ge.graph_for(*inputs))

        inputs = [
            torch.randn(2, 2, dtype=torch.float, device='npu'),
            torch.tensor(3., dtype=torch.float, device='npu'),
        ]
    ...
    def test_grad_sum_to_size_elimination(self):
        ...
        s1 = torch.randn(5, 1, requires_grad=True, device='npu')
        s2 = torch.randn(5, 5, requires_grad=True, device='npu')
