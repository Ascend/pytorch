...
class TestNpuFormatCastAclnn(TestCase):
    ...
    def test_noncontiguous_with_internal_format_raises(self):
        ...
        # v2.10.0 may route this case through a newer backend path that can
...
class TestNpuFormatCastDtypeParam(TestCase):
    ...
    def test_npu_format_cast_int32_customize_dtype_nd_to_nz(self):
        ...
        out = torch_npu.npu_format_cast(
            t, self.ACL_FORMAT_FRACTAL_NZ, customize_dtype=self.DTYPE_INT32)
        self.assertEqual(torch_npu.get_npu_format(out), self.ACL_FORMAT_FRACTAL_NZ)
        self.assertEqual(out.shape, t.shape)
    ...
    # ------------------------------------------------------------------ #
    # Storage size verification: verify storage can hold logical NZ data.
    # v2.10.0 A2 CANN/Soc may allocate with 32 padding, so physical storage bytes
    # can be larger than the logical C0-based formula used by v2.7.1.
    # ------------------------------------------------------------------ #

    @SupportedDevices(['Ascend910B', 'Ascend910_93'])
    def test_npu_format_cast_c0_8_storage_size_2d(self):
        """Verify C0=8 storage capacity for (15, 17) int32.
        ...
        self.assertGreaterEqual(out.untyped_storage().size(), expected_bytes)

    @SupportedDevices(['Ascend910B', 'Ascend910_93'])
    def test_npu_format_cast_c0_8_vs_default_c0_16_storage_differs(self):
        """C0=8 and default C0=16 produce valid storage.
        ...
        out_c0_8 = torch_npu.npu_format_cast(
            t, self.ACL_FORMAT_FRACTAL_NZ, customize_dtype=self.DTYPE_INT32)
        # v2.7.1 can observe different physical storage sizes here. v2.10.0 A2
        # may pad both allocations to the same 32-aligned size, so validate
        # logical capacity instead of requiring physical bytes to differ.
        expected_default_bytes = self._expected_nz_storage_bytes((15, 17), 16, 4)
        expected_c0_8_bytes = self._expected_nz_storage_bytes((15, 17), 8, 4)
        self.assertGreaterEqual(out_default.untyped_storage().size(), expected_default_bytes)
        self.assertGreaterEqual(out_c0_8.untyped_storage().size(), expected_c0_8_bytes)
    ...
    def test_npu_format_cast_c0_8_storage_size_3d(self):
        """Verify C0=8 storage capacity for 3D (4, 15, 17) int32.
