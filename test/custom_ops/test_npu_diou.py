...
class TestDiou(TestCase):
    def diou(self, prediction, gtbox):
    ...
    def cpu_to_exec(self, boxes1, boxes2):
        ...
        dious = [self.diou(boxes1[i], boxes2[i]) for i in range(n)]
        out = torch.tensor(dious, dtype=torch.float32).reshape(1, -1)
        return out.cpu().numpy()

    def npu_to_exec(self, boxes1, boxes2):
        out = torch_npu.npu_diou(
