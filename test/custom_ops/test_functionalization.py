...
def forward(self, arg0_1):
    ...
    @SupportedDevices(['Ascend910B'])
    def test_image_normalize_functionalize(self):
        @impl(m, "image_normalize")
        def image_normalize_(self_, mean, std):
            return torch.empty_like(self_)

        @impl(m, "image_normalize_")
        def image_normalize_(self_, mean, std):
            return self_
        ...
        def f(a):
            ...
            return torch.ops.npu.image_normalize_(a, mean, std)

        a = torch.randn(1, 3, 355, 432).npu()
        logs = self.get_logs(f, a)
        self.assertExpectedInline(logs, """\
...
def forward(self, arg0_1):
    image_normalize = torch.ops.npu.image_normalize.default(arg0_1, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    copy_ = torch.ops.aten.copy_.default(arg0_1, image_normalize);  arg0_1 = None
