...
class TestUnaryUfuncs(TestCase):
    ...
    # should this or assertEqual also validate that strides are equal?
    ...
    def test_complex_edge_values(self, device, dtype):
        ...
        if not (IS_WINDOWS and dtype == torch.cdouble and "npu" in device):
    ...
    # run on non-native device types
    @dtypes(torch.double)
    ...
    # allow large opinfo values to be opted-into via metadata
    @dtypes(torch.long)
    ...
    # rationalize with exp OpInfo
    @dtypes(*floating_and_complex_types_and(torch.bfloat16))
    @dtypesIfPRIVATEUSE1(*floating_and_complex_types_and(torch.half, torch.bfloat16))
...
instantiate_device_type_tests(TestUnaryUfuncs, globals(), only_for='privateuse1')
