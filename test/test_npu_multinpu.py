...
class TestCudaMultiGPU(TestCase):
    ...
    def test_cuda_memory_leak_detection(self):
        ...
        def leak_gpu0():
            # increasing to 8MB to force acquiring a new block and overcome blocksize differences across platforms
            ll.append(torch.randn(1024 * 1024 * 8, device=torch.device("cuda:0")))
        ...
        if TEST_MULTIGPU:
            ...
            def leak_gpu1():
                # increasing to 8MB to force acquiring a new block and overcome blocksize differences across platforms
                ll.append(torch.randn(1024 * 1024 * 8, device=torch.device("cuda:1")))
...
class TestCudaComm(TestCase):
    def _test_broadcast(self, input1):
        ...
        for i, t in enumerate(results):
            ...
            if input1.is_cuda and input1.get_device() == i:  # test not copying on same device
        ...
        for inplace in [True, False]:
            if inplace:
                ...
                outputs = [input1.cuda(0), torch.empty_like(input1, device=1)]
        ...
        with self.assertRaisesRegex(RuntimeError,
            ...
            comm.broadcast(input1, out=[input1.cuda(0), input1.cpu()])
        with self.assertRaisesRegex(RuntimeError,
                                    r"Expected all output tensors to have same shape as the source .+ at index 1"):
            comm.broadcast(input1, out=[input1.cuda(0), input1.cuda(1).unsqueeze(0)])
