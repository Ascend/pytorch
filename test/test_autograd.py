...
from torch.utils.checkpoint import checkpoint, checkpoint_sequential, CheckpointPolicy, \
...
class TestAutograd(TestCase):
    ...
    def test_invalid_gradients(self):
        ...
        with self.assertRaisesRegex(RuntimeError, 'expected shape'):
            input_ = torch.randn(5, 5, dtype=torch.float, requires_grad=True)
            MyFunction.apply(input_).sum().backward()
    ...
    def test_post_accumulate_grad_hook_e2e(self):
        ...
        for _ in range(iters):
            loss = model(input_).sum()
            loss.backward()

            loss_copy = model_copy(input_).sum()
            loss_copy.backward()
            optim_copy.step()
    ...
    def test_sharded_grad(self):
        ...
        intermediates = [leave * i + leave * leave for i, leave in enumerate(leaves)]
        loss = sum(v * i for i, v in enumerate(intermediates)).sum()

        # define a helper for dividing intermediates into groups
        def group(leave, group_size):
            return (leave[i:i + group_size] for i in range(0, len(leave), group_size))
        ...
        for i, leave in enumerate(leaves):
            self.assertEqual(leave.grad, i * i * (1 + leave))
    ...
    def test_retain_grad(self):
        input_ = torch.rand(1, 3, requires_grad=True)
        h1 = input_ * 3
        out = (h1 * h1).sum()
        ...
        input_.retain_grad()
        out.backward()
        self.assertEqual(input_ * 18, input_.grad)
    ...
    def test_to_sparse_backward(self):
        ...
        def check_inversion_possible(t, layout1, layout1_params, layout2, layout2_params):
            ...
            for l1, l2, p1, p2 in ((*layout, *p), (*layout[::-1], *p[::-1])):
                try:
    ...
    def test_save_none_for_backward(self):
        ...
        class MyFn(Function):
            @staticmethod
            def forward(ctx, input_):
                ctx.save_for_backward(None, input_, None)
                return input_ * input_

            @staticmethod
            def backward(ctx, grad_output):
                n1, input_, n2 = ctx.saved_tensors
                test_case.assertIsNone(n1)
                test_case.assertIsNone(n2)
                return 2 * input_ * grad_output
    ...
    def test_dep_nograd(self):
        ...
        class F2(Function):
            @staticmethod
            def forward(ctx, input_, ignored):
                return input_
    ...
    def test_simple_reentrant(self):
        ...
        # Reentrant starts on CPU thread, finishes on NPU thread
        x = torch.randn(2, 2, requires_grad=True)
        out = Reenter.apply(x)
        out.sum().backward()
    ...
    def test_pow_zero_tensor_gradient(self):
        def run_test(input_size, exponent):
            input_ = torch.zeros(*input_size, requires_grad=True)
            input_.pow(exponent).sum().backward()
            self.assertEqual(input_.grad.abs().sum(), 0)
    ...
    def test_current_graph_task_id(self):
        ...
        def hook(_):
            id_[0] = (torch._C._current_graph_task_id())
        ...
        base = id_[0]
        t.backward(retain_graph=True)
        self.assertEqual(id_[0] - base, 1)
        t.backward(retain_graph=True)
        self.assertEqual(id_[0] - base, 2)
...
Done""")
    ...
    def test_profiler_aggregation_lstm(self):
        ...
        print(prof.table(
            sort_by="self_cpu_time_total", row_limit=10, max_src_column_width=300, header="TEST",
    ...
    def test_no_grad_copy_sparse(self):
        ...
        loss = F.embedding_bag(emb_matrix, input_, offsets, sparse=True).sum()
        loss.backward(retain_graph=True)
        p_g = MyFunc.static_grad_ptr
...
"""
    ...
    @unittest.skipIf(IS_MACOS, "Fails with SIGBUS on macOS; pytorch issue 25941")
    ...
    @unittest.skip("skip pytorch issue 191723")
    ...
    @skipIfTorchDynamo("pytorch/issues/127115")
    ...
    def _test_reentrant_with_callbacks(self, install_callbacks_in_depths):
        ...
        class MyReentrantFunc(Function):
            ...
            def backward(ctx, input_):
                ...
                tmp_inp = input_.detach().requires_grad_()
                with torch.enable_grad():
    ...
    def _do_test_autograd_simple_views_python(self, dtype):
        ...
        fn_id_to_inplace_on_view_err_msg = {
            "one_output": ("Output 0 of IdOneOutput is a view and is being "
                           "modified inplace. This view was created inside a custom Function"),
            "two_output": ("Output 0 of IdTwoOutput is a view and is being modified inplace."
                           " This view is the output of a function that returns multiple views."),
            "view_of_temp": ("Output 0 of ViewOfTemp is a view and is being "
                             "modified inplace. This view was created inside a custom Function")
        }
...
class TestAutogradForwardModeBatchedGrad(TestCase):
    ...
    def test_out_of_place_not_same_layout(self):
        input_ = torch.zeros([2, 2]).transpose(0, 1)
        tangent = torch.zeros([2, 2, 2])

        def jvp(tangent):
            with fwAD.dual_level():
                x = fwAD.make_dual(input_, tangent)
...
class TestAutogradForwardMode(TestCase):
    ...
    def test_metadata_check_checks_ignores_size_zero(self):
        ...
        input_ = torch.rand([0, 1], dtype=torch.complex128, requires_grad=True)
        func = partial(torch.diagonal, offset=0)
        torch.autograd.gradcheck(func, (input_,), check_forward_ad=True)
...
class TestAutogradDeviceType(TestCase):
    ...
    def test_scatter_index_reduce_amin_amax_backprops_to_all_values(self, device):
        ...
        for fn, reduction in product(fns, reduces):
            input_ = torch.randn((2, 3), device=device, dtype=torch.float64, requires_grad=True)
            src = input_.clone().detach_().requires_grad_(True)
            idx = torch.arange(2).to(dtype=torch.long, device=device)
            if fn == torch.scatter_reduce:
                idx = idx.unsqueeze(-1).expand((2, 3))

            gradcheck(fn, (input_, 0, idx, src, reduction), check_batched_grad=False)
    ...
    def test_scatter_index_reduce_prod_gradgrad_error(self, device):
        ...
        input_ = torch.tensor([1.], device=device, dtype=torch.float64, requires_grad=True)
        ...
        for fn in (torch.scatter_reduce, torch.index_reduce):
            ...
            gradcheck(fn, (input_, 0, idx, src, 'prod'), check_batched_grad=False)
            with self.assertRaisesRegex(RuntimeError, "Double backward is unsupported for"):
                gradgradcheck(fn, (input_, 0, idx, src, 'prod'))
    ...
    def test_pyscalar_conversions(self, device):
        def _test_pyscalar_conversions(t, integral_conv):
            # integral -> integral
            d = t(torch.zeros(1, 1, 1, dtype=torch.long))
            pyscalar = -12345
            d[0] = pyscalar
            self.assertEqual(integral_conv(d), pyscalar)
            ...
            d[0] = pyscalar
            self.assertEqual(float(d), float(pyscalar))
            ...
            test_nonzero(d, 0, False)
            test_nonzero(d, -2, True)
    ...
    def test_rnn_backward_to_input_but_not_parameters(self, device):
        ...
        lstm = torch.nn.LSTM(2, 3).to(device)
    ...
    def test_simple_reentrant_cross_device(self, device):
        ...
        # Reentrant starts on NPU thread, finishes on NPU thread
        ...
        # Reentrant starts on NPU thread, finishes on CPU thread
        x = torch.randn(2, 2, device=device, requires_grad=True)
...
class TestAutogradInferenceMode(TestCase):
    ...
    def test_inference_mode_decorator(self):
        ...
        for mode, use_kwarg in product((True, False, None), (True, False)):
            ...
            for requires_grad in (True, False):
                c = torch.ones(1, 2, 3, requires_grad=requires_grad, device="npu")
                d = decorated(c)
    ...
    def test_inference_mode_tensor_creation(self):
        with torch.inference_mode():
            ...
            tmp = torch.ones(1, 2, 3, requires_grad=True, device="npu")
    ...
    def test_inference_mode_existing_autograd_session(self):
        s = torch.ones(1, 2, 3, requires_grad=True, device="npu")
        a = s.clone()
    ...
    def test_normal_tensor_inplace_output_in_inference_mode(self):
        def run_test(fn):
            for requires_grad in (True, False):
                s = torch.ones(1, 2, 3, requires_grad=requires_grad, device="npu")
                a = s.clone()
...
class TestNestedCheckpoint(TestCase):
    ...
    def test_nested_checkpoint_two_children(self, early_stop):
        with torch.utils.checkpoint.set_checkpoint_early_stop(early_stop):
            grad, sum_, c = self.grad, self.sum, self.checkpoint
            ...
            a = torch.randn(3, 3, requires_grad=True)
            expected = grad(sum_(grad(sum_(h))))(a)
            actual = grad(sum_(grad(sum_(c(hc)))))(a)
            self.assertTrue(torch.allclose(expected, actual))

            actual = grad(sum_(c(grad(sum_(c(hc))))))(a)
            self.assertTrue(torch.allclose(expected, actual))

            self.check_graph_dies(grad(c(hc)))
            self.check_graph_dies(grad(sum_(grad(sum_(c(hc))))))
            self.check_graph_dies(grad(sum_(c(grad(sum_(c(hc)))))))
...
class TestSelectiveActivationCheckpoint(TestCase):
    @unittest.skipIf(not TEST_PRIVATEUSE1, "requires NPU")
    def test_flops_and_mem(self):
        def get_act_mem(f):
            out = f()
            out.backward()
            # Why do one forward and backward?
            start_mem = torch.npu.memory_stats()["requested_bytes.all.current"]
            out = f()
            cur_mem = torch.npu.memory_stats()["requested_bytes.all.current"]
        ...
        y = torch.randn(512, 512, requires_grad=True, device="npu")
