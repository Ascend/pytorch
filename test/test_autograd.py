...
class TestAutograd(TestCase):
    ...
    # opinfo this or move to the sparse test suite
    ...
    def test_lobpcg(self):
        ...
        # review if this can be ported to OpInfos or moved to test_linalg.py
    ...
    # review porting these to OpInfo tests
...
""")
        # Uncomment after update to hooks behavior
        # self.assertEqual(predicted[0], grad_fns(*actual))
        actual = []
...
class TestAutogradDeviceType(TestCase):
    ...
    def test_scatter_index_reduce_amin_amax_backprops_to_all_values(self, device):
        ...
        for fn, reduction in product(fns, reduces):
            input_ = torch.randn((2, 3), device=device, dtype=torch.float, requires_grad=True)
    ...
    def test_inplace_on_view_gradcheck(self, device):
        ...
        b = torch.randn(2, 2, dtype=torch.float, device=device, requires_grad=True)
        ...
        go = torch.randn(a.size(), dtype=torch.float, device=device, requires_grad=True)
    ...
    def test_inplace_on_view_modify_base(self, device):
        ...
        # correctly
        r = torch.ones(1, dtype=torch.float, device=device, requires_grad=True)

        def fn(r):
            x = torch.ones(5, dtype=torch.float, device=device)
            v = x.select(0, 1)
    ...
    def test_mv_grad_stride_0(self, device):
        mat = torch.randn(2, 2, dtype=torch.float, device=device)
        vec = torch.randn(1, dtype=torch.float, device=device).requires_grad_(True)
    ...
    def test_gradcheck_input_output_different_device(self, device):
        x = torch.ones((1,), dtype=torch.float, device="npu", requires_grad=True)
        gradcheck(lambda x: x.to("cpu"), (x,))

        x = torch.ones((1,), dtype=torch.float, device="cpu", requires_grad=True)
    ...
    def test_mix_inference_and_normal_tensor_functional_op(self):
        for requires_grad in (True, False):
            ...
            if requires_grad:
                ...
                # Test this with an autograd.Function when it works
