...
class TestShapeOps(TestCase):
    ...
    @onlyPRIVATEUSE1  # CPU is too slow
    @largeTensorTest('17GB')  # 4 tensors of 4GB (in, out) x (torch, numpy) + 1GB
    @largeTensorTest("81GB", "cpu")  # sufficient system memory is required
    ...
    # update once warning flag is available to always trigger ONCE warnings
