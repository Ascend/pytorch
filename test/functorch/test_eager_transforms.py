...
class TestGradTransform(TestCase):
    ...
    @skipIfTorchDynamo("see pytorch torchdynamo issue 103613")
...
class TestAutogradFunction(TestCase):
    ...
    def _get_NumpyCubeNotComposable(self):
        class NumpyCubeNotComposable(torch.autograd.Function):
            @staticmethod
            def forward(input1):
                input_np = input1.cpu().numpy()
                return torch.tensor(input_np ** 3, device=input1.device), input_np
...
class TestAutogradFunctionVmapAPI(TestCase):
    def test_no_vmap_staticmethod_and_no_generate_vmap_rule(self, device):
        class NumpyCube(torch.autograd.Function):
            @staticmethod
            def forward(input1):
                input_np = to_numpy(input1)
                dinput = torch.tensor(3 * input_np ** 2, device=input1.device)
                return torch.tensor(input_np ** 3, device=input1.device), dinput
    ...
    def test_has_vmap_staticmethod_and_has_generate_vmap_rule(self, device):
        class NumpyCube(torch.autograd.Function):
            ...
            @staticmethod
            def setup_context(ctx, outputs, input1):
                ctx.save_for_backward(input1, outputs[1])
    ...
    def test_none_returns(self, device):
        class Zeros(torch.autograd.Function):
            @staticmethod
            def forward(input1):
                return torch.zeros(input1.shape, device=input1.device)
            ...
            def vmap(info, in_dims, input1):
                assert in_dims == (0,)
                return torch.zeros(input1.shape[1:], device=input1.device), None
        ...
        class TwoZeros(torch.autograd.Function):
            @staticmethod
            def forward(input1):
                r = torch.zeros(input1.shape, device=input1.device)
                return r, r
            ...
            def vmap(info, in_dims, input1):
                assert in_dims == (0,)
                r = torch.zeros(input1.shape[1:], device=input1.device)
                return (r, r), None
...
class TestLinearize(TestCase):
    @skipIfTorchDynamo("see pytorch torchdynamo issue 96559")
    @dtypes(torch.float)
...
class TestExamplesCorrectness(TestCase):
    ...
    @with_tf32_off  # pytorch issues 86798
...
class TestFunctionalize(TestCase):
    ...
    # See pytorch functorch issues 780
    ...
    # See the fix / discussion at pytorch pull 76085
