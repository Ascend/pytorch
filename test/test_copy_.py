...
class TestTensorCopy_(TestCase):
    ...
    def setUp(self):
        ...
        self.precision_compare_cases = [
            (torch.bool, torch.float32, [False, True, False, True, True, False, True, False]),
            (torch.int8, torch.float16, [-127, -31, -1, 0, 1, 7, 42, 127]),
            (torch.int16, torch.float32, [-32768, -1025, -1, 0, 1, 255, 1024, 32767]),
            (torch.uint16, torch.float32, [0, 1, 17, 255, 1024, 4096, 32768, 65535]),
    ...
    def _make_host_source(self, dtype, pin_memory=False):
        base_dtype = torch.complex128 if dtype.is_complex else torch.float32
        values = (
            [[-7.5 + 1.25j, -3.25 - 0.5j, -1.0 + 2.0j, 0.0 - 1.0j],
             [1.5 + 0.75j, 2.25 - 3.5j, 5.0 + 4.0j, 9.75 - 8.5j]]
            if dtype.is_complex
            else [[-7.5, -3.25, -1.0, 0.0], [1.5, 2.25, 5.0, 9.75]]
        )
        src = torch.tensor(values, dtype=base_dtype).to(dtype)
    ...
    @SupportedDevices(['Ascend910B', 'Ascend910_93', 'Ascend950'])
    def test_copy_npu_from_pinned_cpu_aclnn_cast_unsupported_complex32_src_fallback(self):
        dst = torch.empty(2, 4, dtype=torch.complex64, device=self.device)
        src = self._make_host_source(torch.complex32, pin_memory=True)
    ...
    @SupportedDevices(['Ascend910B', 'Ascend910_93', 'Ascend950'])
    def test_copy_pinned_cpu_from_npu_aclnn_cast_unsupported_complex32_dst_fallback(self):
        dst = torch.empty(2, 4, dtype=torch.complex32).pin_memory()
        src = self._make_device_source(torch.complex64)
