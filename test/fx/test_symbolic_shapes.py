...
This file validates ShapeEnv.produce_guards_expression, ShapeEnv.produce_guards_verbose, ShapeEnv.replace, ShapeEnv.add_backed_var_to_val, ShapeEnv.set_unbacked_var_to_val, ShapeEnv.simplify (extendable).
"""
...
class TestShapeEnvNPU(TestCase):
    """Issue #1627: direct NPU unit tests for ShapeEnv guard / sympy methods (v2.11.0)."""
    ...
    @unittest.skipUnless(torch.npu.is_available(), "requires npu")
    @unittest.skipUnless(
        _shape_env_has("add_backed_var_to_val"),
        "torch.fx.experimental.symbolic_shapes.ShapeEnv.add_backed_var_to_val "
        "is not available on this PyTorch version",
    )
    def test_torch_fx_experimental_symbolic_shapes_ShapeEnv_add_backed_var_to_val(self):
        """Verify add_backed_var_to_val binds a concrete value to a backed sympy symbol (2.11.0+)."""
        env = ShapeEnv()
