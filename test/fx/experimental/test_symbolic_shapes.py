...
class TestShapeEnvSymbolicShapes(TestCase):
    ...
    def test_add_var_to_val_with_npu_tensor_shape(self):
        # Verify the core functionality of ShapeEnv.add_var_to_val using NPU tensor shapes.
        dummy_tensor = torch.ones(7).to(device_type)
        shape_env = ShapeEnv()
        sym = sympy.Symbol("s0")
        actual_val = dummy_tensor.size(0)
        shape_env.add_var_to_val(sym, actual_val)
