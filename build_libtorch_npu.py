...
def generate_bindings_code(base_dir):
    ...
    if subprocess.call(generate_code_cmd) != 0:
        print(f"Failed to generate ATEN bindings: {generate_code_cmd}", file=sys.stderr)
        sys.exit(1)
...
def build_stub(base_dir):
    ...
    if subprocess.call(build_stub_cmd) != 0:
        print(f"Failed to build stub: {build_stub_cmd}", file=sys.stderr)
        sys.exit(1)


def clean_generated_files():
    with open(os.path.join(BASE_DIR, ".gitignore")) as f_ignore:
    ...
    for library_file in library_files:
        subprocess.check_call(
            [
                ...
                library_file,
                "-f",
                str(dbg_dir.joinpath(library_file.name)) + ".debug",
            ],
            cwd=BASE_DIR,
        )  # Compliant
...
def run_cmake():
    ...
    if os.getenv("_GLIBCXX_USE_CXX11_ABI") is not None:
        cmake_args.append(
            "-DGLIBCXX_USE_CXX11_ABI=" + os.getenv("_GLIBCXX_USE_CXX11_ABI")
        )
    ...
    use_ninja = (
        os.environ["CMAKE_GENERATOR"].lower() == "ninja"
        if "CMAKE_GENERATOR" in os.environ
        else which("ninja") is not None
    )
    if use_ninja:
...
def copy_file(infile, outfile, preserve_mode=1, preserve_times=1, link=None, level=1):
    ...
    return file_util.copy_file(
        infile, outfile, preserve_mode, preserve_times, True, link
    )
...
def copy_hpp():
    def get_src_py_and_dst():
        ...
        for src in acl_header_files:
            ...
            os.makedirs(os.path.dirname(wrapper_src), exist_ok=True)
            with open(wrapper_src, "w", encoding="utf-8", newline="\n") as wrapper_file:
                wrapper_file.write("#pragma once\n#include <{}>\n".format(
                    relative_header.replace(os.sep, "/")
                ))
            wrapper_dst = os.path.join(
                BASE_DIR,
                "libtorch_npu/include/third_party/acl/inc",
                relative_header,
            )
            os.makedirs(os.path.dirname(wrapper_dst), exist_ok=True)
            ret.append((wrapper_src, wrapper_dst))
