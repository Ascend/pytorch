...
def _normalize_nodeid(nodeid: str) -> str:
    """归一化 nodeid：去除开头的 'test/' 前缀，与 pytest 在 test 目录下收集的格式对齐"""
    if nodeid.startswith("test/"):
        return nodeid[len("test/"):]
    return nodeid
...
def load_skip_list(skip_list_paths) -> Dict[str, Dict]:
    ...
    for path in skip_list_paths:
        ...
        for nodeid, meta in single.items():
            norm = _normalize_nodeid(nodeid)
            if norm not in skip_dict:
                skip_dict[norm] = meta
    return skip_dict
    ...
    try:
        ...
        if result.returncode in (0, 3):
            ...
            try:
                ...
                r = subprocess.run(["find", "/usr/local/Ascend", "-name", "libhccl.so"], capture_output=True, text=True, timeout=10)
                diag_lines.append("find libhccl.so: " + (r.stdout.strip() or "NOT FOUND"))
                r2 = subprocess.run(["cat", "/usr/local/Ascend/cann/version.cfg"], capture_output=True, text=True, timeout=5)
                diag_lines.append("CANN version: " + (r2.stdout.strip() or "MISSING"))
                r3 = subprocess.run(
                    ["python3", "-c", "import torch; print('torch:', torch.__version__)"],
                    capture_output=True, text=True, timeout=10, env=os.environ, cwd="/tmp",
                )
...
def collect_all_cases(
    ...
    with ThreadPoolExecutor(max_workers=parallel) as executor:
        ...
        for future in as_completed(futures):
            ...
            if completed % 100 == 0:
                print(
                    f"  [Progress: {completed}/{len(test_files)} files, "
                    f"{successful_count} ok, {failed_count} failed, {total_cases} cases]"
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--error-log-dir",
        help="Output directory for collection error logs (default: output-dir/collection_errors)",
    )
