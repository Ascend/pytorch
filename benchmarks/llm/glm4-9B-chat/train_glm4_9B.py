...
class GLM4Trainer:
    ...
    def setup_training(self):
        logger.info("Loading GLM-4 model from %s", self.args.model_path)
    ...
    def get_lora_target_modules(self):
        ...
        model_modules = sorted([name for name, _ in self.model.named_modules()])
        available_modules = []
        ...
        for target in target_modules:
            ...
            for name in model_modules:
                if target in name:
                    ...
                    break
            if not found:
                logger.warning("Target module %s not found in model", target)

        available_modules = list(dict.fromkeys(available_modules))

        if not available_modules:
            available_modules = ["qkv", "proj", "fc1", "fc2"]
            logger.warning("No available, GLM-4 using LoRA target modules: %s", available_modules)
        else:
            logger.info("GLM-4 LoRA target modules: %s", available_modules)

        return available_modules


    def load_and_preprocess_data(self) -> Dataset:
        logger.info("Loading dataset from %s", self.args.data_path)
        ...
        max_length_ref = self.args.max_length

        def preprocess_function(examples):
            tokenized = tokenizer_ref(
                examples["text"],
                truncation=True,
        ...
        logger.info("Dataset size: %s", len(processed_dataset))
    ...
    def create_trainer(self, train_dataset, eval_dataset=None):
        ...
        training_args = TrainingArguments(
            ...
            max_steps=self.args.max_steps if self.args.max_steps > 0 else 30,
    ...
    def train(self):
        logger.info("Starting GLM-4 training...")
        ...
        trainer.save_state()
        logger.info("Training completed! Model saved to %s", self.args.output_dir)
        return metrics


def main():
    parser = argparse.ArgumentParser(description="Train GLM-4 model")
    parser.add_argument("--model_path", type=str, default="ZhipuAI/glm-4-9b-chat",
                        help="Path to the pretrained GLM-4 model")
    parser.add_argument("--data_path", type=str, required=True,
                        help="Path to training data (json/jsonl file or dataset name)")
    parser.add_argument("--output_dir", type=str, default="./glm4-finetuned",
    ...
    parser.add_argument("--num_proc", type=int, default=0,
                        help="Number of processes for data preprocessing (0 = single process)")
    parser.add_argument("--pad_to_max_length", action="store_true",
                        help="Pad sequences to max_length")
    parser.add_argument("--overwrite_cache", action="store_true",
                        help="Overwrite cached features")
    parser.add_argument("--group_by_length", action="store_true",
                        help="Group sequences by length for efficient training")
    ...
    trainer = GLM4Trainer(args)
    metrics = trainer.train()

    print("\n" + "=" * 50)
    print("GLM-4 Training completed successfully!")
