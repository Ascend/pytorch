...
data = pd.read_csv('CTR_Algorithm/data/data.csv')
data_X = data.iloc[:, 2:]
...
def get_profile(profiler_start_step: int, profiler_end_step: int, profiling_save_path: str):
    ...
    if device == 'npu':
        ...
        print("[Warning]: No supported acceleration device (CUDA/NPU) detected, profiler will be disabled")
        return None
...
from CTR_Algorithm.pytorch.model.DCNv2 import DeepCrossNetv2
model = DeepCrossNetv2(feature_fields=fields, embed_dim=16, layer_num=2, mlp_dims=(32, 16), dropout=0.1, cross_method='Matrix')
